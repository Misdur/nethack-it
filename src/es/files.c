...
/* LENGUA:  For compatibility with the English version, filenames, environment
   variables, and option names are left untranslated.  It may be desirable to
   provide alternatives in Spanish for the variables and options. */
...
int whichprefix, buffnum;
{
	...
	if (buffnum < 0 || buffnum >= FQN_NUMBUF) {
		impossible("fqn_filename_buffer inválido especificado: %d",
								buffnum);
		buffnum = 0;
	}
	if (strlen(fqn_prefix[whichprefix]) + strlen(basename) >=
						    FQN_MAX_FILENAME) {
		impossible("fqname demasiado largo: %s + %s", fqn_prefix[whichprefix],
						basename);
		return basename;	/* XXX */
	}
...
char *reasonbuf;
{
	...
	for (prefcnt = 1; prefcnt < PREFIX_COUNT; prefcnt++) {
		...
		if ((fp = fopen(filename, "w"))) {
			...
			Sprintf(panicbuf1,"%s inválido", fqn_prefix_names[prefcnt]);
...
# endif
	...
	if (fd >= 0)
	    ...
	    Sprintf(errbuf,
		    "No puedo crear archivo \"%s\" para nivel %d (errno %d).",
		    lock, lev, errno);
...
# endif
	...
	if (fd < 0 && errbuf)
	    Sprintf(errbuf,
		    "No puedo abrir archivo \"%s\" para nivel %d (errno %d).",
		    lock, lev, errno);
...
# endif
	...
	if (fd < 0 && errbuf) /* failure explanation */
	    Sprintf(errbuf,
		    "No puedo crear huesos \"%s\", id %s (errno %d).",
		    lock, *bonesid, errno);
...
#ifdef WIZARD
	if (wizard && ret != 0)
		pline("no puedo cambiar nombre de %s a %s", tempname, fq_bones);
#endif
}
...
boolean uncomp;
{
	if (freopen(filename, mode, stream) == (FILE *)0) {
		(void) fprintf(stderr, "freopen de %s para %scomprimir falló\n",
			filename, uncomp ? "des" : "");
...
# endif
		...
		perror((char *)0);
		(void) fprintf(stderr, "Exec para %scomprimir %s falló.\n",
			uncomp ? "des" : "", filename);
		terminate(EXIT_FAILURE);
	} else if (f == -1) {
		perror((char *)0);
		pline("Fork para %scomprimir %s falló.",
			uncomp ? "des" : "", filename);
		return;
	}
...
# endif
	if (i == 0) {
	    ...
	    if (uncomp) {
		raw_printf("No puedo descomprimir %s", filename);
		(void) unlink(filename);
	    } else {
...
# pragma unused(filename, retryct)
	...
	nesting++;
	if (nesting > 1) {
	    impossible("TRATÉ DE ANIDAR LOCKS");
	    return TRUE;
	}
...
# endif
		...
		if (retryct--) {
		    HUP raw_printf(
			    "Esperando acceso a %s.  (se quedan %d atentados).",
			    filename, retryct);
...
# endif
		    ...
		    HUP (void) raw_print("Lo desisto.  Lo siento.");
		    HUP raw_printf("¿Tal vez haya un %s viejo?",
					lockname);
		    nesting--;
		    return FALSE;
		}
		...
		break;
	    case ENOENT:
		HUP raw_printf("¡No puedo encontrar %s para hacer lock!", filename);
		nesting--;
		return FALSE;
	    case EACCES:
		HUP raw_printf("¡No tengo permisión de escribir para hacer lock por %s!", filename);
		nesting--;
		return FALSE;
# ifdef VMS			/* c__translate(vmsfiles.c) */
	    case EPERM:
		/* could be misleading, but usually right */
		HUP raw_printf("No puedo hacer lock de %s a causa de protección de directorio.",
			       filename);
		nesting--;
		return FALSE;
# endif
		...
		HUP perror(lockname);
		HUP raw_printf(
			     "No puedo hacer lock de %s por razón desconocida (%d).",
			       filename, errnosv);
		nesting--;
		return FALSE;
	    }
...
# endif
	if (OPENFAILURE(lockptr)) {
	    raw_printf("Esperando acceso a %s.  (se quedan %d atentados).",
			filename, retryct);
	    Delay(50);
	}
    }
    if (!retryct) {
	raw_printf("Lo desisto.  Lo siento.");
	nesting--;
	return FALSE;
    }
...
#endif
{
	...
	if (nesting == 1) {
		...
		if (unlink(lockname) < 0)
			HUP raw_printf("No puedo quitar %s.", lockname);
# ifdef NO_FILE_LINKS
		(void) close(lockfd);
# endif
...
const char *filename;
{
	...
	if (filename) {
		...
		if (access(filename, 4) == -1) {
			 ...
			 * control
			 */
			raw_printf("Acceso a %s negado (%d).",
					filename, errno);
			wait_synch();
		...
		if ((fp = fopenp(filename, "r")) != (FILE *)0) {
		    ...
		    raw_printf("No puedo abrir archivo pedido de config %s (%d).",
					filename, errno);
		    wait_synch();
		    /* fall through to standard names */
#endif
...
#endif
		details = "";
	    raw_printf("No puedo abrir archivo por defecto de config %s %s(%d).",
    ...
    const char *name;		/* name of option for error message */
{
	...
	switch(*bufp) {
	    ...
	    default:
gi_error:
		raw_printf("Error de sintaxis en %s", name);
		wait_synch();
		return count;
	}
    }
...
# endif
	...
	while (fgets(buf, 4*BUFSZ, fp)) {
		if (!parse_config_line(fp, buf, tmp_ramdisk, tmp_levels)) {
			raw_printf("Mala linea de opción:  \"%s\"", buf);
			wait_synch();
		}
	}
	(void) fclose(fp);
...
# else	/* should be only UNIX left */
	...
	if ((fp = fopenp(tmp_wizkit, "r")) != (FILE *)0)
		...
		/* e.g., problems when setuid NetHack can't search home
		 * directory restricted to user */
		raw_printf("No puedo abrir archivo default de wizkit %s (%d).",
					tmp_wizkit, errno);
		wait_synch();
	}
# endif
...
read_wizkit()
{
	...
	while (fgets(buf, (int)(sizeof buf), fp)) {
		...
		if (buf[0]) {
			otmp = readobjnam(buf, (struct obj *)0, FALSE, FALSE);
			if (otmp) {
			    if (otmp != &zeroobj)
				otmp = addinv(otmp);
			} else {
			    /* .60 limits output line width to 79 chars */
			    raw_printf("Mala entrada de wizkit: \"%.50s\"", buf);
			    bad_items = TRUE;
			}
		}
	    }
...
# ifdef VMS	/* must be stream-lf to use UPDATE_RECORD_IN_PLACE */
		if (!file_is_stmlf(fd)) {
		    raw_printf(
		  "Aviso: archivo de escor %s no es en formato de stream_lf",
				fq_record);
		    wait_synch();
		}
# endif
...
# endif /* VMS && !SECURE */
	    ...
	    raw_printf("Aviso: No puedo escribir archivo de escor %s", fq_record);
	    wait_synch();
	}
#endif  /* !UNIX && !VMS */
#if defined(MICRO) || defined(WIN32)
...
# endif
	raw_printf("Aviso: no puedo escribir registro %s", tmp);
		wait_synch();
	    } else
		(void) close(fd);
	} else		/* open succeeded */
...
recover_savefile()
{
	...
	if (read(gfd, (genericptr_t) &hpid, sizeof hpid) != sizeof hpid) {
	    raw_printf(
"\nDados de recobración incompletamente escritos o sobreescritos después.  Es imposible recobrar.");
	    (void)close(gfd);
	    return FALSE;
	}
	if (read(gfd, (genericptr_t) &savelev, sizeof(savelev))
							!= sizeof(savelev)) {
	    raw_printf("\nRecobración no estaba en efecto para %s -- es imposible recobrar.\n",
			lock);
	    (void)close(gfd);
	    return FALSE;
	}
	if ((read(gfd, (genericptr_t) savename, sizeof savename)
		...
		!= sizeof version_data)) {
	    raw_printf("\nError leyendo %s -- no puedo recobrar.\n", lock);
	    (void)close(gfd);
	    return FALSE;
	}
	...
	sfd = create_savefile();
	if (sfd < 0) {
	    raw_printf("\nNo puedo recobrar archivo de guardar %s.\n", SAVEF);
	    (void)close(gfd);
	    return FALSE;
	}
	...
	if (write(sfd, (genericptr_t) &version_data, sizeof version_data)
		!= sizeof version_data) {
	    raw_printf("\nError escribiendo %s; recobración falló.", SAVEF);
	    (void)close(gfd);
	    (void)close(sfd);
	    delete_savefile();
	    return FALSE;
