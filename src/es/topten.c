...
/* LENGUA:  The five entries in English are special to the top-ten listing
   code and must not be translated; listings will substitute them with
   Spanish text */
NEARDATA const char * const killed_by_prefix[] = {
	"matad@ por ", "choked on ", "poisoned by ", "murió de ", "se ahogó en ",
	"burned by ", "fundió en ", "crushed to death by ", "petrified by ",
	"convertid@ en limo por ", "matad@ por ", "", "", "", "", ""
};
...
#undef TTFIELDS
		...
		if (tt->ver_major < 3 ||
		    ...
		    Strcpy(tt->plrace, "?");
		    /* LENGUA:  Leave this alone for compatibility */
...
#define HUP
	...
	switch (killer_format) {
		default: impossible("¿Mala forma de matador?");
		case KILLED_BY_AN:
			Strcat(t0->death, killed_by_prefix[how]);
			fixup = strchr(t0->death, '@');
			if (fixup) *fixup = flags.female? 'a' : 'o';
	...
	if (lock_file(LOGFILE, SCOREPREFIX, 10)) {
		...
		HUP raw_print("¡No puedo abrir el archivo \"logfile\"!");
	    } else {
		writeentry(lfile, t0);
		(void) fclose(lfile);
	    }
	...
	if (wizard || discover) {
		...
		topten_print("");
		Sprintf(pbuf,
	      "Porque estuviste en modo de %s, la lista de cuenta no se cambiará.",
		    wizard ? "mago" : "descubrir");
		topten_print(pbuf);
	    }
	    goto showwin;
	}
	...
	if (!rfile) {
		HUP raw_print("¡No puedo abrir el archivo \"record\"!");
		unlock_file(RECORD);
		goto destroywin;
	}
	...
	for(rank = 1; ; ) {
		...
		--occ_cnt <= 0) {
			...
			HUP {
			    char pbuf[BUFSZ];
			    Sprintf(pbuf,
			  "No excediste tu cuenta previa de %ld puntos.",
				    t1->points);
			    topten_print(pbuf);
			    topten_print("");
			}
	...
	if(flg) {	/* rewrite record file */
		...
		if(!(rfile = fopen_datafile(RECORD, "w", SCOREPREFIX))){
			HUP raw_print("No puedo escribir el archivo \"record\"");
			unlock_file(RECORD);
			free_ttlist(tt_head);
			goto destroywin;
		}
		...
		if(!done_stopprint) if(rank0 > 0){
		    if(rank0 <= 10)
			topten_print("¡Estás entre los diez mejores!");
		    else {
			char pbuf[BUFSZ];
			Sprintf(pbuf,
			  "Has llegado al %dº lugar entre los %d mejores.",
				rank0, ENTRYMAX);
			topten_print(pbuf);
		    }
		    topten_print("");
...
outheader()
{
	...
	Strcpy(linebuf, " No  Puntos     Nombre");
	bp = eos(linebuf);
...
boolean so;
{
	...
	if (t1->plalign[0] != '?')
		...
		Strcat(linebuf, " ");
	/* LENGUA: strings compared to t1->death are not translated, so that
	   an English and a Spanish Nethack sharing a record file will be
	   compatible */
	if (!strncmp("escaped", t1->death, 7)) {
	    Sprintf(eos(linebuf), "escapó del calabozo %s[max nivel %d]",
		...
		*bp = (t1->deathdnum == astral_level.dnum) ? '\0' : ' ';
	    ...
	    Sprintf(eos(linebuf), "se ascendió a demidivinidad");
	    second_line = FALSE;
	} else {
	    if (!strncmp(t1->death, "quit", 4)) {
		Strcat(linebuf, "dejó");
		second_line = FALSE;
	    } else if (!strncmp(t1->death, "died of st", 5)) {
		Strcat(linebuf, "murió de hambre");
		second_line = FALSE;
	    } else if (!strncmp(t1->death, "choked", 6)) {
		Strcat(linebuf, "se sufocó sobre su comida");
	    } else if (!strncmp(t1->death, "poisoned", 8)) {
		Sprintf(eos(linebuf), "fue envenenad%c",
			(t1->plgend[0] == 'F') ? 'a' : 'o');
	    } else if (!strncmp(t1->death, "crushed", 7)) {
		Strcat(linebuf, "se estrujó al muerte");
	    } else if (!strncmp(t1->death, "petrified by", 13)) {
		Strcat(linebuf, "se volvió a piedra");
	    } else Strcat(linebuf, "murió");

	    if (t1->deathdnum == astral_level.dnum) {
		const char *arg, *fmt = " en el Plano de %s";
		...
		Sprintf(eos(linebuf), " en %s", dungeons[t1->deathdnum].dname);
		if (t1->deathdnum != knox_level.dnum)
		    Sprintf(eos(linebuf), " en nivel %d", t1->deathlev);
	...
	if (second_line)
	    Sprintf(eos(linebuf), "  %c%s.", toupper2(*(t1->death)), t1->death+1);
...
int uid;
{
	...
	for (i = 0; i < playerct; i++) {
		...
		i++;
		    ...
		    strcmp(players[i], "todo") == 0 ||
		    strcmp(players[i], "todos") == 0 ||
		    strcmp(players[i], "toda") == 0 ||
		    strcmp(players[i], "todas") == 0 ||
...
char **argv;
{
	...
	if (argc < 2 || strncmp(argv[1], "-s", 2)) {
		raw_printf("prscore: malos argumientos (%d)", argc);
		return;
	}

	rfile = fopen_datafile(RECORD, "r", SCOREPREFIX);
	if (!rfile) {
		raw_print("¡No puedo abrir el archivo \"record\"!");
		return;
	}
	...
	if (argc <= 1) {
		...
		if (!*player0)
# ifdef AMIGA
			player0 = "todo";	/* single user system */
# else
			player0 = "hackplayer";
# endif
		playerct = 1;
	...
	if (match_found) {
	    ...
	    Sprintf(pbuf, "No puedo encontrar ningunas entradas%s sobre ",
				current_ver ? " actuales" : "");
	    if (playerct < 1) Strcat(pbuf, "ti.");
	    else {
		if (playerct > 1) Strcat(pbuf, "ningunos de ");
		for (i = 0; i < playerct; i++) {
	    ...
	    raw_print(pbuf);
	    raw_printf("Usaje: %s -s [-v] <tiposjugadores> [maxrank] [nombresjugadores]",
			 hname);
	    raw_printf("Tipos de jugadores son: [-p rol] [-r raza]");
	}
	free_ttlist(tt_head);
#ifdef	AMIGA
	{
	...
	boolean fem;
{
	...
	impossible("¿Qué extraño rol es esto? (%s)", plch);
...
struct obj *otmp;
{
	...
	rfile = fopen_datafile(RECORD, "r", SCOREPREFIX);
	if (!rfile) {
		impossible("¡No puedo abrir el archivo \"record\"!");
		return (struct obj *)0;
	}
