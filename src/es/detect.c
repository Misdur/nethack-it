...
register struct obj *sobj;
{
	...
	if (sobj) {
		...
		if (youmonst.data == &mons[PM_GOLD_GOLEM]) {
			Sprintf(buf, "¡Te sientes como un millón de %s!",
				currency(2L));
		} else if (hidden_gold() ||
#ifndef GOLDOBJ
				u.ugold)
...
#endif
			Sprintf(buf,
				"Te sientes preocupad%c por tu futura situación financiera.",
				poly_gender()==1? 'a' : 'o');
		else
			Strcpy(buf, "Te sientes materiamente pobre.");
    ...
    if (stale) docrt();
    pline("Nots algo de oro entre tus %s.", makeplural(body_part(FOOT)));
    return(0);
...
outgoldmap:
    ...
    newsym(u.ux,u.uy);
    Te_sientes("muy avar%c, y percibes oro!", poly_gender()==1? 'a' : 'o');
    exercise(A_WIS, TRUE);
...
register struct obj	*sobj;
{
    ...
    const char *what = confused ? something : "comida";
    int uw = u.uinwater;
	...
	if (stale) {
	    docrt();
	    Sientes("una falta de %s en las cercanías.", what);
	    if (sobj && sobj->blessed) {
		if (!u.uedibility) pline("Sientes un hormigueo en tu %s.", body_part(NOSE));
		u.uedibility = 1;
	    }
	} else if (sobj) {
	    char buf[BUFSZ];
	    Sprintf(buf, "Tu %s tiembla%s.", body_part(NOSE),
			(sobj->blessed && !u.uedibility) ? " y entonces empiezas a sentir un hormigueo" : "");
	...
	known = TRUE;
	pline("%s %s en las cercanías.", sobj ? "Hueles" : "Sientes", what);
	if (sobj && sobj->blessed) {
		if (!u.uedibility) pline("Sientes un hormigueo en tu %s.", body_part(NOSE));
		u.uedibility = 1;
	}
	...
	if (sobj) {
	    if (sobj->blessed) {
	    	pline("Tu %s %s y hueles %s.", body_part(NOSE),
	    		u.uedibility ? "sigue hormigueando" : "empieza a hormiguear", what);
		u.uedibility = 1;
	    } else
		pline("Tu %s hormiguea y hueles %s.", body_part(NOSE), what);
	}
	else pline("Percibes %s.", what);
	display_nhwindow(WIN_MAP, TRUE);
	exercise(A_WIS, TRUE);
	docrt();
	u.uinwater = uw;
...
int		class;		/* an object class, 0 for all */
{
	...
	impossible("object_detect:  clase ilegal %d", class);
	class = 0;
    }
	...
	Strcpy(stuff, something);
    	...
    	Strcpy(stuff, class ? oclass_names[class] : "objetos");
    if (boulder && class != ROCK_CLASS) Strcat(stuff, " y/o piedras grandes");
	...
	if (!ctu) {
	    if (detector)
		strange_feeling(detector, "Sientes una falta de algo.");
	    return 1;
	}

	Sientes("%s en las cercanías.", stuff);
	return 0;
    }
 ...
 */
    ...
    newsym(u.ux,u.uy);
    pline("Descubres la %s de %s.", ct ? "presencia" : "ausencia", stuff);
    display_nhwindow(WIN_MAP, TRUE);
    /*
...
int mclass;			/* monster class, 0 for all */
{
	...
	if (otmp) {
	    ...
	    if (Hallucination)
		Strcpy(str, "Te llenas de ansiedad nerviosa.");
	    else
		Sprintf(str, "Te sientes amenazad%c.",
			poly_gender()==1? 'a' : 'o');

	    strange_feeling(otmp, str);
	}
	return 1;
    } else {
	boolean woken = FALSE;
	...
	display_self();
	Sientes("la presencia de monstruos.");
	if (woken)
	    pline("Los monstruos sienten tu presencia.");
	display_nhwindow(WIN_MAP, TRUE);
	docrt();
	if (Underwater) under_water(2);
	if (u.uburied) under_ground(2);
...
/* sobj is null if crystal ball, *scroll if gold detection scroll */
{
    ...
    if (!found) {
	char buf[42];
	Sprintf(buf, "Tus %s dejan de picar.", makeplural(body_part(TOE)));
	strange_feeling(sobj,buf);
	return(1);
    }
    /* traps exist, but only under me - no separate display required */
    pline("Tus %s pican.", makeplural(body_part(TOE)));
    return(0);
outtrapmap:
    cls();
    ...
    newsym(u.ux,u.uy);
    Te_sientes("%s%c.",
	  sobj && sobj->cursed ? "muy avar" : "atrampad",
	  poly_gender()==1? 'a' : 'o');
    display_nhwindow(WIN_MAP, TRUE);
    docrt();
    u.uinwater = uw;
...
d_level *where;
{
    ...
    if (ll < 0) {
	if (ll < (-8 - rn2(3)))
	    if (!indun)	return "muy lejos";
	    else	return "muy abajo";
	else if (ll < -1)
	    if (!indun)	return "muy abajo tuyo";
	    else	return "abajo tuyo";
	else
	    if (!indun)	return "en la distancia";
	    else	return "un poco abajo";
    } else if (ll > 0) {
	if (ll > (8 + rn2(3)))
	    if (!indun)	return "muy lejos";
	    else	return "muy encima";
	else if (ll > 1)
	    if (!indun)	return "muy encima tuyo";
	    else	return "encima tuyo";
	else
	    if (!indun)	return "en la distancia";
	    else	return "un poco encima";
    } else
	    if (!indun)	return "en la distancia";
	    else	return "cerca";
}
...
static const struct {
  ...
  { "Delfos", &oracle_level },
  { "la guarida de Medusa", &medusa_level },
  { "un castillo", &stronghold_level },
  { "el torre del Mago de Yéndor", &wiz1_level },
};
...
struct obj *obj;
{
    ...
    if (Blind) {
	pline("Es una lástima que no puedas ver %s", the(xname(obj)));
	return;
    }
    oops = (rnd(20) > ACURR(A_INT) || obj->cursed);
    if (oops && (obj->spe > 0)) {
	switch (rnd(obj->oartifact ? 4 : 5)) {
	case 1 : pline("¡%s demasiado para comprender!", Tobjnam(obj, "es"));
	    break;
	case 2 : pline("¡%s!", Tobjnam(obj, "te confunde"));
	    make_confused(HConfusion + rnd(100),FALSE);
	    break;
	case 3 : if (!resists_blnd(&youmonst)) {
		pline("¡%s tu visión!", Tobjnam(obj, "daña"));
		make_blinded(Blinded + rnd(100),FALSE);
		if (!Blind) pline(vision_clears);
	    } else {
		pline("%s tu visión.", Tobjnam(obj, "asalta"));
		pline("¡No eres afectad%c!", poly_gender()==1? 'a' : 'o');
	    }
	    break;
	/* LENGUA:  ¿zap -> agitar?  */
	case 4 : pline("¡%s tu mente!", Tobjnam(obj, "agita"));
	    (void) make_hallucinated(HHallucination + rnd(100),FALSE,0L);
	    break;
	case 5 : pline("¡%s!", Tobjnam(obj, "explota"));
	    useup(obj);
	    obj = 0;	/* it's gone */
	    losehp(rnd(30), "explosión de bola de cristal", KILLED_BY_AN);
	    break;
	}
    ...
    /* LENGUA:  Hay mucha jerga aquí, que es difícil de traducir. */
    if (Hallucination) {
	if (!obj->spe) {
	    /* LENGUA:  "funky" -> loco? */
	    pline("No ves nada más que una loca niebla %s.",
		  makeplural(feminize(hcolor((char *)0), TRUE)));
	} else {
	    switch(rnd(6)) {
	    /* LENGUA:  "Grok", un neologismo de Heinlein, se traduce a
	       "groquear" aquí */
	    /* LENGUA:  "groovy" -> ? */
	    case 1 : pline("Groqueas algunos glóbulitos de lava incandescente.");
		break;
	    /* LENGUA:  babe -> bebé; dude -> hombre */
	    case 2 : pline("¡Whoa!  ¡Colores psiquedélicas, %s!",
			   poly_gender() == 1 ? "bebé" : "hombre");
		break;
	    case 3 : pline("¡El cristal pulsa con luz %s siniestra!",
			   feminize(hcolor((char *)0), TRUE));
		break;
	    case 4 : pline("Ves carpas doradas nadando arriba de piedras fluorescentes.");
		break;
	    case 5 : pline("Ves copitos de nieve dando vueltas alrededor de una granja en miniatura.");
		break;
	    default: pline("¡Oh vaya... como un calidoscopio!");
		break;
	    }
	    check_unpaid(obj);
	    obj->spe--;
    ...
    if (flags.verbose) pline("Puedes buscar un símbolo de objeto o monstruo.");
    ch = yn_function("¿Qué quieres buscar?", (char *)0, (char *)0, '\0');
    /* Don't filter out ' ' here; it has a use */
    if ((ch != def_monsyms[S_GHOST]) && index(quitchars,ch)) { 
	if (flags.verbose) pline(Never_mind);
	return;
    }
    pline("Miras en %s...", the(xname(obj)));
    nomul(-rnd(10));
    nomovemsg = "";
    if (obj->spe <= 0)
	pline("La visión es incierta.");
    else {
	int class;
	int ret = 0;
	...
	if (ret) {
		...
		pline("Ves al Mago de Yéndor contemplándote.");
	    else pline("La visión es incierta.");
	}
    }
    return;
}
...
genericptr_t num;
{
	...
	if(levl[zx][zy].typ == SDOOR || (levl[zx][zy].typ == DOOR &&
		...
		if(levl[zx][zy].doormask & D_TRAPPED) {
		    if(distu(zx, zy) < 3) b_trapped("puerta", 0);
		    else Norep("¡%s una explosión!",
				cansee(zx, zy) ? "Ves" :
				   (flags.soundok ? "Oyes" :
						"Sientes la sacudida de"));
...
openit()	/* returns number of things found and opened */
{
	...
	if(u.uswallow) {
		if (is_animal(u.ustuck->data)) {
			if (Blind) pline("¡Su boca se abre!");
			else pline("¡%s abre la boca!", Monnam(u.ustuck));
		}
...
struct trap *trap;
{
    ...
    pline("Encuentras %s.", an(defsyms[trap_to_defsym(tt)].explanation));
	...
	if(u.uswallow) {
		...
		find:		exercise(A_WIS, TRUE);
				if (!canspotmon(mtmp)) {
					...
					Sientes("un monstruo inadvertido!");
					map_invisible(x, y);
				    }
				} else if (!sensemon(mtmp))
				    pline("Encuentras %s.", al(a_monnam(mtmp)));
				return(1);
			    }
			    if(!canspotmon(mtmp)) {
				if (mtmp->mundetected &&
