...
int distance;
{
	...
	while (mtmp) {
		...
		if (canseemon(mtmp)) {
		    if (!could_see_mon)
			pline("Noticias %s, meneando con la música.",
			    a_monnam(mtmp));
		    else
			pline("%s se bloquea, y entonces menea con la música%s.",
			      Monnam(mtmp),
			      was_peaceful ? "" : ", y ahora parece más en calma");
		}
	    }
	    mtmp = mtmp->nmon;
	}
...
int distance;
{
	...
	while (mtmp) {
		...
		if (canseemon(mtmp))
		    pline(
		     "%s escucha alegramente la música, y entonces parece más en calma.",
			  Monnam(mtmp));
	    }
	    mtmp = mtmp->nmon;
	}
...
awaken_soldiers()
{
	...
	while(mtmp) {
		...
		mtmp->mcanmove = 1;
		if (canseemon(mtmp))
		    pline("¡%s ahora está list%c para batalla!", Monnam(mtmp),
			mon_gender(mtmp)? 'a' : 'o');
		else
		    Norep("Oyes el traqueteo de aperos de batalla siendo listados.");
	    }
	    mtmp = mtmp->nmon;
	}
}
...
int force;
{
	...
	for (x=start_x; x<=end_x; x++) for (y=start_y; y<=end_y; y++) {
		...
		if (mtmp->mundetected && is_hider(mtmp->data)) {
		    mtmp->mundetected = 0;
		    if (cansee(x,y))
			pline("¡%s se sacude libre del techo!",
							    Amonnam(mtmp));
		    else
			Oyes("un sonido de golpazo.");
		    if (x==u.ux && y==u.uy) {
			char *mon = mon_nam_acc(mtmp, "evitas");
			pline("Facilmente %s%s.", mon,
			    strncmp(mon, "lo ", 3)==0? "" : " cayente");
		    }
		    newsym(x,y);
		}
	    }
	    if (!rn2(14 - force)) switch (levl[x][y].typ) {
		  case FOUNTAIN : /* Make the fountain disappear */
			if (cansee(x,y))
				pline("La fuente se cae en un abismo.");
			goto do_pit;
#ifdef SINKS
		  case SINK :
			if (cansee(x,y))
				pline("El fregadero se cae en un abismo.");
			goto do_pit;
#endif
		  case ALTAR :
			...
			if (cansee(x,y))
				pline("El altar se cae en un abismo.");
			goto do_pit;
		  case GRAVE :
			if (cansee(x,y))
				pline("La lápida se tumba en un abismo.");
			goto do_pit;
		  case THRONE :
			if (cansee(x,y))
				pline("El trono se cae en un abismo.");
...
do_pit:		    chasm = maketrap(x,y,PIT);
			...
			if (cansee(x, y))
			   pline("¡CADUM! La roca enorme se cae en un abismo%s!",
			      ((x == u.ux) && (y == u.uy)) ? " bajo ti" : "");
			if (mtmp)
				mtmp->mtrapped = 0;
			...
			if(!is_flyer(mtmp->data) && !is_clinger(mtmp->data)) {
			    mtmp->mtrapped = 1;
			    if(cansee(x,y))
				pline("¡%s se cae en un abismo!", Monnam(mtmp));
			    else if (flags.soundok && humanoid(mtmp->data))
				Oyes("un grito!");
			    mselftouch(mtmp, "Cayendo, ", TRUE);
			    if (mtmp->mhp > 0)
				if ((mtmp->mhp -= rnd(6)) <= 0) {
				    if(!cansee(x,y))
					pline("¡Es destruid%c!",
					...
					pline("¡Destruyes %s!", al(mtmp->mtame ?
					    al(x_monnam(mtmp, ARTICLE_THE, "pobrecito",
			    ...
			    if (Levitation || Flying ||
				    ...
				    pline("¡Un abismo se abre bajo ti!");
				    pline("¡No te caes!");
			    } else {
				    pline("¡Te caes en un abismo!");
				    u.utrap = rn1(6,2);
				    u.utraptype = TT_PIT;
				    losehp(rnd(6),"se cayó en un abismo",
					NO_KILLER_PREFIX);
				    selftouch("Cayendo,");
			    }
		    } else newsym(x,y);
...
# endif
	...
	if (!do_spec)
	    pline("Lo que produces es bastante lejos de música...");
	else
	    pline("Empiezas a tocar %s.", the(xname(instr)));
	...
	case WOODEN_FLUTE:		/* May charm snakes */
	    ...
	    pline("%s.", Tobjnam(instr, do_spec ? "trina" : "silba"));
	...
	case FIRE_HORN:			/* Idem wand of fire */
		...
		if (!getdir((char *)0)) {
		    pline("%s.", Tobjnam(instr, "vibra"));
		    break;
		} else if (!u.dx && !u.dy && !u.dz) {
		    if ((damage = zapyourself(instr, TRUE)) != 0) {
			losehp(damage, "usar un cuerno mágico contra sí mismo",
                               KILLED_BY);
		    }
		} else {
	...
	case TOOLED_HORN:		/* Awaken or scare monsters */
	    pline("Produces un sonido espantoso y grave.");
	    awaken_monsters(u.ulevel * 30);
	    exercise(A_WIS, FALSE);
	    break;
	case BUGLE:			/* Awaken & attract soldiers */
	    pline("Extraes un ruido fuerte %s.", del(the(xname(instr))));
	    awaken_soldiers();
	    exercise(A_WIS, FALSE);
	    break;
	case MAGIC_HARP:		/* Charm monsters */
		...
		pline("%s música muy atractiva.", Tobjnam(instr, "produce"));
	...
	case WOODEN_HARP:		/* May calm Nymph */
	    ...
	    pline("%s %s.", The(xname(instr)),
		  do_spec ? "produce una melodia ondulante" : "hace tañidos");
	...
	case DRUM_OF_EARTHQUAKE:	/* create several pits */
		...
		pline("¡Produces un poderoso, estruendoso retumbo!");
		pline("¡Todo el calabozo se sacude alrededor de ti!");
	...
	case LEATHER_DRUM:		/* Awaken monsters */
	    pline("¡Suenas un redoble estruendoso!");
	    awaken_monsters(u.ulevel * 40);
	    exercise(A_WIS, FALSE);
	    break;
	default:
	    impossible("¡Qué instrumento extraño (%d)!", instr->otyp);
	    break;
	}
...
struct obj *instr;
{
    ...
    if (Underwater) {
	No_puedes("tocar la música bajo el agua!");
	return(0);
    }
    ...
    if (c == 'n') {
	if (u.uevent.uheard_tune == 2 && yn("¿Tocar la tonada clave?") == 'y') {
	    Strcpy(buf, tune);
	} else {
	    getlin("¿Qué tonada tocas? [5 notas, A-G]", buf);
	    (void)mungspaces(buf);
	...
	pline("¡Extraes un sonido extraño %s!", del(the(xname(instr))));
#ifdef UNIX386MUSIC
	...
	if(Is_stronghold(&u.uz)) {
		...
		if(ok) { /* There is a drawbridge near */
		    ...
		    if(tumblers)
			if(gears)
			    Oyes("%s golpecito%s de %d voltero%s y la%s vuelta%s de %d rueda%s.",
				tumblers>1L? "los" : "el", plur(tumblers),
				tumblers, plur(tumblers),
				plur(gears), plur(gears),
				gears, plur(gears));
			else
			    Oyes("%s golpecito%s de %d voltero%s.",
				tumblers>1L? "los" : "el", plur(tumblers),
				tumblers, plur(tumblers));
		    else if(gears) {
			Oyes("la%s vuelta%s de %d rueda%s.",
			    plur(gears), plur(gears),
			    gears, plur(gears));
