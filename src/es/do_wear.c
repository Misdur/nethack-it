...
static NEARDATA const char see_yourself[] = "verte";
static NEARDATA const char unknown_type[] = "Clase desconocido de %s (%d)";
static NEARDATA const char c_armor[]  = "armadura",
			   c_suit[]   = "traje",
#ifdef TOURIST
			   c_shirt[]  = "camisa",
#endif
...
register struct obj *otmp;
{
	if (flags.verbose) {
	    ...
	    how[0] = '\0';
	    if (otmp->otyp == TOWEL)
		Sprintf(how, " sobre tu %s", body_part(HEAD));
	    pline("Ahora llevas %s%s.",
...
Boots_on()
{
	...
	case SPEED_BOOTS:
		...
		if (!oldprop && !(HFast & TIMEOUT)) {
			...
			Te_sientes("%smás rapid%c.",
				(oldprop || HFast) ? "un poco " : "",
				poly_gender()==1? 'a' : 'o');
		}
		break;
	case ELVEN_BOOTS:
		if (!oldprop && !HStealth && !BStealth) {
			makeknown(uarmf->otyp);
			pline("Te mueves muy silenciosamente.");
		}
		break;
	case FUMBLE_BOOTS:
...
Boots_off()
{
	...
	case ELVEN_BOOTS:
		if (!oldprop && !HStealth && !BStealth && !cancelled_don) {
			makeknown(otyp);
			pline("Eres ruidos%c por cierto.",
				poly_gender()==1? 'a' : 'o');
		}
		break;
	case FUMBLE_BOOTS:
...
Cloak_on()
{
	...
	case MUMMY_WRAPPING:
		...
		if ((HInvis || EInvis || pm_invisible(youmonst.data)) && !Blind) {
		    newsym(u.ux,u.uy);
		    pline(See_invisible ? "¡Ya no puedes ver través de ti!"
			: "¡Puedes verte!");
		}
		break;
	case CLOAK_OF_INVISIBILITY:
		...
		if (!oldprop && !HInvis && !Blind) {
		    ...
		    newsym(u.ux,u.uy);
		    pline("De repente, %s.",
			See_invisible ? "puedes ver través de ti"
				      : "no puedes verte");
		}
		break;
	case OILSKIN_CLOAK:
		pline("%s muy estrechamente.", Tobjnam(uarmc, "te entalla"));
		break;
...
Cloak_off()
{
	...
	case CLOAK_OF_INVISIBILITY:
		if (!oldprop && !HInvis && !Blind) {
		    ...
		    newsym(u.ux,u.uy);
		    pline("De repente, %s.",
			See_invisible ? "ya no puedes ver través de ti"
				      : "puedes verte");
		}
		break;
	/* Alchemy smock gives poison _and_ acid resistance */
...
Helmet_on()
{
	...
	case DUNCE_CAP:
		if (!uarmh->cursed) {
		    const char *que = xname(uarmh);
		    if (Blind)
			pline("%s por un momento.", Tobjnam(uarmh, "vibra"));
		    else
			pline("%s %s por un momento.",
			      Tobjnam(uarmh, "alumbra"),
			      feminize(hcolor(NH_BLACK), isfeminine(que)));
		    curse(uarmh);
		}
		flags.botl = 1;		/* reveal new alignment or INT & WIS */
		if (Hallucination) {
		    pline("¡Me duele el cerebro!"); /* Monty Python's Flying Circus */
		} else if (uarmh->otyp == DUNCE_CAP) {
		    pline(	/* track INT change; ignore WIS */
		  ACURR(A_INT) <= (ABASE(A_INT) + ABON(A_INT) + ATEMP(A_INT)) ?
			     "Tienes ganas de sentarte en un rincón" : "Te sientes frívolo");
		} else {
		    pline("Tu mente oscila brevemente.");
...
Gloves_off()
{
	...
	pline("Blandes %s en tus %s desnud%cs.",
	      the(corpse_xname(uwep, TRUE)), makeplural(mano),
	      isfeminine(mano)? 'a' : 'o');
	...
	pline("Blandes %s en tu %s desnud%c.",
	      the(corpse_xname(uswapwep, TRUE)), mano,
	      isfeminine(mano)? 'a' : 'o');
...
Amulet_on()
{
	...
	case AMULET_OF_CHANGE:
		...
		if (orig_sex != poly_gender()) {
		    ...
		    pline("¡De repente, eres muy %s!", flags.female ? "femenina"
			: "masculino");
		    flags.botl = 1;
		} else
		    /* already polymorphed into single-gender monster; only
		       changed the character's base sex */
		    pline("No te sientes como tú mism%c.",
			  poly_gender()==1? 'a' : 'o');
		pline("¡El amuleto se desintegra!");
...
Amulet_off()
{
	...
	case AMULET_OF_MAGICAL_BREATHING:
		if (Underwater) {
		    ...
		    if (!breathless(youmonst.data) && !amphibious(youmonst.data)
						&& !Swimming) {
			pline("¡De repente inhalas una insaludable cantidad de agua!");
		    	(void) drown();
		    }
		    return;
		}
	...
	case AMULET_OF_STRANGULATION:
		if (Strangled) {
			pline("¡Puedes respirar más fácilmente!");
			Strangled = 0;
		}
		break;
	case AMULET_OF_RESTFUL_SLEEP:
...
register struct obj *obj;
{
	...
	case RIN_SEE_INVISIBLE:
		...
		if (Invis && !oldprop && !HSee_invisible &&
		    ...
		    newsym(u.ux,u.uy);
		    pline("¡De repente, eres transparente, pero estás ahí!");
...
boolean gone;
{
	...
	impossible("Extraño... No sabía que tuvieras este anillo.");
	...
	case RIN_SEE_INVISIBLE:
		...
		if (Invisible && !Blind) {
		    newsym(u.ux,u.uy);
		    pline("De repente, no puedes verte.");
		    makeknown(RIN_SEE_INVISIBLE);
		}
		break;
	case RIN_INVISIBILITY:
		if (!Invis && !BInvis && !Blind) {
		    newsym(u.ux,u.uy);
		    pline("Parece que tu cuerpo se materializa%s.",
			  See_invisible ? " completamente" : "..");
		    makeknown(RIN_INVISIBILITY);
		}
		break;
	case RIN_LEVITATION:
...
register struct obj *otmp;
{
	...
	if (Blind && !already_blind) {
	    changed = TRUE;
	    if (flags.verbose) pline("Ya no puedes ver.");
	...
	if (!armorpieces) {
		...
		if (uskin)
		    pline("¡%s a tu piel!",
				...
				? "Las escamas de dragón se unen"
				: "La cota de escamas de dragón se une");
		else
		    pline("No llevas ninguna armadura.%s", (iflags.cmdassist && 
				(uleft || uright || uamul || ublindf)) ?
			  "  Usa el comando 'R' para removerte accesorios." : "");
		return 0;
	}
	if (armorpieces > 1)
		otmp = getobj(clothes, "", "quitarte");
	if (otmp == 0) return(0);
...
#define MOREACC(x) if (x) { Accessories++; otmp = x; }
	...
	if(!Accessories) {
		pline("No llevas ningún accesorio.%s", (iflags.cmdassist &&
			    (uarm || uarmc ||
#ifdef TOURIST
			     uarmu ||
#endif
			     uarms || uarmh || uarmg || uarmf)) ?
		      "  Usa el comando 'T' para quitarte armadura." : "");
		return(0);
	}
	if (Accessories != 1) otmp = getobj(accessories, "", "removerte");
	if(!otmp) return(0);
	...
	if (otmp == uright || otmp == uleft) {
		...
		impossible("¿Quitarte acesorio extraño?");
	}
	return(1);
}
...
register struct obj *otmp;
{
	...
	if((otmp == uwep) ? welded(otmp) : (int)otmp->cursed) {
		pline("No puedes.  Está%s maldit%c%s.",
			(is_boots(otmp) || is_gloves(otmp) || otmp->quan > 1L)
			? "n" : "",
			isfeminine(aobjnam(otmp, (char *)0))? 'a' : 'o',
			(is_boots(otmp) || is_gloves(otmp) || otmp->quan > 1L)
			? "s" : "");
		otmp->bknown = TRUE;
		return(1);
...
register struct obj *otmp;
{
	...
	if(delay) {
		nomul(delay);
		if (is_helmet(otmp)) {
			nomovemsg = "Acabas de quitarte el yelmo.";
			afternmv = Helmet_off;
		     }
		else if (is_gloves(otmp)) {
			nomovemsg = "Acabas de quitarte los guantes.";
			afternmv = Gloves_off;
		     }
		else if (is_boots(otmp)) {
			nomovemsg = "Acabas de quitarte las botas.";
			afternmv = Boots_off;
		     }
		else {
			nomovemsg = "Acabas de quitarte el traje.";
			afternmv = Armor_off;
		}
	} else {
...
const char *cc1, *cc2;
{
	No_puedes("ponerte %s porque ya llevas %s ahí.", cc1, cc2);
}
...
boolean noisy;
{
	...
	if (noisy) pline("%s no entalla tu cuerpo.", the(which));
	return 0;
	...
	if (noisy)
	    No_puedes("hacer esto mientras retienes tu %s.",
	...
	if (uarmh) {
	    ...
	    if (noisy)
		pline("%s no encajará tu%s cuerno%s.",
			  The(c_helmet),
	...
	if (uarms) {
	    ...
	    if (noisy) 
		No_puedes("vestir un escudo mientras blandas un %s de dos manos.",
		    is_sword(uwep) ? c_sword :
		    (uwep->otyp == BATTLE_AXE) ? c_axe : c_weapon);
	    err++;
	} else if (u.twoweap) {
	    if (noisy)
		No_puedes("vestir un escudo mientras blandas dos armas.");
	    err++;
	} else
	    *mask = W_ARMS;
    } else if (is_boots(otmp)) {
	if (uarmf) {
	    ...
	    if (noisy) pline("No tienes pies...");	/* not body_part(FOOT) */
	    err++;
	} else if (Upolyd && youmonst.data->mlet == S_CENTAUR) {
	    /* break_armor() pushes boots off for centaurs,
	       so don't let dowear() put them back on... */
	    if (noisy) pline("Tienes demasiadas pesuñas para vestir %s.",
		...
		if (noisy) {
		    ...
		    pline("¡Tu %s está entrampad%c!", pie, isfeminine(pie)? 'a' : 'o');
		}
	    } else {
		if (noisy) {
		    const char *pie = body_part(FOOT);
		    pline("¡Tus %s están aprisionad%cs en %s!",
			 makeplural(pie),
			 isfeminine(pie)? 'a' : 'o',
			 surface(u.ux, u.uy));
		}
	...
	if (uarmg) {
	    ...
	    if (noisy) No_puedes("vestir guantes sobre tu %s.",
			   is_sword(uwep) ? c_sword : c_weapon);
	    err++;
	} else
	    *mask = W_ARMG;
	...
	if (uarm || uarmc || uarmu) {
		...
		if (noisy) No_puedes("vestir esto sobre tu %s.",
	...
	if (uarmc) {
	    const char *name = cloak_simple_name(uarmc);
	    if (noisy) No_puedes("vestir armadura sobre un%s %s.",
		                 isfeminine(name)? "a" : "", name);
	    err++;
	} else if (uarm) {
	    if (noisy) already_wearing("armadura");
	    err++;
	} else
	    *mask = W_ARM;
    } else {
	...
	if (noisy) silly_thing("vestir", otmp);
	err++;
    }
...
dowear()
{
	...
	if ((verysmall(youmonst.data) || nohands(youmonst.data))) {
		pline("No te molestes.");
		return(0);
	}

	otmp = getobj(clothes, "", "vestir");
	if(!otmp) return(0);
	...
	if (otmp->otyp == HELM_OF_OPPOSITE_ALIGNMENT &&
		...
		if (u.ualignbase[A_CURRENT] == u.ualignbase[A_ORIGINAL])
			pline("Por poco pierdes toda oportunidad de alcanzar tu objetivo.");
		else	/* converted */
			pline("Vergüenza te alcanza de repente y cambias de idea.");
		u.ublessed = 0; /* lose your god's protection */
		makeknown(otmp->otyp);
		flags.botl = 1;
		return 1;
	...
	if(delay){
		...
		nomovemsg = "Acabas de tu maniobra de vestuario.";
	} else {
...
doputon()
{
	...
	if(uleft && uright && uamul && ublindf) {
		...
		pline("Tus %s%s están llen%cs, y ya llevas un amuleto y %s.",
			dedos,
			humanoid(youmonst.data) ? " anulares" : "",
			isfeminine(dedos) ? 'a' : 'o',
			ublindf->otyp==LENSES ? "anteojos" : "una venda");
		return(0);
	}
	otmp = getobj(accessories, "", "ponerte");
	if(!otmp) return(0);
	if(otmp->owornmask & (W_RING | W_AMUL | W_TOOL)) {
	...
	if(otmp->oclass == RING_CLASS || otmp->otyp == MEAT_RING) {
		if(nolimbs(youmonst.data)) {
			No_puedes("hacer que el anillo se adhiera a tu cuerpo.");
			return(0);
		}
		if(uleft && uright){
			const char *dedos = makeplural(body_part(FINGER));
			pline("No hay otr%cs %s%s para llenar.",
				isfeminine(dedos)? 'a' : 'o',
				dedos,
				humanoid(youmonst.data) ? " anulares" : "");
			return(0);
		...
		if(uleft) mask = RIGHT_RING;
			...
			char fem = isfeminine(dedo)? 'a' : 'o';

			Sprintf(qbuf, "¿Qué %s%s, Derech%c o Izquierd%c?",
				dedo,
				humanoid(youmonst.data) ? " anular" : "",
				fem, fem);
			if(!(answer = yn_function(qbuf, "di", "lr", '\0')))
				return(0);
			switch(answer){
			case 'l':
			case 'L':
		...
		if (uarmg && uarmg->cursed) {
			uarmg->bknown = TRUE;
		    No_puedes("quitarte los guantes para ponerte el anillo.");
			return(0);
		}
		if (welded(uwep) && bimanual(uwep)) {
			/* welded will set bknown */
	    No_puedes("liberar tus manos de arma para ponerte el anillo.");
			return(0);
		}
		if (welded(uwep) && mask==RIGHT_RING) {
			/* welded will set bknown */
	    No_puedes("liberar una mano de arma para ponerte el anillo.");
			return(0);
		}
		if (otmp->oartifact && !touch_artifact(otmp, &youmonst))
		    return 1; /* costs a turn even though it didn't get worn */
		...
		if (ublindf) {
			if (ublindf->otyp == TOWEL) {
				...
				pline("Tu %s ya está cubiert%c por una toalla.",
					cara, isfeminine(cara)? 'a' : 'o');
			} else if (ublindf->otyp == BLINDFOLD) {
				if (otmp->otyp == LENSES)
					already_wearing2("anteojos", "una venda");
				else
					already_wearing("una venda");
			} else if (ublindf->otyp == LENSES) {
				if (otmp->otyp == BLINDFOLD)
					already_wearing2("una venda", "anteojos");
				else
					already_wearing("anteojos");
			} else
				already_wearing(something); /* ??? */
			return(0);
		}
		if (otmp->otyp != BLINDFOLD && otmp->otyp != TOWEL && otmp->otyp != LENSES) {
			No_puedes("ponerte esto!");
			return(0);
		}
		if (otmp->oartifact && !touch_artifact(otmp, &youmonst))
		    return 1;
...
glibr()
{
	...
	if (!uarmg && (leftfall || rightfall) && !nolimbs(youmonst.data)) {
		...
		if (leftfall && rightfall)
		    pline("Tus anillos se resbalan de tus %s.",
			    makeplural(body_part(FINGER)));
		else
		    pline("Tu anillo se resbala de tu %s.", body_part(FINGER));
		xfl++;
		if (leftfall) {
			otmp = uleft;
			Ring_off(uleft);
	...
	if (u.twoweap && otmp) {
		...
		pline("Tu %s %sse resbala de tus %s.",
			otherwep,
			xfl ? "también " : "",
	...
	if (otmp && !welded(otmp)) {
		...
		pline("Tu %s%s %sse resbala de tus %s.",
			otherwep ? (isfeminine(thiswep)? "otra " : "otro ")
				 : "",
			thiswep,
			xfl ? "también " : "",
			makeplural(body_part(HAND)));
...
int otyp;
{
	...
	impossible("stuck_ring: ¿ni izquierdo ni derecho?");
	return (struct obj *)0;
    }
...
register struct obj *otmp;
{
	...
	*buf = '\0';			/* lint suppresion */
	...
	if (otmp == uright || otmp == uleft) {
	    if (nolimbs(youmonst.data)) {
		pline("El anillo está pegado.");
		return 0;
	    }
	    why = 0;	/* the item which prevents ring removal */
	    if (welded(uwep) && (otmp == uright || bimanual(uwep))) {
		Sprintf(buf, "liberar %s de arma", an(body_part(HAND)));
		why = uwep;
	    } else if (uarmg && uarmg->cursed) {
		Sprintf(buf, "quitarte %s", the(c_gloves));
		why = uarmg;
	    }
	    if (why) {
		No_puedes("%s para removerte el anillo.", buf);
		why->bknown = TRUE;
		return 0;
	    }
	}
	...
	if (otmp == uarmg) {
	    if (welded(uwep)) {
		No_puedes("quitarte los %s mientras blandes esta %s.\n",
		    c_gloves, is_sword(uwep) ? c_sword : c_weapon);
		uwep->bknown = TRUE;
		return 0;
	    } else if (Glib) {
		const char *dedo = body_part(FINGER);
		No_puedes("quitarte los %s resbalosos con tus %s resbalos%cs.",
			 c_gloves, makeplural(dedo),
			 isfeminine(dedo)? 'a' : 'o');
		return 0;
	    }
	}
	...
	if (otmp == uarmf) {
		...
		pline("La trampa para osos te impide sacarte tu %s.",
			  the(body_part(FOOT)));
		return 0;
	    } else if (u.utrap && u.utraptype == TT_INFLOOR) {
		pline("Estás aprisionad%c en %s, y no puedes sacar tus %s.",
		      poly_gender()==1? 'a' : 'o', surface(u.ux, u.uy),
	...
	if (otmp == uarm
		...
		Sprintf(buf, "quitarte %s", the(cloak_simple_name(uarmc)));
		why = uarmc;
#ifdef TOURIST
	    } else if (otmp == uarmu && uarm && uarm->cursed) {
		Sprintf(buf, "quitarte %s", the(c_suit));
		why = uarm;
#endif
	    } else if (welded(uwep) && bimanual(uwep)) {
		Sprintf(buf, "soltar tu %s",
			is_sword(uwep) ? c_sword :
			(uwep->otyp == BATTLE_AXE) ? c_axe : c_weapon);
		why = uwep;
	    }
	    if (why) {
		No_puedes("%s para quitarte %s.", buf, the(xname(otmp)));
		why->bknown = TRUE;
		return 0;
	    }
	}
...
do_takeoff()
{
	...
	if (taking_off == W_WEP) {
	  if(!cursed(uwep)) {
	    ...
	    setuwep((struct obj *) 0);
	    pline("Tienes %s vací%cs.", manos,
		  isfeminine(manos)? 'a' : 'o');
	    u.twoweap = FALSE;
	  }
	} else if (taking_off == W_SWAPWEP) {
	  setuswapwep((struct obj *) 0);
	  pline("Ya no tienes una segunda arma preparada.");
	  u.twoweap = FALSE;
	} else if (taking_off == W_QUIVER) {
	  setuqwep((struct obj *) 0);
	  pline("Ya no tienes municiónes listas.");
	...
	} else impossible("do_takeoff: quitándote %lx", taking_off);
...
take_off()
{
	...
	if (taking_off == 0L) {
	  ...
	  impossible("take_off: quitándote %lx", taking_off);
	  return 0;	/* force done */
	}
...
doddoremarm()
{
	...
	/* LENGUA:  need a gerund here */
	pline("Continúas %s.", disrobing);
	set_occupation(take_off, disrobing, 0);
	return 0;
    } else if (!uwep && !uswapwep && !uquiver && !uamul && !ublindf &&
		!uleft && !uright && !wearing_armor()) {
	pline("No llevas ningún accesorio.");
	return 0;
    }

    add_valid_menu_class(0); /* reset */
    if (flags.menu_style != MENU_TRADITIONAL ||
	    (result = ggetobj("quitarte", select_off, 0, FALSE, (unsigned *)0)) < -1)
	result = menu_remarm(result);

    if (takeoff_mask) {
	/* default activity for armor and/or accessories,
	   possibly combined with weapons */
	disrobing = "desvestirte";
	/* specific activity when handling weapons only */
	if (!(takeoff_mask & ~(W_WEP|W_SWAPWEP|W_QUIVER)))
	    disrobing = "desarmarte";
	(void) take_off();
    }
...
int retry;
{
	...
	n = query_category("¿Qué tipo de cosas quieres quitarte?",
	...
	all_worn_categories = FALSE;
	if (ggetobj("quitarte", select_off, 0, TRUE, (unsigned *)0) == -2)
	    all_worn_categories = TRUE;
    }

    n = query_objlist("¿Qué quieres quitarte?", invent,
	...
	pline("No hay nada más que puedas quitarte o dejar de esgrimir.");
    }
    return 0;
}
...
#define DESTROY_ARM(o) ((otmp = (o)) != 0 && \
	...
	if (DESTROY_ARM(uarmc)) {
		...
		pline("¡Tu %s se desmenuza y se vuelve polvo!",
		     cloak_simple_name(uarmc));
		(void) Cloak_off();
		useup(otmp);
	} else if (DESTROY_ARM(uarm)) {
		if (donning(otmp)) cancel_don();
		pline("¡Tu armadura se vuelve polvo y cae %s!",
		...
		if (donning(otmp)) cancel_don();
		pline("¡Tu camisa se desmenuza en hilachas y se despedaza!");
		(void) Shirt_off();
		useup(otmp);
#endif
	} else if (DESTROY_ARM(uarmh)) {
		if (donning(otmp)) cancel_don();
		pline("¡Tu yelmo se vuelve polvo y se vuela!");
		(void) Helmet_off();
		useup(otmp);
	} else if (DESTROY_ARM(uarmg)) {
		if (donning(otmp)) cancel_don();
		pline("¡Tus guantes se desvanecen!");
		(void) Gloves_off();
		useup(otmp);
		selftouch("Tú");
	} else if (DESTROY_ARM(uarmf)) {
		if (donning(otmp)) cancel_don();
		pline("¡Tus botas se desintegran!");
		(void) Boots_off();
		useup(otmp);
	} else if (DESTROY_ARM(uarms)) {
		if (donning(otmp)) cancel_don();
		pline("¡Tu escudo se desmenuza!");
		(void) Shield_off();
		useup(otmp);
	} else {
