...
amulet()
{
	...
	if ((((amu = uamul) != 0 && amu->otyp == AMULET_OF_YENDOR) ||
		...
		if(ttmp->ttyp == MAGIC_PORTAL) {
		    const char *que = xname(amu);
		    int du = distu(ttmp->tx, ttmp->ty);
		    if (du <= 9)
			pline("¡%s caliente!", Tobjnam(amu, "siente"));
		    else if (du <= 64)
			pline("%s muy cálid%c.", Tobjnam(amu, "siente"),
				isfeminine(que)? 'a' : 'o');
		    else if (du <= 144)
			pline("%s cálid%c.", Tobjnam(amu, "siente"),
				isfeminine(que)? 'a' : 'o');
	...
	for(mtmp = fmon; mtmp; mtmp = mtmp->nmon)
		...
		if (distu(mtmp->mx,mtmp->my) > 2)
		    pline(
    "Tienes el sentido hormigueoso que alguien noticiaba que tomaste el Amuleto."
		    );
		return;
	    }
}
	...
	switch (strat) {
		...
		if(where == STRAT_GROUND) {
			...
			if ((otmp = on_ground(which_arti(targ))) != 0) {
				...
				pline("%s %s.",
				    Mon_verb_mon(mtmp, "recoge", (struct monst *)0),
...
resurrect()
{
	...
	if (mtmp) {
		...
		set_malign(mtmp);
		pline("Una voz resone...");
		verbalize("Así que creíais que pudisteis %sme, tont%c.", verb,
			poly_gender()==1? 'a' : 'o');
	}
...
intervene()
{
	...
	switch (which) {
	    case 0:
	    case 1:	Te_sientes("vagamente nervos%c.",
				poly_gender()==1? 'a' : 'o');
			break;
	    case 2:	if (!Blind)
			    pline("Noticias una incandescencia %s alrededor de ti.",
...
/* LENGUA: Gender inflections are marked as follows:
                masculine   feminine
   @                o          a
   #             (none)        a
   &                e          a
   *               ón         ona
   More may be defined if needed.  */
const char * const random_insult[] = {
#if 0 /* The original English */
	"antic",
	"blackguard",
	"caitiff",
	"chucklehead",
...
/* LENGUA:  To accommodate adjectives that follow the noun, these are converted
   into format strings with a single %s.  The gender inflection markers for
   random_insult may also be used here. */
const char *random_malediction[] = {
	"¡El infierno recibirá pronto vuestros restos, %s!",
	"¡Os río, tu %s patétic@!",
	"¡Preparad para morir, %s!",
	"¡Resistancia es inútil, %s!",
	"¡Rendeos o morid, %s!",
	"¡No habrá misericordia, %s!",
	"¡Os arrepentiréis de vuestra astucia, %s!",
	"¡A mí sois como una pulga, %s!",
	"¡Estáis condenad@, %s!",
	"¡Vuestro destino está cerrada, %s!",
	"¡De verdad, seráis un# %s muert@!"
};
...
register struct monst	*mtmp;
{
	if (mtmp->iswiz) {
		...
		pline("%s se ría diabólicamente.", Monnam(mtmp));
	    else {
		...
		if (u.uhave.amulet && !rn2(SIZE(random_insult)))
		    sprintf(buf, "¡Abandona el amuleto, %s!",
			  random_insult[rn2(SIZE(random_insult))]);
		else if (u.uhp < 5 && !rn2(2))	/* Panic */
		    sprintf(buf, rn2(2) ?
			  "¡Ya ahora tu fuerza de vida decae, %s!" :
			  "¡Disfruta tu respira, %s, porque es tu última!",
			  random_insult[rn2(SIZE(random_insult))]);
		...
		for (i=0; buf[i]!=0; i++) {
		    ...
		    /* LENGUA:  "¡Estás condenada, bribna" isn't right */
		    case '*':
			if (fem) {
			    strcpy(buf+j, "ona");
			    j += 3;
	    ...
	    if (!rn2(5))
		pline("%s insulta a tus ancestros.", Monnam(mtmp));
	    else
