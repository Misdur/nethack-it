...
const char	* const plusattr[] = {
	"fuerte", "inteligente", "sabio", "ágil", "duro", "carismático"
},
		* const minusattr[] = {
	"débil", "estúpido", "tonto", "torpe", "frágil", "repulsivo"
};
...
const struct innate {
	...
	const char *gainstr, *losestr;
		     ...
		     {   1, &(HFast), "", "" },
		     {  10, &(HSearching), "perceptivo", "" },
		     {	 0, 0, 0, 0 } },

	bar_abil[] = { {	 1, &(HPoison_resistance), "", "" },
		     {   7, &(HFast), "rápido", "lento" },
		     {  15, &(HStealth), "furtivo", "" },
		     {	 0, 0, 0, 0 } },

	cav_abil[] = { {	 7, &(HFast), "rápido", "lento" },
	...
	mon_abil[] = { {   1, &(HFast), "", "" },
		     ...
		     {   9, &(HSearching), "perceptivo", "desatento" },
		     {  11, &(HFire_resistance), "fresco", "más tibio" },
		     {  13, &(HCold_resistance), "tibio", "más fresco" },
		     {  15, &(HShock_resistance), "aislado", "conductivo" },
		     {  17, &(HTeleport_control), "controlado","descontrolado" },
		     {   0, 0, 0, 0 } },

	pri_abil[] = { {	15, &(HWarning), "sensitivo", "" },
		     {  20, &(HFire_resistance), "fresco", "más tibio" },
		     {	 0, 0, 0, 0 } },
	...
	tou_abil[] = { {	10, &(HSearching), "perceptivo", "" },
		     {	20, &(HPoison_resistance), "robusto", "" },
		     {	 0, 0, 0, 0 } },
	...
	elf_abil[] = { {	4, &(HSleep_resistance), "despierto", "cansado" },
		     {	 0, 0, 0, 0 } },
...
{			    /* negative => conditional (msg if change made) */
	...
	if ((ndx == A_INT || ndx == A_WIS)
		...
		if (msgflg == 0)
		    pline("Tu gorra te aprieta brevemente, y después se relaja.");
		return FALSE;
	}
	...
	if (incr > 0) {
		...
		if (msgflg == 0 && flags.verbose)
		    pline("Ya eres tan %s como puedes ser.",
			  feminize(plusattr[ndx], poly_gender()==1));
		...
		if (msgflg == 0 && flags.verbose)
		    pline("Ya eres tan %s como puedes ser.",
			  feminize(minusattr[ndx], poly_gender()==1));
	...
	if (msgflg <= 0)
	    Te_sientes("%s%s!",
		  (incr > 1 || incr < -1) ? "muy ": "",
		  feminize((incr > 0) ? plusattr[ndx] : minusattr[ndx],
			   poly_gender()==1));
	flags.botl = 1;
...
exerper()
{
	if(!(moves % 10)) {
...
#ifdef DEBUG
		pline("exerper: Inspecciones de hambre");
#endif
		switch (hs) {
		...
		/* Encumberance Checks */
#ifdef DEBUG
		pline("exerper: Inspecciones de impediencia");
#endif
	...
	if(!(moves % 5)) {
#ifdef DEBUG
		pline("exerper: Inspecciones de estado");
#endif
...
#ifdef DEBUG
	if(moves >= next_check)
		pline("exerchk: listo para probar. multi = %d.", multi);
#endif
	/*	Are we ready for a test?	*/
	if(moves >= next_check && !multi) {
#ifdef DEBUG
	    pline("exerchk: probando.");
#endif
	    /*
...
#ifdef DEBUG
		/* LENGUA:  Need to change Str etc. */
		pline("exerchk: probando %s (%d).",
			(i == A_STR) ? "Fue" : (i == A_WIS) ? "Sab" :
			(i == A_DEX) ? "Des" : "Con", AEXE(i));
#endif
		/*
...
#ifdef DEBUG
		pline("exerchk: cambiando %d.", i);
#endif
		if(adjattrib(i, mod_val, -1)) {
#ifdef DEBUG
		    pline("exerchk: cambiado %d.", i);
#endif
		    ...
		    case A_STR: pline((mod_val >0) ?
				    "Debes de haber ejercitado." :
				    "Debes de haber abusado de tu cuerpo.");
				break;
		    case A_WIS: if (mod_val >0)
				     pline("Debes de haber sido muy observador%s.",
                                              poly_gender()==1? "a" : "");
				else pline("No has prestado atención.");
				break;
		    case A_DEX: pline((mod_val >0) ?
				    "Debes de haber trabajado de tus reflejos." :
				    "No has trabajado de reflejos últimamente.");
				break;
		    case A_CON: pline((mod_val >0) ?
				    "Debes de haber llevado un estilo de vida saludable." :
				    "No has cuidado de tu salud.");
				break;
		    }
		}
	    }
...
#ifdef DEBUG
	    pline("exerchk: próxima inspección a %ld.", next_check);
#endif
	}
}
	...
	while (abil || rabil) {
		...
		if(oldlevel < abil->ulevel && newlevel >= abil->ulevel) {
			...
			if(!(*(abil->ability) & INTRINSIC & ~mask)) {
			    if(*(abil->gainstr))
				Te_sientes("%s!", feminize(abil->gainstr, poly_gender()==1));
			}
		} else if (oldlevel >= abil->ulevel && newlevel < abil->ulevel) {
			*(abil->ability) &= ~mask;
			if(!(*(abil->ability) & INTRINSIC)) {
			    if(*(abil->losestr))
				Te_sientes("%s!", feminize(abil->losestr, poly_gender()==1));
			    else if(*(abil->gainstr))
				Te_sientes("menos %s!", feminize(abil->gainstr, poly_gender()==1));
			}
		}
