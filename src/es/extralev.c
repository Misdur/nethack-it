	...
	if (dir==DOWN) {
		...
		if (!r[x][y].real) {
			...
			if (!IS_WALL(levl[fromx][fromy].typ))
				impossible("abajo: ¿ninguna pared a %d,%d?",fromx,
									fromy);
		...
		if(y >= 2) {
			impossible("¿pared abajo de %d,%d va a ninguna parte?",x,y);
			return;
		}
		y++;
		r[x][y].doortable &= ~UP;
		if (!r[x][y].real) {
			...
			if (!IS_WALL(levl[tox][toy].typ))
				impossible("arriba: ¿ninguna pared a %d,%d?",tox,toy);
		...
		if(x >= 2) {
			impossible("¿pared derecha a %d,%d va a ninguna parte?",x,y);
			return;
		}
		x++;
		...
		if (!r[x][y].real) {
			...
			tox += 1 + 26*x; toy += 7*y;
			if (!IS_WALL(levl[tox][toy].typ))
				impossible("izquierda: ¿ninguna pared a %d,%d?",tox,toy);
			dodoor(tox, toy, &rooms[r[x][y].nroom]);
			levl[tox][toy].doormask = D_NODOOR;
			tox--;
		}
		...
		return;
	} else impossible("¿corredor en dirección %d?",dir);
}
...
#define here r[x][y]
	...
	for(y=0; y<3; y++) for(x=0; x<3; x++) {
		...
		if (here.doortable & LEFT)
			impossible ("¿fin izquierdo de %d, %d nunca conectado?",x,y);
		if (here.doortable & UP)
			impossible ("¿fin alto de %d, %d nunca conectado?",x,y);
	}
}
