...
int shotlimit;
{
	...
	if(!canletgo(obj,"tirar"))
		return(0);
	if (obj->oartifact == ART_MJOLLNIR && obj != uwep) {
	    pline("%s debe blandirse antes de poder tirarse.",
		The(xname(obj)));
		return(0);
	}
	if ((obj->oartifact == ART_MJOLLNIR && ACURR(A_STR) < STR19(25))
	   || (obj->otyp == BOULDER && !throws_rocks(youmonst.data))) {
		pline("Es demasiado pesado.");
		return(1);
	}
	if(!u.dx && !u.dy && !u.dz) {
		No_puedes("tirarte un objeto a ti mism%c.", poly_gender()==1? 'a' : 'o');
		return(0);
	}
	u_wipe_engr(2);
	if (!uarmg && !Stone_resistance && (obj->otyp == CORPSE &&
		...
		pline("Tiras el cadáver de %s con tu %s desnud%c.",
		    mons[obj->corpsenm].mname, mano,
		    isfeminine(mano)? 'a' : 'o');
		Sprintf(killer_buf, "cadáver de %s", an(mons[obj->corpsenm].mname));
...
dothrow()
{
	...
	if (notake(youmonst.data)) {
	    pline("No eres físicamente capaz de tirar nada.");
	    return 0;
	}

	if(check_capacity((char *)0)) return(0);
	obj = getobj(uslinging() ? bullets : toss_objs, "", "tirar");
...
dofire()
{
	...
	if (notake(youmonst.data)) {
	    pline("No eres físicamente capaz de hacer eso.");
	    return 0;
	}
	...
	if (!uquiver) {
		if (!flags.autoquiver) {
			...
			pline("¡No tienes municiones listas!");
			return(dothrow());
		}
		autoquiver();
		if (!uquiver) {
			pline("¡No tienes nada apropiado para tu aljaba!");
			return(dothrow());
		} else {
			pline("Llenas tu aljaba:");
...
register struct obj *obj;
{
	...
	if (IS_ALTAR(levl[u.ux][u.uy].typ))
		...
		pline("%s aterriza%s en %s.", Doname2(obj),
		      (obj->quan == 1L) ? "" : "n", surface(u.ux,u.uy));
    ...
    int x, y;
{
    ...
    if (!isok(x,y)) {
	Sientes("a los espíritus reteniéndote.");
	return FALSE;
	...
	if (levl[x][y].typ == IRONBARS) {
	    pline("Te chocas con unas barras de hierro.  ¡Ouch!");
	    losehp(rnd(2+*range), "chocar con barras de hierro", KILLED_BY);
	    return FALSE;
	}
	if ((obj = sobj_at(BOULDER,x,y)) != 0) {
	    pline("Topas %s.  ¡Ouch!", al(an(xname(obj))));
	    losehp(rnd(2+*range), "topar una roca enorme", KILLED_BY);
	    return FALSE;
	}
	if (!may_pass) {
	    /* did we hit a no-dig non-wall position? */
	    pline("¡Te topas con algo!");
	    losehp(rnd(2+*range), "tocar el borde del universo", KILLED_BY);
	    return FALSE;
	}
	if ((u.ux - x) && (u.uy - y) &&
		...
		pline("%s en una hendedura.",
			too_much ? "Tú y todos tus bienes se apretujan" : "Te apretujas");
		losehp(rnd(2+*range), "apretujarse en una hendedura estrecha", KILLED_BY);
		return FALSE;
	    }
	}
    }
	...
	/* Move at a diagonal. */
	if (In_sokoban(&u.uz)) {
	    pline("¡Llegas a una parada repentina!");
	    return FALSE;
	}
    }
    ...
    if ((ttmp = t_at(x, y)) != 0) {
		...
		*range = 0;
		...
		if (ttmp->tseen)
		    pline("Pasas directamente sobre %s.",
    ...
    boolean verbose;
{
	...
	Sientes("un tirón de la bola de hierro.");
	nomul(0);
	return;
    } else if (u.utrap) {
	pline("Estás anclad%c por %s.",
	    poly_gender()==1? 'a' : 'o',
	    u.utraptype == TT_WEB ? "la telaraña" : u.utraptype == TT_LAVA ? "la lava" :
		u.utraptype == TT_INFLOOR ? surface(u.ux,u.uy) : "la trampa");
	nomul(0);
	return;
    }
    ...
    nomul(-range);
    if (verbose)
	pline("%s en la dirección opuesta.", range > 1 ? "Te lanzas" : "Flotas");
    /* if we're in the midst of shooting multiple projectiles, stop */
    if (m_shot.i < m_shot.n) {
	/* last message before hurtling was "you shoot N arrows" */
	pline("Paras de %s después del primer %s.",
	      m_shot.s ? "disparar" : "tirar",
	      m_shot.s ? "disparo"  : "tiro");
...
boolean hitsroof;
{
    ...
    if (hitsroof) {
	if (breaktest(obj)) {
		pline("%s topa %s.", Doname2(obj), ceiling(u.ux, u.uy));
		breakmsg(obj, !Blind);
    ...
    pline("%s%s choca contra %s, y entonces cae en tu %s.",
	...
	case CREAM_PIE:
	case BLINDING_VENOM:
		pline("¡Lo tienes sobre tod%c tu %s!",
			isfeminine(body_part(FACE))? 'a' : 'o',
			body_part(FACE));
		if (blindinc) {
 ...
 petrify:
		...
		killer = "física elemental";	/* "what goes up..." */
	...
	hitfloor(obj);
	losehp(dmg, "objeto cayente", KILLED_BY_AN);
    }
    return TRUE;
}
...
boolean twoweap; /* used to restore twoweapon mode if wielded weapon returns */
{
	...
	if ((obj->cursed || obj->greased) && (u.dx || u.dy) && !rn2(7)) {
		...
		pline("¡%s!", Tobjnam(obj, "maldispara"));
	    else {
		/* only slip if it's greased or meant to be thrown */
		if (obj->greased || throwing_weapon(obj))
		    /* BUG: this message is grammatically incorrect if obj has
		       a plural name; greased gloves or boots for instance. */
		    /* LENGUA:  The above remains true in Spanish */
		    pline("¡%s cuando l%c tiras!", Tobjnam(obj, "se resbala"),
	...
	if ((u.dx || u.dy || (u.dz < 1)) &&
	    ...
	    !Is_airlevel(&u.uz)) {
	    pline("Tienes tan poca vitalidad que %s se te cae.",
		the(xname(obj)));
	...
	if(u.uswallow) {
		...
		const char *ceil = ceiling(u.ux,u.uy);
		pline("¡%s %s y regresa a tu mano!",
		      Tobjnam(obj, "topa"), the(ceil));
		obj = addinv(obj);
		(void) encumber_msg();
		setuwep(obj);
		u.twoweap = twoweap;
	...
	if (u.uswallow) {
		...
		if (obj->oartifact == ART_MJOLLNIR &&
		    ...
		    if (!impaired && rn2(100)) {
			pline("¡%s a tu mano!", Tobjnam(obj, "regresa"));
			obj = addinv(obj);
			(void) encumber_msg();
			setuwep(obj);
			u.twoweap = twoweap;
			...
			int dmg = rn2(2);
			if (!dmg) {
			    pline(Blind ? "%s aterriza %s tus %s." :
					"%s a ti, aterrizando %s tus %s.",
				  Blind ? Something : Tobjnam(obj, "regresa"),
				  Levitation ? "bajo" : "a",
				  makeplural(body_part(FOOT)));
			} else {
			    dmg += rnd(3);
			    pline(Blind ? "¡%s tu %s!" :
					"¡%s para ti, golpeando tu %s!",
				  Tobjnam(obj, Blind ? "golpea" : "se vuela"),
				  body_part(ARM));
		...
		if(flooreffects(obj,bhitpos.x,bhitpos.y,"cae")) return;
...
struct monst *mon;
{
	...
	pline("%s %s.", The(missile), otense(obj, "falla"));
    else
	miss(missile, mon);
    if (!rn2(3)) wakeup(mon);
    return;
...
register struct obj   *obj;
{
	...
	if (uarmg && uwep && objects[uwep->otyp].oc_skill == P_BOW) {
		...
		break;
	    default:
		impossible("Clase desconocida de guantes (%d)", uarmg->otyp);
		break;
	    }
	}
	...
	if (obj->oclass == GEM_CLASS && is_unicorn(mon->data)) {
	    if (mon->mtame) {
		pline("%s atrapa y tira %s.", Monnam(mon), the(xname(obj)));
		return 0;
	    } else {
		pline("%s atrapa %s.", Monnam(mon), the(xname(obj)));
	...
	if (quest_arti_hits_leader(obj, mon)) {
		...
		if (mon->mpeaceful) {
		    ...
		    pline("%s te %s %s.", Monnam(mon), 
			  (next2u ? "devuelve" : "tira de vuelta"), the(xname(obj))); 
	...
	if (obj->oclass == WEAPON_CLASS || is_weptool(obj) ||
	    ...
	    pline("%s se desvanece en las %s %s.",
		The(xname(obj)),
		is_animal(u.ustuck->data) ? "entrañas" : "corrientes",
		del(mon_nam(mon)));
	} else {
	    tmiss(obj, mon);
	}
...
register struct obj *obj;
{
	...
	int ret = 0;
	static NEARDATA const char nogood[] = "A %s no le interesa tu basura.";
	static NEARDATA const char acceptgift[] = " acepta tu regalo.";
	static NEARDATA const char maybeluck[] = "%s con vacilación";
	static NEARDATA const char noluck[] = "%s graciosamente";
	static NEARDATA const char addluck[] = "%s agradecidamente";
	...
	if(obj->dknown && objects[obj->otyp].oc_name_known) {
		if(is_gem) {
			if(is_buddy) {
				Sprintf(buf,addluck,Monnam(mon));
				change_luck(5);
			} else {
				Sprintf(buf,maybeluck,Monnam(mon));
				change_luck(rn2(7)-3);
			}
		} else {
			Sprintf(buf,nogood,Monnam(mon));
			goto nopick;
		}
	/* making guesses */
			...
			if (obj->otyp == POT_OIL && obj->lamplit) {
				...
				if (obj->otyp != POT_WATER) {
					if (!breathless(youmonst.data))
					    ...
					    pline("Hueles un olor peculiar...");
					else {
					    int numeyes = eyecount(youmonst.data);
					    pline("Tu%s %s te llora%s.",
						 (numeyes == 1) ? "" : "s",
...
boolean in_view;
{
	...
	switch (obj->oclass == POTION_CLASS ? POT_WATER : obj->otyp) {
		default: /* glass or crystal wand */
			...
			impossible("¿rompiendo objeto extraño?");
		...
		case EXPENSIVE_CAMERA:
#endif
			to_pieces = " en mil pedazos";
			/*FALLTHRU*/
		case POT_WATER:		/* really, all potions */
			if (!in_view)
			    Oyes("%s rompiéndose!", something);
			else
			    pline("¡%s se rompe%s%s!", Doname2(obj),
				(obj->quan==1) ? "" : "n", to_pieces);
			break;
		case EGG:
		case MELON:
			pline("¡Splat!");
...
struct obj *obj;
{
	...
	if(!u.dx && !u.dy && !u.dz) {
		...
		dealloc_obj(obj);
#endif
		No_puedes("tirarte oro a ti mism%c.",
			poly_gender()==1? 'a' : 'o');
		return(0);
	}
#ifdef GOLDOBJ
	...
	if(u.uswallow) {
		...
		const char *gold = "oro";
#else
		const char *gold = "dinero";
#endif
		if (is_animal(u.ustuck->data))
		    pline("El %s desaparece en las entrañas %s.",
		          gold, del(mon_nam(u.ustuck)));
		else
		    pline("El %s desaparece en %s.",
		          gold, mon_nam(u.ustuck));
#ifndef GOLDOBJ
	...
	if(u.dz) {
		if (u.dz < 0 && !Is_airlevel(&u.uz) &&
					!Underwater && !Is_waterlevel(&u.uz)) {
	pline("El oro choca contra %s, y entonces cae en tu %s.",
		    ceiling(u.ux,u.uy), body_part(HEAD));
		    /* some self damage? */
		    if(uarmh) pline("¡Afortunadamente, estás llevando un yelmo!");
		}
		bhitpos.x = u.ux;
		bhitpos.y = u.uy;
	} else {
	...
	if(flooreffects(obj,bhitpos.x,bhitpos.y,"cae")) return(1);
	if(u.dz > 0)
		pline("El oro golpea %s.", surface(bhitpos.x,bhitpos.y));
	place_object(obj,bhitpos.x,bhitpos.y);
