    ...
    boolean waslit, rockit;
{
    ...
    if(dist >= 3)
	impossible("mkcavepos llamada con dist %d", dist);
    if(Blind)
	feel_location(x, y);
    else newsym(x,y);
}
...
register boolean rockit;
{
    ...
    if(rockit) pline("¡Crash!  ¡El techo se derrumba a tu alrededor!");
    else pline("¡Una fuerza misteriosa %s cueva a tu alrededor!",
	     (levl[u.ux][u.uy].typ == CORR) ? "crea una" : "extiende la");
	...
	int		x, y;
{
	...
	const char *verb = (madeby == BY_YOU && uwep && is_axe(uwep)) ? "cortar" : "cavar";

	if (On_stairs(x, y)) {
	    if (x == xdnladder || x == xupladder) {
		if(verbose) pline("La escalera de mano resiste tu esfuerzo.");
	    } else if(verbose) pline("La escalera es demasiado dura para %s.", verb);
	    return(FALSE);
	} else if (IS_THRONE(levl[x][y].typ) && madeby != BY_OBJECT) {
	    if(verbose) pline("El trono es demasiado duro para romper.");
	    return(FALSE);
	} else if (IS_ALTAR(levl[x][y].typ) && (madeby != BY_OBJECT ||
				Is_astralevel(&u.uz) || Is_sanctum(&u.uz))) {
	    if(verbose) pline("El altar es demasiado duro para romper.");
	    return(FALSE);
	} else if (Is_airlevel(&u.uz)) {
	    if(verbose) No_puedes("%s en el aire.", verb);
	    return(FALSE);
	} else if (Is_waterlevel(&u.uz)) {
	    if(verbose) pline("El agua chapotea y se calma.");
	    return(FALSE);
	    ...
	    if(verbose) {
		const char *que = surface(x,y);
		boolean fem = isfeminine(que);
		pline("%s aquí es demasiado dur%c para %s.",
		      capitalize(que), fem? 'a' : 'o', verb);
	    }
	    return(FALSE);
	} else if (sobj_at(BOULDER, x, y)) {
	    if(verbose) pline("No hay bastante espacio para %s aquí.", verb);
	    return(FALSE);
...
dig()
{
	...
	const char *verb =
	    (!uwep || is_pick(uwep)) ? "cavar" : "cortar";
	...
	if (digging.down) {
		...
		pline("Este árbol parece petrificado.");
		return(0);
	    }
	    if (IS_ROCK(lev->typ) && !may_dig(dpx,dpy) &&
			dig_typ(uwep, dpx, dpy) == DIGTYP_ROCK) {
		pline("Esta pared es demasiado dura para %s.", verb);
		return(0);
	    }
	}
	if(Fumbling && !rn2(3)) {
	    ...
	    case 0:
		if(!welded(uwep)) {
		    pline("Chapuceas y dejas caer tu %s.", xname(uwep));
		    dropx(uwep);
		} else {
#ifdef STEED
		    if (u.usteed)
			pline("¡Tu %s %s y %s!",
			     xname(uwep),
			     otense(uwep, "bounce"),
			     mon_nam_acc(u.usteed, otense(uwep, "golpea")));
		    else
#endif
			pline("¡Ouch!  ¡Tu %s %s y te %s!",
			      xname(uwep),
			      otense(uwep, "rebota"), otense(uwep, "golpea"));
		    set_wounded_legs(RIGHT_SIDE, 5 + rnd(5));
		}
		break;
	    case 1:
		pline("¡Bang!  ¡Golpeas con el lado ancho %s!",
		      del(the(xname(uwep))));
		break;
	    default: pline("Tu golpe falla su marca.");
		break;
	    }
	    return(0);
	}
	...
	if (digging.effort > 100) {
		...
		if ((obj = sobj_at(STATUE, dpx, dpy)) != 0) {
			...
			digtxt = "La roca enorme se rompe en pedazos.";
			...
			if (IS_TREE(lev->typ)) {
			    digtxt = "Talas el árbol.";
			    lev->typ = ROOM;
			    if (!rn2(5)) (void) rnd_treefruit_at(dpx, dpy);
			} else {
			    digtxt = "Tienes éxito sacando un poco de piedra.";
			    lev->typ = CORR;
			}
			...
			digtxt = "Haces una abertura en la pared.";
		} else if(lev->typ == SDOOR) {
			cvt_sdoor_to_door(lev);	/* ->typ = DOOR */
			digtxt = "¡Rompes una puerta secreta!";
			if(!(lev->doormask & D_TRAPPED))
				lev->doormask = D_BROKEN;
		} else if(closed_door(dpx, dpy)) {
			digtxt = "Rompes la puerta.";
			if(shopedge) {
		...
		if(Is_earthlevel(&u.uz) && !rn2(3)) {
		    ...
		    if(mtmp) pline("¡Los escombros de tu excavación cobran vida!");
		}
...
cleanup:
		...
		static const char *const d_target[6] = {
			"", "la piedra", "la estatua", "la roca enorme", "la puerta", "el árbol"
		};
		int dig_target = dig_typ(uwep, dpx, dpy);

		if (IS_WALL(lev->typ) || dig_target == DIGTYP_DOOR) {
		    if(*in_rooms(dpx, dpy, SHOPBASE)) {
			pline("Esta %s parece demasiado dura para %s.",
			      IS_DOOR(lev->typ) ? "puerta" : "pared", verb);
			return(0);
		    }
		} else if (!IS_ROCK(lev->typ) && dig_target == DIGTYP_ROCK)
		    return(0); /* statue or boulder got taken */
		if(!did_dig_msg) {
		    pline("Golpeas %s con toda tu fuerza.",
			d_target[dig_target]);
		    did_dig_msg = TRUE;
		}
	}
...
int ttyp;
{
	...
	if (ttyp != PIT && !Can_dig_down(&u.uz)) {
	    impossible("digactualhole: no puedo cavar %s en este nivel.",
	...
	if (ttyp == PIT) {
	    ...
	    if(madeby_u) {
		pline("Cavas un hoyo en %s.", surface_type);
		if (shopdoor) pay_for_damage("arruinar", FALSE);
	    } else if (!madeby_obj && canseemon(madeby))
		pline("%s cava un hoyo en %s.",
		      Monnam(madeby), surface_type);
	    else if (cansee(x, y) && flags.verbose)
		pline("Un hoyo aparece en %s.", surface_type);
		...
		if(is_flyer(mtmp->data) || is_floater(mtmp->data)) {
		    if(canseemon(mtmp))
			pline("%s %s sobre el hoyo.",
			      Monnam(mtmp),
			      (is_flyer(mtmp->data)) ? "vuela" : "flota");
		} else if(mtmp != madeby)
		    (void) mintrap(mtmp);
	    }
	    ...
	    if(madeby_u)
		pline("Caves una abertura a través %s.", del(surface_type));
	    else if(!madeby_obj && canseemon(madeby))
		pline("%s cave una abertura a través %s.",
			Monnam(madeby), del(surface_type));
	    else if(cansee(x, y) && flags.verbose)
		pline("Una abertura aparece en %s.", surface_type);

	    if (at_u) {
		if (!u.ustuck && !wont_fall && !next_to_u()) {
		    pline("¡Eres tirad%c atrás por tu mascota!",
			  poly_gender()==1? 'a' : 'o');
		    wont_fall = TRUE;
		}
		...
		if (u.ustuck || wont_fall) {
		    ...
		    if (oldobjs != newobjs)
			(void) pickup(1);
		    if (shopdoor && madeby_u) pay_for_damage("arruinar", FALSE);
		    ...
		    else if (!madeby_u) pay_for_damage("cavar en", TRUE);
		...
		if (mtmp) {
			...
			if (Is_stronghold(&u.uz)) {
			    ...
			    if (canseemon(mtmp))
				pline("%s evita la trampa.", Monnam(mtmp));
			    return;
			} else {
...
boolean pit_only;
{
	...
	if ((ttmp && (ttmp->ttyp == MAGIC_PORTAL || nohole)) ||
	   (IS_ROCK(lev->typ) && lev->typ != SDOOR &&
		...
		boolean fem = isfeminine(que);
		pline("%s aquí es demasiado dur%c para cavar.",
		      capitalize(que), fem? 'a' : 'o');

	} else if (is_pool(u.ux, u.uy) || is_lava(u.ux, u.uy)) {
		pline("%s se agita furiosamente por un momento, y entonces se calma.",
			is_lava(u.ux, u.uy) ? "La lava" : "El agua");
		...
		if (pit_only) {
		    pline("El puente levadizo parece demasiado duro para cavar.");
		    return FALSE;
		} else {
		    int x = u.ux, y = u.uy;
		    /* if under the portcullis, the bridge is adjacent */
		...
		if (ttmp && (ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT) &&
		    rn2(2)) {
			pline("La roca enorme se asienta en el hoyo.");
			 ...
			 */
			pline("¡KADOOM! ¡La roca se desploma!");
 ...
 liquid_flow:
		...
		pline("¡Tan pronto lo cavas, el agujero se llena de %s!",
		      typ == LAVAPOOL ? "lava" : "agua");
	...
	/* the following two are here for the wand of digging */
		...
		pline("El trono es demasiado duro para romper.");

	} else if (IS_ALTAR(lev->typ)) {
		pline("El altar es demasiado duro para romper.");
...
dig_up_grave()
{
	...
	if (Role_if(PM_ARCHEOLOGIST)) {
	    ...
	    Te_sientes("como un despreciable ladrón de tumbas!");
	} else if (Role_if(PM_SAMURAI)) {
	    adjalign(-sgn(u.ualign.type));
	    pline("¡Perturbas a los honorables difuntos!");
	} else if ((u.ualign.type == A_LAWFUL) && (u.ualign.record > -10)) {
	    adjalign(-sgn(u.ualign.type));
	    pline("¡Has violado la sanctidad de esta tumba!");
	}

	switch (rn2(5)) {
	case 0:
	case 1:
	    pline("Desentierras un cadáver.");
	    if (!!(otmp = mk_tt_object(CORPSE, u.ux, u.uy)))
	    	otmp->age -= 100;		/* this is an *OLD* corpse */;
	    break;
	case 2:
	    if (!Blind) pline(Hallucination ? "¡Hombre!  ¡El muerto vivo!" :
 			"¡El dueño de la tumba está muy disgustado!");
 	    (void) makemon(mkclass(S_ZOMBIE,0), u.ux, u.uy, NO_MM_FLAGS);
	    break;
	case 3:
	    /* LENGUA:  I suppose the mommy/mummy pun almost works in Spanish
	       (mamá/momia).... */
	    if (!Blind) pline(Hallucination ? "¡Quiero a mi momia!" :
 			"¡Has molestado una tumba!");
 	    (void) makemon(mkclass(S_MUMMY,0), u.ux, u.uy, NO_MM_FLAGS);
	    break;
	default:
	    /* No corpse */
	    pline("La tumba parece vacía.  Extraño....");
	    break;
	}
	levl[u.ux][u.uy].typ = ROOM;
...
struct obj *obj;
{
	...
	/* Check tool */
	if (obj != uwep) {
	    if (!wield_tool(obj, "blandir", "blandes")) return 0;
	    else res = 1;
	}
	ispick = is_pick(obj);
	verb = ispick ? "cavar" : "cortar";

	if (u.utrap && u.utraptype == TT_WEB) {
	    pline("%s no puedes %s mientras estés enredad%c en una telaraña.",
		  /* res==0 => no prior message;
		     res==1 => just got "You now wield a pick-axe." message */
		  !res ? "Desgraciadamente," : "Pero", verb,
		  poly_gender()==1? 'a' : 'o');
	    return res;
	}
	...
	*dsp = 0;
	Sprintf(qbuf, "¿En qué dirección quieres %s? [%s]", verb, dirsyms);
	if(!getdir(qbuf))
		return(res);
...
struct obj *obj;
{
	...
	const char *verb    = ispick ? "cavar" : "cortar";
	const char *verbing = ispick ? "cavando" : "cortando";

	if (u.uswallow && attack(u.ustuck)) {
		;  /* return(1) */
	} else if (Underwater) {
		pline("Una turbulencia torpedea tu intento de %s.", verb);
	} else if(u.dz < 0) {
		if(Levitation)
			pline("No tienes punto de apoyo.");
		else
			No_puedes("alcanzar %s.", ceiling(u.ux, u.uy));
		...
		if (dam <= 0) dam = 1;
		pline("Te golpeas con %s.", yname(uwep));
		Sprintf(buf, "su propio %s",
		...
		if (dig_target == DIGTYP_UNDIGGABLE) {
			...
			if (trap && trap->ttyp == WEB) {
			    if (!trap->tseen) {
				seetrap(trap);
				Hay("una telaraña allí!");
			    }
			    pline("Tu %s en la telaraña.",
				  aobjnam(obj, "se enrieda"));
			    /* you ought to be able to let go; tough luck */
			    /* (maybe `move_into_trap()' would be better) */
			    nomul(-d(2,2));
			    nomovemsg = "Te sueltas.";
			} else if (lev->typ == IRONBARS) {
			    pline("¡Clang!");
			    wake_nearby();
			} else if (IS_TREE(lev->typ))
			    pline("Necesitas una hacha para talar un árbol.");
			else if (IS_ROCK(lev->typ))
			    pline("Necesitas un pico para cavar en la piedra.");
			else if (!ispick && (sobj_at(STATUE, rx, ry) ||
					     sobj_at(BOULDER, rx, ry))) {
			    boolean vibrate = !rn2(3);
			    pline("El golpe hace salir chispas de la %s.%s",
				sobj_at(STATUE, rx, ry) ? "estatua" : "roca enorme",
				vibrate ? " El mango del hacha vibra violentamente!" : "");
			    if (vibrate) losehp(2, "pegar hachazos a un objeto duro", KILLED_BY);
			}
			else
			    pline("Blandes tu %s por el aire.",
				aobjnam(obj, (char *)0));
		} else {
			static const char * const d_action[6] = {
						...
						"cavar",
						"astillar la estatua",
						"golpear la roca enorme",
						"tajar la puerta",
						"talar el árbol"
			};
			static const char * const d_action_ger[6] = {
						"blandiendo",
						"cavando",
						"astillando la estatua",
						"golpeando la roca enorme",
						"tajando la puerta",
						"talando el árbol"
			};
			did_dig_msg = FALSE;
			...
			if (digging.pos.x != rx || digging.pos.y != ry ||
			    ...
			    digging.effort = 0;
			    if (!digging.quiet)
				pline("Empiezas a %s.", d_action[dig_target]);
			} else {
			    if (digging.chew)
				pline("%s %s.", "Comienzas a",
					    d_action[dig_target]);
			    else
				pline("%s %s.", "Continúas",
					    d_action_ger[dig_target]);
			    digging.chew = FALSE;
			}
			set_occupation(dig, verb, 0);
		}
	} else if (Is_airlevel(&u.uz) || Is_waterlevel(&u.uz)) {
		/* it must be air -- water checked above */
		pline("Blandas tu %s por el aire.", aobjnam(obj, (char *)0));
	} else if (!can_reach_floor()) {
		No_puedes("alcanzar %s.", surface(u.ux, u.uy));
	} else if (is_pool(u.ux, u.uy) || is_lava(u.ux, u.uy)) {
		/* Monsters which swim also happen not to be able to dig */
		No_puedes("quedarte bajo %s por bastante tiempo.",
				is_pool(u.ux, u.uy) ? "el agua" : "la lava");
	} else if (!ispick) {
		pline("Tu %s sólo rasga %s.",
		...
		if (digging.pos.x != u.ux || digging.pos.y != u.uy ||
		    ...
		    assign_level(&digging.level, &u.uz);
		    digging.effort = 0;
		    pline("Comienzas a %s hacia abajo.", verb);
		    if (*u.ushops) shopdig(0);
		} else
		    pline("Continúas %s hacia abajo.", verbing);
		did_dig_msg = FALSE;
    ...
    boolean zap;
{
	...
	if (in_town(x, y) &&
		...
		if(zap || digging.warned) {
		    verbalize("¡Alto, vándal%c!  ¡Estás arrestad%c!",
			      poly_gender()==1? 'a' : 'o',
			      poly_gender()==1? 'a' : 'o');
			...
			str = "esta fuente";
		    verbalize("¡Hey, deja de dañar %s!", str);
		    digging.warned = TRUE;
		}
		if (is_digging())
		    stop_occupation();
...
register struct monst *mtmp;
{
	...
	if (closed_door(mtmp->mx, mtmp->my)) {
		...
		if (!rn2(3) && flags.verbose)	/* not too often.. */
		    Sientes("una inesperada corriente de aire.");
	...
	if ((here->wall_info & W_NONDIGGABLE) != 0) {
	    impossible("mdig_tunnel:  %s a (%d,%d) es incavable",
		       (IS_WALL(here->typ) ? "pared" : "piedra"),
...
zap_dig()
{
	...
	if (u.uswallow) {
		...
		if (is_animal(mtmp->data))
		    pline("¡Atraviesas %s %s!",
			  the(mbodypart(mtmp, STOMACH)),
			  del(mon_nam(mtmp)));
	...
	if (u.dz) {
		...
		if (u.dz < 0 || On_stairs(u.ux, u.uy)) {
		    const char *que = ceiling(u.ux, u.uy);
		    boolean fem = isfeminine(que);
		    if (On_stairs(u.ux, u.uy)) {
			pline("El rayo rebota en la escalera%s y golpea %s.",
			      (u.ux == xdnladder || u.ux == xupladder) ?
			      " de mano" : "", que);
		    }
		    pline("Sueltas una roca de%s %s.", fem? " la" : "l", que);
		    pline("¡Cae en tu %s!", body_part(HEAD));
		    losehp(rnd((uarmh && is_metallic(uarmh)) ? 2 : 6),
			   "roca cayente", KILLED_BY_AN);
	...
	while (--digdepth >= 0) {
		...
		if (IS_WALL(room->typ)) {
			...
			pline("La pared brilla y entonces se apaga.");
		    break;
			...
			pline("El árbol se sacude pero no tiene daño.");
		    break;
			...
			pline("La piedra brilla y entonces se apaga.");
		    break;
		}
	...
	if (shopdoor || shopwall)
	    pay_for_damage(shopdoor ? "destruir" : "cavar en", FALSE);
	return;
}
...
long timeout;	/* unused */
{
	...
	if (on_floor) {
		...
		boolean iswep = (obj == uwep);
		boolean isplural = (obj->quan > 1L);
		pline("%sTu%s %s%s se %s%s",
		      iswep? "¡" : "",
		      isplural? "s" : "",
		      cname,
		      iswep? (isplural? " blandidos" : " blandido") : "",
		      otense(obj, "pudre"),
		      iswep? "!" : ".");
	    }
	    if (obj == uwep) {
...
/* LENGUA:  No se traduce estas funcciones inusadas */
#if 0
void
bury_monst(mtmp)
struct monst *mtmp;
