 ...
 * "El Corazón de Ahriman", save, resume in the Spanish version, and have
    ...
    struct monst *mon;
{
	...
	if (!yours) return 0;
	pline("¡El poder %s te da escalofríos!",
	      del(the(xname(obj))));
...
struct monst *mtmp;
{
	...
	if (weap->spfx & SPFX_DMONS) {
		...
		default:	impossible("Extraño ataque especial de arma.");
	    }
	}
	return(0);
}
...
char *hittee;			/* target's name: "you" or mon_nam(mdef) */
{
	...
	*dmgptr += rnd(4);		/* (4..6)d4 */
	...
	result = TRUE;
	if (youdefend)
	    pline("¡La hoja antimágica te %s!", verb);
	else
	    pline("¡La hoja antimágica %s!", mon_nam_acc(mdef, verb));
	...
	if (youattack && (mb->spe == 0 || !rn2(3 * abs(mb->spe)))) {
	    /* LENGUA: using a verb as a noun; possibilities are prueba and
	       pica; is this OK? */
	    /* Andres:  Esta no es la solución mas elegante posible: */
	    if (strcmp(verb, "sondea") == 0)
		pline("El sondeo es esclarecedor.");
	    else if (strcmp(verb, "instiga") == 0)
		pline("La instigación es esclarecedora.");
	    else
...
int dieroll; /* needed for Magicbane and vorpal blades */
{
	...
	*dmgptr += spec_dbon(otmp, mdef, *dmgptr);
	...
	if (attacks(AD_COLD, otmp)) {
	    if (realizes_damage) {
		if (youdefend)
		    pline("%sLa hoja gélida te hiela%s", excl1, excl2);
		else
		    pline("%sLa hoja gélida %s%s", excl1,
	...
	if (attacks(AD_ELEC, otmp)) {
	    if (realizes_damage) {
		if (youdefend) {
		    /* Andrés: ¿Es correcto usar 'masivo' como traducción de
			'massive'? */
		    if (spec_dbon_applies)
			pline("¡El martillo masivo te golpea!  ¡Te cae un relámpago!");
		    else
			pline("El martillo masivo te golpea.");
		} else {
		    if (spec_dbon_applies)
			pline("¡El martillo masivo golpea!  ¡%s un relámpago!",
	...
	if (attacks(AD_MAGM, otmp)) {
		...
		if (youdefend) {
		    if (spec_dbon_applies)
			pline("¡El objeto imaginario te golpea!  ¡Un granizo de proyectiles mágicos te golpea!");
		    else
			pline("El objeto imaginario te golpea.");
		} else {
		    if (spec_dbon_applies)
			pline("¡El objeto imaginario golpea!  ¡Un granizo de proyectiles mágicos %s!",
	...
	if (spec_ability(otmp, SPFX_BEHEAD)) {
		...
		if (!youdefend) {
			if (!has_head(mdef->data) || notonhead || u.uswallow) {
				if (youattack)
					pline("Por alguna razón, %s atrozmente.",
						mon_nam_acc(mdef, "fallas"));
				else if (vis)
					pline("Por alguna razón, %s le falla atrozmente.",
						mon_nam(magr));
				*dmgptr = 0;
			...
			*dmgptr = 2 * mdef->mhp + FATAL_DAMAGE_MODIFIER;
			...
			if (!has_head(youmonst.data)) {
				pline("Por alguna razón, %s te falla malamente.",
				      magr ? mon_nam(magr) : wepdesc);
				*dmgptr = 0;
				return TRUE;
			}
			if (noncorporeal(youmonst.data) || amorphous(youmonst.data)) {
				pline("%s corta través de tu %s.",
    ...
    register struct obj *obj;
{
	...
	if(obj->age > monstermoves) {
	    ...
	    Sientes("que %s no te %s atención.",
	...
	case CREATE_PORTAL: {
	    ...
	    end_menu(tmpwin, "¿Abrir un portal a qué calabozo?");
...
nothing_special:
	...
	case CONFLICT:
	    if(on) Te_sientes("como un%s demagog%s.",
			poly_gender()==1? "a" : "",
			poly_gender()==1? "a" : "");
	    else Sientes("la tensión disminuyéndo a tu alrededor.");
	    break;
	case LEVITATION:
	    if(on) {
		float_up();
    ...
    struct obj *obj;
{
	...
	if (!*line)
		line = "El archivo de rumores de NetHack está cerrado por renovación.";
