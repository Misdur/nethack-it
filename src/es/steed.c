...
rider_cant_reach()
{
     pline("No tienes la destreza de extendirte %s.", del(y_monnam(u.usteed)));
}
	...
	struct obj *otmp;
{
	...
	if (!u.dx && !u.dy) {
	    pline("¿Quieres ensillarte a ti mismo?  Muy divertido...");
	    return 0;
	}
	if (!isok(u.ux+u.dx, u.uy+u.dy) ||
			!(mtmp = m_at(u.ux+u.dx, u.uy+u.dy)) ||
			!canspotmon(mtmp)) {
	    pline("No veo a nadie allí.");
	    return 1;
	}
	...
	if (mtmp->misc_worn_check & W_SADDLE ||
	    ...
	    pline("%s no necesita otra silla.", Monnam(mtmp));
	    return 1;
	}
	ptr = mtmp->data;
	if (touch_petrifies(ptr) && !uarmg && !Stone_resistance) {
	    ...
	    pline("%s.", mon_nam_acc(mtmp, "Tocas"));
 	    if (!(poly_when_stoned(youmonst.data) && polymon(PM_STONE_GOLEM))) {
		Sprintf(kbuf, "atentar ensillar %s", an(mtmp->data->mname));
		instapetrify(kbuf);
 	    }
	}
	if (ptr == &mons[PM_INCUBUS] || ptr == &mons[PM_SUCCUBUS]) {
	    pline("¡Vergüenza sobre ti!");
	    exercise(A_WIS, FALSE);
	    return 1;
	}
	if (mtmp->isminion || mtmp->isshk || mtmp->ispriest ||
			mtmp->isgd || mtmp->iswiz) {
	    pline("Pienso que %s objetaría.", mon_nam(mtmp));
	    return 1;
	}
	if (!can_saddle(mtmp)) {
		No_puedes("ensillar criaturas como esa.");
		return 1;
	}
	...
	/* Make the attempt */
	if (rn2(100) < chance) {
	    pline("Pones la silla sobre %s.", mon_nam(mtmp));
...
#ifdef WIZARD
	if (wizard && yn("¿Fuerzas que el montar suceda?") == 'y')
		forcemount = TRUE;
#endif
	...
	boolean force;		/* Quietly force this animal */
{
	...
	/* Sanity checks */
	if (u.usteed) {
	    pline("Ya estás cabalgando %s.", al(mon_nam(u.usteed)));
	    return (FALSE);
	}

	/* Is the player in the right form? */
	if (Hallucination && !force) {
	    pline("Tal vez debas buscar un manejador designado.");
	    return (FALSE);
	}
	 ...
	 */
	if (Wounded_legs) {
	    pline("Tus %s no están en ninguna condición para cabalgar.", makeplural(body_part(LEG)));
#ifdef WIZARD
	    if (force && wizard && yn("¿Curar tus piernas?") == 'y')
	...
	if (!otmp) {
	    pline("%s no está ensillad%c.", Monnam(mtmp),
		    mon_gender(mtmp)? 'a' : 'o');
	    return (FALSE);
	}
	ptr = mtmp->data;
	if (touch_petrifies(ptr) && !Stone_resistance) {
	    ...
	    pline("%s.", mon_nam_acc(mtmp, "Tocas"));
	    Sprintf(kbuf, "atentar montar %s", an(mtmp->data->mname));
	    instapetrify(kbuf);
	}
	if (!mtmp->mtame || mtmp->isminion) {
	    pline("Creo que %s objetaría.", mon_nam(mtmp));
	    return (FALSE);
	}
	if (mtmp->mtrapped) {
	    struct trap *t = t_at(mtmp->mx, mtmp->my);

	    No_puedes("montar %s mientras está trampad%c en %s.",
		      al(mon_nam(mtmp)),
		      mon_gender(mtmp)? 'a' : 'o',
	...
	if (!force && !Role_if(PM_KNIGHT) && !(--mtmp->mtame)) {
	    ...
	    pline("¡%s resiste%s!", Monnam(mtmp),
		  mtmp->mleashed ? " y su traílla se suelta libre" : "");
	    if (mtmp->mleashed) m_unleash(mtmp, FALSE);
	    return (FALSE);
	}
	if (!force && Underwater && !is_swimmer(ptr)) {
	    No_puedes("cabalgar a esa criatura mientras estés en agua.");
	    return (FALSE);
	}
	if (!can_saddle(mtmp) || !can_ride(mtmp)) {
	    No_puedes("cabalgar a criaturas como ésa.");
	    return (0);
	}
	...
	if (!force && !is_floater(ptr) && !is_flyer(ptr) &&
	    ...
	    No_puedes("alcanzar %s.", mon_nam(mtmp));
	    return (FALSE);
	}
	if (!force && uarm && is_metallic(uarm) &&
			greatest_erosion(uarm)) {
	    pline("Tu armadura %s es demasiado inflexible para poder montar %s.",
			uarm->oeroded ? "oxidada" : "corroida",
			mon_nam(mtmp));
	    return (FALSE);
	}
	if (!force && (Confusion || Fumbling || Glib || Wounded_legs ||
	    ...
	    pline("Te resbalas mientras intentas montar %s.", mon_nam(mtmp));

	    Sprintf(buf, "resbaló montando %s",
	...
	if (!force) {
	    if (Levitation && !is_floater(ptr) && !is_flyer(ptr))
	    	...
	    	pline("¡%s mágicamente se flota!", Monnam(mtmp));
	    pline("Montas %s.", mon_nam(mtmp));
	}
...
kick_steed()
{
	...
	pline("¡%s galopea!", Monnam(u.usteed));
	u.ugallop += rn1(20, 30);
	return;
}
	...
	int reason;		/* Player was thrown off etc. */
{
	...
	switch (reason) {
		...
		verb = poly_gender()==1? "Eres tirada" : "Eres tirado";
	    case DISMOUNT_FELL:
		pline("¡%s tirada %s!", verb, del(mon_nam(mtmp)));
		if (!have_spot) have_spot = landing_spot(&cc,reason,1);
		losehp(rn1(10,10), "accidente de cabalgar", KILLED_BY_AN);
		HWounded_legs += rn1(5, 5);
		EWounded_legs |= BOTH_SIDES;
		repair_leg_damage = FALSE;
		break;
	    case DISMOUNT_POLY:
		pline("No puedes cabalgar %s más.", al(mon_nam(u.usteed)));
		if (!have_spot) have_spot = landing_spot(&cc,reason,1);
		break;
	    ...
	    default:
		if (otmp && otmp->cursed) {
		    pline("No puedes.  La silla %s maldita.",
			  otmp->bknown ? "está" : "parece");
		    otmp->bknown = TRUE;
		    return;
		}
		if (!have_spot) {
		    pline("No puedes.  No hay ningún lugar en que puedas estar.");
		    return;
		}
		if (!mtmp->mnamelth) {
			/* LENGUA:  Translating the song lyrics breaks the rhyme */
			pline("Has ido través del calabozo en %s sin nombre.",
	    			an(mtmp->data->mname));
			if (Hallucination)
				pline("Sintió bien salirte de la lluvia.");
		} else
			pline("Desmontas %s.", del(mon_nam(mtmp)));
	}
	...
	if (!DEADMONSTER(mtmp)) {
		...
		if (!is_flyer(mdat) && !is_floater(mdat) && !is_clinger(mdat)) {
			...
			pline("¡%s es tirad%c en la lava!", Monnam(mtmp),
				mon_gender(mtmp)? 'a' : 'o');
			if (!likes_lava(mdat)) {
			    killed(mtmp);
			    adjalign(-1);
...
int x, y;
{
	...
	impossible("¿Poner %s en el mapa?",
		   (mon == u.usteed) ? "corcel" : "monstruo defunto");
	return;
    }
    mon->mx = x, mon->my = y;
