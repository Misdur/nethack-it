...
boolean newlev;
{
	...
	if (!*leavestring && shkp->mcanmove && !shkp->msleeping) {
	    ...
	    verbalize(NOTANGRY(shkp) ?
		      "¡%s!  Págueme por favor antes de salir." :
...
struct monst *shkp;
{
	...
	if (eshkp->credit >= total) {
	    pline("Se usa tu crédito de %ld %s para pagar tu cuenta.",
	...
	eshkp->robbed += total;
	pline("Has robado mercancías que valen %ld %s.",
...
register char *enterstring;
{
	...
	if (ANGRY(shkp)) {
	    verbalize("¡¿Así que, %s, se atrieve volver %s %s de %s?!",
		      plname,
...
shopper_financial_report()
{
	...
	if (this_shkp &&
	    ...
	    pline("No tiendes crédito ni deuda aquí.");
...
register struct monst *shkp;
{
	...
	if(credit >= tmp) {
		pline("Se deduce el precio de tu crédito.");
		ESHK(shkp)->credit -=tmp;
		tmp = 0L;
	} else {
		pline("Tu crédito paga parcialmente el precio.");
		ESHK(shkp)->credit = 0L;
		tmp -= credit;
	}
	return(tmp);
...
register xchar ox,oy;
{
	...
	pline("¡%s %s!", Monnam(shkp),
	      !ANGRY(shkp) ? "se enoja" : mon_gender(shkp)? "está furiosa" : "está furioso");
	shkp->mx = sx,  shkp->my = sy;
	hot_pursuit(shkp);
}

STATIC_VAR const char no_money[] = "Además, no tienes dinero.";
STATIC_VAR const char perhaps_no_money[] = "Además, parece que no tengas dinero.";
STATIC_VAR const char not_enough_money[] =
			    "Además, no tienes bastante dinera para interesar%s.";
...
dopay()
{
	...
	if ((!sk && (!Blind || Blind_telepat)) || (!Blind && !seensk)) {
	    pline("Parece que no hay tendero aquí para recibir tu pago.");
		return(0);
	}
	...
	if (seensk == 1) {
		...
		if (shkp != resident && distu(shkp->mx, shkp->my) > 2) {
		    pline("%s no está bastante acerca para recibir tu pago.",
					     Monnam(shkp));
		    return(0);
		}
	} else {
		...
		mtmp = m_at(cx, cy);
		if(!mtmp) {
		     pline("No hay nadie allí para recibir tu pago.");
		     return(0);
		}
		if(!mtmp->isshk) {
		...
		if (mtmp != resident && distu(mtmp->mx, mtmp->my) > 2) {
		     pline("%s está demasiado lejos para recibir tu pago.",
				    Monnam(mtmp));
		     return(0);
		}
		shkp = mtmp;
...
proceed:
	...
	if (!eshkp->billct && !eshkp->debit) {
		...
		if(!ltmp && NOTANGRY(shkp)) {
			...
			pline("Pero %s está tan enojad%c como siempre.",
	...
	if(eshkp->debit) {
		...
		if(loan) {
			...
			Strcat(sbuf, "que has recogado en la tienda.");
			   ...
			   "por oro recogado y el uso de mercancías.");
		} else Strcat(sbuf, "por el uso de mercancías.");
		pline(sbuf);
#ifndef GOLDOBJ
		...
		if (umoney + eshkp->credit < dtmp) {
			...
			eshkp->debit = 0L;
			eshkp->loan = 0L;
			pline("Tu crédito paga tu deuda.");
			...
			eshkp->debit = 0L;
			eshkp->loan = 0L;
			pline("Tu crédito paga en parte esta deuda.");
			pline("Pagas lo demás.");
			flags.botl = 1;
		    }
		    paid = TRUE;
		}
	...
	if (eshkp->billct) {
...
#endif
		pline("No %s ni dinero ni crédito.",
		...
		if(stashed_gold)
		    pline("¿Quizás tengas algo de oro escondido?");
		return(0);
	    }
		...
		while (tmp < eshkp->billct) {
			...
			impossible("Administración del tendero no funciona.");
...
boolean itemize;
{
...
#endif
                pline("%s ni dinero ni crédito más.",
	...
	if (itemize) {
		...
		verbalize("%sle%s otr%c%s %s antes de comprar %s.",
			  ANGRY(shkp) ? "Págue" : "Por favor, págue",
			  plur(obj->quan),
			  fem? 'a' : 'o',
			  plur(obj->quan),
			  que,
			  fem? (obj->quan > 1)? "éstas" : "ésta"
			     : (obj->quan > 1)? "éstos" : "éste");
	...
	if (buy == PAY_BUY && umoney + ESHK(shkp)->credit < ltmp) {
		...
		(ESHK(shkp)->credit > 0L) ? " ni crédito" : "",
		doname(obj));
...
int croaked;
{
	...
	if (numsk > 1) {
		...
		pline("%s %smira tu cadáver%s y %s.",
		      Monnam(shkp),
...
speak:
	if (shkp->mcanmove && !shkp->msleeping && !silent) {
		...
		static const char *honored[5] = {
		  "buen@", "honorad@", "más gracios@", "estimad@",
		  "más renombrad@ y sagrad@"
		};
		...
		if (!is_human(youmonst.data)) {
		    ...
		    Strcat(buf, (flags.female) ? "señora " : "señor ");
		...
		que = xname(obj);
		obj->quan = quan;
		pline("%s; sólo %ld %s %s.\"", buf, ltmp,
			(quan > 1L) ? "por"
                                    : isfeminine(que)? "por esta" : "por este",
                        que);
	    } else
		pline("%s l%c costará%s %ld %s%s.",
			The(xname(obj)),
			poly_gender()==1? 'a' : 'o',
			(obj->quan > 1L)? "n" : "",
			ltmp, currency(ltmp),
...
register struct obj *obj, *otmp;
{
	...
	if(!bp) {
		impossible("splitbill: ¿no está en cuenta?");
		return;
	}
...
register struct monst *shkp;
{
	...
	if((bp = onbill(obj, shkp, FALSE)) != 0) {
		...
		*bp = ESHK(shkp)->bill_p[ESHK(shkp)->billct];
		...
		impossible("sub_one_frombill: objeto no pagado no está en cuenta");
		obj->unpaid = 0;
	}
}
...
register boolean peaceful, silent;
{
	...
	if(peaceful) {
		...
		if (credit_use) {
		    if (ESHK(shkp)->credit) {
			pline("Te queda %ld %s de crédito.",
				 ESHK(shkp)->credit, currency(ESHK(shkp)->credit));
			return value;
		    } else if (!value) {
			pline("No te queda ningún crédito.");
			return 0;
		    }
		    you_owe = "Todavía debes";
		}
		if(obj->oclass == COIN_CLASS)
		...
		if(cansee(shkp->mx, shkp->my)) {
		    Norep("%s truena: \"¡%s, usted es %s!\"",
				Monnam(shkp), plname,
				poly_gender()==1? "una ladrona" : "un ladrón");
		} else  Norep("Oyes una grita, \"¡%s!\"",
				poly_gender()==1? "Ladrona" : "Ladrón");
	    }
	    hot_pursuit(shkp);
	    (void) angry_guards(FALSE);
	}
...
xchar x, y;
{
	...
	if(eshkp->robbed) {  /* shkp is not angry? */
		...
		if((eshkp->robbed -= offer < 0L))
			eshkp->robbed = 0L;
		if(offer) verbalize(
  "Gracias por su contribución para surtir esta tienda recientemente saqueada.");
		subfrombill(obj, shkp);
		return;
	}
	...
	if(isgold || cgold) {
		...
		if(eshkp->debit >= gltmp) {
		    ...
		    pline("Se añade%s %ld %s a tu crédito.",
				delta > 1L ? "n" : "",
...
move_on:
	...
	if(!money_cnt(shkp->minvent)) {
		...
		if (sell_how == SELL_NORMAL || auto_credit) {
		    ...
		    Sprintf(qbuf,
			    "¿Aceptarás %ld %s de crédito por %s?",
		...
		if (c == 'y') {
		    shk_names_obj(shkp, obj, (sell_how != SELL_NORMAL) ?
			    "cambiaste%s%s por %ld zorkmid%s de crédito%s." :
			    "dejas%s%s y adquieres %ld zorkmid%s de crédito%s.",
			    tmpcr, "es",
		...
		"dejas%s%s y recibes %ld pieza%s de oro para compensación.%s",
				    offer, "s", "");
			    break;
...
int mode;		/* 0: deliver count 1: paged */
{
	...
	for (bp = eshkp->bill_p, end_bp = &eshkp->bill_p[eshkp->billct];
	    ...
	    obj = bp_to_obj(bp);
	    if(!obj) {
		impossible("Mala administración de tendero.");
		goto quit;
	    }
...
register xchar x, y;
{
	...
	if (shkp->mcanmove && !shkp->msleeping &&
		...
		if (mnearto(shkp, x, y, TRUE))
		    verbalize("¡Váyase, cerd%c!",
			    poly_gender()==1? 'a' : 'o');
		if (cansee(x, y)) {
		    pline("%s ágilmente%s prende %s.",
			  Monnam(shkp),
	...
	if (saw_walls) {
	    pline("¡De repente, %s secci%s de pared se cierra%s!",
		  (saw_walls == 1) ? "una" : (saw_walls <= 3) ?
						  "unas" : "severales",
		  (saw_walls == 1) ? "ón" : "ones", (saw_walls == 1) ? "" : "n");
	    if (saw_door)
		...
		pline("¡De repente, se va el daño del piso!");
	    else if (saw_untrap)
	        pline("¡De repente, se quita la trampa del piso!");
	    else if (inside_shop(u.ux, u.uy) == ESHK(shkp)->shoproom)
		Te_sientes("más claustrofóbic%c que antes.",
			poly_gender()==1? 'a' : 'o');
	    else if (flags.soundok && !rn2(10))
		Norep("Las acústicas del calabozo se cambian notablemente.");
	}
	if (stop_picking)
		stop_occupation();
}
...
#define vert(i)  ((i/3)-1)
		 ...
		 */
		verbalize("¡Quítese su basura de mi pared!");
	   ...
	   (shkp->data != &mons[PM_GRID_BUG] || (omx==u.ux || omy==u.uy))) {
		...
		if(eshkp->following) {
			...
			if(moves > followmsg+4) {
			    verbalize("¡%s, %s!  ¿No se le olvidó pagar?",
				    Hello(shkp), plname);
			    followmsg = moves;
			    if (!rn2(9)) {
...
register int fall;
{
	...
	if (Role_if(PM_KNIGHT)) {
	    Te_sientes("como %s vulgar.", poly_gender()==1? "una ladrona" : "un ladrón");
	    adjalign(-sgn(u.ualign.type));
	}
	return;
    }
	...
	if (lang == 2) {
		...
		verbalize(
			"Cuidado, señor%s, o usted podría caer través del piso.",
			flags.female ? "a" : "");
	    else
	        verbalize("¡Señor%s, no dañe el piso aquí!",
			flags.female ? "a" : "");
	}
	if (Role_if(PM_KNIGHT)) {
	    Te_sientes("como %s vulgar.", poly_gender()==1? "ladrona" : "ladrón");
	    adjalign(-sgn(u.ualign.type));
	}
		...
		if (distu(shkp->mx, shkp->my) > 2) {
		    if (lang == 2)
			pline("¡%s te maldice en enojo y frustración!",
			      shkname(shkp));
		    rile_shk(shkp);
		    return;
...
boolean cant_mollify;
{
	...
	if(uinshp) {
		...
		if(flags.soundok) {
		    ...
		    verbalize("¡Váyase, cerd%c!", poly_gender()==1? 'a' : 'o');
		    wait_synch();
	...
	if((um_dist(x, y, 1) && !uinshp) || cant_mollify ||
	   ...
	   (money_cnt(invent) + ESHK(shkp)->credit) < cost_of_damage
		...
		if(um_dist(x, y, 1) && !uinshp) {
		    ...
		    verbalize("¿Quién se atrevía %s mi %s?", dmgstr,
					 dugwall ? "tienda" : "puerta");
		} else {
getcad:
		    verbalize("¿Cómo se atrieve %s mi %s?", dmgstr,
					 dugwall ? "tienda" : "puerta");
		}
		hot_pursuit(shkp);
		return;
	...
	if (Invis) pline("¡Tu invisibilidad no engaña a %s!", shkname(shkp));
	Sprintf(qbuf,"\"¡Malcriad%c!  ¡Hiciste %ld %s de daño!\"  ¿Pagas? ",
		 poly_gender()==1? 'a' : 'o',
...
#endif
		flags.botl = 1;
		pline("Molificad%c, %s acepta tu restitución.",
			mon_gender(shkp)? 'a' : 'o',
			shkname(shkp));
...
boolean fem;
{
	...
	switch (choice) {
		...
		if (o == FOOD_CLASS) return ", ¡delicia del gastrónomo!";
	    ...
	    case 3: return ", calidad más grande.";
	    case 2: return ", una selección excellente.";
	    case 1: return ", una ganga verdadera.";
	   default: break;
	}
    } else if (itm->oartifact) {
	return fem ? ", ¡única!" : ", ¡único!";
    }
    return ".";
}
#endif /*OVL3*/
...
const char *Izchak_speaks[]={
    ...
    "%s dice: 'No me gusta café débil... deme Supremo Colombiano.'",
    "%s dice que obtener el acuerdo del devteam sobre alguno es difícil.",
    ...
    "%s dice: 'Usted probablemente necesitará algo de esta tienda en el porvenir.'",
...
struct monst *shkp;
{
	...
	if (ANGRY(shkp))
		...
		pline("%s dice que tu cuenta añade a %ld %s.",
		...
		pline("%s te alenta de usar tu%s %ld %s de crédito.",
...
boolean altusage;
{
	...
	if (otmp->oclass == SPBOOK_CLASS) {
	    fmt = "%sDebes%s %ld %s.";
	    arg1 = rn2(2) ? "Ésta no es una biblioteca gratis, ¡idiota!  " : "";
	    arg2 = ESHK(shkp)->debit > 0L ? " un adicional" : "";
	} else if (otmp->otyp == POT_OIL) {
	    fmt = "%s%sEso costará %ld %s (Impuesto Yendoriano de Combustible).";
	} else {
...
register long amount;
{
	...
	if(eshkp->credit >= amount) {
	    if(eshkp->credit > amount)
		pline("Se reduce tu crédito por %ld %s.",
					amount, currency(amount));
	    else pline("Se borra tu crédito.");
	    eshkp->credit -= amount;
	} else {
	    delta = amount - eshkp->credit;
	    if(eshkp->credit)
		pline("Se borra tu crédito.");
	    if(eshkp->debit)
