...
static NEARDATA const char *deaths[] = {		/* the array of death */
	/* Conjugation:  preterite, 3rd person singular, or past participle;
	   some are nouns or infinitives
	   @'s are replaced by 'o' or 'a' as appropriate
	   The eight entries in English must be left that way for compatibility;
	   they will actually be printed in Spanish */
	"se murió", "choked", "poisoned", "starvation", "ahogar",
	"quemar", "disolverse bajo calor y presión",
	"crushed", "turned to stone", "se volvió barro",
	"genocidad@", "pánico", "engañad@",
	"quit", "escaped", "ascended"
};


static NEARDATA const char *ends[] = {		/* "when you..." */
	/* Conjugation:  preterite, 2nd person singular */
	/* These are printed to the screen, not saved to files, and may be
	   entirely in Spanish */
	"moriste", "te atragantaste", "fuiste envenenad@",
	"moriste de hambre", "te ahogaste",
	"te quemaste", "te disolviste en la lava",
	"fuiste aplastad@", "te volviste piedra", "te volviste barro",
	"fuiste genocidad@", "entraste en pánico", "fuiste embaucad@",
	"dejaste", "te escapaste", "ascendiste"
};
...
done2()
{
	...
	if(wizard) {
	    int c;
# ifdef VMS
	    const char *tmp = "¿Entrar debugger?";
# else
#  ifdef LATTICE
	    const char *tmp = "¿Crear SnapShot?";
#  else
	    const char *tmp = "¿Descargar la memoria?";
#  endif
# endif
	    if ((c = ynq(tmp)) == 'y') {
		(void) signal(SIGINT, (SIG_RET_TYPE) done1);
...
register struct monst *mtmp;
{
	...
	if (distorted) {
		Strcat(buf, "distorcionad");
		Strcat(buf, mon_gender(mtmp)? "a" : "o");
		Strcat(buf, mon_gender(mtmp)? "a" : "o");
		Strcat(buf, " por alucinaciones");
	}

	if(mtmp->data == &mons[PM_GHOST]) {
		Strcat(buf, "espectro");
		if (mtmp->mnamelth) Sprintf(eos(buf), " de %s", NAME(mtmp));
	} else if(mtmp->isshk) {
		if (mtmp->female)
			Sprintf(eos(buf), "la Sra. %s, la tendera", shkname(mtmp));
		else
			Sprintf(eos(buf), "el Sr. %s, el tendero", shkname(mtmp));
		killer_format = KILLED_BY;
		...
		if (mtmp->mnamelth)
		    Sprintf(eos(buf), " llamad%c %s", mon_gender(mtmp)? 'a' : 'o', NAME(mtmp));
	}

	if (multi) Sprintf(eos(buf), ", estando indefens%c", flags.female? 'a' : 'o');
	killer = buf;
...
panic VA_DECL(const char *, str)
	...
	raw_print(program_state.gameover ?
		  /* LENGUA:  Need a translation for this */
		  "Postgame wrapup disrupted." :
		  !program_state.something_worth_saving ?
		  "La initialización del programa ha fallado." :
		  "De repente, el calabozo se derrumba.");
#if defined(WIZARD) && !defined(MICRO)
# if defined(NOTIFY_NETHACK_BUGS)
	if (!wizard)
	    raw_printf("Reporta el error a \"%s\".",
			"rchason@smart.net");
	else if (program_state.something_worth_saving)
	    raw_print("\nGuardando archivo de grabación de errores.\n");
# else
	if (!wizard)
	    raw_printf("Reporta el error a \"%s\"%s.",
...
#  endif
			!program_state.something_worth_saving ? "" :
			" y podría ser posible reconstruir");
# endif
...
boolean taken;
{
	...
	if (invent) {
	    if(taken)
		Sprintf(qbuf,"¿Quieres ver lo que tenías cuando %s?",
			(how == QUIT) ? "abandonaste" : "te moriste");
	    else
		Strcpy(qbuf,"¿Quieres identificar tus posesiones?");

	    ask = should_query_disclose_option('i', &defquery);
	    if (!done_stopprint) {
		c = ask ? yn_function(qbuf, es_ynqchars, ynqchars, defquery) : defquery;
		if (c == 'y') {
			struct obj *obj;
	...
	if (!done_stopprint) {
	    c = ask ? yn_function("¿Quieres ver tus atributos?",
				  es_ynqchars, ynqchars, defquery) : defquery;
	    if (c == 'y')
	...
	if (!done_stopprint) {
	    if (ask)
		c = yn_function("¿Quieres ver tu conducta?",es_ynqchars,ynqchars,defquery);
...
int how;
{
	...
	nomovemsg = "Sobreviviste ese atentado contra tu vida.";
	flags.move = 0;
...
winid endwin;
{
	...
	if (otmp->oartifact ||
		...
		name = otmp->oartifact ? artifact_name(xname(otmp), &dummy) :
			    OBJ_NAME(objects[otmp->otyp]);
		Sprintf(pbuf, "%s (que vale %ld %s y %ld puntos)",
			the_unique_obj(otmp) ? The(name) : name,
...
int how;
{
...
#ifdef WIZARD
	    if (wizard) {
		pline("Eres un%s mag%c muy trampos%c, según parece.",
			flags.female? "a" : "",
			flags.female? 'a' : 'o',
			flags.female? 'a' : 'o');
		return;
	...
	if (Lifesaved && (how <= GENOCIDED)) {
		...
		pline("Tu medallón %s!",
		      !Blind ? "empieza a brillar" : "se siente caliente");
		if (how == CHOKING) pline("Vomitas...");
		Te_sientes("mucho mejor!");
		pline("¡El medallón se desmenuza en polvo!");
		...
		savelife(how);
		if (how == GENOCIDED)
			pline("Desgraciadamente, aún estás genocidad%c...",
				flags.female? 'a' : 'o');
		else {
			killer = 0;
			killer_format = 0;
	...
	if ((
		...
		if(yn("¿Morir?") == 'y') goto die;
		pline("Muy bien, no te %s.",
			(how == CHOKING) ? "atragantas" : "mueres");
...
die:
	...
	if (moves <= 1 && how < PANICKED)	/* You die... --More-- */
	    pline("No pases por ir.  No cobres 200 %s.", currency(200L));
...
# endif
	...
	if (bones_ok && u.ugrave_arise < LOW_PM) {
		...
		corpse = mk_named_object(CORPSE, &mons[mnum],
				       u.ux, u.uy, plname);
		Strcpy(prefix, killed_by_prefix[how]);
		if ((p = strchr(prefix, '@')) != 0)
		    *p = flags.female? 'a' : 'o';
		Sprintf(pbuf, "%s, %s%s", plname,
			killer_format == NO_KILLER_PREFIX ? "" : prefix,
	...
	if (how == QUIT) {
		...
		if (u.uhp < 1) {
			...
			Strcpy(kilbuf, "abandonó cuando ya estaba en la barca de Caronte");
		}
	}
	...
	if (strcmp(flags.end_disclose, "none")
	 && strcmp(flags.end_disclose, "nada")
	 && how != PANICKED)
		disclose(how, taken);
	...
	if (bones_ok) {
#ifdef WIZARD
	    if (!wizard || yn("¿Guardar los huesos?") == 'y')
#endif
		savebones(corpse);
...
*/
	if (u.uhave.amulet) Strcat(kilbuf, " (con el Amuleto)");
	else if (how == ESCAPED) {
	    if (Is_astralevel(&u.uz))	/* offered Amulet to wrong deity */
		Strcat(kilbuf, " (en desgracia celestial)");
	    else if (carrying(FAKE_AMULET_OF_YENDOR))
		Strcat(kilbuf, " (con un Amuleto falso)");
		/* don't bother counting to see whether it should be plural */
	}

	if (!done_stopprint) {
	    Sprintf(pbuf, "%s %s %s %s...", Goodbye(), plname,
		   flags.female ? "la" : "el",
		   how != ASCENDED ?
		      (const char *) ((flags.female && urole.name.f) ?
		         urole.name.f : urole.name.m) :
		      (const char *) (flags.female ? "Semidiosa" : "Semidios"));
	    putstr(endwin, 0, pbuf);
	    putstr(endwin, 0, "");
	}
	...
	if (how == ESCAPED || how == ASCENDED) {
	    ...
	    mtmp = mydogs;
	    if (!done_stopprint) Strcpy(pbuf, "Tú");
	    plural = mtmp != (struct monst *)0;
	    if (mtmp) {
		while (mtmp) {
		    if (!done_stopprint)
			Sprintf(eos(pbuf), " y %s", mon_nam(mtmp));
		    if (mtmp->mtame)
			u.urexp += mtmp->mhp;
		    mtmp = mtmp->nmon;
		}
	    ...
	    if (!done_stopprint) {
		/* LENGUA:  In American Spanish the plural forms would be
		   "fueron a su..." and "escaparon..." */
		Sprintf(eos(pbuf), "%s con %ld punto%s,",
			plural?(how==ASCENDED ? "fueron por su recompensa"
					      : "escaparon del calabozo")
			      :(how==ASCENDED ? "fuiste por tu recompensa"
		...
		for (i = 0; i < val->size && !done_stopprint; i++) {
			...
			Sprintf(pbuf,
				"%8ld pieza%s de vidrio coloreado sin valor",
				count, plur(count));
		    }
		    putstr(endwin, 0, pbuf);
		}
		...
		Sprintf(pbuf, "%s más allá de los confínes del calabozo",
			(u.uz.dlevel < 0) ? "Falleciste" : ends[how]);
	    } else {
		/* more conventional demise */
		const char *where = dungeons[u.uz.dnum].dname;

		if (Is_astralevel(&u.uz)) where = "El Plano Astral";
		Sprintf(pbuf, "%s en %s", ends[how], where);
		if (!In_endgame(&u.uz) && !Is_knox(&u.uz))
		    Sprintf(eos(pbuf), " en nivel %d del calabozo",
			    In_quest(&u.uz) ? dunlev(&u.uz) : depth(&u.uz));
	    }
	    pbuf[0] = toupper2(pbuf[0]);
	    p = pbuf;
	    while ((p = strchr(p, '@')) != (char *)0)
		    *p = gen;

	    Sprintf(eos(pbuf), " con %ld punto%s,",
		    u.urexp, plur(u.urexp));
	    putstr(endwin, 0, pbuf);
	}

	if (!done_stopprint) {
	    Sprintf(pbuf, "y %ld pieza%s de oro, después de %ld movimiento%s.",
		    umoney, plur(umoney), moves, plur(moves));
	    putstr(endwin, 0, pbuf);
	}
	if (!done_stopprint) {
	    Sprintf(pbuf,
	     "Eras de nivel %d con un máximo de %d punto%s de golpe cuando %s.",
...
boolean identified, all_containers;
{
	...
	for (box = list; box; box = box->nobj) {
		...
		if (box->otyp == BAG_OF_TRICKS) {
		    ...
		    Sprintf(buf, "Contenido de %s:", the(xname(box)));
		    putstr(tmpwin, 0, buf);
		    putstr(tmpwin, 0, "");
		    ...
		    const char *caja = xname(box);
		    pline("%s vací%c.", Tobjnam(box, "está"), isfeminine(caja)? 'a' : 'o');
...
boolean ask;
{
     ...
     */
    if (ntypes != 0) {
	c = ask ? yn_function("¿Quieres un listado de las criaturas vencidas?",
			      es_ynqchars, ynqchars, defquery) : defquery;
	if (c == 'q') done_stopprint++;
	if (c == 'y') {
	    klwin = create_nhwindow(NHW_MENU);
	    putstr(klwin, 0, "Criaturas vencidas:");
	    putstr(klwin, 0, "");
		...
		if (mons[i].mlevel == lev && (nkilled = mvitals[i].died) > 0) {
			...
			Sprintf(buf, "%s%s",
				...
				mons[i].mname);
			if (nkilled > 1)
			    Sprintf(eos(buf)," (%d veces)", nkilled);
		    } else {
	    ...
	    if (ntypes > 1) {
		putstr(klwin, 0, "");
		Sprintf(buf, "%ld criaturas vencidas.", total_killed);
		putstr(klwin, 0, buf);
	    }
...
boolean ask;
{
    ...
    if (ngenocided != 0) {
	c = ask ? yn_function("¿Quieres una lista de especies genocidadas?",
			      es_ynqchars, ynqchars, defquery) : defquery;
	if (c == 'q') done_stopprint++;
	if (c == 'y') {
	    klwin = create_nhwindow(NHW_MENU);
	    putstr(klwin, 0, "Especies genocidadas:");
	    putstr(klwin, 0, "");

	    for (i = LOW_PM; i < NUMMONS; i++)
		if (mvitals[i].mvflags & G_GENOD) {
		    if ((mons[i].geno & G_UNIQ) && i != PM_HIGH_PRIEST)
			/* LENGUA:  The English version prints "the" when the
			   monster's name is a proper name.  This is a bug,
			   corrected here, but it probably doesn't matter since
			   no existing unique monster can be genocided */
			Sprintf(buf, "%s%s",
				!type_is_pname(&mons[i])
					? ((is_female(&mons[i]))
						? "La "
	    ...
	    putstr(klwin, 0, "");
	    Sprintf(buf, "%d especie%s genocidada%s.", ngenocided,
