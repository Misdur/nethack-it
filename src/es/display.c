    ...
    register xchar worm_tail;	/* mon is actually a worm tail */
{
	...
	switch (mon->m_ap_type) {
	    default:
		impossible("display_monster:  valor incorrecto de m_ap_type [ = %d ]",
							(int) mon->m_ap_type);
	    case M_AP_NOTHING:
    ...
    register struct monst *mon;
{
	...
	glyph = mon_to_glyph(mon);
    	...
    	impossible("¿display_warning no no concide con tipo de advertencia?");
        return;
    }
    ...
    int x, y;
{
    ...
    if (!tglyph) panic("tmp_at: tglyph no inicializado");
    ...
    int x, y, glyph;
{
	...
	if (glyph >= GLYPH_WARNING_OFF) {	/* a warning */
	    text = "advertencia";	offset = glyph - GLYPH_WARNING_OFF;
	} else if (glyph >= GLYPH_SWALLOW_OFF) {	/* swallow border */
	    text = "borde de tragar";	offset = glyph - GLYPH_SWALLOW_OFF;
	} else if (glyph >= GLYPH_ZAP_OFF) {		/* zap beam */
	    text = "rayo de agitación";	offset = glyph - GLYPH_ZAP_OFF;
	} else if (glyph >= GLYPH_EXPLODE_OFF) {	/* explosion */
	    text = "explosión";		offset = glyph - GLYPH_EXPLODE_OFF;
	} else if (glyph >= GLYPH_CMAP_OFF) {		/* cmap */
	    text = "cmap_index";	offset = glyph - GLYPH_CMAP_OFF;
	} else if (glyph >= GLYPH_OBJ_OFF) {		/* object */
	    text = "objeto";		offset = glyph - GLYPH_OBJ_OFF;
	} else if (glyph >= GLYPH_RIDDEN_OFF) {		/* ridden mon */
	    text = "mon cabalgado";	offset = glyph - GLYPH_RIDDEN_OFF;
	} else if (glyph >= GLYPH_BODY_OFF) {		/* a corpse */
	    text = "cadáver";		offset = glyph - GLYPH_BODY_OFF;
	} else if (glyph >= GLYPH_DETECT_OFF) {		/* detected mon */
	    text = "mon detectado";	offset = glyph - GLYPH_DETECT_OFF;
	} else if (glyph >= GLYPH_INVIS_OFF) {		/* invisible mon */
	    text = "mon invisible";	offset = glyph - GLYPH_INVIS_OFF;
	} else if (glyph >= GLYPH_PET_OFF) {		/* a pet */
	    text = "mascota";		offset = glyph - GLYPH_PET_OFF;
	} else {					/* a monster */
	    text = "monstruo";		offset = glyph;
	}

	impossible("show_glyph:  mala pos %d %d con glifo %d [%s %d].",
						x, y, glyph, text, offset);
	return;
    }

    if (glyph >= MAX_GLYPH) {
	impossible("show_glyph:  mal glifo %d [max %d] en (%d,%d).",
					glyph, MAX_GLYPH, x, y);
	return;
    }
    ...
    xchar x,y;
{
	...
	case DRAWBRIDGE_DOWN:
	    ...
	    break;
	default:
	    impossible("back_to_glyph:  clase de nivel desconocida [ = %d ]",ptr->typ);
	    idx = S_room;
	    break;
    }
    ...
    int loc;
{
	...
	impossible("swallow_to_glyph: mal lugar de tragar");
	loc = S_sw_br;
    }
    ...
    int beam_type;
{
	...
	impossible("zapdir_to_glyph:  clase de rayo ilegal");
	beam_type = 0;
    }
...
type_to_name(type)
    int type;
{
    return (type < 0 || type > MAX_TYPE) ? "desconocido" : type_names[type];
}
...
set_wall_state()
{
	...
	if (bad_count[x]) {
	    if (y == 0) {
		y = 1;	/* only print once */
		pline("set_wall_type: problemas con modo de pared: ");
	    }
...
do_twall:
		switch (lev->wall_info & WM_MASK) {
			...
			break;
		    default:
			impossible("wall_angle: modo de pared T desconocido %d",
	...
	case VWALL:
		switch (lev->wall_info & WM_MASK) {
		    ...
		    case 2: idx = seenv & (SV0|SV1|SV5|SV6|SV7) ? S_vwall :
			    ...
			    break;
		    default:
			impossible("wall_angle: modo de vwall desconocido %d",
				lev->wall_info & WM_MASK);
			idx = S_stone;
			break;
		}
...
horiz:
		switch (lev->wall_info & WM_MASK) {
		    ...
		    case 2: idx = seenv & (SV0|SV1|SV2|SV3|SV7) ? S_hwall :
			    ...
			    break;
		    default:
			impossible("wall_angle: modo de hwall desconocido %d",
				lev->wall_info & WM_MASK);
			idx = S_stone;
			break;
		}
...
#define set_corner(idx, lev, which, outer, inner, name)	\
	...
	default:							    \
	    impossible("wall_angle: modo desconocido de %s %d", name,		    \
		(lev)->wall_info & WM_MASK);				    \
	    idx = S_stone;						    \
	    break;							    \
    }
...
do_crwall:
			if (seenv == SV4)
				...
				impossible(
				    "wall_angle: verificación de crwall de abajo");
				col = C_crwall;
			    }
		    ...
		    default:
			impossible("wall_angle: modo de crosswall desconocido");
			idx = S_stone;
			break;
		}
		break;
	...
	default:
	    impossible("wall_angle: tipo inesperado de wall %d", lev->typ);
	    idx = S_stone;
    }
    return idx;
}
