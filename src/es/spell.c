...
static const char explodes[] = "radia energía explosiva";
	...
	struct obj *bp;
{
	...
	case 6:
		if(Antimagic) {
		    shieldeff(u.ux, u.uy);
		    pline("¡El libro %s, pero no eres dañad%c!", explodes,
			  poly_gender()==1? 'a' : 'o');
		} else {
...
struct obj *spellbook;
{
	...
	"Confundid%c, tienes difficultades manejando tus acciones.",
	    ...
	    pline("Te encuentras leyendo la %s línea una y otra vez.",
		  spellbook == book ? "próxima" : "primera");
	}
	return gone;
}
...
struct obj *book2;
{
    ...
    pline("Vuelves las páginas del Libro de los Muertos...");
	...
	if(!u.uhave.bell || !u.uhave.menorah) {
	    pline("Un escalofrío se corre través de tu %s.", body_part(SPINE));
	    if(!u.uhave.bell) Oyes("un campaneo leve...");
	    if(!u.uhave.menorah) pline("El doppelganger de Vlad está entretenido.");
	    return;
	}
	...
	if(arti_cursed) {
	    pline("¡Se falla la invocación!");
	    pline("A lo menos uno de tus artefactos está maldito...");
	    ...
	    pline("Tienes un sentido de que %s está errado...", something);
	    goto raise_dead;
	}
	return;
    }
...
raise_dead:
	...
	switch(rn2(3)) {
	case 0:
	    pline("¡Tus ancestros están enfadados de ti!");
	    break;
	case 1:
	    pline("¡Las lápidas del cementerio empiezan mover!");
	    break;
	default:
	    pline("¡Ay mío!  ¡Tu nombre aparece en el libro!");
	}
    }
    return;
}
	...
	for (i = 0; i < MAXSPELL; i++)  {
		if (spellid(i) == booktype)  {
			if (book->spestudied > MAX_SPELL_STUDY) {
			    pline("Este libro es demasiado indistinto para leerse más.");
			    book->otyp = booktype = SPE_BLANK_PAPER;
			} else if (spellknow(i) <= 1000) {
			    pline("Tu saber de %s está más agudo.", splname);
			    incrnknow(i);
			    book->spestudied++;
			...
			incrnknow(i);
			book->spestudied++;
			pline(i > 0 ? "Añades %s a tu repertorio." : "Aprendes %s.",
			    splname);
			makeknown((int)booktype);
			break;
		}
	...
	if (delay && !confused && spellbook == book &&
		...
		if (!spellbook->blessed &&
			...
			if (Role_if(PM_WIZARD) && read_ability < 20 &&
			    ...
			    char qbuf[QBUFSZ];
			    Sprintf(qbuf,
		      "Este libro es %sdifícil de comprender. ¿Seguir?",
				    (read_ability < 12 ? "muy " : ""));
			    if (yn(qbuf) != 'y') {
	...
	int *spell_no;
{
	...
	if (spellid(0) == NO_SPELL)  {
	    pline("No sabes ningún hechizo ahora.");
	    return FALSE;
	}
	if (flags.menu_style == MENU_TRADITIONAL) {
	    ...
	    for(;;)  {
		Sprintf(qbuf, "¿Echas qué hechizo? [%s ?]", lets);
...
cast_protection()
{
	...
	if (gain > 0) {
		...
		if (u.uspellprot)
		    pline("La niebla %s alrededor de ti se hace más densa.",
			      hgolden);
		else
	    ...
	    pline("Tu piel siente cálida por un momento.");
	}
}
...
boolean atme;
{
	...
	if (spellknow(spell) <= 0) {
	    pline("Tu saber de este hechizo está retorcido.");
	    pline("Invoca imágenes pesadilladas en tu mente...");
	    spell_backfire(spell);
	    return(0);
	} else if (spellknow(spell) <= 100) {
	    pline("Te fuerzas para recordar el hechizo.");
	} else if (spellknow(spell) <= 1000) {
	    pline("Tu saber de este hechizo se pone débil.");
	}
	...
	if (u.uhunger <= 10 && spellid(spell) != SPE_DETECT_FOOD) {
		...
		"Tu concentración se falla mientras traigas tantas cosas.")) {
	    return (1);
	} else if (!freehand()) {
	...
	if (u.uhave.amulet) {
		Sientes("el amuleto consumiendo tu energía.");
		energy += rnd(2*energy);
	}
	if(energy > u.uen)  {
		pline("No tienes bastante energía para echar ese hechizo.");
		return(0);
	} else {
	...
	case SPE_STONE_TO_FLESH:
		if (!(objects[pseudo->otyp].oc_dir == NODIR)) {
			if (atme) u.dx = u.dy = u.dz = 0;
			    ...
			    pline("¡La energía mágica se libera!");
			}
			if(!u.dx && !u.dy && !u.dz) {
			    if ((damage = zapyourself(pseudo, TRUE)) != 0) {
				losehp(damage,
	...
	case SPE_CURE_SICKNESS:
		if (Sick) pline("No estás mal%c más.",
				poly_gender()==1? 'a' : 'o');
		if (Slimed) {
...
throwspell()
{
	...
	if (u.uinwater) {
	    pline("¡Estás bromando! ¿En estos tiempos?"); return 0;
	} else if (Is_waterlevel(&u.uz)) {
	    pline("Tendrías que esperar hasta que el sol salga."); return 0;
	}

	pline("¿A dónde quieres echar el hechizo?");
	cc.x = u.ux;
	cc.y = u.uy;
	...
	if (distmin(u.ux, u.uy, cc.x, cc.y) > 10) {
	    ...
	    pline("¡Tu mente no puede fijarse a aquella locación!");
	    return 0;
	} else {
	    u.dx=cc.x;
	    u.dy=cc.y;
...
struct obj *obj;
{
	...
	for (i = 0; i < MAXSPELL; i++) {
	    if (spellid(i) == obj->otyp) {
	         pline("Error: Hechizo %s ya está sabido.",
