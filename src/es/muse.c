...
#define POTION_OCCUPANT_CHANCE(n) (13 + 2*(n))	/* also in potion.c */
	    ...
	    if (potion_descr && !strcmp(potion_descr, "láctea")) {
			...
			if (vis) {
			    const char *que = Hallucination ? rndmonnam() : (const char *)"fantasma";
			    boolean fem = isfeminine(que);
			    pline("¡Como %s abre la botella, %s enorm%c emerge!",
				mon_nam(mon), an(que), fem? 'a' : 'o');
			    pline("%s se espanta a la muerte, y no puede mover.",
				    Monnam(mon));
			}
			mon->mcanmove = 0;
			mon->mfrozen = 3;
	    ...
	    if (potion_descr && !strcmp(potion_descr, "humosa") &&
		...
		if (!mtmp) {
		    ...
		    if (vis)
			pline("¡En una nube de humo, %s emerge!",
							a_monnam(mtmp));
		    pline("%s habla.", vis ? Monnam(mtmp) : Something);
		/* I suspect few players will be upset that monsters */
		/* can't wish for wands of death here.... */
		    if (rn2(2)) {
			verbalize("¡Me has librado!");
			mtmp->mpeaceful = 1;
			set_malign(mtmp);
		    } else {
			verbalize("A buenas horas.");
			if (vis) pline("%s se desvanece.", Monnam(mtmp));
			mongone(mtmp);
		    }
		}
		return 2;
	...
	if (obj->oclass == WAND_CLASS && obj->cursed && !rn2(100)) {
	    ...
	    if (flags.soundok) {
		if (vis) pline("¡%s agita %s, que de repente estalla!",
			Monnam(mon), an(xname(obj)));
		else Oyes("un zap y una explosión en la distancia.");
	    }
	    m_useup(mon, obj);
	    if (mon->mhp <= dam) {
...
boolean self;
{
	if (!canseemon(mtmp)) {
		...
		pline("¡%s se agita con %s!", Monnam(mtmp), doname(otmp));
	else {
		pline("¡%s agita %s!", Monnam(mtmp), an(xname(otmp)));
		stop_occupation();
	}
}
...
struct obj *otmp;
{
	...
	if (vismon)
	    pline("¡%s lee %s!", Monnam(mtmp), onambuf);
	else
	    Oyes("%s leyendo %s.",
		al(x_monnam(mtmp, ARTICLE_A, (char *)0,
		    (SUPPRESS_IT|SUPPRESS_INVISIBLE|SUPPRESS_SADDLE), FALSE)),
		onambuf);

	if (mtmp->mconf)
	    pline("Confundid%c, %s pronuncia mal las palabras mágicas...",
		  mon_gender(mtmp)? 'a' : 'o',
...
struct obj *otmp;
{
	if (canseemon(mtmp)) {
		otmp->dknown = 1;
		pline("¡%s bebe %s!", Monnam(mtmp), singular(otmp, doname));
	} else
		if (flags.soundok)
			Oyes("un sonido de beber.");
}
...
struct monst *mtmp;
{
	...
	const char *mcsa = "%s puede ver de nuevo.";
...
#define m_flee(m)	if (fleetim && !m->iswiz) \
	...
	case MUSE_UNICORN_HORN:
		if (vismon) {
		    if (otmp)
			pline("¡%s usa un cuerno de unicornio!", Monnam(mtmp));
		    else
			pline("¡El punto del cuerno %s deslumbra!", del(mon_nam(mtmp)));
		}
		if (!mtmp->mcansee) {
		    mtmp->mcansee = 1;
		    mtmp->mblinded = 0;
		    ...
		    if (vismon)
			pline("%s parece más estable ahora.", Monnam(mtmp));
		} else impossible("¿No necesidad para cuerno de unicornio?");
		return 2;
	case MUSE_BUGLE:
		if (vismon)
			pline("¡%s toca %s!", Monnam(mtmp), doname(otmp));
		else if (flags.soundok)
			Oyes("un clarín tocando la reveille!");
		awaken_soldiers();
		return 2;
...
mon_tele:
		...
		if ((
		    ...
		    if (vismon)
			pline("%s parece desorientad%c por un momento.",
				Monnam(mtmp),
				mon_gender(mtmp)? 'a' : 'o');
		    return 2;
		}
	...
	case MUSE_SCR_TELEPORTATION:
		...
		if (obj_is_cursed || mtmp->mconf) {
			...
			if (mon_has_amulet(mtmp) || In_endgame(&u.uz)) {
			    if (vismon)
				pline("%s parece muy desorientad%c por un momento.",
					Monnam(mtmp),
					mon_gender(mtmp)? 'a' : 'o');
			    return 2;
			}
			...
			if (nlev == depth(&u.uz)) {
			    if (vismon)
				pline("%s se tembla por un momento.",
								Monnam(mtmp));
			    return 2;
			}
			get_level(&flev, nlev);
	...
	case MUSE_WAN_DIGGING:
		...
		if (IS_FURNITURE(levl[mtmp->mx][mtmp->my].typ) ||
		    ...
		    (sstairs.sx && sstairs.sx == mtmp->mx &&
				   sstairs.sy == mtmp->my)) {
			pline("El rayo de cavar es inefectivo.");
			return 2;
		}
		if (!Can_dig_down(&u.uz)) {
		    if(canseemon(mtmp)) {
			const char *piso = surface(mtmp->mx, mtmp->my);
		...
		seetrap(ttmp);
		if (vis) {
		    pline("%s ha hecho una abertura en %s.", Monnam(mtmp),
				surface(mtmp->mx, mtmp->my));
		    pline("%s se %s través...", Monnam(mtmp),
			  is_flyer(mtmp->data) ? "zambulle" : "cae");
		} else if (flags.soundok)
			Oyes("%s estrellándose través %s.", something,
				del(surface(mtmp->mx, mtmp->my)));
	...
	case MUSE_TRAPDOOR:
		...
		if (vis) {
			struct trap *t;
			t = t_at(trapx,trapy);
			pline("¡%s %s en una %s!", Monnam(mtmp),
			locomotion(mtmp->data, "se salta"),
			t->ttyp == TRAPDOOR ? "trampa" : "abertura");
	...
	case MUSE_UPSTAIRS:
		...
		if (ledger_no(&u.uz) == 1) {
			if (mon_has_special(mtmp))
				return 0;
			if (vismon)
			    pline("¡%s escapa del calabozo!", Monnam(mtmp));
			mongone(mtmp);
			return 2;
		}
		m_flee(mtmp);
		if (Inhell && mon_has_amulet(mtmp) && !rn2(4) &&
		    ...
		    if (vismon) pline(
     "Como %s sube la escalera, una fuerza misteriosa l%c curcunda por un momento...",
				     mon_nam(mtmp), mon_gender(mtmp)? 'a' : 'o');
		    ...
		    if (vismon) pline("¡%s escapa subiendo la escalera!", Monnam(mtmp));
		    migrate_to_level(mtmp, ledger_no(&u.uz) - 1,
	...
	case MUSE_DOWNSTAIRS:
		m_flee(mtmp);
		if (vismon) pline("¡%s escapa bajando la escalera!", Monnam(mtmp));
		migrate_to_level(mtmp, ledger_no(&u.uz) + 1,
				 MIGR_STAIRS_UP, (coord *)0);
		return 2;
	case MUSE_UP_LADDER:
		m_flee(mtmp);
		if (vismon) pline("¡%s escapa subiendo la escalera de mano!", Monnam(mtmp));
		migrate_to_level(mtmp, ledger_no(&u.uz) - 1,
				 MIGR_LADDER_DOWN, (coord *)0);
		return 2;
	case MUSE_DN_LADDER:
		m_flee(mtmp);
		if (vismon) pline("¡%s escapa bajando la escalera de mano!", Monnam(mtmp));
		migrate_to_level(mtmp, ledger_no(&u.uz) + 1,
	...
	case MUSE_SSTAIRS:
		...
		if (sstairs.up) {
			if (vismon)
			    pline("¡%s escapa subiendo la escalera!", Monnam(mtmp));
			if(Inhell) {
		    ...
		    pline("¡%s escapa bajando la escalera!", Monnam(mtmp));
		migrate_to_level(mtmp, ledger_no(&sstairs.tolev),
	...
	case MUSE_TELEPORT_TRAP:
		m_flee(mtmp);
		if (vis) {
			pline("¡%s %s sobre una telepuerta!", Monnam(mtmp),
				locomotion(mtmp->data, "salta"));
			if (levl[trapx][trapy].typ == SCORR) {
			    levl[trapx][trapy].typ = CORR;
			    unblock_point(trapx, trapy);
			}
	...
	case MUSE_POT_HEALING:
		...
		if (vismon) pline("%s parece mejor.", Monnam(mtmp));
	...
	case MUSE_POT_EXTRA_HEALING:
		...
		if (vismon) pline("%s parece mucho mejor.", Monnam(mtmp));
	...
	case MUSE_POT_FULL_HEALING:
		...
		if (vismon) pline("%s parece completamente curad%c.",
	...
	default: impossible("¡%s quería hacer la acción %d?", Monnam(mtmp),
			m.has_defense);
		break;
	}
	return 0;
	...
	case WAN_STRIKING:
		...
		if (mtmp == &youmonst) {
			...
			if (Antimagic) {
			    ...
			    pline("¡La varita te golpea!");
			    tmp = d(2,12);
			    if(Half_spell_damage) tmp = (tmp+1) / 2;
			    losehp(tmp, "varita", KILLED_BY_AN);
			} else pline("La varita te falla.");
			stop_occupation();
			nomul(0);
		...
		if (mtmp == &youmonst) {
			...
			if (mtmp->ispriest &&
				*in_rooms(mtmp->mx, mtmp->my, TEMPLE)) {
				...
				pline("¡%s resiste la mágia!", Monnam(mtmp));
			    mtmp->msleeping = 0;
			    if(mtmp->m_ap_type) seemimic(mtmp);
...
struct monst *mtmp;
{
	...
	case MUSE_FROST_HORN:
		if (oseen) {
			makeknown(otmp->otyp);
			pline("¡%s toca %s!", Monnam(mtmp), an(xname(otmp)));
		} else
			Oyes("un cuerno estando tocado.");
		otmp->spe--;
		m_using = TRUE;
	...
	case MUSE_SCR_EARTH:
		...
		if (canspotmon(mtmp)) {
		    pline("¡%s retumba %s %s!",
				capitalize(ceiling(mtmp->mx, mtmp->my)),
	    			otmp->blessed ? "alrededor" : "arriba",
				del(mon_nam(mtmp)));
		    if (oseen) makeknown(otmp->otyp);
	    	...
	    	for (x = mmx-1; x <= mmx+1; x++) {
				...
				if (cansee(mtmp2->mx, mtmp2->my)) {
				    pline("¡%s es golpead%c por %s!", Monnam(mtmp2),
						mon_gender(mtmp)? 'a' : 'o',
	    	    	    			doname(otmp2));
				...
				if (helmet) {
					...
					if (canspotmon(mtmp2))
					    pline("Por fortuna, %s está llevando un yelmo duro.", mon_nam(mtmp2));
					else if (flags.soundok)
					    Oyes("un sonido de tantán.");
					if (mdmg > 2) mdmg = 2;
				    } else {
					if (canspotmon(mtmp2)) {
					    const char *que = xname(helmet);
					    pline("%s %s %s no l%c protege.",
						isfeminine(que)? "La" : "El",
						que,
						del(mon_nam(mtmp2)),
						mon_gender(mtmp2)? 'a' : 'o');
					    }
				    }
				}
	    	    	    	mtmp2->mhp -= mdmg;
	    	    	    	if (mtmp2->mhp <= 0) {
				    pline("Se mata %s.", mon_nam(mtmp2));
	    	    	    	    mondied(mtmp2);
				}
	    	    	    }
	...
	case MUSE_SCR_FIRE:
		...
		if (mtmp->mconf) {
			if (vis)
			    pline("¡Oh, que lindo fuego!");
		} else {
			struct monst *mtmp2;
			int num;

			if (vis)
			    pline("¡El pergamino se arroja en una torre de fuego!");
			shieldeff(mtmp->mx, mtmp->my);
			pline("No se daña %s.", mon_nam(mtmp));
			...
			else losehp(num, "pergamino de fuego", KILLED_BY_AN);
...
skipmsg:
			if (vismon) {
			    pline("%s parece inquiet%c.", Monnam(mtmp),
				mon_gender(mtmp)? 'a' : 'o');
			    if(!objects[POT_GAIN_LEVEL].oc_name_known
			      && !objects[POT_GAIN_LEVEL].oc_uname)
				docall(otmp);
		...
		if (vismon) pline("%s parece más expert%c.", Monnam(mtmp),
			mon_gender(mtmp)? 'a' : 'o');
	...
	case MUSE_POT_INVISIBILITY:
		...
		if (vismon && mtmp->minvis) {	/* was seen, now invisible */
		    if (See_invisible)
			pline("El cuerpo %s adquiere una transparencia %s.",
			      del(nambuf),
			      Hallucination ? "normal" : "extraña");
		    else
			pline("De repente, no puedes ver %s.", al(nambuf));
		    if (oseen) makeknown(otmp->otyp);
		}
	...
	case MUSE_POT_POLYMORPH:
		mquaffmsg(mtmp, otmp);
		if (vismon) pline("¡%s de repente muta!", Monnam(mtmp));
	...
	case MUSE_POLY_TRAP:
		if (vismon)
		    pline("¡%s deliberadamente %s sobre una trampa de polímorfo!",
			Monnam(mtmp),
			locomotion(mtmp->data, "salta"));
	...
	case MUSE_BULLWHIP:
		...
		if (uwep && !rn2(5)) {
		    const char *The_whip = vismon ? "El látigo" : "Un látigo";
		    int where_to = rn2(4);
		    struct obj *obj = uwep;
		    const char *hand;
		    ...
		    if (vismon)
			pline("¡%s echa un látigo para tu%s %s!",
			      Monnam(mtmp), bimanual(obj)? "s" : "", hand);
		    if (obj->otyp == HEAVY_IRON_BALL) {
			pline("%s falla de envolver %s.",
			      The_whip, the_weapon);
			return 1;
		    }
		    pline("¡%s envuelve %s que blandes!",
			  The_whip, the_weapon);
		    if (welded(obj)) {
			pline("%sEstá%s soldad%c%s a tu%s %s%c",
			      !obj->bknown ? "¡" : "",
			      (obj->quan == 1L) ? "" : "n",
			      isfeminine(the_weapon)? 'a' : 'o',
			      (obj->quan == 1L) ? "" : "s",
			      bimanual(obj)? "s" : "",
			      hand, !obj->bknown ? '!' : '.');
		    ...
		    if (!where_to) {
			pline("El látigo se resbala libre.");  /* not `The_whip' */
			return 1;
			...
			case 1:		/* onto floor beneath mon */
			    pline("¡%s arranca %s de tu%s %s!", Monnam(mtmp),
				  the_weapon, bimanual(obj)? "s" : "", hand);
...
struct monst *mtmp;
{
	pline("Por alguna razón, la presencia %s es conocida de ti.",
		del(noit_mon_nam(mtmp)));
	cls();
#ifdef CLIPPING
	...
	if (unconscious()) {
		...
		multi = -1;
		sprintf(buf, "Molestad%c, eres choquead%c a toda conciencia.",
			fem, fem);
		nomovemsg = buf;
	}
	newsym(mtmp->mx,mtmp->my);
...
const char *str;
{
	...
	if (orefl && orefl->otyp == SHIELD_OF_REFLECTION) {
	    ...
	    if (str)
		reflector = "la armadura";
	    reflect = TRUE;
	} else if (mon->data == &mons[PM_SILVER_DRAGON] ||
		mon->data == &mons[PM_CHROMATIC_DRAGON]) {
	    /* Silver dragons only reflect when mature; babies do not */
	    if (str)
		reflector = "las escamas";
	    reflect = TRUE;
	}
        if (reflector) {
	    /* Grammatical hack:  form possesive correctly when the monster
	       name is a pronoun */
	    whose = del(mon_nam(mon));
	    if (strcmp(whose, "de él") == 0) {
		char its_reflector[20];
		char buf[BUFSZ];
		int len;

		/* "sus escamas" instead of "las escamas de él" */
		Sprintf(its_reflector, "su%s", reflector+2);
		Sprintf(buf, str, reflector, "");
		len = strlen(buf);
		/* "...es reflejada por sus escamas ."; drop the space
		   before the period */
		if (len >= 2 && buf[len-2] == ' ') {
		    buf[len-2] = buf[len-1];
		    buf[len-1] = 0;
...
const char *fmt, *str;
{
	...
	if (EReflecting & W_ARMS) {
	    ...
	    /* Due to wielded artifact weapon */
	    if (fmt && str)
	    	Sprintf(buf, fmt, str, " arma");
	    reflect = TRUE;
	} else if (EReflecting & W_AMUL) {
	    if (fmt && str) {
	    	Sprintf(buf, fmt, str, " medallón");
	    	makeknown(AMULET_OF_REFLECTION);
	    }
	    reflect = TRUE;
	} else if (EReflecting & W_ARM) {
	    if (fmt && str)
	    	Sprintf(buf, fmt, str, " armadura");
	    reflect = TRUE;
	} else if (youmonst.data == &mons[PM_SILVER_DRAGON]) {
	    if (fmt && str)
	    	Sprintf(buf, fmt, str, "s escamas");
	    reflect = TRUE;
	}
	if (buf[0] != 0) {
	    /* Grammatical hack:  Replace "La mirada de él" with
	       "Su mirada".  TODO: generalize this to other nouns */
	    if (strncmp(buf, "La mirada de él ", 16) == 0)
		pline("Su mirada %s", buf+16);
	    else
		pline(buf);
	}
	return reflect;
...
boolean stoning;
{
	...
	pline("%s %ss %s.", Monnam(mon),
		    (obj->otyp == POT_ACID) ? "bebe" : "come",
		    distant_name(obj,doname));
	obj->quan = save_quan;
    } else if (flags.soundok)
	Oyes("sonidos de %s.", (obj->otyp == POT_ACID) ? "bebida" : "mascada");
    m_useup(mon, obj);
    if (((obj->otyp == POT_ACID) || acidic(&mons[obj->corpsenm])) &&
		    !resists_acid(mon)) {
	mon->mhp -= rnd(15);
	pline("%s tiene un caso muy mal de ácido de estómago.",
	    Monnam(mon));
    }
    if (mon->mhp <= 0) {
	pline("¡%s se muere!", Monnam(mon));
	...
	if (Hallucination)
    pline("Qué lástima - ¡%s ha ruinado una pieza futura de arte!",
	    mon_nam(mon));
	else
	    pline("¡%s parece flexible!", Monnam(mon));
    }
    if (obj->otyp == CORPSE && obj->corpsenm == PM_LIZARD && mon->mconf) {
	mon->mconf = 0;
	if (canseemon(mon))
	    pline("%s parece más estable ahora.", Monnam(mon));
    }
