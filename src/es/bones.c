...
struct obj *corpse;
{
...
#ifdef WIZARD
		if (wizard) {
		    if (yn("Ya existe el archivo de huesos.  ¿Quieres reemplazarlo?") == 'y') {
			if (delete_bonesfile(&u.uz)) goto make_bones;
			else pline("No puedo borrar los huesos viejos.");
		    }
		}
#endif
		return;
 ...
 make_bones:
	...
	if (u.ugrave_arise == (NON_PM - 1)) {
		...
		mtmp = christen_monst(mtmp, plname);
		newsym(u.ux, u.uy);
		pline("Tu cuerpo se levanta de entre los muertos como %s...",
...
# ifdef WIZARD
		if (wizard)
			pline("Espacio insuficiente para crear el archivo de huesos.");
# endif
		(void) close(fd);
		cancel_bonesfile();
		return;
...
getbones()
{
	...
	if ((ok = uptodate(fd, bones)) == 0) {
	    ...
	    if (!wizard)
#endif
		pline("Desechando huesos inútiles; que no cunda el pánico...");
	} else {
#ifdef WIZARD
		if(wizard)  {
			if(yn("¿Quieres cargar los huesos?") == 'n') {
				(void) close(fd);
				compress_bonesfile();
				return(0);
			}
		...
		if (strcmp(bonesid, oldbonesid) != 0) {
			...
			Sprintf(errbuf, "¡Esto es el nivel de huesos '%s', no '%s'!",
				oldbonesid, bonesid);
#ifdef WIZARD
			if (wizard) {
				pline("%s", errbuf);
			...
			for(mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
				...
				if (wizard)
				    pline("Quitando monstruo difunto %s de los huesos.",
					mtmp->data->mname);
#endif
				mongone(mtmp);
			    } else
	...
	if (!delete_bonesfile(&u.uz)) {
		 ...
		 */
		/* pline("No puedo borrar los huesos."); */
		return(0);
	}
	return(ok);
}
