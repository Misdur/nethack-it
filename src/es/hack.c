...
moverock()
{
	...
	if (Levitation || Is_airlevel(&u.uz)) {
	    ...
	    pline("No tienes un buen punto de apoyo para empujar %s.", the(xname(otmp)));
	...
	if (verysmall(youmonst.data)
	    ...
	    if (Blind) feel_location(sx, sy);
	    pline("Eres demasiado pequeñ%c para empujar esa %s.",
		  poly_gender()==1? 'a' : 'o',
		  xname(otmp));
	    goto cannot_push;
	}
	if (isok(rx,ry) && !IS_ROCK(levl[rx][ry].typ) &&
		...
		const char *where = surface(sx, sy)+3; /* skip article */
	    	if (Blind) feel_location(sx,sy);
	    	pline("%s no puede rodar diagonalmente en est%c %s.",
	        		The(xname(otmp)),
				isfeminine(where)? 'a' : 'e',
				where);
	    	goto cannot_push;
	    }

	    if (revive_nasty(rx, ry, "Percibes movimiento al otro lado."))
		return (-1);
		...
		if (Blind) feel_location(sx, sy);
		if (canspotmon(mtmp))
		    Hay("%s al otro lado.", a_monnam(mtmp));
		else {
		    Oyes("un monstruo detrás %s.", del(the(xname(otmp))));
		    map_invisible(rx, ry);
		}
		if (flags.verbose) {
#ifdef STEED
		    if (u.usteed)
			pline("Tal vez esa sea la razón por la qué %s no puede moverla.",
				mon_nam(u.usteed));
		    else
#endif
			pline("Tal vez esa sea la razón por la qué no puedes moverla.");
		}
		goto cannot_push;
	    }
		...
		case LANDMINE:
			...
			unblock_point(sx, sy);
			newsym(sx, sy);
			pline("¡¡CAABLAMM!!!  %s estallar %s mina terrestre.",
			      Tobjnam(otmp, "hace"),
			      ttmp->madeby_u ? "tu" : "una");
		...
		case PIT:
		    ...
		    if (!flooreffects(otmp, rx, ry, "cae")) {
			place_object(otmp, rx, ry);
		    }
		...
		case TRAPDOOR:
		    if (Blind)
			pline("¡Kerplunk!  Ya no sientes %s.",
				the(xname(otmp)));
		    else
			pline("¡%s en %s en %s y %s tapona!",
			      Tobjnam(otmp, "cae"),
			      (ttmp->ttyp == TRAPDOOR) ? "la puerta trampa" : "el hoyo",
			      surface(rx, ry),
			      (ttmp->ttyp == TRAPDOOR) ? "la" : "lo");
		    deltrap(ttmp);
		    delobj(otmp);
		    bury_objs(rx, ry);
		    if (cansee(rx,ry)) newsym(rx,ry);
...
#ifdef STEED
		    if (u.usteed)
			pline("¡%s empuja %s, haciéndola desaparecer!",
				Monnam(u.usteed), the(xname(otmp)));
		    else
#endif
			pline("¡Empujas %s y de repente desaparece!",
			    the(xname(otmp)));
		...
		if (!u.usteed) {
		  ...
		  if (moves > lastmovetime+2 || moves < lastmovetime)
		    pline("Con %s esfuerzo mueves %s.",
			  throws_rocks(youmonst.data) ? "poco" : "mucho",
			  the(xname(otmp)));
		  exercise(A_STR, TRUE);
#ifdef STEED
		} else 
...
#ifdef STEED
	  if (u.usteed)
	    pline("%s trata de mover %s, pero no puede.",
		  upstart(y_monnam(u.usteed)), the(xname(otmp)));
	  else
#endif
	    pline("Tratas de mover %s, pero en vano.", the(xname(otmp)));
	    if (Blind) feel_location(sx, sy);
	cannot_push:
	    if (throws_rocks(youmonst.data)) {
#ifdef STEED
		if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
		    pline("No tienes maña para %s %s desde %s.",
			(flags.pickup && !In_sokoban(&u.uz))
			    ? "recoger" : "empujar al lado",
			the(xname(otmp)), y_monnam(u.usteed));
		} else
#endif
		{
		    pline("Sin embargo, puedes %s fácilmente.",
			(flags.pickup && !In_sokoban(&u.uz))
			    ? "recogerla" : "empujarla a un lado");
		    if (In_sokoban(&u.uz))
		...
		pline("Sin embargo, puedes apretujarte en una pequeña abertura.");
		if (In_sokoban(&u.uz))
		    change_luck(-1);	/* Sokoban guilt */
		break;
	    } else
    ...
    xchar x, y;
{
	...
	pline("Te lastimas los dientes en %s.", 
	      IS_TREE(lev->typ) ? "el árbol" : "la roca dura.");
	nomul(0);
	return 1;
	...
	pline("Empiezas a mascar %s %s.",
	    boulder ? "una" : "una abertura en",
	    boulder ? "roca enorme" : IS_ROCK(lev->typ) ? "la piedra" : "la puerta");
	watch_dig((struct monst *)0, x, y, FALSE);
	return 1;
    } else if ((digging.effort += (30 + u.udaminc)) <= 100)  {
	if (flags.verbose)
	    pline("%s %s.",
		  digging.chew ? "Sigues mascando" : "Empiezas a mascar",
		  boulder ? "la roca enorme" :
		  IS_TREE(lev->typ) ? "el árbol" :
		  IS_ROCK(lev->typ) ? "la piedra" : "la puerta");
	digging.chew = TRUE;
	watch_dig((struct monst *)0, x, y, FALSE);
	return 1;
    }
	...
	pline("Comes la roca enorme.");	/* yum */
	...
	digtxt = "Mascas una abertura en la pared.";
	...
	digtxt = "Mascas a través del árbol.";
	lev->typ = ROOM;
    } else if (lev->typ == SDOOR) {
	if (lev->doormask & D_TRAPPED) {
	    lev->doormask = D_NODOOR;
	    b_trapped("puerta secreta", 0);
	} else {
	    digtxt = "Mascas través de la puerta secreta.";
	...
	if (lev->doormask & D_TRAPPED) {
	    ...
	    digtxt = "Mascas través de la puerta.";
	    lev->doormask = D_BROKEN;
	}

    } else { /* STONE or SCORR */
	digtxt = "Mascas un pasadizo través de la piedra.";
	lev->typ = CORR;
    }
...
#ifdef SINKS
static NEARDATA const char fell_on_sink[] = "cayó en un fregadero";
...
dosinkfall()
{
	...
	if (is_floater(youmonst.data) || (HLevitation & FROMOUTSIDE)) {
	    pline("Te tambaleas inestablemente por un momento.");
	} else {
	    ...
	    pline("¡Te estrellas contra el piso!");
...
int mode;
{
	...
	/* MRKR: Automatic digging when wielding the appropriate tool */
		...
		if (Is_stronghold(&u.uz) && is_db_wall(x,y))
		    pline("¡El puente levantizo está arriba!");
		if (Passes_walls && !may_passwall(x,y) && In_sokoban(&u.uz))
	    		pline("Las paredes del Sokoban resisten tu habilidad.");
	    }
	    return FALSE;
	}
	...
	if (closed_door(x,y)) {
		...
		if (mode == DO_MOVE) pline("Te rezumas bajo la puerta.");
	    } else if (tunnels(youmonst.data) && !needspick(youmonst.data)) {
		/* Eat the door. */
		if (mode == DO_MOVE && still_chewing(x,y)) return FALSE;
	    } else {
		if (mode == DO_MOVE) {
			...
			pline("Tratas de rezumarte bajo la puerta, pero no puedes pasar tus posesiones contigo.");
		    else if (x == ux || y == uy) {
			if (Blind || Stunned || ACURR(A_DEX) < 10 || Fumbling) {
#ifdef STEED
			    if (u.usteed) {
				No_puedes("ordenar %s que pase a través de esta puerta cerrada.",
				         al(y_monnam(u.usteed)));
			    } else
#endif
			    {
				pline("¡Ouch!  Topas una puerta.");
			        exercise(A_DEX, FALSE);
			    }
			} else pline("Esa puerta está cerrada.");
		    }
	...
	if (In_sokoban(&u.uz)) {
	    if (mode == DO_MOVE)
	    	No_puedes("pasar por ahí.");
	    return FALSE;
	}
	if (bigmonst(youmonst.data)) {
	    if (mode == DO_MOVE)
		pline("Tu cuerpo es demasiado grande para caber.");
	    return FALSE;
	}
	if (invent && (inv_weight() + weight_cap() > 600)) {
	    if (mode == DO_MOVE)
		pline("Estás trayendo demasiado para pasar.");
	    return FALSE;
	}
    }
...
domove()
{
	...
	if(((wtcap = near_capacity()) >= OVERLOADED
	   ...
	   && !Is_airlevel(&u.uz)) {
	    if(wtcap < OVERLOADED) {
		pline("No tienes suficiente vitalidad para moverte.");
		exercise(A_CON, FALSE);
	    } else
		pline("Colapsas bajo tu carga.");
	    nomul(0);
	    return;
	}
	if(u.uswallow) {
		...
		if (Is_airlevel(&u.uz) && rn2(4) &&
		    ...
		    switch(rn2(3)) {
		    case 0:
			pline("Das volteretas en el lugar.");
			exercise(A_DEX, FALSE);
			break;
		    case 1:
			No_puedes("controlar muy bien tus movimientos."); break;
		    case 2:
			pline("Es difícil caminar en aire.");
			exercise(A_DEX, TRUE);
			break;
		    }
		    return;
		...
		if (u.ustuck && (x != u.ustuck->mx || y != u.ustuck->my)) {
			 ...
			 */
			pline("%s.", mon_nam_acc(u.ustuck, "Liberas"));
			u.ustuck = 0;
		    } else {
	...
	if(mtmp) {
		...
		if(mtmp->m_ap_type && !Protection_from_shape_changers
		    ...
		    pline("Perdón, %s.", m_monnam(mtmp));
		else
		    pline("Te mueves derecho hacia %s.", mon_nam(mtmp));
		return;
	    }
		...
		if(wtcap >= HVY_ENCUMBER && moves%3) {
			...
			pline("¡Te desmayas del esfuerzo!");
			exercise(A_CON, FALSE);
	...
	if (flags.forcefight ||
		...
		if (Underwater) {
		    if (is_pool(x, y))
		        pline("%s agua vacía.",
		              expl ? "Estallas al" : "Atacas el");
		    else
		        pline("%s un lugar vacío en %s.",
		              expl ? "Estallas a" : "Atacas", the(surface(x,y)));
		} else
		    pline("%s aire vacío.", expl ? "Estallas al" : "Atacas el");
	...
	if (u.usteed && !u.usteed->mcanmove && (u.dx || u.dy)) {
		pline("¡%s no quiere moverse!", upstart(y_monnam(u.usteed)));
		nomul(0);
		return;
	} else
#endif
	if(!youmonst.data->mmove) {
		pline("Estás arraigad%c %s.",
		    poly_gender()==1? 'a' : 'o',
	...
	if(u.utrap) {
		if(u.utraptype == TT_PIT) {
			...
			pline("Tu %s queda pegad%c en una grieta.",
			    pierna, isfeminine(pierna)? 'a' : 'o');
			display_nhwindow(WIN_MESSAGE, FALSE);
			clear_nhwindow(WIN_MESSAGE);
			pline("Liberas tu %s.", pierna);
		    } else if (!(--u.utrap)) {
			pline("%s a la orilla del hoyo.",
				(In_sokoban(&u.uz) && Levitation) ?
				"Forcejeas contra las corrientes del aire y flotas" :
#ifdef STEED
				u.usteed ? "Cabalgas" :
#endif
				"Te arrastras");
...
#ifdef STEED
			if (u.usteed)
			    Norep("%s todavía está en un hoyo.",
				  upstart(y_monnam(u.usteed)));
			else
#endif
			Norep( (Hallucination && !rn2(5)) ?
				"Te has caído, y no puedes leventarte." :
				"Todavía estás en un hoyo." );
		    }
		} else if (u.utraptype == TT_LAVA) {
		    if(flags.verbose) {
#ifdef STEED
			if (u.usteed)
			    	Norep("%s está atrapad%c en la lava.",
				      upstart(y_monnam(u.usteed)),
				      mon_gender(u.usteed)? 'a' : 'o');
			else
#endif
			Norep("Estás atrapad%c en la lava.",
			    poly_gender()==1? 'a' : 'o');
		    }
		    if(!is_lava(x,y)) {
			u.utrap--;
...
#ifdef STEED
			    if (u.usteed)
				pline("Llevas %s a la orilla de la lava.",
				    al(y_monnam(u.usteed)));
			    else
#endif
			     pline("Te tiras a la orilla de la lava.");
			    u.utrap = 0;
			}
		    }
		    u.umoved = TRUE;
			...
			u.utrap = 0;
			pline("¡Aguijón corta la telaraña!");
			return;
		    }
		    if(--u.utrap) {
			if(flags.verbose) {
#ifdef STEED
			    if (u.usteed)
			    	Norep("%s está pegad%c a la telaraña.",
				      upstart(y_monnam(u.usteed)),
				      mon_gender(u.usteed)? 'a' : 'o');
			    else
#endif
			    Norep("Estás pegad%c a la telaraña.",
			        poly_gender()==1? 'a' : 'o');
			}
		    } else {
#ifdef STEED
			if (u.usteed)
			    pline("%s se escapa de la telaraña.",
				  upstart(y_monnam(u.usteed)));
			else
#endif
			pline("Te desenredas.");
...
#ifdef STEED
			    if (u.usteed)
			    	Norep("%s está pegad%c en %s.",
				      upstart(y_monnam(u.usteed)),
				      mon_gender(u.usteed)? 'a' : 'o', surface(u.ux, u.uy));
			    else
#endif
			    Norep("Estás pegad%c en %s.",
			        poly_gender()==1? 'a' : 'o',
				surface(u.ux, u.uy));
			}
		    } else {
#ifdef STEED
			if (u.usteed)
			    pline("%s finalmente se escapa culebreando.",
				  upstart(y_monnam(u.usteed)));
			else
#endif
			pline("Finalmente te escapas culebreando.");
		    }
		} else {
		    if(flags.verbose) {
#ifdef STEED
			if (u.usteed)
			    	Norep("%s está aprisionad%c en una trampa para osos.",
				      upstart(y_monnam(u.usteed)),
				      mon_gender(u.usteed)? 'a' : 'o');
			else
#endif
			Norep("Estás aprisionad%c en una trampa para osos.",
			    poly_gender()==1? 'a' : 'o');
		    }
	...
	if (is_safepet(mtmp) && !(is_hider(mtmp->data) && mtmp->mundetected)) {
		...
		u.ux = u.ux0,  u.uy = u.uy0;	/* didn't move after all */
		pline("Paras.  %s no cabe ahí.", upstart(y_monnam(mtmp)));
	    } else {
		char pnambuf[BUFSZ];
		...
		case 0:
		    pline("%s %s.", mtmp->mtame ? "Desplazas" : "Espantas",
			al(pnambuf));
		    break;
		case 1:		/* trapped */
		...
		case 2:
		     ...
		     */
		    if (rn2(4)) {
			Te_sientes("culpable por perder a tu mascota de esta manera.");
			u.ugangr++;
			adjalign(-15);
		    }
		    ...
		    break;
		default:
		    pline("¡Qué raro, resultado de mintrap desconocido!");
		    break;
		}
	    }
	}
...
#ifdef STEED
	    if (u.usteed) Sprintf(buf, "bajo %s", y_monnam(u.usteed));
	    else
#endif
	    if (Levitation || Flying) Strcpy(buf, "abajo tuyo");
	    else Sprintf(buf, "bajo tus %s", makeplural(body_part(FOOT)));

	    Sientes("una vibración extraña %s.", buf);
	    if (otmp && otmp->spe == 7 && otmp->lamplit)
		pline("¡%s %s!", The(xname(otmp)),
		    Blind ? "late palpablemente" : "brilla con una luz extraña");
	}
}
	...
	if(u.uinwater) {
		...
		if (!is_pool(u.ux,u.uy)) {
			if (Is_waterlevel(&u.uz))
				pline("Saltas en una burbuja de aire.");
			else if (is_lava(u.ux, u.uy))
				pline("Sales del agua...");	/* oops! */
			else
				pline("Estás sobre %s otra vez.",
				    is_ice(u.ux, u.uy) ? "hielo sólido" : "tierra sólida");
		}
		else if (Is_waterlevel(&u.uz))
			goto stillinwater;
		else if (Levitation)
			pline("¡Saltas del agua como un corcho!");
		else if (Flying)
			pline("Sales volando del agua.");
		else if (Wwalking)
			pline("Subes lentamente por encima de la superficie.");
		else
			goto stillinwater;
...
stillinwater:;
	...
	if((mtmp = m_at(u.ux, u.uy)) && !u.uswallow) {
		...
		switch(mtmp->data->mlet) {
		    case S_PIERCER:
			pline("¡%s de repente cae %s!",
			      Amonnam(mtmp),
			      del(ceiling(u.ux,u.uy)));
			if(mtmp->mtame) /* jumps to greet you, not attack */
			    ;
			else if(uarmh && is_metallic(uarmh))
			    pline("Su golpe rebota en tu casco.");
			else if (u.uac + 3 <= rnd(20))
			    pline("¡Casi eres golpead%c por %s!",
				poly_gender()==1? 'a' : 'o',
				x_monnam(mtmp, ARTICLE_A, "cayente", 0, TRUE));
			else {
			    int dmg;
			    pline("¡Eres golpead%c por %s!",
				poly_gender()==1? 'a' : 'o',
				x_monnam(mtmp, ARTICLE_A, "cayente", 0, TRUE));
			    dmg = d(4,6);
			...
			if(mtmp->mtame)
			    pline("%s salta cerca tuyo desde %s.",
				  Amonnam(mtmp), ceiling(u.ux,u.uy));
			else if(mtmp->mpeaceful) {
				pline("¡Sorprendes %s!",
				    Blind && !sensemon(mtmp) ?
				    something : al(a_monnam(mtmp)));
				mtmp->mpeaceful = 0;
			} else
			    pline("¡%s te ataca por sorpresa!",
					Amonnam(mtmp));
			break;
		}
...
register boolean newlev;
{
	...
	for (ptr = &u.uentered[0]; *ptr; ptr++) {
	    ...
	    switch (rt) {
		case ZOO:
		    pline("¡Bienvenid%c a la zoológico del tesoro de David!",
			  poly_gender()==1? 'a' : 'o');
		    break;
		case SWAMP:
		    pline("%s un poco %s aquí abajo.",
			  Blind ? "Se siente" : "Parece",
			  Blind ? "húmido" : "barroso");
		    break;
		case COURT:
		    pline("¡Entras en un salón del trono opulento!");
		    break;
		case LEPREHALL:
		    pline("¡Entras en una antecámara de leprechaunes!");
		    break;
		case MORGUE:
		    if(midnight()) {
		       /*
			* LENGUA:  In the English version, this called
			* locomotion() to get the verb; but this was the only
			* use of locomotion() with an imperative verb, and
			* putting this in locomotion() would complicate that
			* function.
			*
			* Instead, we use the verbs that locomotion() would
			* have returned, in the appropriate conjugation.
			*/
			const char *run =
			    is_floater(youmonst.data) ? "Flota" :
			    is_flyer(youmonst.data)   ? "Vuela" :
			    slithy(youmonst.data)     ? "Deslízate" :
			    amorphous(youmonst.data)  ? "Rezúmate" :
			    nolimbs(youmonst.data)    ? "Arrástrate" :
							"Corre";
			pline("¡%s fuera!  ¡%s fuera!", run, run);
		    } else
			pline("Tienes una sensación misteriosa...");
		    break;
		case BEEHIVE:
		    pline("¡Entras en una colmena gigante!");
		    break;
		case COCKNEST:
		    pline("¡Entras en un nido repugnante!");
		    break;
		case ANTHOLE:
		    pline("¡Entras en un hormiguero!");
		    break;
		case BARRACKS:
			...
			pline("¡Entras en un cuartel militar!");
		    else
			pline("Entras en un cuartel abandonado.");
		    break;
		case DELPHI:
		    if(monstinroom(&mons[PM_ORACLE], roomno))
			verbalize("¡%s, %s, bienvenid%c a Delfos!",
...
dopickup()
{
	...
	if(u.uswallow) {
		...
		if (is_animal(u.ustuck->data)) {
		    pline("Recoges la lengua de %s.", mon_nam(u.ustuck));
		    pline("Pero está un poco viscosa, así que la dejas.");
		} else
		    pline("No %s nada aquí para recoger.",
			  Blind ? "sientes" : "ves");
		return(1);
	    } else {
	    	int tmpcount = -count;
	...
	if(is_pool(u.ux, u.uy)) {
	    if (Wwalking || is_floater(youmonst.data) || is_clinger(youmonst.data)
			|| (Flying && !Breathless)) {
		No_puedes("zambullirte en el agua para recoger cosas.");
		return(0);
	    } else if (!Underwater) {
		No_puedes("ni siquiera ver el fondo, mucho menos recoger alguna cosa.");
		return(0);
	    }
	}
	if (is_lava(u.ux, u.uy)) {
	    if (Wwalking || is_floater(youmonst.data) || is_clinger(youmonst.data)
			|| (Flying && !Breathless)) {
		No_puedes("alcanzar el fondo para recoger cosas.");
		return(0);
	    } else if (!likes_lava(youmonst.data)) {
		pline("Te achicharrarrías tratando de recoger cosas.");
		return(0);
	    }
	}
	if(!OBJ_AT(u.ux, u.uy)) {
		pline("Aquí no hay nada que recoger.");
		return(0);
	}
	if (!can_reach_floor()) {
#ifdef STEED
		if (u.usteed && P_SKILL(P_RIDING) < P_BASIC)
			pline("No tienes la habilidad para alcanzar %s desde %s.",
				surface(u.ux,u.uy),
				y_monnam(u.usteed));
		else
#endif
 	...
 	if (traphere && traphere->tseen) {
		...
		if ((traphere->ttyp == PIT || traphere->ttyp == SPIKED_PIT) &&
			...
			No_puedes("alcanzar el fondo del hoyo.");
			return(0);
		}
	}
...
maybe_wail()
{
	...
	who = (Role_if(PM_WIZARD) || Role_if(PM_VALKYRIE)) ?
		urole.name.m : (flags.female? "Elfa" : "Elfo");
	if (u.uhp == 1) {
	    pline("%s está a punto de morir.", who);
	} else {
	    for (i = 0, powercnt = 0; i < SIZE(powers); ++i)
		if (u.uprops[powers[i]].intrinsic & INTRINSIC) ++powercnt;

	    pline(powercnt >= 4 ? "%s, todos tus poderes se perderán..."
				: "%s, tu fuerza de vida se acaba.", who);
	}
    } else {
	Oyes(u.uhp == 1 ? "los lamentos de la Banshee..."
			 : "el aullido de los CwnAnnwn...");
    }
}
...
const char *str;
{
	...
	if(str)
	    ...
	    No_puedes("hacer eso mientras traes tantas cosas.");
	return 1;
    }
    return 0;
}
