...
#define MAX_GOOD 15
    ...
    if (!mdat) {
#ifdef DEBUG
	pline("enexto() llamado con mdat==0");
#endif
...
boolean force_it;
{
	...
	if (mtmp->mleashed) {
	    ...
	    if (!otmp) {
		impossible("%s está encorread%c, sin correa.", Monnam(mtmp),
			mon_gender(mtmp)? 'a' : 'o');
		goto release_it;
	    }
...
tele()
{
	...
	if (level.flags.noteleport) {
		...
		if (!wizard) {
#endif
		    pline("¡Una fuerza misteriosa previene que te teleportes!");
		    return;
#ifdef WIZARD
		}
#endif
	...
	if ((Teleport_control && !Stunned)
	    ...
	    if (unconscious()) {
		pline("Estando inconsciente, no puedes manejar tu telepuerta.");
	    } else {
#ifdef STEED
		    if (u.usteed)
			pline("¿A qué posición queráis tú y %s teleportaros?",
				mon_nam(u.usteed));
		    else
#endif
		        pline("¿A qué posición quieres teleportarte?");
		    cc.x = u.ux;
		    cc.y = u.uy;
...
dotele()
{
	...
	if (trap) {
		if (trap->once) {
			pline("Esto es una telepuerta de caja fuerte, usable sólo una vez.");
			if (yn("¿Saltar al dentro?") == 'n')
				trap = 0;
			else {
				deltrap(trap);
				newsym(u.ux, u.uy);
		...
		if (trap)
			pline("%s al dentro de la telepuerta.",
	...
	if (!trap) {
		...
		if (!wizard) {
		    ...
		    if (!castit) {
			if (!Teleportation)
			    pline("No sabes ese hechizo.");
			else No_puedes("teleportarte a voluntad.");
			return(0);
		    }
#ifdef WIZARD
		}
		...
		if (!wizard) {
#endif
			pline("Te falta la fuerza para %steleportar.",
			    castit ? "un hechizo de " : "");
			return 1;
#ifdef WIZARD
		}
#endif
...
#endif
		{
			pline("Te falta la energía para %steleportar.",
			    castit ? "un hechizo de " : "");
			return 1;
		}
	    }
	    ...
	    if (check_capacity(
			"Se pierde tu concentración de traer tantas cosas."))
		return 1;
...
level_tele()
{
	...
	if ((u.uhave.amulet || In_endgame(&u.uz) || In_sokoban(&u.uz))
	    ...
	    Te_sientes("muy disorientad%c por un momento.",
		    poly_gender()==1? 'a' : 'o');
	    return;
	}
	if ((Teleport_control && !Stunned)
	   ...
	   || wizard
	    ...
	    Strcpy(qbuf, "¿A qué nivel quieres teleportar?");
	    do {
		if (++trycnt == 2) {
#ifdef WIZARD
			if (wizard) Strcat(qbuf, " [tecla un número o ? por un menú]");
			else
#endif
			Strcat(qbuf, " [tecla un número]");
		}
		getlin(qbuf, buf);
		...
		if (wizard && !strcmp(buf,"?")) {
			...
			if (In_endgame(&newlevel) && !In_endgame(&u.uz)) {
				Sprintf(buf,
				    "Destinación es la nivel de tierra");
				if (!u.uhave.amulet) {
					struct obj *obj;
		...
		if (trycnt >= 10)
		    goto random_levtport;
		Sprintf(buf, "Ir a ningún lugar.  ¿Estás segur%c?",
			poly_gender()==1? 'a' : 'o');
		if (ynq(buf) != 'y') return;
		pline("%s de agonía como tu cuerpo empieza deformarse...",
		    is_silent(youmonst.data) ? "Te retuerces" : "Gritas");
		display_nhwindow(WIN_MESSAGE, FALSE);
		pline("Dejas de existir.");
		if (invent) pline("Tus posesiones se aterrizan en %s con un thud.",
				surface(u.ux, u.uy));
		killer_format = NO_KILLER_PREFIX;
		killer = "cometió suicida";
		done(DIED);
		pline("Una nube energizada de polvo empieza a coalescer.");
		pline("Tu cuerpo se rematerializa%s.", invent ?
			", y recoges todas tus posesiones" : "");
		return;
	    }
 ...
 random_levtport:
	...
	if (newlev < 0 && !force_dest) {
		...
		if (newlev <= -10) {
			pline("Llegas en el cielo.");
			verbalize("Estáis tempran%c, pero os admitiremos.",
				poly_gender()==1? 'a' : 'o');
			killer_format = NO_KILLER_PREFIX;
			killer = "fue al cielo premaduramente";
		} else if (newlev == -9) {
			/* LENGUA:  Does "Cloud 9" translate? */
			Te_sientes("deliriosamente feliz.");
			pline("(¡De hecho, estás en Nube 9!) ");
			display_nhwindow(WIN_MESSAGE, FALSE);
		} else
			pline("Ahora estás muy arriba de los nubes...");

		if (killer) {
		    ;		/* arrival in heaven is pending */
		} else if (Levitation) {
		    escape_by_flying = "Flotas suavemente abajo a la tierra";
		} else if (Flying) {
		    escape_by_flying = "Vuelas abajo a la tierra";
		} else {
		    pline("Desgraciadamente, no sabes volar.");
		    pline("Te caes unos miles de metros a tu muerte.");
		    killer = "se teleportó fuera del calabozo y cayó a su muerte";
		    killer_format = NO_KILLER_PREFIX;
		}
	}
	...
	if (killer) {	/* the chosen destination was not survivable */
	    ...
	    escape_by_flying = "Te encuentras en la superficie de nuevo";
	    u.uz = lsav;	/* restore u.uz so escape code works */
	}

	/* calls done(ESCAPED) if newlevel==0 */
	if (escape_by_flying) {
	    pline("%s.", escape_by_flying);
...
register struct trap *ttmp;
{
	...
	pline("¡Has activado un portal mágico!");
	...
	if (In_endgame(&u.uz) && !u.uhave.amulet) {
	    Te_sientes("desmayad%c por un momento, pero no pasa nada...",
		    poly_gender()==1? 'a' : 'o');
	    return;
	}
	...
	schedule_goto(&target_level, FALSE, FALSE, 1,
		      poly_gender()==1
			? "Te sientes desmayada por un momento, pero la sensación pasa."
		        : "Te sientes desmayado por un momento, pero la sensación pasa.",
		      (char *)0);
}
...
struct trap *trap;
{
	pline("¡%s sobre una telepuerta de nivel!",
		      Levitation ? (const char *)"Flotas" :
				  locomotion(youmonst.data, "Andas"));
	if (Antimagic) {
	    shieldeff(u.ux, u.uy);
	}
	...
	if (!Blind)
	    pline("Estás cegad%c por un momento por un destello de luz.",
		  poly_gender()==1? 'a' : 'o');
	else
	    pline("Estás disorientad%c por un momento.",
		  poly_gender()==1? 'a' : 'o');
	deltrap(trap);
	newsym(u.ux,u.uy);	/* get rid of trap symbol */
	level_tele();
...
boolean suppress_impossible;
{
	...
	if (!suppress_impossible)
		impossible("rloc(): no podía relocar monstruo");
	return FALSE;
...
struct monst *mon;
{
	if (level.flags.noteleport) {
		if (canseemon(mon))
		    pline("¡Una fuerza misteriosa previene que %s se teleporte!",
			mon_nam(mon));
		return TRUE;
	}
	return FALSE;
...
int in_sight;
{
	...
	if (teleport_pet(mtmp, FALSE)) {
	    ...
	    if (in_sight) {
		if (canseemon(mtmp))
		    pline("%s parece disorientad%c.", monname,
			  mon_gender(mtmp)? 'a' : 'o');
		else
		    pline("¡%s de repente desaparece!", monname);
		seetrap(trap);
	    }
	}
}
...
int in_sight;
{
	...
	if (teleport_pet(mtmp, force_it)) {
		...
		if (Is_stronghold(&u.uz)) {
			...
			pline("%s evita la %s.", Monnam(mtmp),
			(tt == HOLE) ? "abertura" : "trampa");
		    return 0;
		} else {
		...
		if (In_endgame(&u.uz) &&
			...
			pline("%s parece relucir por un momento.",
							Monnam(mtmp));
			seetrap(trap);
		    }
		    return 0;
		...
		if (mon_has_amulet(mtmp) || In_endgame(&u.uz)) {
		    if (in_sight)
			pline("%s parece muy disorientad%c por un momento.",
		...
		if (nlev == depth(&u.uz)) {
		    if (in_sight)
			pline("%s tembla por un momento.", Monnam(mtmp));
		    return 0;
		}
	    ...
	    if (in_sight) {
		pline("De repente, %s desaparece afuera de vista.", mon_nam(mtmp));
		seetrap(trap);
	    }
...
boolean give_feedback;
{
	...
	if (mtmp->ispriest && *in_rooms(mtmp->mx, mtmp->my, TEMPLE)) {
	    if (give_feedback)
		pline("¡%s resiste tu magia!", Monnam(mtmp));
	    return FALSE;
	} else if (level.flags.noteleport && u.uswallow && mtmp == u.ustuck) {
	    if (give_feedback)
		pline("¡Ya no estás dentro de %s!", mon_nam(mtmp));
	    unstuck(mtmp);
	    (void) rloc(mtmp, FALSE);
