...
d_level *dlev;
{
	...
	if (first > LAST_GEM || objects[first].oc_class != GEM_CLASS ||
		...
		raw_printf("¿Joyas de menos? - first=%d j=%d LAST_GEM=%d",
			first, j, LAST_GEM);
		wait_synch();
	    }
...
# define COPY_OBJ_DESCR(o_dst,o_src) o_dst.oc_descr_idx = o_src.oc_descr_idx
	...
	check:
		...
		if(sum != 1000)
			error("error de init-prob por clase %d (%d%%)", oclass, sum);
		first = last;
	}
...
find_skates()
{
	...
	if ((s = OBJ_DESCR(objects[i])) != 0 && !strcmp(s, "botas de nieve"))
	    return i;

    impossible("botas de nieve no encontradas");
...
register int oindx;
{
	...
	else impossible("objeto designado no está en disco");
	update_inventory();
    }
}
...
dodiscovered()				/* free after Robert Viduya */
{
    ...
    putstr(tmpwin, 0, "Descubrimientos");
    putstr(tmpwin, 0, "");
	...
	if (objects[uniq_objs[i]].oc_name_known) {
	    if (!dis++)
		putstr(tmpwin, iflags.menu_headings, "Artículos Únicos:");
	...
	*s = '\0';
    ...
    if (ct == 0) {
	pline("Ya no has descubierto nada...");
    } else
