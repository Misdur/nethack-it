...
boolean talk;
{
	...
	if (!xtime && old) {
		if (talk)
		    Te_sientes("%s ahora.",
			Hallucination ? "menos tambaleante" : "un poco más estable");
	}
	if (xtime && !old) {
		if (talk) {
#ifdef STEED
...
static const char vismsg[] = "Tu visión parece %s por un momento pero ahora está %s.";
...
boolean talk;
{
	...
	if (can_see_now && !u_could_see) {	/* regaining sight */
	    if (talk) {
		if (Hallucination)
		    pline("¡Caramba!  ¡Todo es cósmico de nuevo!");
		else
		...
		if (!haseyes(youmonst.data)) {
		    ...
		    pline(vismsg, "volverse más clara",
			 Hallucination ? "más triste" : "normal");
		}
	    }
	}
	...
	if (u_could_see && !can_see_now) {	/* losing sight */
	    if (talk) {
		if (Hallucination)
		    pline("¡Qué lío!  ¡Todo está oscuro!  ¡Ayúdame!");
		else
		...
		if (!haseyes(youmonst.data)) {
		    ...
		    pline(vismsg, "volverse más oscura",
			 Hallucination ? "más feliz" : "normal");
		}
	    }
	}
...
long mask;	/* nonzero if resistance status should change by mask */
{
	...
	if (mask) {
		...
		if (!haseyes(youmonst.data)) {
		    ...
		    pline(vismsg, "volverse más insípida", "normal");
		}
	    }
	}
...
ghost_from_bottle()
{
	...
	if (!mtmp) {
		pline("Descubres que la botella está vacía.");
		return;
	}
	if (Blind) {
	...
	pline("¡Al abrir la botella, un%s %s enorm%c emerge!",
		fem? "a" : "", que, fem? 'a' : 'o');
	if(flags.verbose)
	    pline("Estás espantad%c a la muerte, y no puedes mover.",
		    poly_gender()==1? 'a' : 'o');
	nomul(-3);
...
dodrink()
{
	...
	if (Strangled) {
		pline("Si no puedes respirar aire, ¿cómo puedes beber líqido?");
		return 0;
	}
	...
	register struct obj	*otmp;
{
	...
	case SPE_RESTORE_ABILITY:
		unkn++;
		if(otmp->cursed) {
		    pline("¡Ulch!  ¡Ésta hace que sientes mediocre!");
		    break;
		} else {
		    if (otmp->blessed) {
			if (unfixable_trouble_count(FALSE))
			    pline("¡Vaya!  ¡Ésta te hace sentir mejor!");
			else
			    pline("¡Vaya!  ¡Ésta te hace sentir magnífic%c!",
				  poly_gender()==1? 'a' : 'o');
		    } else {
			pline("¡Vaya!  ¡Ésta te hace sentir buen%c!",
			      poly_gender()==1? 'a' : 'o');
		    }
	...
	case POT_WATER:
		...
		if(is_undead(youmonst.data) || is_demon(youmonst.data) ||
		    ...
		    if(otmp->blessed) {
			pline("¡Ésta te quema como ácido!");
			exercise(A_CON, FALSE);
			if (u.ulycn >= LOW_PM) {
			...
			losehp(d(2,6), "poción de agua bendita", KILLED_BY_AN);
		    } else if(otmp->cursed) {
			...
			if(u.ualign.type == A_LAWFUL) {
			    pline("¡Ésta te quema como ácido!");
			    losehp(d(2,6), "poción de agua maldita",
				KILLED_BY_AN);
			} else
	...
	case POT_BOOZE:
		unkn++;
		pline("¡Ooph!  ¡Ésta sabe a %s%s!",
		      otmp->odiluted ? "diluido " : "",
		      Hallucination ? "vino de diente de león" : "fuego líquido");
		if (!otmp->blessed)
	...
	case POT_INVISIBILITY:
		...
		if(otmp->cursed) {
		    pline("Por alguna razón, sientes que se sabe tu presencia.");
		    aggravate();
		}
		break;
	...
	case POT_FRUIT_JUICE:
		...
		unkn++;
		if (otmp->cursed)
		    pline("¡Yecch!  Ésta sabe %s.",
			  Hallucination ? "sobremadura" : "podrida");
		else
		    pline(Hallucination ?
		      "Ésta sabe a bebido todo-natural de 10%% real jugo%s de %s." :
		      "Ésta sabe a jugo%s de %s.",
		...
		if (msg && !Blind) { /* Blind possible if polymorphed */
		    pline("¡Puedes ver través de ti, pero estás visible!");
		    unkn--;
		}
		break;
	    }
	case POT_PARALYSIS:
		if (Free_action)
			...
			pline("Estás suspendid%c sin moción.",
				poly_gender()==1? 'a' : 'o');
#ifdef STEED
		    else if (u.usteed)
			pline("¡Estás paralizad%c y no puedes mover!",
				poly_gender()==1? 'a' : 'o');
#endif
		    else {
			const char *pie = body_part(FOOT);
			pline("¡Tus %s están pegad%cs %s!",
			     makeplural(pie),
			     isfeminine(pie)? 'a' : 'o',
	...
	case SPE_DETECT_TREASURE:
		...
		break;
	case POT_SICKNESS:
		pline("¡Yecch!  Esta poción sabe a veneno.");
		if (otmp->blessed) {
		    pline("(Pero de hecho era %s templadamente rancio.)",
			  fruitname(TRUE));
		    if (!Role_if(PM_HEALER)) {
			/* NB: blessed otmp->fromsink is not possible */
			losehp(1, "poción templadamente contaminada", KILLED_BY_AN);
		    }
		} else {
		    if(Poison_resistance)
			pline(
		    "(Pero de hecho era %s biológicamente contaminado.)",
			      fruitname(TRUE));
		    if (Role_if(PM_HEALER))
			...
			if(!Poison_resistance) {
				...
				losehp(rnd(10)+5*!!(otmp->cursed),
				       "poción contaminada", KILLED_BY_AN);
			}
			exercise(A_CON, FALSE);
		    }
		}
		if(Hallucination) {
			pline("¡Eres chocad%c atrás a ti!",
				poly_gender()==1? 'a' : 'o');
	...
	case POT_GAIN_ABILITY:
		if(otmp->cursed) {
		    pline("¡Ulch!  ¡Esta poción sabía sucia!");
		    unkn++;
		} else if (Fixed_abil) {
		    nothing++;
	...
	case SPE_HASTE_SELF:
		if(!Very_fast) /* wwf@doe.carleton.ca */
			pline("Estás moviendo %smás rápidamente de repente.",
				Fast ? "" : "mucho ");
		else {
			pline("Tus %s ganan nueva energía.",
	...
	case POT_GAIN_LEVEL:
		if (otmp->cursed) {
			...
			if((ledger_no(&u.uz) == 1 && u.uhave.amulet) ||
			    ...
			    const char *riseup ="¡Te levantas, a través %s!";
			    if(ledger_no(&u.uz) == 1) {
	...
	case POT_GAIN_ENERGY:			/* M. Stephenson */
		{	register int num;
			if(otmp->cursed)
			    ...
			    pline("Energías mágicas corren través de tu cuerpo.");
	...
	case POT_OIL:				/* P. Winner */
		{
			...
			if (otmp->lamplit) {
				...
				losehp(d(Fire_resistance ? 1 : 3, 4),
				       "poción quemante de aceite", KILLED_BY_AN);
			    }
			} else if(otmp->cursed)
	...
	case POT_ACID:
		if (Acid_resistance)
			...
			pline("¡Esta quema%s!", otmp->blessed ? " un poco" :
					otmp->cursed ? " mucho" : " como ácido");
			losehp(d(otmp->cursed ? 2 : 1, otmp->blessed ? 4 : 8),
					"poción de ácido", KILLED_BY_AN);
			exercise(A_CON, FALSE);
		}
	...
	case POT_POLYMORPH:
		Te_sientes("un poco %s.",
			Hallucination ? "normal"
				      : feminize("extraño", poly_gender()==1));
		if (!Unchanging) polyself(FALSE);
		break;
	default:
		impossible("¡Qué poción extraña! (%u)", otmp->otyp);
		return(0);
	}
	return(-1);
}
...
boolean your_fault;
{
	...
	if(isyou) {
		...
		losehp(rnd(2), "poción tirada", KILLED_BY_AN);
	} else {
	...
	case POT_OIL:
		...
		break;
	case POT_POLYMORPH:
		Te_sientes("un poco %s.", Hallucination ? "normal" : feminize("extraño", poly_gender()==1));
 ...
 do_healing:
	...
	case POT_SICKNESS:
		...
		if (dmgtype(mon->data, AD_DISE) ||
			   ...
			   resists_poison(mon)) {
		    if (canseemon(mon))
			pline("%s no parece dañad%c.", Monnam(mon),
				mon_gender(mon)? 'a' : 'o');
		    break;
		}
 do_illness:
	...
	case POT_WATER:
		if (is_undead(mon->data) || is_demon(mon->data) ||
			...
			angermon = FALSE;
			if (canseemon(mon))
			    pline("%s parece más san%c.", Monnam(mon),
				    poly_gender()==1? 'a' : 'o');
			mon->mhp += d(2,6);
			if (mon->mhp > mon->mhpmax) mon->mhp = mon->mhpmax;
...
register struct obj *obj;
{
	...
	case POT_GAIN_ABILITY:
		if(obj->cursed) {
			...
			pline("¡Ulch!  ¡Esta poción huele terrible!");
	...
	case POT_SICKNESS:
		...
		break;
	case POT_HALLUCINATION:
		pline("Tienes una visión por un momento.");
		break;
	case POT_CONFUSION:
	case POT_BOOZE:
		if(!Confusion)
	...
	case POT_INVISIBILITY:
		if (!Blind && !Invis) {
			...
			See_invisible ? "pudiste ver través de ti"
			: "no pudiste verte");
		}
		break;
	case POT_PARALYSIS:
	...
	case POT_SLEEPING:
		...
		break;
	case POT_SPEED:
		if (!Fast) pline("Tus rodillas parecen más flexibles ahora.");
...
/* returns TRUE if something happened (potion should be used up) */
{
	...
	switch (obj->oclass) {
		...
		if (obj->otyp == POT_ACID) {
			...
			pline("¡Eres asid%c en la explosión!",
				poly_gender()==1? 'a' : 'o');
			losehp(rnd(10), "química elementaria", KILLED_BY);
			makeknown(obj->otyp);
			update_inventory();
			return (TRUE);
		}
	...
	pline("%s de repente se calenta; vapor sube y el libro se queda seco.",
		...
		if (!obj->oerodeproof && is_rustprone(obj) &&
			...
			pline("%s se oxida%s %s.",
			      Your_buf, vplural, obj->oeroded ? "más" : "algo");
			obj->oeroded++;
			update_inventory();
			return TRUE;
		} else break;
...
dodip()
{
	...
	if (potion == obj && potion->quan == 1L) {
		pline("¡Ésta es una botella de poción, no una botella Klein!");
		return 0;
	}
	...
	poof:
			...
			if (obj->blessed) {
				if (useeit)
				    pline("%s %s %s.",
					  Your_buf,
					  otense(obj, "alumbra"),
					  feminize(hcolor((const char *)"marrón"),
						   isfeminine(Your_buf)));
				unbless(obj);
				obj->bknown=1;
				goto poof;
		...
		if (obj->cursed || obj->otyp == POT_ACID || !rn2(10)) {
			...
			useup(obj);
			useup(potion);
			losehp(rnd(10), "ráfaga alquímica", KILLED_BY_AN);
			return(1);
		}
	...
	if (potion->otyp == POT_INVISIBILITY && !obj->oinvis) {
		obj->oinvis = TRUE;
		if (!Blind) {
		    if (!See_invisible) pline("¿A dónde ha ido %s?",
		    		the(xname(obj)));
	...
	if (potion->otyp == POT_OIL) {
		...
		if (catch_lit(obj)) {
		    ...
		    pline("El aceite ardiente %s %s.",
			    obj->oeroded == MAX_ERODE ? "destruye" : "daña",
			    yname(obj));
		...
		pline("La poción se derrama y cubre tus %s con aceite.",
	...
	if ((obj->otyp == OIL_LAMP || obj->otyp == MAGIC_LAMP) &&
	   (potion->otyp == POT_OIL)) {
	    ...
	    if (obj->age > 1000L) {
		const char *nombre = Yname2(obj);
		pline("%s %s llen%c.", nombre, otense(obj, "está"),
	...
	if ((obj->otyp == UNICORN_HORN || obj->otyp == AMETHYST) &&
		...
		if (Blind) {
			...
			Strcpy(qualifier, "");
		    pline("La poción %s%s%s.", oldbuf, qualifier, newbuf);
