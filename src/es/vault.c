...
register boolean forceshow;
{
	...
	if(grd->mhp <= 0) {
	    pline("El corredor desaparece.");
	    /* LENGUA:  need translation */
	    if(IS_ROCK(levl[u.ux][u.uy].typ))
		pline("Estás encerrad%c en piedra.",
		      poly_gender()==1? 'a' : 'o');
	}
	return(TRUE);
}
...
incr_radius: ;
	...
	impossible("¿¿Ni un solo corredor en este nivel??");
	tele();
	return;
fnd:
	gx = x; gy = y;
	...
	if (canspotmon(guard))
	    pline("¡Entra de repente un%c de los %s de la Caja Fuerte!",
		  mon_gender(guard)? 'a' : 'o',
		  makeplural(g_monnam(guard)));
	else
	    pline("Alguien más ha entrado la Caja Fuerte.");
	newsym(guard->mx,guard->my);
	if (youmonst.m_ap_type == M_AP_OBJECT || u.uundetected) {
	    if (youmonst.m_ap_type == M_AP_OBJECT &&
			youmonst.mappearance != GOLD_PIECE) {
		const char *obj = mimic_obj_name(&youmonst);
	    	verbalize("¡Hey! ¿Quién dejó es%c %s aquí?",
			isfeminine(obj)? 'a' : 'e', obj);
	    }
	    /* You're mimicking a pile of gold or you're hidden. */
	    pline("Dudos%c, se vuelve y se va.",
		  mon_gender(guard)? 'a' : 'o');
	    mongone(guard);
	    return;
	}
	if (Strangled || is_silent(youmonst.data) || multi < 0) {
	    ...
	    verbalize("¡Volveré cuando estás list%c de hablar conmigo!",
			poly_gender()==1? 'a' : 'o');
	    mongone(guard);
	    return;
	}
	...
	do {
	    char msg[BUFSZ];
	    Sprintf(msg, "\"Hola extranjer%c, ¿quién es usted?\" -",
		    poly_gender()==1? 'a' : 'o');
	    getlin(msg, buf);
	    (void) mungspaces(buf);
	} while (!isalpha2(buf[0]) && --trycount > 0);

	if (u.ualign.type == A_LAWFUL &&
	    /* ignore trailing text, in case player includes character's rank */
	    strncmpi2(buf, plname, (int) strlen(plname)) != 0) {
		adjalign(-1);		/* Liar! */
	}

	if (!strcmpi2(buf, "Croesus") || !strcmpi2(buf, "Kroisos")
		|| !strcmpi2(buf, "Creso")
#ifdef TOURIST
		|| !strcmpi2(buf, "Creosote")
#endif
	    ) {
	    if (!mvitals[PM_CROESUS].died) {
		verbalize("Oh, sí, por supuesto.  Lo siento que os molesté.");
		mongone(guard);
	    } else {
		setmangry(guard);
		verbalize("¿Vuelt%c de la muerte, eres?  ¡Remediaré eso!",
			poly_gender()==1? 'a' : 'o');
	...
	verbalize("No te conozco.");
#ifndef GOLDOBJ
	if (!u.ugold && !hidden_gold())
	    verbalize("Sígueme, por favor.");
	else {
	    if (!u.ugold)
		verbalize("Tienes oro escondido.");
	    verbalize("En toda probabilidad, todo tu oro era robado de esta caja fuerte.");
	    verbalize("Haz caer este oro, por favor, y sígueme.");
	}
#else
	...
	if (!umoney && !hidden_gold())
	    ...
	    if (!umoney)
		verbalize("Tienes dinero escondido.");
	    verbalize("En toda probabilidad, todo tu dinero era robado de esta caja fuerte.");
	    verbalize("Haz caer este dinero, por favor, y sígueme.");
	}
#endif
	EGD(guard)->gdx = gx;
	EGD(guard)->gdy = gy;
...
struct monst *grd;
{
	...
	if(movedgold || fixed) {
		...
		pline("%s %s cuchichea una incantación.",
		      mon_article(grd)? "La" : "El",
		      g_monnam(grd));
	    else Oyes("un canto distante.");
	    if(movedgold)
		pline("Una fuerza misteriosa mueve el oro en el caja fuerte.");
	    if(fixed)
		pline("¡Se restoran mágicamente las paredes dañadas de la caja fuerte!");
	}
}
...
register struct monst *grd;
{
	...
	if(egrd->fcend == 1) {
		...
		if(egrd->warncnt == 3)
#ifndef GOLDOBJ
			verbalize("¡Repito, %ssígueme!",
				u_carry_gold ? (!u.ugold ?
					  "haz caer este oro escondido y " :
					  "haz caer este oro y ") : "");
#else
			verbalize("¡Repito, %ssígueme!",
				u_carry_gold ? (!u.ugold ?
					  "haz caer este dinero escondido y " :
					  "haz caer este dinero y ") : "");
#endif
		if(egrd->warncnt == 7) {
			boolean fem = poly_gender()==1;
			m = grd->mx;
			n = grd->my;
			verbalize("¡Has sido avisad%c, brib%s!",
				  fem? 'a' : 'o',
				  fem? "ona" : "ón");
			mnexto(grd);
...
letknow:
			...
			Oyes("el sonido agudo de un silbato de guarda.");
		    else
			if (um_dist(grd->mx, grd->my, 2))
			    pline("Ves a un%s %s enojad%c acercándote",
				  mon_gender(grd)? "a" : "",
				  g_monnam(grd),
				  mon_gender(grd)? 'a' : 'o');
			else
			    pline("Eres confrontad%c por un%s %s enojado",
				  poly_gender()==1? 'a' : 'o',
				  mon_gender(grd)? "a" : "",
				  l_monnam(grd));
		    return(-1);
	...
	if(egrd->fcend > 1) {
		...
		pline("%s %s, confundid%c, desaparece.",
		      mon_article(grd)? "La" : "El",
		      g_monnam(grd),
		      mon_gender(grd)? 'a' : 'o');
		...
		if(egrd->warncnt < 6) {
			egrd->warncnt = 6;
			verbalize("¡Haz caer todo tu oro, bellac%c!",
				poly_gender()==1? 'a' : 'o');
			return(0);
		} else {
			verbalize("¡Sea lo que sea, pícar%c!",
				poly_gender()==1? 'a' : 'o');
			grd->mpeaceful = 0;
			return(-1);
		}
	...
	if(goldincorridor && !egrd->gddone) {
		...
		if(cansee(m,n))
		    pline("%s%s el oro.", Mon_verb_mon(grd, "recoge", (struct monst *)0),
				grd->mpeaceful ? " se calma y" : "");
	...
	if(um_dist(grd->mx, grd->my, 1) || egrd->gddone) {
		if(!egrd->gddone && !rn2(10)) verbalize("¡Muévete!");
		restfakecorr(grd);
...
cleanup:
		...
		if(!semi_dead && (in_fcorridor(grd, u.ux, u.uy) ||
			...
			pline("De repente, %s %s desaparece.",
			      mon_article(grd)? "la" : "el",
			      g_monnam(grd));
		    return(1);
		}
...
paygd()
{
	...
	if (u.uinvault) {
	    pline("Tus %ld %s van a la Caja de Memoria Mágica.",
#ifndef GOLDOBJ
		u.ugold,
		currency(u.ugold));
#else
	    ...
	    mnexto(grd);
	    pline("%s remite tu oro a la caja fuerte.", Monnam(grd));
	    gx = rooms[EGD(grd)->vroom].lx + rn2(2);
	    gy = rooms[EGD(grd)->vroom].ly + rn2(2);
	    Sprintf(buf,
		"Para Creso: aquí está el oro recubierto desde %s %s %s.",
		plname, flags.female? "la" : "el",
		feminize(mons[u.umonster].mname, flags.female));
