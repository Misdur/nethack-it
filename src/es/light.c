    ...
    genericptr_t id;
{
	...
	impossible("new_light_source:  rango ilegal %d", range);
	return;
    }
    ...
    genericptr_t id;
{
    ...
    impossible("del_light_source: no encontrado type=%d, id=0x%lx", type, (long)id);
}
    ...
    int fd, mode, range;
{
	...
	if (actual != count)
	    panic("¡la cuenta %d fuentes de luz, da %d! [range=%d]",
		  count, actual, range);
    }
	...
	for (prev = &light_base; (curr = *prev) != 0; ) {
	    if (!curr->id) {
		impossible("save_light_sources: ¡sin id! [range=%d]", range);
		is_global = 0;
	    } else
	    switch (curr->type) {
	    case LS_OBJECT:
	    ...
	    default:
		is_global = 0;
		impossible("save_light_sources: mal tipo (%d) [range=%d]",
			   curr->type, range);
		break;
	    }
    ...
    boolean ghostly;
{
	...
	if (ls->flags & LSF_NEEDS_FIXUP) {
		...
		if (ghostly) {
			...
			impossible("relink_light_sources: sin mapa id");
		} else
		    nid = (unsigned) ls->id;
		if (ls->type == LS_OBJECT) {
		    which = 'o';
		...
		if (!ls->id)
		    impossible("relink_light_sources: no se encuentra %c_id %d",
			       which, nid);
	    } else
		impossible("relink_light_sources: mal tipo (%d)", ls->type);
    ...
    boolean write_it;
{
	...
	default:
	    is_global = 0;
	    impossible("maybe_write_ls: clase mala (%d) [range=%d]",
		       ls->type, range);
	    break;
	}
    ...
    light_source *ls;
{
	...
	if (ls->flags & LSF_NEEDS_FIXUP)
		...
		if (find_oid((unsigned)ls->id) != otmp)
		    panic("write_ls: ¡no se encuentra obj #%u!", (unsigned)ls->id);
#endif
		...
		if (find_mid((unsigned)ls->id, FM_EVERYWHERE) != mtmp)
		    panic("write_ls: ¡no se encuentra mon #%u!", (unsigned)ls->id);
#endif
	    }
	    ls->flags |= LSF_NEEDS_FIXUP;
	    bwrite(fd, (genericptr_t)ls, sizeof(light_source));
	...
	impossible("write_ls: mal tipo (%d)", ls->type);
    }
}
...
wiz_light_sources()
{
    ...
    Sprintf(buf, "Fuentes mobiles de luz: héroe @ (%2d,%2d)", u.ux, u.uy);
    putstr(win, 0, buf);
    putstr(win, 0, "");

    if (light_base) {
	putstr(win, 0, "locación límites banderas clase   id");
	putstr(win, 0, "-------- ------- -------- ----- -------");
	for (ls = light_base; ls; ls = ls->next) {
