...
int x1, y1, x2, y2;
{
	...
	if (x1<0 || x2>=COLNO || x1>x2 || y1<0 || y2>=ROWNO || y1>y2)
	    panic("wallification: malos límites (%d,%d) a (%d,%d)",x1,y1,x2,y2);
    ...
    d_level	*lev;
{
    ...
    impossible("¡No podía poner tipo de lregion %d!", rtype);
}
...
register const char *s;
{
	...
	if(*s) {
	    if(sp && sp->rndlevs) Sprintf(protofile, "%s-%d", s,
            ...
            pline("Debug: proto=%s", dungeons[u.uz.dnum].proto);
	...
	if(*protofile) {
	    ...
	    impossible("No podía leer \"%s\" - haciendo un laberinto.", protofile);
	}
...
#define INVPOS_DISTANCE 11
		...
		panic("inv_pos: ¡laberinto es demasiado pequeño! (%d x %d)",
...
register int dir;
{
	switch(dir){
		...
		case 2: (*y)++; break;
		case 3: --(*x); break;
		default: panic("move: mala dirección");
	}
}
	...
	coord	*cc;
{
	...
	if (cpt >= 100) {
		...
		panic("mazexy: ¡no puedo encontrar un lugar!");
	}
	return;
}
...
register xchar x, y, todnum, todlevel;
{
	...
	if (!ttmp) {
		impossible("portal sobre portal??");
		return;
	}
#ifdef DEBUG
...
movebubbles()
{
	...
	for (b = up ? bbubbles : ebubbles; b; b = up ? b->next : b->prev) {
		...
		for (j = 0, y = b->y; j < (int) b->bm[1]; j++, y++)
		    if (b->bm[j + 2] & (1 << i)) {
			if (!isok(x,y)) {
			    impossible("movebubbles: mala pos (%d,%d)", x,y);
			    continue;
			}
...
water_friction()
{
	...
	if (eff) pline("Turbulencia de agua afecta tus movimientos.");
}
...
/* LENGUA:  Need a translation for swamp */
...
xchar x,y;
{
	...
	if (!isok(x,y))
		return "la bebida";		/* should never happen */
	lev = &levl[x][y];
	ltyp = lev->typ;
...
register int x, y, n;
{
	...
	if (n >= SIZE(bmask)) {
		impossible("n demasiado grande (mk_bubble)");
		n = SIZE(bmask) - 1;
	}
	b = (struct bubble *)alloc(sizeof(struct bubble));
...
register boolean ini;
{
	...
	for (cons = b->cons; cons; cons = ctemp) {
		...
		default:
		    impossible("mv_bubble: contenidos de burbuja desconocidos");
		    break;
	    }
