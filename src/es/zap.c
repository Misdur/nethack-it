...
#else
STATIC_VAR const char are_blinded_by_the_flash[] = "¡Eres cegad%c por el rayo!";

const char * const flash_types[] = {	/* also used in buzzmu(mcastu.c) */
	"proyectil mágico",	/* Wands must be 0-9 */
	"rayo de fuego",
	"rayo de frío",
	"rayo de sueño",
	"rayo de muerte",
	...
	"proyectil mágico",	/* Spell equivalents must be 10-19 */
	"bola de fuego",
	"cono de frío",
	"rayo de sueño",
	"dedo de muerte",
	"rayo de relámpago",	/* There is no spell, used for retribution */
	"",
	"",
	"",
	"",

	"ráfaga de proyectiles",	/* Dragon breath equivalents 20-29*/
	"ráfaga de fuego",
	"ráfaga de hielo",
	"ráfaga de gas de sueño",
	"ráfaga de desintegración",
	"ráfaga de relámpago",
	"ráfaga de gas envenenado",
	"ráfaga de ácido",
	"",
	""
};
...
struct obj *otmp;
{
	...
	const char *zap_type_text = "hechizo";
	struct obj *obj;
	...
	case SPE_SLOW_MONSTER:
		if (!resist(mtmp, otmp->oclass, 0, NOTELL)) {
			...
			if (u.uswallow && (mtmp == u.ustuck) &&
			    is_whirly(mtmp->data)) {
				pline("¡%s!", mon_nam_acc(mtmp, "Desbaratas"));
				pline("Una abertura grandísima se abre...");
	...
	case WAN_MAKE_INVISIBLE:
		...
		if (!oldinvis && knowninvisible(mtmp)) {
		    pline("¡%s se vuelve transpariente!", nambuf);
		    makeknown(otyp);
		}
		break;
	    }
	...
	case SPE_KNOCK:
		...
		if (u.uswallow && mtmp == u.ustuck) {
			if (is_animal(mtmp->data)) {
				if (Blind) Sientes("un chorro súbito de aire!");
				else pline("%s se abre la boca!", Monnam(mtmp));
			}
			expels(mtmp, mtmp->data, TRUE);
#ifdef STEED
	...
	case SPE_EXTRA_HEALING:
		...
		if (canseemon(mtmp)) {
		    ...
		    } else pline("%s se ve%s mejor.", Monnam(mtmp),
			         otyp == SPE_EXTRA_HEALING ? " mucho" : "" );
		}
	...
	case SPE_STONE_TO_FLESH:
		if (monsndx(mtmp->data) == PM_STONE_GOLEM) {
			...
			if (canseemon(mtmp))
			    pline("¡%s se vuelve carne!", name);
		    } else {
			if (canseemon(mtmp))
			    pline("%s parece bastante carnos%c por un momento.",
				  name, mon_gender(mtmp)? 'a' : 'o');
		    }
		} else
		    wake = FALSE;
		break;
	case SPE_DRAIN_LIFE:
		...
		if (resists_drli(mtmp))
			...
			mtmp->m_lev--;
			if (canseemon(mtmp))
			    pline("¡%s de repente parece más débil!", Monnam(mtmp));
		    }
		}
		break;
	default:
		impossible("Qué efecto interesante (%d)", otyp);
		break;
	}
	if(wake) {
	    if(mtmp->mhp > 0) {
...
struct monst *mtmp;
{
	...
	if (mtmp->minvent) {
	    ...
	    pline("%s no trae nada.", noit_Monnam(mtmp));
	}
}
			...
			if (obj->oxlth && (obj->oattached == OATTACHED_M_ID)) {
			    ...
			    ghost = find_mid(m_id, FM_FMON);
		    	    if (ghost && ghost->data == &mons[PM_GHOST]) {
		    		    ...
		    		    if (canseemon(ghost))
		    		  	pline("¡%s de repente se trae en su cuerpo anterior!",
						Monnam(ghost));
				    mondead(ghost);
...
struct monst *mon;
{
	...
	while ((otmp = otmp2) != 0) {
		...
		if (youseeit) {
		    if (!once++) {
			if (mon == &youmonst)
			    Sprintf(owner, "Tu %s", corpse);
			else
			    Sprintf(owner, "%s %s", The(corpse),
				    del(mon_nam(mon)));
		    }
		    pline("¡%s de repente se hace vivo!", owner);
		} else if (canseemon(mtmp2))
		    pline("¡%s de repente aparece!", Amonnam(mtmp2));
	    }
	}
	return res;
}
...
register struct obj *obj;
{
	...
	case OBJ_INVENT:
		if (obj->unpaid) {
		    ...
		    if (!shkp) return;
		    Norep("¡Usted cancela un objeto sin pagar, usted lo paga!");
	...
	case OBJ_FLOOR:
		...
		if (costly_spot(u.ux, u.uy) && objroom == *u.ushops) {
		    Norep("¡Usted lo cancela, usted lo paga!");
		    bill_dummy_object(obj);
		} else
    ...
    int okind;
{
	...
	if(mtmp && cansee(mtmp->mx, mtmp->my)) {
	    pline("Unos objetos%s se funden, y %s se levanta de la pila!",
...
no_unwear:
	...
	if ((!carried(otmp) || obj->unpaid) &&
		...
		if(shkp->mpeaceful) {
			...
			pline("¡%s se enoja!", Monnam(shkp));
			hot_pursuit(shkp);
		    }
		} else Norep("¡%s está furios%c!", Monnam(shkp),
				mon_gender(shkp)? 'a' : 'o');
	    }
	}
	delobj(obj);
...
struct obj *obj, *otmp;
{
...
#ifdef DEBUG
			pline("%s por un momento.", Tobjnam(obj, "pulsa"));
#endif
			obj->bypass = 0;
		}
	}
	...
	if (!(obj->where == OBJ_FLOOR || otmp->otyp == SPE_STONE_TO_FLESH))
	    impossible("bhito: obj no es piso ni el hechizo Piedra a Carne");
	...
	case WAN_PROBING:
		...
		if (Is_container(obj) || obj->otyp == STATUE) {
			...
			pline("%s vací%c.", Tobjnam(obj, "está"),
			      isfeminine(que)? 'a' : 'o');
		    } else {
			struct obj *o;
...
smell:
			if (herbivorous(youmonst.data) &&
			    ...
			    Norep("Hueles el olor de carne.");
			else
			    Norep("Hueles un olor delicioso.");
			break;
		...
		break;
	default:
		impossible("Qué efecto interesante (%d)", otmp->otyp);
		break;
	}
	return res;
}
...
register struct obj *wand;
{
	if(wand->spe < 0 || (wand->spe == 0 && rn2(121)))
		return 0;
	if(wand->spe == 0)
		pline("Arrebatas una final carga de la varita gastada.");
	wand->spe--;
	return 1;
}
...
register struct obj *obj;
{
	...
	switch(obj->otyp) {
		...
		case WAN_WISHING:
			known = TRUE;
			if(Luck + rn2(5) < 0) {
				pline("Desgraciadamente, no pasa nada.");
				break;
			}
			makewish(FALSE);
			break;
		case WAN_ENLIGHTENMENT:
			known = TRUE;
			Te_sientes("como si te conocieras...");
			display_nhwindow(WIN_MESSAGE, FALSE);
			enlightenment(FALSE);
			pline("El sentido subsida.");
			exercise(A_WIS, TRUE);
			break;
	}
...
struct obj *otmp;
{
	...
	pline("¡%s de repente explota!", The(xname(otmp)));
	losehp(d(otmp->spe+2,6), "varita explotante", KILLED_BY_AN);
	useup(otmp);
}
...
dozap()
{
	...
	if(!zappable(obj)) pline(nothing_happens);
		...
		if (!Blind)
		    pline("%s alumbra y se obscura.", The(xname(obj)));
		/* make him pay for knowing !NODIR */
	} else if(!u.dx && !u.dy && !u.dz && !(objects[obj->otyp].oc_dir == NODIR)) {
	    if ((damage = zapyourself(obj, TRUE)) != 0) {
		losehp(damage, "se agitó con una varita", NO_KILLER_PREFIX);
	    }
	} else {
	...
	if (obj && obj->spe < 0) {
	    pline("%s polvo.", Tobjnam(obj, "se vuelve"));
	    useup(obj);
	}
...
boolean ordinary;
{
	...
	switch(obj->otyp) {
		...
		case WAN_LIGHTNING:
			...
			shieldeff(u.ux, u.uy);
			pline("Te agitas, pero pareces indañad%c.",
				poly_gender()==1? 'a' : 'o');
		    ...
		    if (!resists_blnd(&youmonst)) {
			    pline(are_blinded_by_the_flash, poly_gender()==1? 'a' : 'o');
		...
		case SPE_FIREBALL:
		    pline("¡Explotas una bola de fuego sobre ti!");
		...
		case FIRE_HORN:
		    if (Fire_resistance) {
			shieldeff(u.ux, u.uy);
			Te_sientes("bastante cálid%c.",
				poly_gender()==1? 'a' : 'o');
			ugolemeffects(AD_FIRE, d(12,6));
		    } else {
			pline("¡Te has encendido!");
			damage = d(12,6);
		    }
		    burn_away_slime();
		...
		case FROST_HORN:
		    if (Cold_resistance) {
			shieldeff(u.ux, u.uy);
			Te_sientes("un poco frí%c.", poly_gender()==1? 'a' : 'o');
			ugolemeffects(AD_COLD, d(12,6));
		    } else {
			pline("¡Imitas un popsicle!");
			damage = d(12,6);
		    }
		...
		case SPE_MAGIC_MISSILE:
		    if(Antimagic) {
			shieldeff(u.ux, u.uy);
			pline("¡Los proyectiles rebotan!");
		    } else {
			damage = d(4,6);
			pline("¡Idiota!  ¡De has disparado!");
		    }
		    break;
		...
		case WAN_MAKE_INVISIBLE: {
			...
			/* A mummy wrapping absorbs it and protects you */
		        Te_sientes("un poco hormigos%c bajo tu %s.",
				poly_gender()==1? 'a' : 'o',
				xname(uarmc));
		        break;
		    }
		...
		case WAN_SPEED_MONSTER:
			...
			if (!Fast)
			    ...
			    pline("Tu rapidez siente más natural.");
		...
		case SPE_SLEEP:
		    if(Sleep_resistance) {
			shieldeff(u.ux, u.uy);
			pline("¡No tienes sueño!");
		    } else {
			pline("¡El rayo de sueño te golpea!");
		...
		case SPE_FINGER_OF_DEATH:
			...
			if (obj->otyp == WAN_DEATH)
			    pline("La varita te dispara un rayo aparentemente sin daño.");
			else
			    pline("Pareces no más muert%c que antes.",
				  poly_gender()==1? 'a' : 'o');
			break;
		    }
		    killer = "se disparó con un rayo de muerte";
		    killer_format = NO_KILLER_PREFIX;
		    pline("¡Te irradias con energía pura!");
		    pline("Te mueres.");
		    makeknown(obj->otyp);
		...
		case SPE_TURN_UNDEAD:
			...
			Te_sientes("espantad%c y %saturdid%c.",
			     fem, Stunned ? "todavía más " : "", fem);
			make_stunned(HStun + rnd(30), FALSE);
		    } else
			pline("Te temblas de pavor.");
		    break;
		case SPE_HEALING:
		case SPE_EXTRA_HEALING:
		    ...
		    Te_sientes("%smejor.",
			obj->otyp == SPE_EXTRA_HEALING ? "mucho " : "");
		    break;
		case WAN_LIGHT:	/* (broken wand) */
		 /* assert( !ordinary ); */
		    damage = d(obj->spe, 25);
		...
		case SPE_KNOCK:
		    if (Punished) pline("Tu cadena tembla por un momento.");
		    break;
		case WAN_DIGGING:
		case SPE_DIG:
		case SPE_DETECT_UNSEEN:
		...
		case SPE_STONE_TO_FLESH:
		    ...
		    break;
		default: impossible("¿objeto %d usado?",obj->otyp);
		    break;
	}
	return(damage);
}
...
boolean			youattack, allow_cancel_kill, self_cancel;
{
	...
	if (youdefend) {
		...
		if (Unchanging)
		    pline("Tu amuleto se pone caliente por un momento, luego se enfría.");
		else
		    rehumanize();
	    }
	} else {
...
struct obj *obj;	/* wand or spell */
{
	...
	case WAN_PROBING:
	    ptmp = 0;
	    if (u.dz < 0) {
		pline("Pruebas hacia %s.", ceiling(x,y));
	    } else {
		ptmp += bhitpile(obj, bhito, x, y);
		pline("Pruebas bajo %s.", del(surface(x,y)));
		ptmp += display_binventory(x, y, TRUE);
	    }
	    if (!ptmp) pline("Tu prueba no revela nada.");
	...
	case SPE_KNOCK:
		...
		pline("La escalera parece ondear por un momento.");
		disclose = TRUE;
	    }
	    break;
	case WAN_STRIKING:
	...
	case SPE_WIZARD_LOCK:
		...
		/* similar to zap_dig() */
		pline("Una piedra se suelta %s y cae sobre tu %s.",
		      del(ceiling(x, y)), body_part(HEAD));
		losehp(rnd((uarmh && is_metallic(uarmh)) ? 2 : 6),
		       "piedra cayente", KILLED_BY_AN);
		...
		if (!Blind) {
			if (ttmp->tseen) {
				pline("Una trampa bajo ti se cierra y entonces desvanece.");
				disclose = TRUE;
			} else {
				pline("Ves una gira de %s bajo ti.",
					is_ice(x,y) ? "hielo" : "polvo");
			}
		} else {
			Oyes("un twang seguido por un thud.");
		}
		deltrap(ttmp);
		ttmp = (struct trap *)0;
		newsym(x, y);
	...
	case SPE_STONE_TO_FLESH:
		...
		pline("Sangre gotea sobre tu %s.", body_part(FACE));
		...
		*/
		...
		if (!(e && e->engr_type == ENGRAVE)) {
			...
			pline("Sangre %s %s tus %s.",
			      is_lava(u.ux, u.uy) ? "se bulle" : "forma un charco",
			      Levitation ? "bajo" : "a",
	...
	if (u.dz > 0) {
		...
		case SPE_STONE_TO_FLESH:
			...
			pline(Hallucination ?
			    "¡El piso se corre como manteca!" :
			    "Los filos del piso se hacen más suave.");
...
register struct	obj	*obj;
{
	...
	if (objects[otyp].oc_dir == IMMEDIATE) {
		...
		impossible("weffects: hechizo o varita inesperada");
	    disclose = TRUE;
	}
...
register const char *force;		/* usually either "." or "!" */
{
	...
	force2  = (strcmp(force, "!") == 0) ? "¡"
		: (strcmp(force, "?") == 0) ? "¿"
		: "";
	if((!cansee(bhitpos.x,bhitpos.y) && !canspotmon(mtmp) &&
	     !(u.uswallow && mtmp == u.ustuck))
	   || !flags.verbose)
	    pline("%s lo %s.", The(str), vtense(str, "golpea"));
	else pline("%s%s %s%s", force2, The(str),
		mon_nam_acc(mtmp, vtense(str, "golpea")), force);
}
...
register struct monst *mtmp;
{
	if ((cansee(bhitpos.x,bhitpos.y) || canspotmon(mtmp))
	       && flags.verbose)
	    pline("%s %s.", The(str), mon_nam_acc(mtmp, vtense(str, "falla")));
	else
	    pline("%s lo %s.", The(str), vtense(str, "falla"));
}
#endif /*OVL0*/
#ifdef OVL1
...
struct obj *obj;			/* object tossed/used */
{
	...
	while(range-- > 0) {
		...
		if ((bobj = sobj_at(BOULDER, x, y)) != 0) {
			...
			if (cansee(x,y))
			    /* LENGUA:  need a better word for jerk */
			    pline("%s se tira a una parada repentina.",
	...
	for (ct = 0; ct < 10; ct++) {
		...
		if(bhitpos.x == u.ux && bhitpos.y == u.uy) { /* ct == 9 */
			if(Fumbling || rn2(20) >= ACURR(A_DEX)) {
				...
				tmp_at(DISP_END, 0);
				pline("Ases el bumerán de destreza.");
				return(&youmonst);
			}
		}
		tmp_at(bhitpos.x, bhitpos.y);
...
struct obj **ootmp;	/* to return worn armor for caller to disintegrate */
{
	...
	*ootmp = (struct obj *)0;
	...
	case ZT_MAGIC_MISSILE:
		...
		if (spellcaster)
		    pline("Daño = %d + %d", tmp-spell_damage_bonus(),
			spell_damage_bonus());
#endif
		break;
	case ZT_FIRE:
...
#ifdef WIZ_PATCH_DEBUG
	pline("monstruo agitado hp = %d (= %d - %d)", mon->mhp-tmp,mon->mhp,tmp);
#endif
	mon->mhp -= tmp;
	return(tmp);
}
...
xchar sx, sy;
{
	...
	case ZT_MAGIC_MISSILE:
	    if (Antimagic) {
		shieldeff(sx, sy);
		pline("¡Los proyectiles se rebotan!");
	    } else {
		dam = d(nd,6);
		exercise(A_STR, FALSE);
	    }
	...
	case ZT_DEATH:
		...
		shieldeff(sx, sy);
		pline("Pareces inefectad%c.",
			poly_gender()==1? 'a' : 'o');
		break;
	    } else if (Antimagic) {
		shieldeff(sx, sy);
		pline("No eres efectad%c.",
			poly_gender()==1? 'a' : 'o');
		break;
	    }
	...
	case ZT_LIGHTNING:
	    ...
	    break;
	case ZT_POISON_GAS:
	    poisoned("ráfaga", A_DEX, "ráfaga envenenada", 15);
	    break;
	case ZT_ACID:
	    if (Acid_resistance) {
		dam = 0;
	    } else {
		pline("¡El ácido quema!");
		dam = d(nd,6);
		exercise(A_STR, FALSE);
	    }
...
boolean u_caused;
{
	...
	for (obj = level.objects[x][y]; obj; obj = obj2) {
		...
		if (delquan) {
		    ...
		    if (give_feedback) {
			if (delquan > 1)
			    pline("%ld %s se queman.", delquan, buf2);
			else
			    pline("%s se quema.", An(buf1));
		    }
		}
	    }
	}
...
register int dx,dy;
{
	...
	else	pline("%s%s se rasga al dentro %s%s",
		      exclam2(tmp), The(fltxt), del(mon_nam(u.ustuck)), exclam(tmp));
	...
	if (mon) {
		...
		if (mon_reflects(mon, (char *)0)) {
			...
			(void) mon_reflects(mon, "¡Pero se refleja desde %s %s!");
		    }
		    dx = -dx;
		    dy = -dy;
		} else {
			...
			if (canseemon(mon)) {
			    hit(fltxt, mon, ".");
			    pline("%s se desintegra.", Monnam(mon));
			    pline("¡El cuerpo %s reintegra al frente de tu%s %s!",
				  del(mon_nam(mon)),
				  (eyecount(youmonst.data) == 1) ?  "" : "s",
				  (eyecount(youmonst.data) == 1) ?
				  	body_part(EYE) : makeplural(body_part(EYE)));
			    pline("¡%s revive!", Monnam(mon));
			}
			mon->mhp = mon->mhpmax;
			...
			if (canseemon(mon)) {
			    hit(fltxt, mon, ".");
			    pline("¡%s absuerbe %s mortal!", Monnam(mon),
				  type == ZT_BREATH(ZT_DEATH) ?
					"la ráfaga" : "el rayo");
			    pline("Parece aún más fuerte que antes.");
			}
			break; /* Out of while loop */
		    }
...
#define oresist_disintegration(obj) \
		...
		range -= 2;
		pline("¡%s te golpea!", The(fltxt));
		if (Reflecting) {
		    if (!Blind) {
		    	(void) ureflects("¡Pero refleja de tu%s%s!", "");
		    } else
			pline("Por alguna razón no eres afectad%c.",
				poly_gender()==1? 'a' : 'o');
		    dx = -dx;
		    dy = -dy;
		    shieldeff(sx, sy);
		...
		pline("¡%s zumba por ti!", The(fltxt));
	    }
 ...
 make_bounce:
    ...
    if (shopdamage)
	pay_for_damage(abstype == ZT_FIRE ?  "quemar" :
		       abstype == ZT_COLD ?  "estrellar" :
		       abstype == ZT_DEATH ? "desintegrar" : "destruir", FALSE);
...
xchar x, y;
{
	...
	newsym(x,y);
	if (cansee(x,y)) Norep("El hielo cruje y se funde.");
	if ((otmp = sobj_at(BOULDER, x, y)) != 0) {
	    if (cansee(x,y)) pline("%s se asenta...", An(xname(otmp)));
	    do {
		obj_extract_self(otmp);	/* boulder isn't being pushed */
		if (!boulder_hits_pool(otmp, x, y, FALSE))
		    impossible("melt_ice: ¿no chorro?");
...
boolean *shopdamage;
{
	...
	if(abstype == ZT_FIRE) {
		...
		if (cansee(x,y)) Norep("¡Una telaraña se estrella en llamas!");
		(void) delfloortrap(t);
		...
		const char *msgtxt = "Oyes gas siseante.";
		if(lev->typ != POOL) {	/* MOAT or DRAWBRIDGE_UP */
		    if (cansee(x,y)) msgtxt = "Algo de agua se evapora.";
		} else {
		    ...
		    if (cansee(x,y)) msgtxt = "El agua se evapora.";
		}
		Norep(msgtxt);
		if (lev->typ == ROOM) newsym(x,y);
	    } else if(IS_FOUNTAIN(lev->typ)) {
		    if (cansee(x,y))
			pline("Vapor se alza desde la fuente.");
		    rangemod -= 1;
		    dryup(x, y, type > 0);
	    }
	}
		...
		if (lev->typ == WATER) {
		    ...
		    if (cansee(x,y))
			pline("El agua se hiela por un momento.");
		    else
			Oyes("un crujido suave.");
		    rangemod -= 1000;	/* stop */
		} else {
		    rangemod -= 3;
		    if (lev->typ == DRAWBRIDGE_UP) {
		    ...
		    if(cansee(x,y)) {
			if(moat)
				Norep("¡El foso está llenado de hielo!");
			else if(lava)
				Norep("La lava se hiela y se solidifica.");
			else
				Norep("El agua se hiela.");
			newsym(x,y);
		    } else if(flags.soundok && !lava)
			Oyes("un sonido de crujido.");
			...
			if (u.uinwater) {   /* not just `if (Underwater)' */
			    ...
			    if (Passes_walls) {
				pline("Pasas través de la piedra sólida.");
			    } else {
				u.utrap = rn1(50,20);
				u.utraptype = TT_INFLOOR;
				pline("Estás firmamente pegad%c en la piedra enfriante.",
					poly_gender()==1? 'a' : 'o');
			    }
			}
	...
	if(closed_door(x, y)) {
		...
		case ZT_FIRE:
		    ...
		    see_txt = "¡Se consume la puerta en llamas!";
		    sense_txt = "Hueles humo.";
		    break;
		case ZT_COLD:
		    new_doormask = D_NODOOR;
		    see_txt = "¡La puerta se hiela y se estrella!";
		    Sprintf(buf, "Te sientes frí%c.", poly_gender()==1? 'a' : 'o');
                    sense_txt = buf;
		    break;
		case ZT_DEATH:
			...
			goto def_case;
		    new_doormask = D_NODOOR;
		    see_txt = "¡La puerta se desintegra!";
		    hear_txt = "madera estallante.";
		    break;
		case ZT_LIGHTNING:
		    new_doormask = D_BROKEN;
		    see_txt = "¡La puerta se estrella!";
		    hear_txt = "crujidos.";
		    break;
		default:
		def_case:
		    if(cansee(x,y)) {
			const char *que =
			      abs(type) < ZT_SPELL(0) ? "el rayo" :
			      abs(type) < ZT_BREATH(0) ? "el hechizo" :
			      "la ráfaga";
			pline("¡La puerta absuerbe %s%s!",
			      (type < 0) ? "" : "tu ",
			      (type < 0) ? que : que+3);
		    } else Sientes("vibraciones.");
		    break;
		}
		if (new_doormask >= 0) {	/* door gets broken */
			...
			if (flags.soundok) Oyes(hear_txt);
		    }
		    if (picking_at(x, y)) {
			stop_occupation();
			reset_pick();
	...
	if(OBJ_AT(x, y) && abstype == ZT_FIRE)
		if (burn_floor_paper(x, y, FALSE, type > 0) && couldsee(x, y)) {
		    newsym(x,y);
		    pline("%s de humo.",
			!Blind ? "Ves una bocanada" : "Hueles un soplo");
		}
	if ((mon = m_at(x,y)) != 0) {
...
register struct obj *obj;
{
	...
	if (Role_if(PM_ARCHEOLOGIST) && !flags.mon_moving && (obj->spe & STATUE_HISTORIC)) {
	    Te_sientes("culpable sobre dañar una estatua tan histórica.");
	    adjalign(-1);
	}
	obj->spe = 0;
	fracture_rock(obj);
...
const char * const destroy_strings[] = {	/* also used in trap.c */
	"se hiela y se estrella", "se hielan y se estrellan", "poción estrellada",
	"bulle y explota", "bullen y explotan", "poción bulliente",
	"se encienda y quema", "se enciendan y queman", "pergamino quemante",
	"se encienda y quema", "se enciendan y queman", "libro quemante",
	"se vuelve polvo y se desvanece", "se vuelven polvo y se desvanecen", "",
	"se rompe y explota", "se rompen y explotan", "varita explotante"
};
...
register int osym, dmgtyp;
{
	...
	for(obj = invent; obj; obj = obj2) {
		...
		case AD_FIRE:
			...
			skip++;
			if (!Blind)
			    pline("%s alumbra un %s extraño, pero queda intato.",
				The(xname(obj)), hcolor("rojo oscuro"));
		    }
		    quan = obj->quan;
		    switch(osym) {
			case POTION_CLASS:
		...
		if(!cnt) continue;
		if(cnt == quan)	mult = (cnt == 1L)? "Tu" : "Tus";
		else if (isfeminine(destroy_strings[dindx*3 + 2]))
			mult = (cnt == 1L) ? "Una de tu" : "Unas de tus";
		else	mult = (cnt == 1L) ? "Uno de tu" : "Unos de tus";
		...
		for (i = 0; i < cnt; i++)
		    useup(obj);
		if(dmg) {
		    if(xresist)	pline("¡No eres dañad%c!",
					poly_gender()==1? 'a' : 'o');
		    else {
...
int osym, dmgtyp;
{
	...
	for(obj = mtmp->minvent; obj; obj = obj2) {
		...
		if(!cnt) continue;
		if (vis) pline("¡%s %s %s!",
			The(xname(obj)), del(mon_nam(mtmp)),
			(cnt > 1L) ? destroy_strings[dindx*3 + 1]
				  : destroy_strings[dindx*3]);
...
boolean wiz;
{
	...
	if (flags.verbose) pline("Puedes desear un objeto.");
retry:
	getlin("¿Por cuál deseas?", buf);
	 ...
	 */
	otmp = readobjnam(buf, &nothing, TRUE, wiz);
	if (!otmp) {
	    pline("Nada que quepa esa descripción existe en el juego.");
	    if (++tries < 5) goto retry;
	    pline(thats_enough_tries);
	    otmp = readobjnam((char *)0, (struct obj *)0, TRUE, wiz);
