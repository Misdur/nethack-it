...
struct obj *wep;
{
	...
	if (!wep) {
		...
		pline("Tienes %s vací%cs.", makeplural(mano),
			isfeminine(mano)? 'a' : 'o');
		setuwep((struct obj *) 0);
		res++;
	    } else {
		const char *mano = body_part(HAND);
		pline("Ya tienes %s vací%cs.", makeplural(mano),
			isfeminine(mano)? 'a' : 'o');
	    }
	    ...
	    pline("Blandes el cadáver de %s en tus %s desnud%cs.",
		  mons[wep->corpsenm].mname, makeplural(mano),
		  isfeminine(mano)? 'a' : 'o');
	    Sprintf(kbuf, "cadáver de %s", mons[wep->corpsenm].mname);
	    instapetrify(kbuf);
		...
		pline("Tienes un sentido incómodo sobre blandir hierro frío.");
		change_luck(-1);
	    }
#endif
...
dowield()
{
	...
	if (!(wep = getobj(wield_objs, "", "blandir")))
	    ...
	    pline("¡Ya estás blandiendo esto!");
...
dowieldquiver()
{
	...
	if (newquiver == &zeroobj) {
		/* Explicitly nothing */
		if (uquiver) {
			pline("Ya no tienes amunición lista.");
...
const char *verb_pres;	/* "frotas",&c */
{
	...
	if (flags.verbose) {
	    ...
	    pline(
	     "Porque tu arma está soldada a tu%s %s, no puedes %s %s %s.",
...
#define NOT_WEAPON(obj) (!is_weptool(obj) && obj->oclass != WEAPON_CLASS)
	if (!could_twoweap(youmonst.data)) {
		...
		pline("Tu%s %s%s%s está%s vací%c%s.",
			dosmanos? "s" : "",
			mano,
		...
		pline("Blandes el cadáver de %s en tu %s desnud%c.",
...
{
	if (u.twoweap) {
		pline("No puedes usar dos armas más.");
		u.twoweap = FALSE;
		update_inventory();
	}
	return;
...
register int amount;
{
	...
	if(uwep->otyp == WORM_TOOTH && amount >= 0) {
		uwep->otyp = CRYSKNIFE;
		uwep->oerodeproof = 0;
		pline("Tu arma parece más aguda ahora.");
		uwep->cursed = 0;
	...
	if(uwep->otyp == CRYSKNIFE && amount < 0) {
		...
		uwep->oerodeproof = 0;
		pline("Tu arma parece más embotada ahora.");
