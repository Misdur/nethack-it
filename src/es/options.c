...
# endif
	...
	{"unicode",   (boolean *)0, FALSE, SET_IN_GAME},
#else
	{"unicode",   &iflags.unicode, FALSE, SET_IN_GAME},
#endif
#ifdef WIN32CON
...
static struct Comp_Opt
{
	...
	{ "accents",  "juego de caracteres para mostrar letras con acentos (0, 1 ó 2)",
						1, SET_IN_GAME },
	{ "align",    "tu alineamiento inicial (justo, neutral, o caótico)",
						8, DISP_IN_GAME },
	{ "align_message", "alineamiento de ventana de mensajes", 20, DISP_IN_GAME }, 	/*WC*/
	{ "align_status", "alineamiento de ventana de estado", 20, DISP_IN_GAME }, 	/*WC*/
	{ "altkeyhandler", "interfaz de tecleado alternativa", 20, DISP_IN_GAME },
	{ "boulder",  "el símbolo usado para mostrar las rocas enormes",
						1, SET_IN_GAME },
	{ "catname",  "el nombre de tu (primer) gato (p.ej., catname:Tabby)",
						PL_PSIZ, DISP_IN_GAME },
	{ "disclose", "las clases de la información para revelar al fin de juego",
						sizeof(flags.end_disclose) * 2,
						SET_IN_GAME },
	{ "dogname",  "el nombre de tu (primer) perro (p.ej., dogname:Fang)",
						PL_PSIZ, DISP_IN_GAME },
	{ "dungeon",  "los símbolos usados para mostrar el mapa del calabozo",
						MAXDCHARS+1, SET_IN_FILE },
	{ "effects",  "los símbolos usados para mostrar los efectos especiales",
						MAXECHARS+1, SET_IN_FILE },
	{ "font_map", "la fuente usada en la ventana de mapa", 40, DISP_IN_GAME },	/*WC*/
	{ "font_menu", "la fuente usada en los menus", 40, DISP_IN_GAME },		/*WC*/
	{ "font_message", "la fuente usada en la ventana de mensajes",
						40, DISP_IN_GAME },		/*WC*/
	{ "font_size_map", "el tamaño de la fuente de mapa", 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_menu", "el tamaño de la fuente de menú", 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_message", "el tamaño de la fuente de mensaje", 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_status", "el tamaño de la fuente de estado", 20, DISP_IN_GAME },	/*WC*/
	{ "font_size_text", "el tamaño de la fuente de texto", 20, DISP_IN_GAME },	/*WC*/
	{ "font_status", "la fuente usada en la ventana de estado", 40, DISP_IN_GAME }, /*WC*/
	{ "font_text", "la fuente usada en la ventana de texto", 40, DISP_IN_GAME },	/*WC*/
	{ "fruit",    "el nombre de una fruta que te gusta comer",
						PL_FSIZ, SET_IN_GAME },
	{ "gender",   "tu sexo inicial (male (masculino) o female (femenina))",
						8, DISP_IN_GAME },
	{ "horsename", "el nombre de tu (primer) caballo (p.ej., horsename:Silver)",
						PL_PSIZ, DISP_IN_GAME },
#ifdef ASCIIGRAPH
	{"IBMgraphics", "El juego de símbolos IBM para mostrar el mapa (0, 1, 2 ó 3)",
						1, SET_IN_GAME },
#endif
	{ "map_mode", "modo de mostrar el mapa en Windows", 20, DISP_IN_GAME },	/*WC*/
	{ "menustyle", "interfaz usuaria para selección de los objetos",
						MENUTYPELEN, SET_IN_GAME },
	{ "menu_deselect_all", "deselegir todos los artículos en un menú", 4, SET_IN_FILE },
	{ "menu_deselect_page", "deselegir todos los artículos en esta página de un menú",
						4, SET_IN_FILE },
	/* LENGUA:  Need a good translation */
	{ "menu_headings", "bold, inverse, or underline headings", 9, SET_IN_GAME },
	{ "menu_first_page", "saltar a la primera página en un menú",
						4, SET_IN_FILE },
	{ "menu_invert_all", "invertir todos los artículos en un menú", 4, SET_IN_FILE },
	{ "menu_invert_page", "invertir todos los artículos en esta página de un menú",
						4, SET_IN_FILE },
	{ "menu_last_page", "saltar a la última página en un menú", 4, SET_IN_FILE },
	{ "menu_next_page", "ir a la próxima página del menú", 4, SET_IN_FILE },
	{ "menu_previous_page", "ir a la previa página del menú", 4, SET_IN_FILE },
	{ "menu_search", "buscar un artículo de menú", 4, SET_IN_FILE },
	{ "menu_select_all", "elegir todos los artículos en el menú", 4, SET_IN_FILE },
	{ "menu_select_page", "elegir todos los artículos en esta página del menú",
						4, SET_IN_FILE },
	{ "monsters", "los símbolos para usar para los monstruos",
						MAXMCLASSES, SET_IN_FILE },
	{ "msghistory", "cuantos mensajes de linea encima para guardar",
						5, DISP_IN_GAME },
# ifdef TTY_GRAPHICS
	{"msg_window", "el tipo necesario de ventana de mensaje",1, SET_IN_GAME},
# else
	{"msg_window", "el tipo necesario de ventana de mensaje", 1, SET_IN_FILE},
# endif
	{ "name",     "el nombre de tu personaje (e.g., name:Merlin-W)",
						PL_NSIZ, DISP_IN_GAME },
	/* LENGUA:  Get a surer translation for "number pad" */
	{ "number_pad", "usar el tecleado numérico", 1, SET_IN_GAME},
	{ "objects",  "los símbolos para usar para los objetos",
						MAXOCLASSES, SET_IN_FILE },
	{ "packorder", "el orden de inventario de los artículos en tu carga",
						MAXOCLASSES, SET_IN_GAME },
#ifdef CHANGE_COLOR
	/* LENGUA:  Necesito una palabra para "palette" */
	{ "palette",  "palette (00c/880/-fff es azul/amarillo/blanco inverso)",
						15 , SET_IN_GAME },
# if defined(MAC)
	{ "hicolor",  "lo mismo como palette, excepto que se inverte el orden",
						15, SET_IN_FILE },
# endif
#endif
	{ "pettype",  "el tipo de mascota que prefieres", 4, DISP_IN_GAME },
	{ "pickup_burden",  "peso máximo escogido antes de apuntar",
						20, SET_IN_GAME },
	{ "pickup_types", "tipos de objetos de escoger automaticamente",
						MAXOCLASSES, SET_IN_GAME },
	{ "race",     "tu raza inicial (e.g., Human, Elf)",
						PL_CSIZ, DISP_IN_GAME },
	{ "role",     "tu rol/papel inicial (e.g., Bárbaro, Valquiria)",
						PL_CSIZ, DISP_IN_GAME },
	{ "runmode", "frequencia de actualizar el mapa cuando `corriendo' o `viajando'",
						sizeof "teleport", SET_IN_GAME },
	{ "scores",   "las partes de la lista de escor que quieres ver",
						32, SET_IN_GAME },
	{ "scroll_amount", "distancia para mover el mapa cuando se alcanza scroll_margin",
						20, DISP_IN_GAME }, /*WC*/
	{ "scroll_margin", "mover el mapa cuando a esta distancia del borde", 20, DISP_IN_GAME }, /*WC*/
#ifdef MSDOS
	{ "soundcard", "tipo de tarjeta de sonido usada", 20, SET_IN_FILE },
#endif
	{ "suppress_alert", "suprimir los avisos sobre los rasgos específicos de versión",
						8, SET_IN_GAME },
	{ "tile_width", "anchura de tejas", 20, DISP_IN_GAME},	/*WC*/
	{ "tile_height", "altura de tejas", 20, DISP_IN_GAME},	/*WC*/
	{ "tile_file", "nombre de archivo de tejas", 70, DISP_IN_GAME},	/*WC*/
	{ "traps",    "los símbolos usados para mostrar las trampas",
						MAXTCHARS+1, SET_IN_FILE },
	{ "vary_msgcount", "mostrar más mensajes viejos juntos", 20, DISP_IN_GAME }, /*WC*/
#ifdef MSDOS
	{ "video",    "manera de actualizar el video", 20, SET_IN_FILE },
#endif
#ifdef VIDEOSHADES
	{ "videocolors", "mapa de colores para rutinas internales de pantalla",
						40, DISP_IN_GAME },
	{ "videoshades", "matices de gris mostradas como negro/gris/blanco",
						32, DISP_IN_GAME },
#endif
#ifdef WIN32CON
	{"subkeyvalue", "override keystroke value", 7, SET_IN_FILE},
#endif
	{ "windowcolors",  "los colores de frente y fondo de ventanas",	/*WC*/
						80, DISP_IN_GAME },
	{ "windowtype", "sistema de ventanas usado", WINTYPELEN, DISP_IN_GAME },
	{ (char *)0, (char *)0, 0, 0 }
};
...
# endif
	...
	obj_descr[SLIME_MOLD].oc_name = "fruta";
...
{
#ifdef MICRO
	pline("\"%s\" puede ponerse sólo desde %s.", optname, configfile);
#else
	pline("%s puede ponerse sólo desde NETHACKOPTIONS o %s.", optname,
			configfile);
#endif
}
...
const char *opts;
{
	if (!initial) {
		...
		pline("Mala sintaxis: %s.  Entra \"?g\" para ayudo.", opts);
	    return;
	}
#ifdef MAC
	else return;
	...
	if(from_file)
	    raw_printf("Mala sintaxis en OPTIONS en %s: %s.", configfile, opts);
	else
	    raw_printf("Mala sintaxis en NETHACKOPTIONS: %s.", opts);
...
{
	pline("La opción %s no puede %snegarse.",
		optname,
		with_parameter ? "tener un valor y también " : "");
}
...
const char *optn;
{
	...
	if (rejectver) {
		if (!initial)
			No_puedes("apagar avisos de rasgos nuevos para versiónes futuras.");
		else {
			Sprintf(buf,
				"\n%s=%s Referencia invalida a una versión futura es desatendida",
				optn, op);
			badoption(buf);
		}
		return 0;
	...
	if (!initial) {
		...
		pline("Avisos de cambios de rasgos son apagados por rasgos de NetHack %s y anterior.",
			buf);
	}
	return 1;
}
...
int on_or_off;
{
	...
	if (on_or_off != 0) {
		...
		if (iflags.opt_booldup)
			impossible("iflags.opt_booldup ya encendido (grieta de memoria)");
		...
		if (iflags.opt_compdup)
			impossible("iflags.opt_compdup ya encendido (grieta de memoria)");
...
int bool_or_comp;	/* 0 == boolean option, 1 == compound */
{
	...
	if ((bool_or_comp == 0) && iflags.opt_booldup && initial && from_file) {
		...
		if (match_optname(opts, boolopt[i].name, 3, FALSE)) {
			...
			if (*optptr == 1) {
			    raw_printf(
				"\nAviso - opción Booleana especificada más que una vez: %s.\n",
					opts);
			        wait_synch();
			}
			*optptr += 1;
		...
		if (match_optname(opts, compopt[i].name, strlen(compopt[i].name), TRUE)) {
			...
			if (*optptr == 1) {
			    raw_printf(
				"\nWarning - opción compuesta especificada más que una vez: %s.\n",
					compopt[i].name);
			        wait_synch();
			}
			*optptr += 1;
...
boolean tinitial, tfrom_file;
{
	...
	if (strlen(opts) > BUFSZ/2) {
		badoption("opción demasiado larga");
		return;
	}
	...
	if (match_optname(opts, "female", 3, FALSE)) {
		if(!initial && flags.female == negated)
			pline("Esto no es anatómicamente posible.");
		else
	...
	if (match_optname(opts, fullname, 3, TRUE)) {
		if ((op = string_for_env_opt(fullname, opts, negated)) != 0) {
			...
			case 'N':
			    ...
			    break;
			default:
			    pline("Tipo de mascota desconocido '%s'.", op);
			    break;
		    }
...
# endif
	...
	if (match_optname(opts, "fruit", 2, TRUE)) {
		...
		if (!initial) {
		    ...
		    if (num >= 100) {
			pline("Hacer esto tantas veces no es muy fructuoso.");
			return;
		    }
		}
goodfruit:
	...
	/* OBJ_NAME(objects[SLIME_MOLD]) won't work after initialization */
		if (!*pl_fruit)
		    nmcpy(pl_fruit, "moho mucoso", PL_FSIZ);
		if (!initial)
		    (void)fruitadd(pl_fruit);
...
/*		if (!(opts = string_for_env_opt(fullname, opts, FALSE))) */
		...
		if (clash) {
			...
			pline(
		  "Mala opción - símbolo de roca enorme '%c' tiene conflicto con un símbolo de %s.",
				opts[0], (clash == 1) ? "monstruo" : "aviso");
		} else {
			/*
	...
	if (match_optname(opts, "pickup_types", 8, TRUE)) {
		...
		if (!op) {
			...
			use_menu = FALSE;
			Sprintf(qbuf, "Nuevos pickup_types: [%s am] (%s)",
				ocl, *tbuf ? tbuf : "todos");
			getlin(qbuf, abuf);
			op = mungspaces(abuf);
		    ...
		    if (use_menu) {
			(void) choose_classes_menu("¿Auto-Pickup cuáles?", 1,
						   TRUE, ocl, tbuf);
			op = tbuf;
		    }
		}
	...
	if (match_optname(opts, "disclose", 7, TRUE)) {
		...
		||  !strcmpi(op, "todo") || !strcmpi(op, "nada")
		||  !strcmpi(op, "ninguno")) {
			if (op && op[0] == 'n') negated = TRUE;
		...
		while (*op && num < sizeof flags.end_disclose - 1) {
			...
			if (dop) {
				...
				if (idx < 0 || idx > NUM_DISCLOSURE_OPTIONS - 1) {
				    impossible("mala indice de revelación %d %c",
							idx, c);
				    continue;
				}
				if (prefix_val != -1) {
...
# endif
	...
	fullname = "accents";
	if (match_optname(opts, fullname, sizeof("accents")-1, TRUE)) {
		op = string_for_opt(opts, TRUE);
		/* For backward compatibility, !accents is mapped to 0
		   and accents without a parameter is mapped to 2 */
		if (negated)
			iflags.accents = 0;
		else if (!op)
			iflags.accents = 2;
		else
			iflags.accents = atoi(op);
		need_redraw = TRUE;
		return;
	}
	...
	for (i = 0; boolopt[i].name; i++) {
		if (match_optname(opts, boolopt[i].name, 3, FALSE)) {
			...
			if (!boolopt[i].addr) {
			    if (!initial && !negated)
				pline("La opción \"%s\" no está disponible.",
					boolopt[i].name);
			    return;
			}
...
# endif
			...
			if ((boolopt[i].addr) == &iflags.unicode) {
			    if (!initial && Is_rogue_level(&u.uz)) {
				assign_rogue_graphics(FALSE);
				assign_rogue_graphics(TRUE);
			    }
    ...
    char *src, *dest;
{
	...
	if (i < 0 || i >= MAXOCLASSES)
	    impossible("oc_to_str:  clase ilegal de objeto %d", i);
	else
	    *dest++ = def_oc_syms[i];
    }
    *dest = '\0';
    ...
    char from_ch, to_ch;
{
	...
	pline("no hay espacio para mapa de menú.");
    else {
...
doset()
{
 ...
 add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
		 "Booleanos (elegir cambiará el valor):", MENU_UNSELECTED);
	any.a_int = 0;
 ...
 add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
		 "Compuestas (elegir apuntará para valor nuevo):",
		 MENU_UNSELECTED);
...
#ifdef AUTOPICKUP_EXCEPTIONS
	any.a_int = -1;
	Sprintf(buf, "excepciones de autopickup (%d actualmente en uso)",
	...
	add_menu(tmpwin, NO_GLYPH, &any, 0, 0, ATR_NONE, "", MENU_UNSELECTED);
	add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
		 "Lugares de campo variables:", MENU_UNSELECTED);
	for (i = 0; i < PREFIX_COUNT; i++)
		doset_add_menu(tmpwin, fqn_prefix_names[i], 0);
#endif
	end_menu(tmpwin, "¿Ajustar qué opciones?");
	need_redraw = FALSE;
	if ((pick_cnt = select_menu(tmpwin, PICK_ANY, &pick_list)) > 0) {
		...
		if (opt_indx < boolcount) {
		    ...
		    if (!special_handling(compopt[opt_indx].name,
							setinitial, fromfile)) {
			Sprintf(buf, "¿Ajustar %s a qué?", compopt[opt_indx].name);
			getlin(buf, buf2);
			if (buf2[0] == '\033')
			    continue;
...
boolean setinitial,setfromfile;
{
	...
	end_menu(tmpwin, "Elige menustyle:");
	...
	end_menu(tmpwin, "Elige nivel de estorbancia:");
    ...
    end_menu(tmpwin, "Cambiar qué opciones de revelación:");
	...
	for (i = 0; i < NUM_DISCLOSURE_OPTIONS; i++) {
		...
		Sprintf(dbuf, "Opciones de revelación para %s:", disclosure_names[i]);
	        tmpwin = create_nhwindow(NHW_MENU);
		start_menu(tmpwin);
		any.a_char = DISCLOSE_NO_WITHOUT_PROMPT;
		add_menu(tmpwin, NO_GLYPH, &any, 'a', 0,
			ATR_NONE,"Nunca revelar y no apuntar", MENU_UNSELECTED);
		any.a_void = 0;
		any.a_char = DISCLOSE_YES_WITHOUT_PROMPT;
		add_menu(tmpwin, NO_GLYPH, &any, 'b', 0,
			ATR_NONE,"Siempre revelar y no apuntar", MENU_UNSELECTED);
		any.a_void = 0;
		any.a_char = DISCLOSE_PROMPT_DEFAULT_NO;
		add_menu(tmpwin, NO_GLYPH, &any, 'c', 0,
			ATR_NONE,"Apuntar y usar \"No\" si no hay respuesta", MENU_UNSELECTED);
		any.a_void = 0;
		any.a_char = DISCLOSE_PROMPT_DEFAULT_YES;
		add_menu(tmpwin, NO_GLYPH, &any, 'd', 0,
			ATR_NONE,"Apuntar y usar \"Sí\" si no hay respuesta", MENU_UNSELECTED);
	...
	end_menu(tmpwin, "Elige modo de mostrar de correr/caminar:");
	...
	add_menu(tmpwin, NO_GLYPH, &any, 's', 0,
		ATR_NONE, "sencillo", MENU_UNSELECTED);
	any.a_char = 'c';
	add_menu(tmpwin, NO_GLYPH, &any, 'c', 0,
		ATR_NONE, "combinación", MENU_UNSELECTED);
	any.a_char = 'f';
	add_menu(tmpwin, NO_GLYPH, &any, 'f', 0,
		ATR_NONE, "pleno", MENU_UNSELECTED);
	any.a_char = 'r';
	add_menu(tmpwin, NO_GLYPH, &any, 'r', 0,
		ATR_NONE, "al revés", MENU_UNSELECTED);
	end_menu(tmpwin, "Elige tipo de mostrar historia de mensajes:");
	...
	add_menu(tmpwin, NO_GLYPH, &any, 't', 0,
		ATR_NONE, "arriba", MENU_UNSELECTED);
	any.a_int = ALIGN_BOTTOM;
	add_menu(tmpwin, NO_GLYPH, &any, 'b', 0,
		ATR_NONE, "abajo", MENU_UNSELECTED);
	any.a_int = ALIGN_LEFT;
	add_menu(tmpwin, NO_GLYPH, &any, 'l', 0,
		ATR_NONE, "izquierdo", MENU_UNSELECTED);
	any.a_int = ALIGN_RIGHT;
	add_menu(tmpwin, NO_GLYPH, &any, 'r', 0,
		ATR_NONE, "derecho", MENU_UNSELECTED);
	Sprintf(abuf, "Elige posición de la ventana de %s relativa al mapa:",
		msg ? "mensaje" : "estado");
	...
	static const char *npchoices[3] =
		{"0 (apagado)", "1 (encendido)", "2 (encendido, compatible con DOS)"};
	...
	end_menu(tmpwin, "Elige modo de number_pad:");
	if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
	...
	static const char *action_titles[] = {
		"a", "añadir nueva excepción de autopickup",
		"m", "mostrar excepciones de autopickup",
		"q", "quitar excepción actual de autopickup",
...
ape_again:
	...
	if (opt_idx == 0) {	/* add new */
		/* LENGUA:  Need translation for pattern */
		getlin("¿Qué nuevo [pattern] de excepción de autopickup?", &apebuf[1]);
		...
		for (pass = AP_LEAVE; pass <= AP_GRAB; ++pass) {
		    ...
		    any.a_void = 0;
		    add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
				(pass == 0) ? "Nunca recoger" : "Siempre recoger",
				MENU_UNSELECTED);
		...
		Sprintf(apebuf, "%s excepciones de autopickup",
			(opt_idx == 1) ? "Lista de" : "Quitar cuál");
...
char *buf;
{
	...
	buf[0] = '\0';
	if (!strcmp(optname,"accents"))
		Sprintf(buf, "%d", iflags.accents);
	else if (!strcmp(optname,"align_message"))
		...
		if (flags.female)
		    Sprintf(buf, "%s", rolestring(flags.initalign, aligns, adj_f));
		else
		...
		Sprintf(buf, "%s",
			(!iflags.num_pad) ? "0=apagado" :
			(iflags.num_pad_mode) ? "2=encendido, compatible con DOS" : "1=encendido");
...
dotogglepickup()
{
	...
	if (flags.pickup) {
	    oc_to_str(flags.pickup_types, ocl);
	    if (ocl[0])
		Sprintf(buf, "ENCENDIDO, para los objetos %s", ocl);
	    else
		Sprintf(buf, "ENCENDIDO, para todos los objetos");
#ifdef AUTOPICKUP_EXCEPTIONS
	    if (iflags.autopickup_exceptions[AP_LEAVE] ||
		iflags.autopickup_exceptions[AP_GRAB])
		Strcat(buf,
			 ((count_ape_maps((int *)0, (int *)0) == 1) ?
			    ", con una excepción" : ", con unas excepciones"));
#endif
	} else {
	    Strcpy(buf, "APAGADO");
	}
...
const char *mapping;
{
	...
	if (sscanf(mapping, "\"%255[^\"]\"", text) == 1) {
		...
		*apehead = ape;
	    ...
	    raw_print("error de sintaxis en AUTOPICKUP_EXCEPTION");
	    return 0;
	}
	return 1;
}
...
static const char *opt_intro[] = {
	"",
	"             Ayuda de Opciones de NetHack:",
	"",
#define CONFIG_SLOT 3	/* fill in next value at run-time */
	(char *)0,
#if !defined(MICRO) && !defined(MAC)
	"o usa `NETHACKOPTIONS=\"<opciones>\"' en tu ambiente",
#endif
	"(<opciones> es una lista de opciones separadas por comas)",
#ifdef VMS
	"-- por ejemplo, $ DEFINE NETHACKOPTIONS \"noautopickup,fruit:kumquat\"",
#endif
	"o prensa \"O\" mientras jugando y usa el menú.",
	"",
 "Opciones booleanas (que pueden negarse prefijándolas con '!' o \"no\"):",
	(char *)0
};

static const char *opt_epilog[] = {
	"",
 "Algunas opciones pueden ponerse sólo antes de empezar el juego; esas",
	"opciones no serán elegibles en el menú del mando 'O'.",
	(char *)0
};
...
option_help()
{
	...
	Sprintf(buf, "Ajustar opciones como OPTIONS=<opciones> en %s", configfile);
    ...
    /* Compound options */
    putstr(datawin, 0, "Opciones compuestas:");
    for (i = 0; compopt[i].name; i++) {
...
char *str;
{
	...
	if (user_specified) {
		...
		/* LENGUA:  Ugh. */
		if (found || numeric ||
		    !strncmp(str, "maldecido ", 10) ||
		    !strncmp(str, "maldecida ", 10) ||
		    !strncmp(str, "maldecidos ", 11) ||
		    !strncmp(str, "maldecidas ", 11) ||
		    !strncmp(str, "maldito ", 8) ||
		    !strncmp(str, "maldita ", 8) ||
		    !strncmp(str, "malditos ", 9) ||
		    !strncmp(str, "malditas ", 9) ||
		    !strncmp(str, "inmaldecido ", 12) ||
		    !strncmp(str, "inmaldecida ", 12) ||
		    !strncmp(str, "inmaldecidos ", 13) ||
		    !strncmp(str, "inmaldecidas ", 13) ||
		    !strncmp(str, "inmaldito ", 10) ||
		    !strncmp(str, "inmaldita ", 10) ||
		    !strncmp(str, "inmalditos ", 11) ||
		    !strncmp(str, "inmalditas ", 11) ||
		    !strncmp(str, "bendecido ", 10) ||
		    !strncmp(str, "bendecida ", 10) ||
		    !strncmp(str, "bendecidos ", 11) ||
		    !strncmp(str, "bendecidas ", 11) ||
		    !strncmp(str, "bendito ", 8) ||
		    !strncmp(str, "bendita ", 8) ||
		    !strncmp(str, "benditos ", 9) ||
		    !strncmp(str, "benditas ", 9) ||
		    !strcmp(eos(str)-16, " comido en parte") ||
		    !strcmp(eos(str)-16, " comida en parte") ||
		    !strcmp(eos(str)-17, " comidos en parte") ||
		    !strcmp(eos(str)-17, " comidas en parte") ||
		    (!strncmp(str, "lata de ", 8) &&
			(!strcmp(str+7, "espinaca") ||
			 name_to_mon(str+7) >= LOW_PM)) ||
		    !strcmp(str, "lata vacia") ||
		    !strcmp(str, "lata vacía") ||
		    !strncmp(str, "cadaver de ", 11) ||
		    !strncmp(str, "cadáver de ", 11) ||
		    !strncmp(str, "huevo de ", 9) ||
		    name_to_mon(str) >= LOW_PM)
			{
				Strcpy(buf, pl_fruit);
				Strcpy(pl_fruit, isfeminine(pl_fruit)? "garapiñada " : "garapiñado ");
...
char *class_select;
{
	...
	switch (category) {
		...
		case 1:
			...
			break;
		default:
			impossible("choose_classes_menu: categoría invalida %d",
					category);
	}
...
int status;
{
	...
	if (status < SET_IN_FILE || status > SET_IN_GAME) {
		impossible("set_option_mod_status: estado inválido %d.",
			   status);
		return;
	}
...
int status;
{
	int k = 0;
	if (status < SET_IN_FILE || status > SET_IN_GAME) {
		impossible("set_wc_option_mod_status: estado inválido %d.",
			   status);
		return;
	}
...
int status;
{
	int k = 0;
	if (status < SET_IN_FILE || status > SET_IN_GAME) {
		impossible("set_wc2_option_mod_status: estado inválido %d.",
			   status);
		return;
	}
