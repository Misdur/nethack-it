...
boolean quietly;
{
	...
	for (otmp = invent; otmp; otmp = otmp2) {
		...
		if (!otmp->in_use) {
		    impossible("inven_inuse: !in_use oro en inventorio");
		}
...
#endif /* GOLDOBJ */
 		if (!quietly) pline("Acabando %s...", xname(otmp));
		useup(otmp);
	    }
	}
}
...
boolean ghostly, frozen;
{
	...
	while(1) {
		...
		if (otmp->oartifact) {
		    /* Artifacts are saved under the English name.
		       Substitute the Spanish one here.  Can't use oname
		       because it trips over the oartifact flag. */
		    const char *es_name = artiname(otmp->oartifact);

		    otmp->onamelth = (uchar)(strlen(es_name) + 1);
		    xl = otmp->onamelth + otmp->oxlth;
		    otmp = (struct obj *)realloc(otmp,
			    (unsigned)(xl) + sizeof(struct obj));
		    Strcpy(ONAME(otmp), es_name);
		}
		if(!first) first = otmp;
	...
	if(first && otmp2->nobj){
		impossible("Restobjchn: error leyendo objchn.");
		otmp2->nobj = 0;
	}
...
boolean ghostly;
{
	...
	while(1) {
		...
		if (mtmp->mw) {
			...
			if (obj) mtmp->mw = obj;
				...
				MON_NOWEP(mtmp);
				impossible("mala restauración de arma de monstruo");
			}
		}
	...
	if(first && mtmp2->nmon){
		impossible("Restmonchn: error leyendo monchn.");
		mtmp2->nmon = 0;
	}
	return(first);
}
...
register struct obj *otmp;
{
	...
	if (!oldf) impossible("¿no fruta vieja?");
...
unsigned int *stuckid, *steedid;	/* STEED */
{
	...
	if (uid != getuid()) {		/* strange ... */
	    ...
	    pline("Juego guardado no era tuyo.");
#ifdef WIZARD
	    if (!wizard)
#endif
		return FALSE;
	...
	if(u.uhp <= 0 && (!Upolyd || u.mh <= 0)) {
	    ...
	    pline("No eras bastante san%c para sobrevivir la restauración.",
		    poly_gender()==1? 'a' : 'o');
...
unsigned int stuckid, steedid;	/* STEED */
{
	...
	if (stuckid) {
		...
		if (!mtmp) panic("No puedo encontrar el monstruo ustuck.");
		u.ustuck = mtmp;
	}
#ifdef STEED
	if (steedid) {
		...
		if (!mtmp) panic("No puedo encontrar el monstruo usteed.");
		u.usteed = mtmp;
...
#endif
{
	...
	if (!savelev(nfd, ltmp, COUNT_SAVE)) {
		 ...
		 */
		pline("No hay bastante espacio en `%s' para restaurar tu juego.",
			levels);
...
# endif
	...
	pline("Vuelves a nivel %d en %s%s.",
		depth(&u.uz), dungeons[u.uz.dnum].dname,
		flags.debug ? " mientras en la moda de debug" :
		flags.explore ? " mientras en la moda de explorar" : "");
	curs(WIN_MAP, 1, 1);
	dotcnt = 0;
	dotrow = 2;
	if (strncmpi("X11", windowprocs.name, 3))
    	  putstr(WIN_MAP, 0, "Restaurando:");
#endif
	while(1) {
#ifdef ZEROCOMP
...
char *reason;
{
	pline("Extraño, este mapa no es como lo recuerdo.");
	pline("Alguien intenta engaños aquí...");
	pline("Este juego es nulo.");
	killer = reason;
	done(TRICKED);
}
...
/* CHECK:  This may prevent restoration */
	...
	if ((pid && pid != hpid) || (lev && dlvl != lev)) {
	    ...
	    if (pid && pid != hpid)
		Sprintf(trickbuf, "¡PID (%d) no iguala PID guardada (%d)!",
			hpid, pid);
	    else
		Sprintf(trickbuf, "¡Esto es nivel %d, no %d!", dlvl, lev);
#ifdef WIZARD
	...
	if (ghostly) {
		...
		case BR_PORTAL: /* max of 1 portal per level */
			...
			for(ttmp = ftrap; ttmp; ttmp = ttmp->ntrap)
				...
				break;
			if (!ttmp) panic("getlev: se necesita portal pero ninguno encontrado");
...
register unsigned len;
{
    ...
    if (fd < 0) error("Error de restaurar; mread tratando de leer archivo %d.", fd);
    mreadfd = fd;
    while (len--) {
	if (inrunlength > 0) {
	    inrunlength--;
...
#endif
		pline("Leyó %d en vez de %u bytes.", rlen, len);
		if(restoring) {
			(void) close(fd);
			(void) delete_savefile();
			error("Error de restaurar juego viejo.");
		}
		panic("Error de leer archivo de nivel.");
	}
}
#endif /* ZEROCOMP */
