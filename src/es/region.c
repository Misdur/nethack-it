...
genericptr_t p2;
{
	...
	if (!Blind)
		pline("Te chocas con %s. ¡Ouch!",
		    Hallucination ? "un árbol invisible" :
			"algún tipo de pared invisible");
	else
	    pline("¡Ouch!");
    } else {
	mtmp = (struct monst *) p2;
	if (canseemon(mtmp))
	    pline("¡%s se choca con algo!", Monnam(mtmp));
    }
    return FALSE;
}
...
genericptr_t p2;
{
	...
	if (!Poison_resistance) {
	    pline("¡Algo quema tus %s!", makeplural(body_part(LUNG)));
	    pline("¡Toses y expectoras sangre!");
	    losehp(rnd(dam) + 5, "nube de gas", KILLED_BY_AN);
	    return FALSE;
	} else {
	    pline("¡Toses!");
	    return FALSE;
	...
	if (!nonliving(mtmp->data) && !breathless(mtmp->data)) {
		...
		if (heros_fault(reg))
		    ...
		    monkilled(mtmp, "nube de gas", AD_DRST);
