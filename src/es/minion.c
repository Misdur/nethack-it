...
boolean talk;
{
	...
	case A_NONE:
	    ...
	    break;
	default:
	    impossible("¿jugador sin alineación?");
	    mnum = ndemon(A_NONE);
	    break;
    }
    if (mnum == NON_PM) {
    ...
    if (mon) {
	if (talk) {
	    pline("La voz de %s resuena:", align_gname(alignment));
	    verbalize("¡Pagaréis por vuestra indiscreción!");
	    if (!Blind)
		pline("%s aparece al frente de ti.", Amonnam(mon));
	}
	mon->mpeaceful = FALSE;
...
register struct monst *mtmp;
{
	...
	if (uwep && uwep->oartifact == ART_EXCALIBUR) {
	    pline("%s se ve muy enojad%c.", Amonnam(mtmp), mon_gender(mtmp)? 'a' : 'o');
	...
	if (is_dprince(mtmp->data) && mtmp->minvis) {
	    ...
	    if (!Blind) pline("%s aparece al frente de ti.", Amonnam(mtmp));
	    newsym(mtmp->mx,mtmp->my);
	}
	if (youmonst.data->mlet == S_DEMON) {	/* Won't blackmail their own. */
	    pline("%s dice, \"Buena caza, Herman%c.\"",
		  Amonnam(mtmp), flags.female ? 'a' : 'o');
	...
	if (!demand) {		/* you have no gold */
	    ...
	    pline("%s demanda %ld %s por pasaje seguro.",
		  Amonnam(mtmp), demand, currency(demand));

	    if ((offer = bribe(mtmp)) >= demand) {
		pline("%s se desvanece, riendo de mortales cobardes.",
		      Amonnam(mtmp));
	    } else if (offer > 0L && (long)rnd(40) > (demand - offer)) {
		pline("%s te frunce el ceño con menaza, luego se desvanece.",
		      Amonnam(mtmp));
	    } else {
...
struct monst *mtmp;
{
	...
	getlin("¿Cuánto quieres ofrecer?", buf);
	if (sscanf(buf, "%ld", &offer) != 1) offer = 0L;

	/*Michael Paddon -- fix for negative offer to monster*/
	/*JAR880815 - */
	if (offer < 0L) {
		pline("Tratas de %s, pero chapuceas.",
			mon_nam_acc(mtmp, "defraudar"));
		return 0L;
	} else if (offer == 0L) {
		pline("Rehusas.");
		return 0L;
		...
		pline("Entregas %s todo tu oro.", al(mon_nam(mtmp)));
		offer = u.ugold;
	} else {
		pline("Entregas %s %ld %s.",
			al(mon_nam(mtmp)), offer, currency(offer));
	}
	u.ugold -= offer;
	mtmp->mgold += offer;
#else
