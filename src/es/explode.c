...
int expltype;
{
	...
	switch (abs(type) % 10) {
		case 0: str = "estallido mágico";
			adtyp = AD_MAGM;
			break;
		case 1: str =   olet == BURNING_OIL ?	"aceite hirviente" :
				olet == SCROLL_CLASS ?	"torre de llama" :
							"bola de fuego";
			adtyp = AD_FIRE;
			break;
		case 2: str = "bola de frío";
			adtyp = AD_COLD;
			break;
		case 4: str =  (olet == WAND_CLASS) ? "campo de la muerte" :
							"campo de desintegración";
			adtyp = AD_DISN;
			break;
		case 5: str = "bola de relámpago";
			adtyp = AD_ELEC;
			break;
		case 6: str = "nube de gas venenoso";
			adtyp = AD_DRST;
			break;
		case 7: str = "salpicadura de ácido";
			adtyp = AD_ACID;
			break;
		default: impossible("¿Tipo básico de explosión %d?", type); return;
	}
	...
	for (i=0; i<3; i++) for (j=0; j<3; j++) {
		...
		if (i+x-1 == u.ux && j+y-1 == u.uy) {
			...
			case AD_ACID:
				...
				break;
			default:
				impossible("¿tipo de explosión %d?", adtyp);
				break;
		    }
		}
	...
	for (i=0; i<3; i++) for (j=0; j<3; j++) {
		...
		if (u.uswallow && mtmp == u.ustuck) {
			char gen = mon_gender(u.ustuck)? 'a' : 'o';
			if (is_animal(u.ustuck->data)) {
				if (adtyp == AD_DISN)
					pline((olet == WAND_CLASS)?
					      "¡%s es irradiad%c por energía pura!" :
					      "¡%s es perforad%c!",
					      Monnam(u.ustuck), gen);
				else if (adtyp == AD_DRST)
					pline("¡%s es envenenad%c!",
					      Monnam(u.ustuck), gen);
				else
					pline("¡%s %s!",
					      Monnam(u.ustuck),
					      (adtyp == AD_FIRE) ? "se quema" :
					      (adtyp == AD_COLD) ? "se hiela" :
					      (adtyp == AD_ELEC) ? "se electriza" :
					      (adtyp == AD_ACID) ? "se indigesta" :
					       "se fríe");
			}
			else {
				if (adtyp == AD_DISN)
					pline((olet == WAND_CLASS)?
					      "¡%s es abrumad%c un poco por energía pura!" :
					      "¡%s es perforad%c un poco!",
					      Monnam(u.ustuck), gen);
				else
					pline("¡%s %s un poco!",
					      Monnam(u.ustuck),
					      (adtyp == AD_FIRE) ? "se tuesta" :
					      (adtyp == AD_COLD) ? "se hiela" : /*o 'se congela'*/
					      (adtyp == AD_ELEC) ? "se electriza" :
					      (adtyp == AD_DRST) ? "se intoxica" :
					      (adtyp == AD_ACID) ? "se quema" :
					       "se fríe");
			}
		} else if (cansee(i+x-1, j+y-1)) {
		    if(mtmp->m_ap_type) seemimic(mtmp);
		    pline("¡%s es asid%c en %s %s!", Monnam(mtmp),
			mon_gender(mtmp)? 'a' : 'o',
			isfeminine(str)? "la" : "el", str);
		}
		 ...
		 */
			...
			if (resist(mtmp, olet, 0, FALSE)) {
				...
				pline("¡%s resiste %s!", Monnam(mtmp), the(str));
			    mdam = dam/2;
			}
			if (mtmp == u.ustuck)
				mdam *= 2;
	...
	if (uhurt) {
		if ((type >= 0 || adtyp == AD_PHYS) &&	/* gas spores */
			...
			pline("¡Eres atrapad%c por %s!",
			      poly_gender()==1? 'a' : 'o', the(str));
		/* do property damage first, in case we end up leaving bones */
		if (adtyp == AD_FIRE) burn_away_slime();
		if (Invulnerable) {
		    damu = 0;
		    pline("¡No sufres ningún daño!");
		...
		if (u.uhp <= 0 || (Upolyd && u.mh <= 0)) {
			...
			if (olet == MON_EXPLODE) {
			    ...
			    Sprintf(killer_buf, "atrapad%c por su propi%c %s",
				poly_gender()==1? 'a' : 'o',
				isfeminine(str)? 'a' : 'o', str);
			} else if (!strncmpi(str,"torre de llama", 8) ||
				   !strncmpi(str,"bola de fuego", 13)) {
			    killer_format = KILLED_BY_AN;
			    Strcpy(killer_buf, str);
			} else {
	...
	if (shopdamage) {
		pay_for_damage(adtyp == AD_FIRE ? "quemar" :
			       adtyp == AD_COLD ? "estrellar" :
			       adtyp == AD_DISN ? "desintegrar" : "destruir",
			       FALSE);
	}
	...
	while ((otmp = individual_object ? obj : level.objects[sx][sy]) != 0) {
		...
		if (otmp->otyp == BOULDER) {
		    ...
		    if ((trap = t_at(sx,sy)) && trap->ttyp == STATUE_TRAP)
			    deltrap(trap);
		    pline("%s.", Tobjnam(otmp, "se desmenuza"));
