...
/* LENGUA: the article arrays are changed to support the feminine articles,
   and A_gush_of_water_hits is reduced to A_gush_of_water so that the object
   can be placed correctly */
#ifndef OVLB
STATIC_VAR const char * const a_your[3];
STATIC_VAR const char * const un_tu[2];
STATIC_VAR const char * const una_tu[2];
STATIC_VAR const char * const Un_Tu[2];
STATIC_VAR const char * const Una_Tu[2];
STATIC_VAR const char * const el_tu[2];
STATIC_VAR const char * const la_tu[2];
STATIC_VAR const char *A_gush_of_water;
STATIC_VAR const char * const blindgas[6];

#else

STATIC_VAR const char * const a_your[3] = { "un", "una", "tu" };
STATIC_VAR const char * const un_tu[2]  = { "un",  "tu" };
STATIC_VAR const char * const una_tu[2] = { "una", "tu" };
STATIC_VAR const char * const Un_Tu[2]  = { "Un",  "Tu" };
STATIC_VAR const char * const Una_Tu[2]  = { "Una",  "Tu" };
STATIC_VAR const char * const el_tu[2] = { "el", "tu" };
STATIC_VAR const char * const la_tu[2] = { "la", "tu" };
STATIC_VAR const char *A_gush_of_water = "¡Un chorro de agua";
/* LENGUA: The modified noun is "gas", masculine */
/* LENGUA: need a word for acrid */
STATIC_VAR const char * const blindgas[6] = 
	{"húmido", "sin olor", "pungitivo", "helante", "ácrido", "mordiente"};
...
#define burn_dmg(obj,descr) rust_dmg(obj, descr, 0, FALSE, victim)
	...
	case 0:
		...
		mat_idx = objects[item->otyp].oc_material;
	    	Sprintf(buf,"yelmo de %s", materialnm[mat_idx] );
	    }
	    if (!burn_dmg(item, item ? buf : "yelmo")) continue;
	    break;
	case 1:
...
struct monst *victim;
{
	static NEARDATA const char * const action[] = { "quema", "se oxida", "pudre", "corroe" };
	static NEARDATA const char * const msg[] =  { "quemad", "oxidad", "podrid", "corroíd" };
	...
	/* LENGUA: retained from 3.3.1 to support Spanish grammar */
	plural = (is_gloves(otmp) || is_boots(otmp)) &&
		!strstri(ostr, "par de ");	/* "pair of *s" is singular */
	fem = isfeminine(ostr);
	...
	if (!vulnerable) {
		...
		if (flags.verbose) {
		    if (victim == &youmonst)
			pline("Por alguna razón, tu%s %s no %s afectad%c%s.",
			      plural? "s" : "",
			      ostr,
			      vtense(ostr, "es"),
			      fem? 'a' : 'o',
			...
			pline("Por alguna razón, %s %s no %s afectad%c%s.",
			      the(ostr),
			      del(mon_nam(victim)),
			      vtense(ostr, "es"),
			      fem? 'a' : 'o',
	    ...
	    if (flags.verbose) {
		if (victim == &youmonst)
		    pline("Tu%s %s %s completamente %s%c%s.",
			plural ? "s" : "",
			ostr,
			vtense(ostr, Blind ? "siente" : "parece"),
			msg[type],
			fem ? 'a' : 'o',
			plural ? "s" : "");
		else if (vismon)
		    pline("%s %s %s completamente %s%c%s.",
			The(ostr), del(mon_nam(victim)),
			vtense(ostr, "parece"), msg[type],
			fem ? 'a' : 'o',
			plural ? "s" : "");
	    }
	}
...
struct monst *victim;
{
	static const char txt[] = "por el estrato de grasa!";
	...
	if (ostr) {
	    plu = isplural(ostr)? "s" : "";
	    fem = isfeminine(ostr)? 'a' : 'o';
	    if (victim == &youmonst)
		pline("¡Tu%s %s %s protegid%c%s %s",
			plu, ostr,
			vtense(ostr, "es"),
			fem, plu, txt);
	    else if (vismon)
		pline("¡%s %s %s protegid%c%s %s",
			The(ostr),
			del(mon_nam(victim)),
			vtense(ostr, "es"),
			fem, plu, txt);
...
boolean td;	/* td == TRUE : trap door or hole */
{
	...
	if(td) {
	    ...
	    if (!In_sokoban(&u.uz)) {
		if (t->ttyp == TRAPDOOR)
			pline("¡Una puerta trampa se abre bajo ti!");
		else 
			pline("¡Hay una abertura grande bajo ti!");
	    }
	} else pline("¡%s se abre bajo ti!", capitalize(surface(u.ux,u.uy)));
	...
	if (In_sokoban(&u.uz) && Can_fall_thru(&u.uz))
	   ...
	   || (Inhell && !u.uevent.invoked &&
	    ...
	    dont_fall = "No te caes.";
	} else if (youmonst.data->msize >= MZ_HUGE) {
	    dont_fall = "No cabes.";
	} else if (!next_to_u()) {
	    dont_fall = "¡Tu mascota te sacude lejos!";
	}
	if (dont_fall) {
	    pline(dont_fall);
		...
		pline("Se cierra la abertura bajo ti.");
	    }
	    return;
	}
	...
	if (!td)
	    Sprintf(msgbuf, "Se cierra la abertura en %s arriba de ti.",
		    ceiling(u.ux,u.uy));
...
int *fail_reason;
{
	...
	if ((x == u.ux && y == u.uy) || cause == ANIMATE_SPELL) {
	    const char *comes_to_life = nonliving(mon->data) ?
					"se mueve" : "se hace viva"; 
	    if (cause == ANIMATE_SPELL)
	    	pline("%s %s!", upstart(statuename),
	    		canspotmon(mon) ? comes_to_life : "desaparece");
	    else
		pline("¡La estatua %s!",
			canspotmon(mon) ? comes_to_life : "desaparece");
	    if (historic) {
		    Te_sientes("culpable de que la estatua histórica ya se ha ido.");
		    adjalign(-1);
	    }
	} else if (cause == ANIMATE_SHATTER)
	    pline("¡En vez de romper, la estatua de repente %s!",
		canspotmon(mon) ? "se hace viva" : "desaparece");
	else { /* cause == ANIMATE_NORMAL */
	    pline("Encuentras %s fingiéndose estatua.",
		canspotmon(mon) ? al(a_monnam(mon)) : something);
	    stop_occupation();
	}
...
unsigned trflags;
{
	...
	if (In_sokoban(&u.uz) &&
	     ...
	     */
	    const char *name = defsyms[trap_to_defsym(ttype)].explanation;
	    pline("¡Corrientes de aire te tiran en %s %s!",
	    	a_your[trap->madeby_u? 2 : isfeminine(name)], name);
	...
	switch(ttype) {
		...
		trap->once = 1;
		seetrap(trap);
		pline("¡Una flecha se te dispara a ti!");
...
#endif
		if (thitu(8, dmgval(otmp, &youmonst), otmp, "flecha")) {
		...
		trap->once = 1;
		seetrap(trap);
		pline("¡Un dardo pequeño se te dispara a ti!");
...
#endif
		if (thitu(7, dmgval(otmp, &youmonst), otmp, "dardo pequeño")) {
		    if (otmp->opoisoned)
			poisoned("dardo", A_CON, "daro pequeño", -10);
		    obfree(otmp, (struct obj *)0);
		} else {
		    place_object(otmp, u.ux, u.uy);
		    if (!Blind) otmp->dknown = 1;
	    ...
	    case ROCKTRAP:
		if (trap->once && trap->tseen && !rn2(15)) {
		    pline("¡Se abre una puerta trampa en %s, pero nada se cae!",
			  ceiling(u.ux,u.uy));
		    deltrap(trap);
		    newsym(u.ux,u.uy);
		} else {
		    ...
		    pline("¡Se abre una puerta trampa en %s y cae %s sobre tu %s!",
			  ceiling(u.ux,u.uy),
			  an(xname(otmp)),
			  body_part(HEAD));
			...
			if(is_metallic(uarmh)) {
			    pline("Por fortuna, llevas un yelmo duro.");
			    dmg = 2;
			} else if (flags.verbose) {
			    pline("Tu %s no te protege.", xname(uarmh));
			}
		    }
		    ...
		    losehp(dmg, "piedra cayente", KILLED_BY_AN);
		...
		if (Levitation || Flying) {
			...
			seetrap(trap);
			if (Hallucination)
				pline("Noticias una arruga en el linoleo.");
			else
				pline("Noticias un tablero desatado bajo ti.");
		    }
		} else {
		    seetrap(trap);
		    pline("Un tablero bajo ti rechina ruidosamente.");
		    wake_nearby();
		}
		break;
		...
		if(amorphous(youmonst.data) || is_whirly(youmonst.data) ||
		    ...
		    pline("%s trampa para osos se cierra sin daño través de ti.",
			    Una_Tu[trap->madeby_u]);
		    break;
		}
		if(
#ifdef STEED
		   ...
		   youmonst.data->msize <= MZ_SMALL) {
		    pline("%s trampa para osos se cierra sin daño sobre ti.",
			    Una_Tu[trap->madeby_u]);
		    break;
		}
		u.utrap = rn1(4, 4);
...
#ifdef STEED
		if (u.usteed)
		    pline("¡%s trampa para osos se cierra sobre %s %s!",
			Una_Tu[trap->madeby_u],
			the(mbodypart(u.usteed, FOOT)),
			del(mon_nam(u.usteed)));
		else
#endif
		{
		    pline("¡%s trampa para osos se cierra sobre tu %s!",
			    Una_Tu[trap->madeby_u], body_part(FOOT));
		    if(u.umonnum == PM_OWLBEAR || u.umonnum == PM_BUGBEAR)
			pline("¡Aullas de enojo!");
		}
		...
		if(Sleep_resistance || breathless(youmonst.data)) {
		    pline("¡Una nube de gas te ronde!");
		    break;
		}
		pline("¡Una nube de gas te hace dormir!");
		...
		if (u.umonnum == PM_IRON_GOLEM) {
		    ...
		    pline("%s te golpea!", A_gush_of_water);
		    pline("¡Estás cubrid%c de herrumbre!",
			  poly_gender()==1? 'a' : 'o');
		    if (Half_physical_damage) dam = (dam+1) / 2;
		    losehp(dam, "herrumbrarse", KILLED_BY);
		    break;
		...
		switch (rn2(5)) {
		    case 0:
			pline("%s te golpea %s!", A_gush_of_water,
				    the(body_part(HEAD)));
			(void) rust_dmg(uarmh, "yelmo", 1, TRUE, &youmonst);
			break;
		    case 1:
			{
			    const char *brazo = body_part(ARM);
			    pline("%s te golpea %s izquierd%c!", A_gush_of_water,
				    the(body_part(ARM)),
				    isfeminine(brazo)? 'a' : 'o');
			}
			if (rust_dmg(uarms, "escudo", 1, TRUE, &youmonst))
			    break;
			if (u.twoweap || (uwep && bimanual(uwep)))
			    erode_obj(u.twoweap ? uswapwep : uwep, FALSE, TRUE);
glovecheck:		(void) rust_dmg(uarmg, "guantes", 1, TRUE, &youmonst);
			...
			{
			    const char *brazo = body_part(ARM);
			    pline("%s te golpea %s derech%c!", A_gush_of_water,
				    the(body_part(ARM)),
				    isfeminine(brazo)? 'a' : 'o');
			}
			...
			if (uarmc)
			    ...
			    (void) rust_dmg(uarmu, "camisa", 1, TRUE, &youmonst);
#endif
		}
		update_inventory();
		break;
		...
		if (!In_sokoban(&u.uz) && is_clinger(youmonst.data)) {
		    if(trap->tseen) {
			pline("Ves %s hoyo%s bajo ti.",
			    un_tu[trap->madeby_u],
			    ttype == SPIKED_PIT ? " con púas" : "");
		    } else {
			pline("¡%s hoyo%s se abre bajo ti!",
			    Un_Tu[trap->madeby_u],
			    ttype == SPIKED_PIT ? " con púas" : "");
			pline("¡No te caes!");
		    }
		...
		if (!In_sokoban(&u.uz)) {
		    ...
		    if (u.usteed) {
			    ...
			    Sprintf(verbbuf,"Conduces %s",
				al(x_monnam(u.usteed,
					 u.usteed->mnamelth ? ARTICLE_NONE : ARTICLE_THE,
				 	 "pobrecito", SUPPRESS_SADDLE, FALSE)));
		    } else
#endif
		    Strcpy(verbbuf,"Te caes");
		    pline("¡%s a %s hoyo!", verbbuf, un_tu[trap->madeby_u]);
		}
		/* wumpus reference */
		if (Role_if(PM_RANGER) && !trap->madeby_u && !trap->once &&
			In_quest(&u.uz) && Is_qlocate(&u.uz)) {
		    pline("Por fortuna, tiene fondo de todo modos...");
		    trap->once = 1;
		}
		/* LENGUA:  this doesn't translate */
		/*
		...
		*/
		if (ttype == SPIKED_PIT) {
		    const char *predicament = "sobre una cama de púas agudas de hierro";
#ifdef STEED
		    if (u.usteed) {
			pline("¡%s se aterriza %s!",
...
#endif
		    pline("¡Te aterrizas %s!", predicament);
		}
		if (!Passes_walls)
		    u.utrap = rn1(6,2);
		u.utraptype = TT_PIT;
...
#endif
		if (ttype == SPIKED_PIT) {
		    losehp(rnd(10),"se cayó sobre un hoyo de púas agudas",
			NO_KILLER_PREFIX);
		    if (!rn2(6))
			poisoned("púas", A_STR, "caída sobre púas envenenadas", 8);
		} else
		    losehp(rnd(6),"se cayó en un hoyo", NO_KILLER_PREFIX);
		...
		if (!Can_fall_thru(&u.uz)) {
		    ...
		    impossible("dotrap: %ss no pueden existir en este nivel.",
		...
		if (amorphous(youmonst.data) || is_whirly(youmonst.data) ||
			...
			if (webmsgok)
			    pline("¡%s %s telaraña!",
				(u.umonnum == PM_FIRE_ELEMENTAL) ? "Quemas" : "Disuelves",
				una_tu[trap->madeby_u]);
			deltrap(trap);
			newsym(u.ux,u.uy);
			break;
		    }
		    if (webmsgok) pline("Flujas través de %s telaraña.",
			    una_tu[trap->madeby_u]);
		    break;
		}
		if (webmaker(youmonst.data)) {
		    if (webmsgok)
			pline(trap->madeby_u ? "Caminas sobre tu telaraña."
		...
		if (webmsgok) {
		    ...
		    Sprintf(verbbuf, "%s", Levitation ? (const char *)"Flotas" :
		      		locomotion(youmonst.data, "Tropezas"));
		    pline("¡%s en %s telaraña!",
			  verbbuf, una_tu[trap->madeby_u]);
		}
		u.utraptype = TT_WEB;
		...
		/* Time stuck in the web depends on your/steed strength. */
		{
			...
			u.utrap = 0;
			if (webmsgok)
			    pline("¡Rasgas a través de %s telaraña!", una_tu[trap->madeby_u]);
			deltrap(trap);
		...
		if (!rn2(30)) {
		    ...
		    pline("¡Estás asid%c en una explosión mágica!",
			  poly_gender()==1? 'a' : 'o');
		    losehp(rnd(10), "explosión mágica", KILLED_BY_AN);
		    pline("¡Tu cuerpo absorbe alguna energía mágica!");
		    u.uen = (u.uenmax += 2);
		} else domagictrap();
#ifdef STEED
		(void) steedintrap(trap, (struct obj *)0);
		...
		if(Antimagic) {
		    shieldeff(u.ux, u.uy);
		    Te_sientes("letárgic%c por un momento.",
			  poly_gender()==1? 'a' : 'o');
...
#endif
		 Sprintf(verbbuf,"%s",
		    Levitation ? (const char *)"Flotas" :
		    locomotion(youmonst.data, "Pisas"));
		pline("¡%s en una trampa de polimorfo!", verbbuf);
		if(Antimagic || Unchanging) {
		    shieldeff(u.ux, u.uy);
		    Te_sientes("diferente por un momento.");
		    ...
		    newsym(u.ux,u.uy);	/* get rid of trap symbol */
		    Sientes("un cambio alcanzándote.");
		    polyself(FALSE);
		}
		break;
	    }
		...
		if (Levitation || Flying) {
		    ...
		    seetrap(trap);
		    pline("%s %s en una pila de suelo bajo ti.",
			    already_seen ? "Hay" : "Descubres",
			    trap->madeby_u ? "el gatillo de tu mina" :
					     "un gatillo");
		    if (already_seen && rn2(3)) break;
		    if (already_seen) {
			pline("¡¡¡CAABLAMM!!!  ¡%s %smina de tierra!",
			      forcebungle ? "Tu atento inepto dispara" :
					    "Los correntes de aire disparan",
			      una_tu[trap->madeby_u]);
		    }
		    else {
			pline("¡¡¡CAABLAMM!!!  ¡%s!",
			      forcebungle ? "Tu atento inepto la dispara" :
					    "Los correntes de aire la disparan");
		    }
		} else {
#ifdef STEED
...
#endif
		    seetrap(trap);
		    pline("¡¡¡CAABLAMM!!!  ¡Has disparado %s mina de tierra!",
					    una_tu[trap->madeby_u]);
#ifdef STEED
		...
		losehp(rnd(16), "mina de tierra", KILLED_BY_AN);
		...
		seetrap(trap);
		pline("¡Clic! ¡Disparas una trampa de roca enorme girante!");
		if(!launch_obj(BOULDER, trap->launch.x, trap->launch.y,
		      trap->launch2.x, trap->launch2.y, style)) {
		    deltrap(trap);
		    newsym(u.ux,u.uy);	/* get rid of trap symbol */
		    pline("Por fortuna de ti, no se soltó ninguna roca enorme.");
		}
		break;
	    }
	    case MAGIC_PORTAL:
	    ...
	    default:
		seetrap(trap);
		impossible("Andas en una trampa de tipo %u", trap->ttyp);
	}
}
...
struct obj *otmp;
{
	...
	switch (tt) {
		case ARROW_TRAP:
			if(!otmp) {
				impossible("¿caballo golpeado por flecha que no existe?");
				return 0;
			}
		...
		case DART_TRAP:
			if(!otmp) {
				impossible("¿caballo golpeado por dardo que no existe?");
				return 0;
			}
		...
		case POLY_TRAP: 
			...
			if (!can_saddle(mtmp) || !can_ride(mtmp)) {
				...
				pline("Tienes que ajustarte en la silla %s.",
					del(x_monnam(mtmp,
...
int style;
{
	...
	while(dist-- > 0 && !used_up) {
		...
		if (style == ROLL) {
			...
			case LANDMINE:
			    if (rn2(10) > 2) {
				  ...
				  cansee(bhitpos.x, bhitpos.y) ?
					" La roca enorme rodante dispara una mina de tierra." : "");
				deltrap(t);
			...
			case TELEP_TRAP:
			    if (cansee(bhitpos.x, bhitpos.y))
				pline("¡De repente, la roca enorme rodante se desaparece!");
			    else
				Oyes("un retumbo acabando de repente.");
			    singleobj->otrapped = 0;
		    ...
		    if (flooreffects(singleobj, bhitpos.x, bhitpos.y, "cae")) {
			used_up = TRUE;
			break;
		    }
		    if (otyp == BOULDER &&
			...
			const char *bmsg =
				" como una roca enorme hace que otra se mueva";

			if (!isok(bhitpos.x + dx, bhitpos.y + dy) || !dist ||
			    IS_ROCK(levl[bhitpos.x + dx][bhitpos.y + dy].typ))
			    bmsg = " como una roca enorme pega otra";

		    	Oyes("una choca fuerte%s!",
		...
		if (otyp == BOULDER && closed_door(bhitpos.x,bhitpos.y)) {
			if (cansee(bhitpos.x, bhitpos.y))
				pline("La roca enorme se estrella través de una puerta.");
...
register struct monst *mtmp;
{
	...
	if (!trap) {
		...
		if (sobj_at(BOULDER, mtmp->mx, mtmp->my) &&
			...
			mtmp->mtrapped = 0;
			if (canseemon(mtmp))
			    pline("%s se tira libre...", Monnam(mtmp));
		...
		if (trap->ttyp == BEAR_TRAP) {
		    if (canseemon(mtmp))
			pline("¡%s come una trampa para osos!", Monnam(mtmp));
		    deltrap(trap);
		    mtmp->meating = 5;
		    mtmp->mtrapped = 0;
		} else if (trap->ttyp == SPIKED_PIT) {
		    if (canseemon(mtmp))
			pline("¡%s muerde algunas púas!", Monnam(mtmp));
		    trap->ttyp = PIT;
		    mtmp->meating = 5;
		}
	    }
		...
		case ARROW_TRAP:
			if (trap->once && trap->tseen && !rn2(15)) {
			    if (in_sight && see_it)
				pline("%s dispara una trampa pero no pasa nada.",
				      Monnam(mtmp));
			    deltrap(trap);
		...
		case ROCKTRAP:
			if (trap->once && trap->tseen && !rn2(15)) {
			    if (in_sight && see_it)
				pline("¡Se abre una puerta trampa arriba de %s, pero nada se cae!",
				      mon_nam(mtmp));
			    deltrap(trap);
			    newsym(mtmp->mx, mtmp->my);
			    break;
		...
		case SQKY_BOARD:
			...
			if (in_sight) {
			    pline("Un tablero bajo %s rechina ruidosamente.", del(mon_nam(mtmp)));
			    seetrap(trap);
			} else
			   Oyes("una chirra afuera.");
		...
		case BEAR_TRAP:
			if(mptr->msize > MZ_SMALL &&
			    ...
			    mtmp->mtrapped = 1;
			    if(in_sight) {
				pline("¡%s está asid%c en %s trampa para osos!",
				...
				if((mptr == &mons[PM_OWLBEAR]
				   ...
				   && flags.soundok)
				    pline("¡Oyes el gruño de un oso enojado!");
			    }
			}
			break;
		...
		case RUST_TRAP:
			...
			case 0:
			    if (in_sight)
				pline("%s %s en %s!", A_gush_of_water,
				    mon_nam_acc(mtmp, "golpea"), the(mbodypart(mtmp, HEAD)));
			    target = which_armor(mtmp, W_ARMH);
			    (void) rust_dmg(target, "yelmo", 1, TRUE, mtmp);
			    break;
			case 1:
			    if (in_sight) {
				const char *what = mbodypart(mtmp, ARM);
				pline("%s %s en %s izquierd%c!", A_gush_of_water,
				    mon_nam_acc(mtmp, "golpea"), the(what),
				    isfeminine(what)? 'a' : 'o');
			    }
			    target = which_armor(mtmp, W_ARMS);
			    if (rust_dmg(target, "escudo", 1, TRUE, mtmp))
				break;
			    target = MON_WEP(mtmp);
			    if (target && bimanual(target))
				erode_obj(target, FALSE, TRUE);
glovecheck:		    target = which_armor(mtmp, W_ARMG);
			    (void) rust_dmg(target, "guantes", 1, TRUE, mtmp);
			    break;
			case 2:
			    if (in_sight) {
				const char *what = mbodypart(mtmp, ARM);
				pline("%s %s en %s derech%c!", A_gush_of_water,
				    mon_nam_acc(mtmp, "golpea"), the(what),
				    isfeminine(what)? 'a' : 'o');
			    }
			...
			default:
				...
				if (target)
				    (void) rust_dmg(target, "armadura", 1, TRUE, mtmp);
#ifdef TOURIST
				else {
				    target = which_armor(mtmp, W_ARMU);
				    (void) rust_dmg(target, "camisa", 1, TRUE, mtmp);
				}
#endif
			    }
			}
			if (mptr == &mons[PM_IRON_GOLEM]) {
				if (in_sight)
				    pline("¡%s se cae en pedazos!", Monnam(mtmp));
				/* LENGUA: this pun doesn't translate */
				/*
				else if(mtmp->mtame)
 ...
 mfiretrap:
			if (in_sight)
			    pline("¡Una torre de %s arroja desde %s bajo %s!",
				  Hallucination ? "vicuñas" : "llamas",
				  surface(mtmp->mx,mtmp->my), mon_nam(mtmp));
			else if (see_it)  /* evidently `mtmp' is invisible */
			    pline("¡Ves una torre de %s arrojando desde %s!",
				Hallucination ? "vicuñas" : "llamas", surface(mtmp->mx,mtmp->my));

			if (resists_fire(mtmp)) {
			    if (in_sight) {
				shieldeff(mtmp->mx,mtmp->my);
				pline("%s no está dañad%c.", Monnam(mtmp),
					mon_gender(mtmp)? 'a' : 'o');
			    }
			} else {
			    int num = d(2,4), alt;
		...
		case SPIKED_PIT:
			...
			if (is_flyer(mptr) || is_floater(mptr) ||
			    ...
			    fallverb = "se arrastra";	/* sokoban pit */
			}
		...
		case TRAPDOOR:
			if (!Can_fall_thru(&u.uz)) {
			 impossible("mintrap: %ss no pueden exister en este nivel.",
			...
			if (is_flyer(mptr) || is_floater(mptr) ||
				...
				if (in_sight) {
				    pline("¡%s parece arrastrado abajo!",
					  Monnam(mtmp));
		...
		case WEB:
			...
			if (amorphous(mptr) || is_whirly(mptr) || unsolid(mptr)){
			    ...
			    if (in_sight) {
				pline("%s fluja través de %s telaraña.",
				      Monnam(mtmp),
				      una_tu[trap->madeby_u]);
				seetrap(trap);
			    }
			...
			switch (monsndx(mptr)) {
			    ...
			    case PM_BUGBEAR:
				if (!in_sight) {
				    Oyes("el gruño de un oso confundido!");
				    mtmp->mtrapped = 1;
				    break;
				}
				/* fall though */
			...
			if (tear_web) {
			    if (in_sight)
				pline("%s rasga través de %s telaraña!",
				      Monnam(mtmp), una_tu[trap->madeby_u]);
			    deltrap(trap);
			    newsym(mtmp->mx, mtmp->my);
			}
			break;
	...
	pline("Un gatillo aparece en una pila de suelo bajo %s.", del(mon_nam(mtmp)));
					seetrap(trap);
				}
				if (rn2(3)) break;
				if (in_sight) {
					newsym(mtmp->mx, mtmp->my);
					pline("¡Los corrientes de aire %s!",
					  already_seen ? "disparan una mina de tierra"
						       : "lo disparan");
				}
			} else if(in_sight) {
			    newsym(mtmp->mx, mtmp->my);
			    pline("¡¡¡CAABLAMM!!!  ¡%s dispara %s mina de tierra!",
				Monnam(mtmp), una_tu[trap->madeby_u]);
			}
			if (!in_sight)
				pline("¡Caablamm!  ¡Oyes una explosión en la distancia!");
			blow_up_landmine(trap);
			if (thitm(0, mtmp, (struct obj *)0, rnd(16), FALSE))
				trapkilled = TRUE;
			else {
			...
			if (unconscious()) {
				multi = -1;
				nomovemsg="¡La explosion te despierta!";
			}
			break;
		...
		case ROLLING_BOULDER_TRAP:
			...
			int style = ROLL | (in_sight ? 0 : LAUNCH_UNSEEN);
		        ...
		        newsym(mtmp->mx,mtmp->my);
			if (in_sight)
			    pline("¡Clic! %s dispara %s.", Monnam(mtmp),
				  trap->tseen ?
				  "una trampa de roca enorme girante" :
				  something);
			if (launch_obj(BOULDER, trap->launch.x, trap->launch.y,
				trap->launch2.x, trap->launch2.y, style)) {
		...
		default:
			impossible("Algún monstruo encontró una trampa de tipo extraño %d.", tt);
	    }
	}
...
boolean byplayer;
{
	...
	if (cansee(mon->mx, mon->my))
		pline("%s se vuelve piedra.", Monnam(mon));
	if (byplayer) {
		stoned = TRUE;
		xkilled(mon,0);
	} else monstone(mon);
...
const char *arg;
{
	...
	if(uwep && uwep->otyp == CORPSE && touch_petrifies(&mons[uwep->corpsenm])
			&& !Stone_resistance) {
		pline("%s tocas el cadáver de %s.", arg,
		        mons[uwep->corpsenm].mname);
		Sprintf(kbuf, "cadáver de %s", mons[uwep->corpsenm].mname);
		instapetrify(kbuf);
	}
	...
	if(u.twoweap && uswapwep && uswapwep->otyp == CORPSE &&
		...
		Sprintf(kbuf, "cadáver de %s", mons[uswapwep->corpsenm].mname);
		instapetrify(kbuf);
	}
}
...
boolean byplayer;
{
	...
	if (mwep && mwep->otyp == CORPSE && touch_petrifies(&mons[mwep->corpsenm])) {
		if (cansee(mon->mx, mon->my)) {
			pline("%s%s toca el cadáver de %s.",
...
float_up()
{
	if(u.utrap) {
		if(u.utraptype == TT_PIT) {
			u.utrap = 0;
			pline("¡Flotas arriba, fuera del hoyo!");
			vision_full_recalc = 1;	/* vision limits change */
			fill_pit(u.ux, u.uy);
		} else if (u.utraptype == TT_INFLOOR) {
			const char *pierna = body_part(LEG);
			pline("Tu cuerpo se tira arriba, pero tus %s ya están pegad%cs.",
			     makeplural(pierna), isfeminine(pierna)? 'a' : 'o');
		} else {
			const char *pierna = body_part(LEG);
			pline("Flotas arriba, pero tu %s ya está pegad%c.",
				pierna, isfeminine(pierna)? 'a' : 'o');
		}
	}
	else if(Is_waterlevel(&u.uz))
		Te_sientes("como si hubieras perdido algo de peso.");
	else if(u.uinwater)
		spoteffects(TRUE);
	else if(u.uswallow) {
		if (is_animal(u.ustuck->data))
		    pline("Flotas afuera %s.", del(surface(u.ux, u.uy)));
		else
		    pline("Giras arriba en %s.", mon_nam(u.ustuck));
	} else if (Hallucination)
		pline("¡Arriba y afuera!  ¡Estás andando en aire!");
	else if(Is_airlevel(&u.uz))
		pline("Puedes manejar tus movimientos.");
	else
		pline("¡Empiezas a flotar en el aire!");
#ifdef STEED
	if (u.usteed && !is_floater(u.usteed->data) &&
						!is_flyer(u.usteed->data)) {
	    if (Lev_at_will)
	    	pline("¡%s magicamente se flota!", Monnam(u.usteed));
	    else {
	    	pline("No puedes quedar sobre %s.", mon_nam(u.usteed));
...
int x, y;
{
	...
	if ((t = t_at(x, y)) &&
		...
		obj_extract_self(otmp);
		(void) flooreffects(otmp, x, y, "se cae");
	}
}
...
long hmask, emask;     /* might cancel timeout */
{
	...
	if(u.uswallow) {
	    pline("Flotas abajo, pero todavía estás %s%c.",
		is_animal(u.ustuck->data) ? "tragad" : "engolfad",
		poly_gender()==1? 'a' : 'o');
	    return(1);
	}
	...
	if(!Flying) {
		if (!u.uswallow && u.ustuck) {
			if (sticks(youmonst.data))
				No_puedes("mantener tu agarro sobre %s.",
					mon_nam(u.ustuck));
			else
				pline("¡Sorpresad%c, %s ya no puede detenerte!",
	...
	if (!trap) {
	    trap = t_at(u.ux,u.uy);
	    if(Is_airlevel(&u.uz))
		pline("Empiezas a voltear en lugar.");
	    else if (Is_waterlevel(&u.uz) && !no_msg)
		Te_sientes("más pesad%c.", poly_gender()==1? 'a' : 'o');
...
#endif
		{
			...
			pline("¡Caramba!  Has %s.",
			      is_pool(u.ux,u.uy) ?
				"chapoteado abajo" : sokoban_trap ? "chocado" :
				"golpeado la tierra");
		    else {
			if (!sokoban_trap)
			    pline("Flotas suavemente %s.",
			     ...
			     */
			    pline("Te caes.");
			    losehp(rnd(2), "vientos peligrosos", KILLED_BY);
#ifdef STEED
 ...
 */
	...
	if ((box && !carried(box)) ? is_pool(box->ox, box->oy) : Underwater) {
	    pline("¡Una cascada de burbujas de vapor arroja desde %s!",
		    box ? the(xname(box)) : surface(u.ux,u.uy));
	    if (Fire_resistance) pline("No te daña.");
	    else losehp(rnd(3), "agua herviente", KILLED_BY);
	    return;
	}
	pline("¡Una torre de %s %s desde %s!",
	      Hallucination ? "vicuñas" : "llamas",
	      box ? "se reventa" : "arroja",
	      box ? the(xname(box)) : surface(u.ux,u.uy));
	if (Fire_resistance) {
	    shieldeff(u.ux, u.uy);
	    num = rn2(2);
	...
	if (!num)
	    ...
	    losehp(num, "torre de llamas", KILLED_BY_AN);
	burn_away_slime();
...
domagictrap()
{
	...
	if (fate < 10) {
	  ...
	  if (!resists_blnd(&youmonst)) {
		pline("¡Te ciega una explosión de luz por un momento!");
		make_blinded((long)rn1(5,10),FALSE);
		if (!Blind) pline(vision_clears);
	  } else if (!Blind) {
		pline("¡Ves una explosión de luz!");
	  }  else
		Oyes("un gruño ensordeciente!");
	  while(cnt--)
	  ...
	  switch (fate) {
	     ...
	     /* odd feelings */
	     case 13:	pline("¡Un escalofrío corre través de tu %s!",
			      body_part(SPINE));
			break;
	     case 14:	Oyes(Hallucination ?
				"la luna aullándote." :
				"una aulla distante.");
			break;
	     case 15:	if (on_level(&u.uz, &qstart_level))
			    Te_sientes("%scomo el hijo prodigal.",
			      (flags.female || (Upolyd && is_neuter(youmonst.data))) ?
				     "extrañamente " : "");
			else
			    /* LENGUA:  English, "Cleveland" */
			    pline("Anhelas de repente %s.",
				Hallucination ? "Oaxaca" :
			    (In_quest(&u.uz) || at_dgn_entrance("La Misión")) ?
						"tu país cercano" :
						"tu país lejano");
			break;
	     case 16:   pline("¡Tu carga se menea violentemente!");
			break;
	     case 17:	pline(Hallucination ?
				"Hueles hamburguesas." :
				"Hueles carne requemada.");
			break;
	     case 18:	Te_sientes("cansad%c.", poly_gender()==1? 'a' : 'o');
			break;
...
xchar x, y;
{
	...
	if (Is_container(obj)) {
	    ...
	    if (in_sight) pline("%s se incienda y se quema.", Yname2(obj));
	    if (Has_contents(obj)) {
		if (in_sight) pline("Sus contenidos se vierten.");
		...
		if (in_sight) pline("Humo se levanta de %s.", the(xname(obj)));
		continue;
	    }
	    ...
	    if (in_sight) {
		pline("%s %s%s.", Yname2(obj), otense(obj, "se quema"),
		      obj->oeroded+1 == MAX_ERODE ? " completamente" :
		      obj->oeroded ? " más" : "");
	    }
	    obj->oeroded++;
	}
...
register boolean force, here;
{
	...
	for (; obj; obj = otmp) {
		...
		if(obj->otyp == CAN_OF_GREASE && obj->spe > 0) {
			...
			if (obj->otyp == SPE_BOOK_OF_THE_DEAD)
				pline("Vapor se levanta desde %s.", the(xname(obj)));
			else obj->otyp = SPE_BLANK_PAPER;
		} else if (obj->oclass == POTION_CLASS) {
			if (obj->otyp == POT_ACID) {
				/* damage player/monster? */
				pline("¡Una poción explota!");
				delobj(obj);
				continue;
...
boolean *lostsome;
{
	...
	while (near_capacity() > (Punished ? UNENCUMBERED : SLT_ENCUMBER)) {
		...
		if (u.ugold) {
		    pline("Por desesperación, haces caer tu billetera.");
...
drown()
{
	...
	if (!u.uinwater) {
	    boolean exclam = !(Amphibious || Swimming);
	    pline("%s%s en el agua%c",
		exclam ? "¡" : "",
		Is_waterlevel(&u.uz) ? "Te zambulles" : "Caes",
		exclam ? '!' : '.');
	    if (!Swimming && !Is_waterlevel(&u.uz))
		    pline("Te hundes como %s.",
			  Hallucination ? "la Titanic" : "una piedra");
	}
	...
	if (u.umonnum == PM_GREMLIN && rn2(3))
	    ...
	    losehp(i, "oxidarse", KILLED_BY);
	}
	if (inpool_ok) return(FALSE);

	if ((i = number_leashed()) > 0) {
		pline("La%s correa%s se resbala%s libre.",
			(i > 1) ? "s" : "",
			(i > 1) ? "s" : "",
			(i > 1) ? "n" : "");
		unleash_all();
	...
	if (Amphibious || Swimming) {
		if (Amphibious) {
			if (flags.verbose)
				pline("Pero no ahogas.");
			if (!Is_waterlevel(&u.uz)) {
				if (Hallucination)
					pline("Tu carina golpea el fondo.");
				else
					pline("Tocas el fondo.");
			}
		}
		if (Punished) {
			unplacebc();
	...
	if ((Teleportation || can_teleport(youmonst.data)) &&
		...
		pline("Atentas un hechizo de teleportar.");	/* utcsri!carroll */
		if (!level.flags.noteleport) {
			(void) dotele();
			if(!is_pool(u.ux,u.uy))
				return(TRUE);
		} else pline("El hechizo de teleportar atentado falla.");
	}
#ifdef STEED
	if (u.usteed) {
		dismount_steed(DISMOUNT_GENERIC);
	...
	if (u.usleep) unmul("¡De repente te despiertas!");
 ...
 crawl:
	if (crawl_ok) {
		...
		pline("Tratas de arrastrarte fuera del agua.");
		if (lost)
			pline("Dejas algunas cosas para perder peso...");
		if (succ) {
			pline("¡Feew!  Fue estrecho.");
			teleds(x,y,TRUE);
			return(TRUE);
		}
	...
	while (!safe_teleds(TRUE)) {
		pline("Todavía estás ahogando.");
		done(DROWNING);
	}
	if (u.uinwater) {
	    u.uinwater = 0;
	    pline("Te encuentras %s de nuevo.", Is_waterlevel(&u.uz) ?
		    "en una burbuja de aire" : "en tierra");
	}
	return(TRUE);
}
...
{
	if (!u.uenmax) return;
	Sientes("que tu energía mágica se escurre!");
	u.uen -= n;
	if(u.uen < 0)  {
		u.uenmax += u.uen;
...
dountrap()	/* disarm a trap */
{
	if (near_capacity() >= HVY_ENCUMBER) {
	    pline("Estás demasiado forzad%c para hacer eso.",
		poly_gender()==1? 'a' : 'o');
	    return 0;
	}
	if ((nohands(youmonst.data) && !webmaker(youmonst.data)) || !youmonst.data->mmove) {
	    pline("¿Y como esperas hacer eso?");
	    return 0;
	} else if (u.ustuck && sticks(youmonst.data)) {
	    pline("Tendrás que soltar %s primero.", al(mon_nam(u.ustuck)));
	    return 0;
	}
	if (u.ustuck || (welded(uwep) && bimanual(uwep))) {
	    const char *mano = body_part(HAND);
	    pline("Tus %s parecen demasiado ocupad%cs para eso.",
		 makeplural(mano), isfeminine(mano)? 'a' : 'o');
	    return 0;
	}
	return untrap(FALSE);
}
...
boolean force_failure;
{
	...
	if (!can_reach_floor()) {
		...
		if (u.usteed && P_SKILL(P_RIDING) < P_BASIC)
			pline("No tienes habilidad de alcanzar desde %s.",
				mon_nam(u.usteed));
		else
#endif
		No_puedes("alcanzar %s!",
			the(defsyms[trap_to_defsym(ttype)].explanation));
		return 0;
	}
	...
	if (force_failure || untrap_prob(ttmp)) {
		if (rnl(5)) {
			...
			if (ttype == BEAR_TRAP) {
				...
				if (ttmp2) {
				    pline("La telaraña te pega.  ¡Estás enredad%c también!",
					    poly_gender()==1? 'a' : 'o');
				...
				pline("%s se queda enredad%c.", Monnam(mtmp),
					mon_gender(mtmp)? 'a' : 'o');
			}
		    } else if (under_u) {
			dotrap(ttmp, 0);
		    ...
		    const char *trampa = defsyms[trap_to_defsym(ttype)].explanation;
		    pline("Es difícil %s %s %s.",
			  (ttype == WEB) ? "quitar" : "desarmar",
			  ttmp->madeby_u ? "tu"
                                         : feminize(under_u ? "este" : "ese",
                                                    isfeminine(trampa)),
			  trampa);
		}
		return 1;
...
struct monst *mtmp;
{
	if (!ttmp->madeby_u) {
		...
		pline("%s está grat%c.", Monnam(mtmp),
			mon_gender(mtmp)? 'a' : 'o');
	    }
		...
		adjalign(1);
		Te_sientes("que has hecho la cosa correcta.");
	    }
	}
}
...
struct trap *ttmp;
{
	...
	if ((mtmp = m_at(ttmp->tx,ttmp->ty)) != 0) {
		mtmp->mtrapped = 0;
		pline("Quitas %s %s %s.", la_tu[ttmp->madeby_u],
			(ttmp->ttyp == BEAR_TRAP) ? "trampa para osos" : "telaraña",
			del(mon_nam(mtmp)));
		reward_untrap(ttmp, mtmp);
	} else {
		if (ttmp->ttyp == BEAR_TRAP) {
			pline("Desarmas %s trampa para osos.", la_tu[ttmp->madeby_u]);
			cnv_trap_obj(BEARTRAP, 1, ttmp);
		} else /* if (ttmp->ttyp == WEB) */ {
			pline("Tienes éxito de quitar %s telaraña.", la_tu[ttmp->madeby_u]);
			deltrap(ttmp);
		}
	}
...
struct trap *ttmp;
{
	...
	if (fails < 2) return fails;
	pline("Desarmas %s mina de tierra.", la_tu[ttmp->madeby_u]);
...
struct trap *ttmp;
{
	...
	pline("Reparas el tablero rechinante.");	/* no madeby_u */
	deltrap(ttmp);
	newsym(u.ux + u.dx, u.uy + u.dy);
...
int otyp;
{
	...
	if (fails < 2) return fails;
	pline("Desarmas %s trampa.", la_tu[ttmp->madeby_u]);
...
boolean stuff;
{
	...
	if (((wt * 2) / wc) >= HVY_ENCUMBER) {
	    pline("%s %s y no puedes levantarl%c.", Monnam(mtmp),
		  stuff ? "trae demasiado"
                        : feminize("es demasiado pesado", mon_gender(mtmp)),
		  mon_gender(mtmp)? 'a' : 'o');
		...
		mtmp->mpeaceful = 1;
		set_malign(mtmp);		/* reset alignment */
		pline("%s siente bien de ti porque trataste.", Monnam(mtmp));
	    }
	    return 0;
	}
	return 1;
...
struct trap *ttmp;
{
	 ...
	 */
	if (!mtmp->mtrapped) {
		pline("%s no está trampad%c.", Monnam(mtmp),
			mon_gender(mtmp)? 'a' : 'o');
		return 0;
	}
	...
	if ((uprob = untrap_prob(ttmp)) && !mtmp->msleeping && mtmp->mcanmove) {
		pline("Tratas extender tus %s, pero %s se retracta escépticamente.",
	...
	if (touch_petrifies(mtmp->data) && !uarmg && !Stone_resistance) {
		const char *mano = body_part(HAND);
		pline("Agarras %s trampad%c con tus %s desnud%cs.",
			the(mtmp->data->mname),
			mon_gender(mtmp)? 'a' : 'o',
			makeplural(mano), isfeminine(mano)? 'a' : 'o');
		...
		if (poly_when_stoned(youmonst.data) && polymon(PM_STONE_GOLEM))
			...
			Sprintf(kbuf, "tratar ayudar %s fuera de un hoyo",
					an(mtmp->data->mname));
			instapetrify(kbuf);
			return 1;
		}
	...
	if (uprob) {
	    pline("Tratas %s, pero no puedes lograr un agarro firmo.",
		mon_nam_acc(mtmp, "agarrar"));
	    if (mtmp->msleeping) {
		mtmp->msleeping = 0;
		pline("%s se despierta.", Monnam(mtmp));
	    }
	    return 1;
	}

	pline("Extiendes tus %s y %s.",
	    makeplural(body_part(ARM)), mon_nam_acc(mtmp, "agarras"));
	...
	pline("%s fuera del hoyo.", mon_nam_acc(mtmp, "Arrastras"));
	mtmp->mtrapped = 0;
	fill_pit(mtmp->mx, mtmp->my);
	reward_untrap(ttmp, mtmp);
	return 1;
...
boolean force;
{
	...
	if ((ttmp = t_at(x,y)) && ttmp->tseen) {
		...
		if (box_here) {
			if (ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT) {
			    No_puedes("hacer mucho con %s%s%s.",
					the_trap, u.utrap ?
					" en que estás pegad" :
					" mientras estás a su borde",
					u.utrap ? (poly_gender()? "a" : "o") : "");
			    trap_skipped = TRUE;
			    deal_with_floor_trap = FALSE;
			} else {
			    Sprintf(qbuf, "Hay %s y %s aquí.  ¿%s %s?",
				(containercnt == 1) ? "una caja" : "unas cajas",
				an(defsyms[trap_to_defsym(ttmp->ttyp)].explanation),
				ttmp->ttyp == WEB ? "Quitar" : "Desarmar", the_trap);
			    switch (ynq(qbuf)) {
				case 'q': return(0);
		...
		if (deal_with_floor_trap) {
		    if (u.utrap) {
			No_puedes("tratar con %s mientras estás trampad%c%s!",
				  the_trap,
				  poly_gender()==1? 'a' : 'o',
				  (x == u.ux && y == u.uy) ?
					(isfeminine(the_trap)? " en ella" : " en él")
					: "");
			return 1;
		    }
		    switch(ttmp->ttyp) {
			...
			case SPIKED_PIT:
				if (!u.dx && !u.dy) {
				    pline("Ya estás en el borde del hoyo.");
				    return 0;
				}
				if (!(mtmp = m_at(x,y))) {
				    pline("Trata llenar el hoyo.");
				    return 0;
				}
				return help_monster_out(mtmp, ttmp);
			default:
				No_puedes("desarmar %s trampa.", (u.dx || u.dy) ? "esa" : "esta");
				return 0;
		    }
		}
	} /* end if */
	...
	if(!u.dx && !u.dy) {
	    for(otmp = level.objects[x][y]; otmp; otmp = otmp->nexthere)
		if(Is_box(otmp)) {
		    Sprintf(qbuf, "Hay %s aquí. ¿Examinar para trampas?",
			safe_qbuf("", sizeof("Hay  aquí. ¿Examinar para trampas?"),
...
#ifdef STEED
		    if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
			pline("No tienes la destreza necesaria para alcanzar desde %s.",
				mon_nam(u.usteed));
			return(0);
		    }
#endif
			...
			pline("¡Encuentras una trampa en %s!", the(xname(otmp)));
			if (!confused) exercise(A_WIS, TRUE);

			switch (ynq("¿Desarmarla?")) {
			    case 'q': return(1);
			...
			if(otmp->otrapped) {
			    ...
			    pline("Est%c %s no tenía trampa.",
				  isfeminine(que)? 'a' : 'e', que);
			}
			return(1);
		    } else {
			pline("No encuentras trampas en %s.", the(xname(otmp)));
			return(1);
		    }
		}

	    pline(trap_skipped ? "No encuentras otras trampas aquí."
			       : "No sabes nada de trampas aquí.");
	    return(0);
	}
	...
	if (!IS_DOOR(levl[x][y].typ)) {
	    if ((ttmp = t_at(x,y)) && ttmp->tseen)
		No_puedes("desarmar esa trampa.");
	    else
		pline("No sabes nada de trampas allí.");
	    return(0);
	}

	switch (levl[x][y].doormask) {
	    case D_NODOOR:
		pline("No %s puerta allí.", Blind ? "sientes" : "ves");
		return(0);
	    case D_ISOPEN:
		pline("Esta puerta está seguramente abierta.");
		return(0);
	    case D_BROKEN:
		pline("Esta puerta esta rota.");
		return(0);
	}
	...
	if ((levl[x][y].doormask & D_TRAPPED
	    ...
	    || (!force && confused && !rn2(3))) {
		pline("¡Encuentras una trampa en la puerta!");
		exercise(A_WIS, TRUE);
		if (ynq("¿Desarmarla?") != 'y') return(1);
		if (levl[x][y].doormask & D_TRAPPED) {
		...
		} else pline("Esta puerta no tenía trampa.");
		return(1);
	} else {
		pline("No encuentras trampas en la puerta.");
		return(1);
	}
}
#endif /* OVL2 */
...
boolean disarm;
{
	...
	pline(disarm ? "¡La disparas!" : "¡Disparas una trampa!");
	display_nhwindow(WIN_MESSAGE, FALSE);
	if (Luck > -13 && rn2(13+Luck) > 7) {	/* saved by luck */
	    /* trap went off, but good luck prevents damage */
	    switch (rn2(13)) {
		case 12:
		case 11:  msg = "la carga explosiva no funciona";  break;
		case 10:
		case  9:  msg = "la carga eléctrica se aterriza";  break;
		case  8:
		case  7:  msg = "la llama se extingue";  break;
		case  6:
		case  5:
		case  4:  msg = "la aguja envenenada falla";  break;
		case  3:
		case  2:
		case  1:
		case  0:  msg = "la nube de gas se disipa";  break;
		default:  impossible("gazapo de desarmar de cofre");  msg = (char *)0;
			  break;
	    }
	    if (msg) pline("¡Pero por suerte %s!", msg);
	} else {
		...
		case 21: {
			  ...
			  pline("¡%s!", Tobjnam(obj, "estalla"));
			  Sprintf(buf, "%s estallante", xname(obj));
			  ...
			  if(costly && loss) {
			      if(insider)
			      pline("Debes %ld %s por objetos destruidos.",
							loss, currency(loss));
			      else {
				  pline("¡Causaste %ld %s de daños!",
							loss, currency(loss));
		...
		case 18:
		case 17:
			pline("Una nube de gas nocivo se alza desde %s.",
							the(xname(obj)));
			poisoned("nube de gas", A_STR, "nube de gas envenenado",15);
		...
		case 14:
		case 13:
			Sientes("una aguja atravesando tu %s.",body_part(bodypart));
			poisoned("aguja", A_CON, "aguja envenenada",10);
			exercise(A_CON, FALSE);
			break;
		case 12:
		case 11:
		...
		case 6: {
			...
			pline("¡Recibes un choque eléctrico!");
			if(Shock_resistance)  {
			    shieldeff(u.ux, u.uy);
			    pline("No pareces afectad%c.",
				poly_gender()==1? 'a' : 'o');
			    dmg = 0;
			} else
			    dmg = d(4, 4);
			...
			if (dmg) losehp(dmg, "choque eléctrico", KILLED_BY_AN);
			break;
		      }
		case 5:
		case 4:
		case 3:
			if (!Free_action) {                        
			pline("¡De repente estás bloquead%c en lugar!",
				poly_gender()==1? 'a' : 'o');
			nomul(-d(5, 6));
			exercise(A_DEX, FALSE);
			nomovemsg = You_can_move_again;
			} else pline("Te vuelves inflexible por un momento.");
			break;
		case 2:
		case 1:
		case 0:
			pline("Una nube de gas %s se alza desde %s.",
				Blind ? blindgas[rn2(SIZE(blindgas))] :
				rndcolor(), the(xname(obj)));
			if(!Stunned) {
			    /* LENGUA:  ¿Cómo se dice "groovy"? */
			    if (Hallucination)
				pline("¡Qué sentido groovy!");
			    else if (Blind)
				pline("%s y te vuelves aturdid%c...",
					stagger(youmonst.data, "Te bamboleas"),
					poly_gender()==1? 'a' : 'o');
			    else
				pline("%s y se nubla tu visión...",
					stagger(youmonst.data, "Te bamboleas"));
			}
			make_stunned(HStun + rn1(7, 16),FALSE);
			(void) make_hallucinated(HHallucination + rn1(5, 16),FALSE,0L);
			break;
		default: impossible("mala trampa de cofre");
			break;
	    }
...
register int bodypart;
{
	...
	pline("¡¡BUM!!  ¡%s tenía trampa!", The(item));
	wake_nearby();
	losehp(dmg, "explosión", KILLED_BY_AN);
	exercise(A_STR, FALSE);
...
boolean nocorpse;
{
	...
	if(!strike) {
		if (obj && cansee(mon->mx, mon->my))
		    pline("¡%s casi es golpead%c por %s!", Monnam(mon),
			    mon_gender(mon)? 'a' : 'o', doname(obj));
	} else {
		int dam = 1;

		if (obj && cansee(mon->mx, mon->my))
			pline("¡%s es golpead%c por %s!", Monnam(mon),
				mon_gender(mon)? 'a' : 'o', doname(obj));
...
unconscious()
{
	return((boolean)(multi < 0 && (!nomovemsg ||
		u.usleep ||
		!strncmp(nomovemsg,"Recobras conci", 14) ||
		!strncmp(nomovemsg,"Estás conscien", 14))));
}

static const char lava_killer[] = "lava fundida";
...
lava_effects()
{
	...
	if(Wwalking) {
	    dmg = d(6,6);
	    pline("¡La lava aquí te quema!");
	    if(dmg < u.uhp) {
		losehp(dmg, lava_killer, KILLED_BY);
		goto burn_stuff;
	    }
	} else
	    pline("¡Te caes en la lava!");
	...
	for(obj = invent; obj; obj = obj2) {
		...
		if(obj->owornmask) {
		    if (usurvive)
			pline("¡%s en llamas!", aobjnam(obj, "arroja"));
	...
	killer_format = KILLED_BY;
	killer = lava_killer;
	pline("Te quemas a crespo...");
	done(BURNING);
	while (!safe_teleds(TRUE)) {
		pline("Todavía estás quemando.");
		done(BURNING);
	}
	where = surface(u.ux, u.uy);
	pline("Te encuentras de nuevo en %s sólid%c.", where+3,
		isfeminine(where)? 'a' : 'o');
	return(TRUE);
    }
	...
	u.utraptype = TT_LAVA;
	pline("¡Te hundes en la lava, pero solo te quema un poco!");
	if (u.uhp > 1)
...
burn_stuff:
	...
	obj = uarmf;
	pline("¡Tu %s se arroja en llamas!", xname(obj));
	(void) Boots_off();
	useup(obj);
    }
    destroy_item(SCROLL_CLASS, AD_FIRE);
