 ...
 */
/* LENGUA:  These names are left untranslated, but names defined elsewhere are
   replaced with their Spanish equivalents */
...
char *nam;
{
	...
	devnam = dev_name();
	if (!devnam)
	    Strcpy(nam, fmlkind ? "Eva" : "Adán");
	else if (fmlkind && !!strcmp(devnam, "Janet"))
	    Strcpy(nam, rn2(2) ? "Maud" : "Eva");
	...
	if (fmlkind || !strcmp(nam, "Janet"))
	    ...
	    mtmp->female = 0;
	Strcat(nam, mon_article(mtmp)? " la " : " el ");
...
register boolean special;
{
	...
	if ((mtmp = makemon(ptr, x, y, NO_MM_FLAGS)) != 0) {
		...
		case PM_WIZARD:
		    ...
		    shield = STRANGE_OBJECT;
		    break;
		default: impossible("monstruo malo de mplayer");
		    weapon = 0;
		    break;
	    }
...
register struct monst *mtmp;
{
	static const char *same_class_msg[3] = {
		"¡No puedo ganar, y tú tampoco!",
		"¡No mereces ganar!",
		"¡Mío debe ser el honor, no tuyo!",
	},		  *other_class_msg[4] = {
		"¿El cochino quiere hablar, eh?",
		"¡Lucha, canalla!",
		"¡Aquí está lo que quiero decir!",
		"¿La cochina quiere hablar, eh?",
	};
	int fem = poly_gender()==1? 1 : 0; /* el cochino -> la cochina */
	...
	pline("¿Hablar? -- %s",
		...
		same_class_msg[rn2(3)] : other_class_msg[rn2(3)+fem]);
}
