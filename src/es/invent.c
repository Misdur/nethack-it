...
struct obj **potmp, **pobj;
{
	...
	if(mergable(otmp, obj)) {
		...
		if (obj->owornmask && carried(otmp)) {
			...
			impossible("uniendo cosas llevadas de manera extraña (%lx)", wmask);
...
struct obj *obj;
{
	if (obj->oclass == COIN_CLASS) {
		...
		if (u.uhave.amulet) impossible("¿Ya tienes amuleto?");
		u.uhave.amulet = 1;
	} else if (obj->otyp == CANDELABRUM_OF_INVOCATION) {
		if (u.uhave.menorah) impossible("¿Ya tienes candelabro?");
		u.uhave.menorah = 1;
	} else if (obj->otyp == BELL_OF_OPENING) {
		if (u.uhave.bell) impossible("¿Ya tienes campana de plata?");
		u.uhave.bell = 1;
	} else if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
		if (u.uhave.book) impossible("¿Ya tienes el libro?");
		u.uhave.book = 1;
	} else if (obj->oartifact) {
		if (is_quest_artifact(obj)) {
		    if (u.uhave.questart)
			impossible("¿Ya tienes artefacto de la misión?");
		    u.uhave.questart = 1;
		    artitouch();
		}
...
struct obj *obj;
{
	if (obj->oclass == COIN_CLASS) {
		...
		if (!u.uhave.amulet) impossible("¿No tienes amuleto?");
		u.uhave.amulet = 0;
	} else if (obj->otyp == CANDELABRUM_OF_INVOCATION) {
		if (!u.uhave.menorah) impossible("¿No tienes candelabro?");
		u.uhave.menorah = 0;
	} else if (obj->otyp == BELL_OF_OPENING) {
		if (!u.uhave.bell) impossible("¿No tienes campana de plata?");
		u.uhave.bell = 0;
	} else if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
		if (!u.uhave.book) impossible("¿No tienes el libro?");
		u.uhave.book = 0;
	} else if (obj->oartifact) {
		if (is_quest_artifact(obj)) {
		    if (!u.uhave.questart)
			impossible("¿No tienes artefacto de la misión?");
		    u.uhave.questart = 0;
		}
...
const char *action;
{
    /* LENGUA:  Do we need to worry about verb conjugation? */
    return !strcmp(action, "quitarte") || !strcmp(action, "removerte");
}
...
{
    /* LENGUA:  Do we need to worry about verb conjugation? */
    return !strcmp(action, "vestir") || !strcmp(action, "ponerte");
}
		    ...
		    *word;	/* Infinitive */
{
	 ...
	 */
	if(allowall && !strcmp(word, "leer")) allowall = FALSE;

	/* another ugly check: show boulders (not statues) */
	if(*let == WEAPON_CLASS &&
	   !strcmp(word, "tirar") && throws_rocks(youmonst.data))
	    useboulder = TRUE;
	...
	for (otmp = invent; otmp; otmp = otmp->nobj) {
		...
		/* Second ugly check; unlike the first it won't trigger a
		 * "más" in "no tienes más que ___".
		 */
		...
		|| (!strcmp(word, "comer") && !is_edible(otmp))
		|| (!strcmp(word, "sacrificar") &&
		    (otyp != CORPSE &&
		     otyp != AMULET_OF_YENDOR && otyp != FAKE_AMULET_OF_YENDOR))
		|| (!strcmp(word, "escribir") && !strcmp(prep, "con") &&
		    (otmp->oclass == TOOL_CLASS &&
		...
		|| (!strncmp(word, "frotar en la piedra", 16) &&
		    *let == GEM_CLASS &&	/* using known touchstone */
		    otmp->dknown && objects[otyp].oc_name_known)
		|| ((!strcmp(word, "usar o aplicar") ||
			!strcmp(word, "desarmar")) && !strcmp(prep, "con") &&
		...
		|| (!strcmp(word, "cargar") && !is_chargeable(otmp))
		    )
			foo--;
	...
	*ap = '\0';
...
#endif
		pline("No tienes nada %s%s%sque %s.",
			foox ? "más " : "",
			prep,
			prep[0]!=0? " " : "",
			word);
	...
	for(;;) {
		...
		if(!buf[0]) {
			Sprintf(qbuf, "¿%s%squé quieres %s? [*]",
				prep,
				prep[0]==0? "" : " ",
				word);
		} else {
			Sprintf(qbuf, "¿%s%squé quieres %s? [%s or ?*]",
				prep,
				prep[0]==0? "" : " ",
				word, buf);
		}
		qbuf[1] = toupper2(qbuf[1]);
#ifdef REDO
		if (in_doagain)
		    ilet = readchar();
		else
#endif
		    ilet = yn_function(qbuf, (char *)0, (char *)0, '\0');
		...
		if(digit(ilet)) {
			pline("No se permiten cuentas con este comando.");
			continue;
		}
		...
		if(ilet == def_oc_syms[COIN_CLASS]) {
			if (!usegold) {
			    if (!strncmp(word, "frotar en ", 10)) {
				/* the dangers of building sentences... */
				No_puedes("frotar oro%s.", word + 6);
			    } else {
				No_puedes("%s oro.", word);
			    }
			    return(struct obj *)0;
#ifndef GOLDOBJ
			} else if (!allowgold) {
				pline("No estás llevando nada de oro.");
				return(struct obj *)0;
#endif
			} 
			 ...
			 */
			if(cnt < 0) {
		pline("Al LRS le interesaría mucho saber que tienes tanto.");
				return(struct obj *)0;
			}
		...
		if(allowcnt == 2 && !strcmp(word,"tirar")) {
		    ...
		    if(cnt == 0 && prezero) return((struct obj *)0);
		    if(cnt > 1) {
			pline("Sólo puedes tirar una cosa por vez.");
			continue;
		    }
		}
#ifdef GOLDOBJ
		...
		if(!otmp) {
			pline("No tienes ese objeto.");
#ifdef REDO
			if (in_doagain) return((struct obj *) 0);
#endif
			continue;
		} else if (cnt < 0 || otmp->quan < cnt) {
			pline("¡No tienes tantos!  Sólo tienes %ld.",
			    otmp->quan);
#ifdef REDO
			if (in_doagain) return((struct obj *) 0);
#endif
...
struct obj *otmp;
{
	...
	if (ocls == ARMOR_CLASS) {
	    if (!strcmp(word, "ponerte"))
		s1 = "W", s2 = "vestir";
	    else if (!strcmp(word, "removerte"))
		s1 = "T", s2 = "quitarte";
	} else if ((ocls == RING_CLASS || otyp == MEAT_RING) ||
		ocls == AMULET_CLASS ||
		(otyp == BLINDFOLD || otyp == TOWEL || otyp == LENSES)) {
	    if (!strcmp(word, "vestir"))
		s1 = "P", s2 = "ponerte";
	    else if (!strcmp(word, "quitarte"))
		s1 = "R", s2 = "removerte";
	}
	if (s1) {
	    fem = isfeminine(xname(otmp));
	    what = fem ? "esa" : "ese";
		...
		what = fem ? "esas" : "esos";
	    pline("Usa el comando '%s' para %s %s.", s1, s2, what);
	} else {
...
unsigned *resultflags;
{
...
#ifndef GOLDOBJ
	allowgold = (u.ugold && !strcmp(word, "dejar")) ? 1 : 0;
#endif
	...
	if(!invent){
#endif
		pline("No tienes nada que %s.", word);
		return(0);
	}
	...
	if (taking_off(word)) {
	    takeoff = TRUE;
	    filter = is_worn;
	} else if (!strcmp(word, "identificar")) {
	    ident = TRUE;
	...
	if (ident && !iletct) {
	    ...
	    ilets[iletct++] = ' ';
	    if (unpaid) ilets[iletct++] = 's';
	    if (count_buc(invent, BUC_BLESSED))  ilets[iletct++] = 'B';
	    if (count_buc(invent, BUC_UNCURSED)) ilets[iletct++] = 'I';
	    if (count_buc(invent, BUC_CURSED))   ilets[iletct++] = 'M';
	...
	for (;;) {
	    Sprintf(qbuf,"¿Qué clases de cosas quieres %s? [%s]",
		    word, ilets);
	    getlin(qbuf, buf);
	...
	while ((sym = *ip++) != '\0') {
		...
		if (index(extra_removeables, oc_of_sym)) {
		    ...
		    pline("No llevas ninguna armadura.");
		    return 0;
		} else if (oc_of_sym == WEAPON_CLASS &&
			!uwep && !uswapwep && !uquiver) {
		    pline("No estás blandiendo nada.");
		    return 0;
		} else if (oc_of_sym == RING_CLASS && !uright && !uleft) {
		    pline("No llevas ningún anillo.");
		    return 0;
		} else if (oc_of_sym == AMULET_CLASS && !uamul) {
		    pline("No llevas ningún amuleto.");
		    return 0;
		} else if (oc_of_sym == TOOL_CLASS && !ublindf) {
		    pline("No tienes los ojos vendados.");
		    return 0;
		}
	    }
		...
		add_valid_menu_class('S');
		ckfn = ckunpaid;
	    } else if (sym == 'B') {
	    	add_valid_menu_class('B');
	    	ckfn = ckvalidcat;
	    } else if (sym == 'M') {
	    	add_valid_menu_class('M');
	    	ckfn = ckvalidcat;
	    } else if (sym == 'I') {
...
register int FDECL((*fn),(OBJ_P)), FDECL((*ckfn),(OBJ_P));
{
	...
	ident = !strcmp(word, "identificar");
	nodot = (!strcmp(word, "nodot") || !strcmp(word, "dejar") ||
		 ident || takeoff);
	ininv = (*objchn == invent);
...
nextclass:
	...
	if (olets && *olets && *++olets)
		goto nextclass;
	if(!takeoff && (dud || cnt)) pline("Eso fue todo.");
	else if(!dud && !cnt) pline("No hay objetos aplicables.");
ret:
	return(cnt);
}
...
int id_limit;
{
    ...
    while (id_limit) {
	Sprintf(buf, "¿Qué quieres identificar %s?",
		first ? "primero" : "a continuación");
...
int id_limit;
{
    ...
    if (!unid_cnt) {
	pline("Ya has identificado todas tus posesiones.");
    } else if (!id_limit) {
	/* identify everything */
	if (unid_cnt == 1) {
	...
	if (flags.menu_style == MENU_TRADITIONAL)
	    do {
		n = ggetobj("identificar", identify, id_limit, FALSE, (unsigned *)0);
...
long quan;		/* if non-0, print this quantity, not obj->quan */
{
	...
	Sprintf(li, "%ld pieza%s de oro%s", obj->quan, plur(obj->quan),
		(dot ? "." : ""));
#endif
    } else {
...
#ifndef GOLDOBJ
	    pline("No traes nada%s.", u.ugold ? " más que oro" : "");
#else
	    pline("No traes nada.");
#endif
	    return 0;
    ...
    int type;
{
	...
	switch(type) {
		...
		break;
	    default:
		impossible("¿se necesita cuenta de estado de maldición %d?", type);
		return 0;
	}
	list = list->nobj;
    }
...
dotypeinv()
{
	...
	const char *prompt = "¿De qué clase de objeto quieres un inventario?";
	...
	if (traditional) {
	    ...
	    *extra_types = '\0';	/* for index() */
	    ...
	    if(class_count > 1) {
		c = yn_function(prompt, types, (char *)0, '\0');
#ifdef REDO
		savech(c);
#endif
		if(c == '\0') {
	...
	if (c == 'x') {
		...
		pline("No hay objetos usados en tu factura de compras.");
	    return 0;
	}
	if (c == 'S') {
	    if (unpaid_count)
		...
		pline("No estás llevando ningún objeto sin pagar.");
	    return 0;
	}
	if (traditional) {
...
char *buf;
{
	...
	if (IS_DOOR(ltyp)) {
	    ...
	    dfeature = "charco de agua";
#ifdef SINKS
	else if (IS_SINK(ltyp))
	    cmap = S_sink;				/* "sink" */
#endif
	else if (IS_ALTAR(ltyp)) {
	    Sprintf(altbuf, "altar de %s (%s)", a_gname(),
	    ...
	    dfeature = "juego de barras de hierro";
...
boolean picked_some;
{
	struct obj *otmp;
	struct trap *trap;
	const char *verb = Blind ? "sientes" : "ves";
	const char *Verb = Blind ? "Sientes" : "Ves";
	...
	if (u.uswallow && u.ustuck) {
	    ...
	    Sprintf(fbuf, "Contenidos %s %s",
		del(the(mbodypart(mtmp, STOMACH))), del(mon_nam(mtmp)));
	    pline("%s lo que está en %s %s.",
		  Blind ? "Intentas sentir" : "Miras a tu alrededor para ver",
		  the(mbodypart(mtmp, STOMACH)), del(mon_nam(mtmp)));
	    otmp = mtmp->minvent;
	    if (otmp) {
		...
		pline("No %s ningunos objetos aquí.", verb);
	    }
	    return(!!Blind);
	}
	...
	if (dfeature && !strcmp(dfeature, "charco de agua") && Underwater)
		dfeature = 0;
	...
	if (Blind) {
		...
		if (dfeature && !strncmp(dfeature, "altar ", 6)) {
		    ...
		    pline("Intentas sentir lo que hay aquí.");
		    fem = FALSE;
		} else {
		    pline("Intentas sentir lo que %s%s.",
		        drift ? "flota aquí" : "yace aquí en ",
		        drift ?	""	     : surface(u.ux, u.uy));
		    fem = drift ? FALSE : isfeminine(surface(u.ux, u.uy));
		}
		if (dfeature && !drift && !strcmp(dfeature, surface(u.ux,u.uy)))
			dfeature = 0;		/* ice already identifed */
		if (!can_reach_floor()) {
			pline("¡Pero no puedes alcanzarl%c!", fem? 'a' : 'o');
			return(0);
		}
	}

	if (dfeature)
		Sprintf(fbuf, "Hay %s aquí.", an(dfeature));

	if (!otmp || is_lava(u.ux,u.uy) || (is_pool(u.ux,u.uy) && !Underwater)) {
		if (dfeature) pline(fbuf);
		read_engr_at(u.ux, u.uy); /* Eric Backus */
		if (!skip_objects && (Blind || !dfeature))
		    pline("No %s ningún objeto aquí.", verb);
		return(!!Blind);
	}
	...
	if (skip_objects) {
	    if (dfeature) pline(fbuf);
	    read_engr_at(u.ux, u.uy); /* Eric Backus */
	    Hay("%s%s objetos aquí.",
		  (obj_cnt <= 10) ? "varios" : "muchos",
		  picked_some ? " más" : "");
...
#endif
	    pline("%s aquí %s.", Verb, doname(otmp));
	    if (otmp->otyp == CORPSE) feel_cockatrice(otmp, FALSE);
	} else {
	    ...
	    putstr(tmpwin, 0, Blind ? "Cosas que sientes aquí:" :
				      "Cosas que hay aquí:");
...
boolean force_touch;
{
	...
	if (will_feel_cockatrice(otmp, force_touch)) {
	    if(poly_when_stoned(youmonst.data)) {
			...
			pline("Tocaste el cadáver de %s con tus %s desnud%cs.",
				mons[otmp->corpsenm].mname, makeplural(mano),
				isfeminine(mano)? 'a' : 'o');
	    } else
			pline("Tocar el cadáver de %s es un error fatal...",
				mons[otmp->corpsenm].mname);
		Sprintf(kbuf, "cadáver de %s", an(mons[otmp->corpsenm].mname));
		instapetrify(kbuf);
	}
}
...
doprgold()
{
...
#ifndef GOLDOBJ
	if(!u.ugold)
	    pline("Tu cartera está vacía.");
	else
	    pline("Tu cartera contiene %ld pieza%s de oro.",
	...
	if(!umoney)
	    ...
	    pline("Tu cartera contiene %ld %s.",
		    umoney, plur(umoney));
#endif
...
doprwep()
{
    if (!uwep) {
	const char *mano = body_part(HAND);
	pline("Tienes %s vací%cs.", the(makeplural(mano)), isfeminine(mano)? 'a' : 'o');
    } else {
...
doprtool()
{
	...
	lets[ct] = '\0';
	if (!ct) pline("No estás usando ninguna herramienta.");
...
doprinuse()
{
	...
	for (otmp = invent; otmp; otmp = otmp->nobj)
		...
		lets[ct++] = obj_to_let(otmp);
	lets[ct] = '\0';
	if (!ct) pline("No llevas ni blandes nada.");
	else (void) display_inventory(lets, FALSE);
	return 0;
}
...
STATIC_VAR NEARDATA const char *names[] = { 0,
	"Objetos ilegales", "Armas", "Armaduras", "Anillos", "Amuletos",
	"Herramientas", "Comestibles", "Pociones", "Pergaminos", "Libros de Hechizos",
	"Varitas", "Monedas", "Joyas", "Rocas Enormes/Estatuas", "Bolas de Hierro",
	"Cadenas", "Venenos"
};
...
boolean unpaid;
{
	...
	len = strlen(class_name) + (unpaid ? sizeof " Sin Pagar" : sizeof "");
	if (len > invbufsiz) {
	...
	if (unpaid)
	    Sprintf(invbuf, "%s Sin Pagar", class_name);
	else
...
doorganize()	/* inventory organizer by Del Lamb */
{
	...
	if (!(obj = getobj(allowall,"","ajustar"))) return(0);
	...
	for (;;) {
		Sprintf(qbuf, "¿Ajustar la letra a cuál [%s]?",buf);
		let = yn_function(qbuf, (char *)0, (char *)0, '\0');
		if(index(quitchars,let)) {
			pline(Never_mind);
			return(0);
		}
		if (let == '@' || !isalpha2(let))
			pline("Elige una letra de casillero de inventario.");
		else
			break;
	}
...
char *title;
{
	...
	Sprintf(tmp,"%s %s:",
		do_all ? "Posesiones" : "Armamentos",
		del(noit_mon_nam(mon)));
	...
	if (do_all ? (mon->minvent != 0)
		...
		if (add_to_minv(mon, &m_gold))
		    panic("display_minventory: objeto estático librado.");
	    }
	    ...
	    invdisp_nothing(title ? title : tmp, "(nada)");
	    n = 0;
	}
...
register struct obj *obj;
{
	...
	int n;
	menu_item *selected = 0;
	char *name = doname(obj);

	Sprintf(tmp,"Contenidos de %s:", name);
	...
	if (obj->cobj) {
	    ...
	    char empty[10];
	    Sprintf(empty, "(vací%c)", isfeminine(name)? 'a' : 'o');
...
boolean as_if_seen;
{
	...
	if (n) {
	    only.x = x;
	    only.y = y;
	    if (query_objlist("Cosas que están enterradas aquí:",
