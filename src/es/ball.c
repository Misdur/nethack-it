...
ballfall()
{
	...
	if (carried(uball)) {
		pline("Sobresaltad%c, dejas caer la bola de hierro.",
			poly_gender()==1? 'a' : 'o');
		if (uwep == uball)
	...
	if(gets_hit){
		int dmg = rn1(7,25);
		pline("La bola de hierro cae en tu %s.",
			body_part(HEAD));
		if (uarmh) {
		...
		losehp(dmg, "con la cabeza aplastada por una bola de hierro", NO_KILLER_PREFIX);
	}
}
...
{
    if (!uchain || !uball) {
	impossible("¿Dónde están tu bola y cadena?");
	return;
    }
...
bc_order()
{
    ...
    impossible("bc_order:  ¡bola y cadena no están en la misma locación!");
    return BCPOS_DIFFER;
}
...
#define SKIP_TO_DRAG { *chainx = oldchainx; *chainy = oldchainy; \
    ...
    goto drag; } 
		...
		default: impossible("mal movimiento de cadena");
		    break;
	    }
#undef SKIP_TO_DRAG
#undef IS_CHAIN_ROCK
...
drag:
	...
	if (near_capacity() > SLT_ENCUMBER && dist2(x, y, u.ux, u.uy) <= 2) {
	    No_puedes("%sarrastrar la bola pesada de hierro.",
			    invent ? "traer todo de eso y también " : "");
	    nomul(0);
	    return FALSE;
	}
	...
	if ((is_pool(uchain->ox, uchain->oy) &&
		...
		pline("¡Eres arrastrad%c por la bola de hierro!",
		      poly_gender()==1? 'a' : 'o');
...
xchar x, y;
{
    ...
    if (x != u.ux || y != u.uy) {
	struct trap *t;
	const char *pullmsg = "La bola te tira desde %s!";

	if (u.utrap && u.utraptype != TT_INFLOOR) {
	    switch(u.utraptype) {
	    case TT_PIT:
		pline(pullmsg, "el hoyo");
		break;
	    case TT_WEB:
		pline(pullmsg, "la telaraña");
		pline("¡La telaraña es destruída!");
		deltrap(t_at(u.ux,u.uy));
		break;
	    case TT_LAVA:
		pline(pullmsg, "la lava");
		break;
	    case TT_BEARTRAP: {
		register long side = rn2(3) ? LEFT_SIDE : RIGHT_SIDE;
		pline(pullmsg, "la trampa para osos");
...
#endif
		{
		    ...
		    pline("Tu %s %s%c está gravamente dañad%c.",
			  pierna,
			  (side == LEFT_SIDE) ? "izquierd" : "derech",
			  fem, fem);
			sprintf(killer, "daño en la pierna por ser arrancad%c una trampa para osos",
				poly_gender()==1? 'a' : 'o');
...
litter()
{
	...
	while (otmp) {
		...
		if ((otmp != uball) && (rnd(capacity) <= (int)otmp->owt)) {
			if (canletgo(otmp, "")) {
				pline("Tu %s bajo la escalera.",
				     aobjnam(otmp, "te sigue"));
				dropx(otmp);
			}
		}
		otmp = nextobj;
...
drag_down()
{
	...
	if (carried(uball))
		pline("No puedes sostener la bola de hierro.");

	if (forward) {
		if(rn2(6)) {
			char killer[50];
			pline("¡La bola de hierro te arrastra por la escalera!");
			sprintf(killer, "arrastrad%c por la escalera por una bola de hierro",
				poly_gender()==1? 'a' : 'o');
			losehp(rnd(6), killer, NO_KILLER_PREFIX);
			litter();
		}
	} else {
		if(rn2(2)) {
			pline("¡La bola de hierro te pega!");
			losehp(rnd(20), "colisión con una bola de hierro", KILLED_BY_AN);
