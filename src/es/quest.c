...
boolean talk;
{
    ...
    boolean fem = poly_gender()==1? 'a' : 'o';

#ifdef WIZARD
    if (wizard && talk) {
	if (u.ualign.type != original_alignment) {
	    pline("Eres actualmente %s en vez de %s.",
		feminize(align_str(u.ualign.type), fem),
		feminize(align_str(original_alignment), fem));
	} else if (u.ualignbase[A_CURRENT] != original_alignment) {
	    pline("Has convertido.");
	} else if (u.ualign.record < MIN_QUEST_ALIGN) {
	    pline("Estás actualmente %d y requieres %d.",
		u.ualign.record, MIN_QUEST_ALIGN);
	    if (yn_function("¿Ajustar?", (char *)0, (char *)0, 'y') == 'y')
		u.ualign.record = MIN_QUEST_ALIGN;
	}
    }
#endif
	...
	register struct monst *mtmp;
{
    ...
    switch(mtmp->data->msound) {
	    ...
	    default:	impossible("quest_chat: Personaje de busca desconocido %s.",
				   mon_nam(mtmp));
	}
}
