...
boolean devour;
{
	...
	if (cansee(x, y) || cansee(mtmp->mx, mtmp->my))
	    pline("%s %s %s.", mon_visible(mtmp) ? noit_Monnam(mtmp) : "Él",
		  devour ? "devora" : "come",
...
register struct edog *edog;
{
	if (monstermoves > edog->hungrytime + 500) {
		...
		if (cansee(mtmp->mx, mtmp->my))
		    pline("%s está confundid%c por el hambre.", Monnam(mtmp),
	                  mon_gender(mtmp)? 'a' : 'o');
		else if (couldsee(mtmp->mx, mtmp->my))
		    beg(mtmp);
		else
		    Te_sientes("preocupad%c sobre %s.",
			  poly_gender()==1? 'a' : 'o',
			  y_monnam(mtmp));
		stop_occupation();
 ...
 dog_died:
		if (mtmp->mleashed
		    ...
		    pline("Tu correa se afloja.");
		else if (cansee(mtmp->mx, mtmp->my))
		    pline("%s se muere de hambre.", Monnam(mtmp));
		else
		    /* LENGUA:  "bummed" (jerga) -> "bajoneado" */
		    Te_sientes("%s por un momento.",
			Hallucination ? feminize("bajoneado", poly_gender()==1)
				      : "triste");
		mondied(mtmp);
		return(TRUE);
	    }
...
#define GDIST(x,y) (dist2(x,y,gx,gy))
	...
	if (Conflict && !resist(mtmp, RING_CLASS, 0, 0)) {
		 ...
		 */
		if (canspotmon(mtmp)) {
		    pline("%s te reprocha, diciendo:", Monnam(mtmp));
		    verbalize("Ya que deseáis el conflicto, ¡tened más!");
		}
		mongone(mtmp);
		i = rnd(4);
		while(i--) {
...
newdogpos:
	if (nix != omx || niy != omy) {
		...
		if (cursemsg[chi] && (cansee(omx,omy) || cansee(nix,niy)))
			pline("%s se mueve sólo renuentemente.", Monnam(mtmp));
