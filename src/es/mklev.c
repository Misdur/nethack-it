...
static NEARDATA const char *trap_engravings[TRAPNUM] = {
			...
			"Vlad estaba aquí", "ad aerarium", "ad aerarium",
			(char *)0, (char *)0, (char *)0, (char *)0, (char *)0,
			(char *)0,
};
    ...
    coord *mp;
{
	...
	do {
	    if (!somexy(croom, mp))
		impossible("¡No puedo poner rama!");
...
{
	if (!x) {
	    impossible("mkstairs:  intento inválido de escalera en <%d,%d>", x, y);
	    return;
	}
...
struct mkroom *croom;
{
	...
	make_grave(m.x, m.y, dobell ? "¡Salvado por la campana!" : (char *) 0);
...
mkinvokearea()
{
    ...
    pline("¡El piso se sacude violentamente bajo ti!");
    pline("¡Las paredes a tu alrededor empiezan a doblarse y romperse!");
    ...
    pline("¡Estás a la cima de una escalera que va abajo!");
...
int dist;
{
	...
	if (dist < (7 - 2))
	    panic("mkinvpos: ¡<%d,%d> (%d) fuera de mapa!", x, y, dist);
	return;
    }
	...
	break;
    default:
	impossible("mkinvpos llamada con dist %d", dist);
	break;
    }
...
xchar x, y;
{
	...
	if (! (u.uz.dnum == oracle_level.dnum	    /* in main dungeon */
		&& !at_dgn_entrance("La Misión")    /* but not Quest's entry */
