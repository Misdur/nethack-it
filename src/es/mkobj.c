...
struct obj *otmp;
{
	...
	break;
    default:
	panic("replace_object: obj posición");
	break;
    }
}
...
boolean artif;
{
	...
	*otmp = zeroobj;
	...
	case COIN_CLASS:
		break;	/* do nothing */
	default:
		impossible("imposible mkobj %d, sim '%c'.", otmp->otyp,
...
boolean init;
{
	...
	if (objtype != CORPSE && objtype != STATUE)
	    impossible("haciendo tipo de corpstat %d", objtype);
...
int x, y;
{
	...
	panic("place_object: obj no libre");
...
register struct obj *otmp;
{
	...
	panic("remove_object: obj no está en piso");
    ...
    struct obj *obj, **head_ptr;
{
    ...
    if (!curr) panic("extract_nobj: objeto perdido");
    obj->where = OBJ_FREE;
}
    ...
    struct obj *obj, **head_ptr;
{
    ...
    if (!curr) panic("extract_nexthere: objeto perdido");
}
    ...
    struct obj *obj;
{
	...
	panic("add_to_minv: obj no libre");
    ...
    struct obj *container, *obj;
{
    ...
    if (obj->where != OBJ_FREE)
	panic("add_to_container: obj no libre");
...
{
    if (obj->where != OBJ_FREE)
	panic("add_to_migration: obj no libre");
...
{
    if (obj->where != OBJ_FREE)
	panic("add_to_buried: obj no libre");
...
{
    if (obj->where != OBJ_FREE)
	panic("dealloc_obj: obj no libre");
...
obj_sanity_check()
{
	...
	if (obj->cobj) {
	    pline("%s obj %s contiene %s! %s\n", mesg,
		fmt_ptr((genericptr_t)obj, obj_address),
		something, doname(obj));
	}
    }

    mesg = "sanidad de minvent";
	...
	for (obj = mon->minvent; obj; obj = obj->nobj) {
	    ...
	    if (obj->ocarry != mon) {
		pline("%s obj %s (%s) no tenido por mon %s (%s)\n", mesg,
			fmt_ptr((genericptr_t)obj, obj_address),
			doname(obj),
...
static const char *obj_state_names[NOBJ_STATES] = {
	"libre",	"piso",		"contenido",	"invent",
	"minvent",	"migrando",	"enterrado",	"encuenta"
};
