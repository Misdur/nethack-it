...
static const char moderateloadmsg[] = "Tienes un poco de dificultad levantando";
static const char nearloadmsg[] = "Tienes mucha dificultad levantando";
static const char overloadmsg[] = "Tienes extrema difficultad levantando";
...
ask_again:
		...
		*one_at_a_time = *everything = FALSE;
		...
		Sprintf(qbuf,"¿Qué tipos de cosas quieres %s? [%s]",
			action, ilets);
		getlin(qbuf,inbuf);
		...
		for (p = inbuf; (sym = *p++); ) {
			...
			if (index(ilets,sym)) {
			    ...
			    if (!where)
				where = !strcmp(action,"recoger")  ? "aquí" :
					!strcmp(action,"sacar") ?
							    "dentro" : "";
			    if (*where)
				pline("No hay %c's %s.", sym, where);
			    else
				pline("No tienes %c's.", sym);
			    not_everything = TRUE;
			}
		    }
		}
...
struct obj *obj;
{
    ...
    if (((index(valid_menu_classes,'S') != (char *)0) && obj->unpaid) ||
	...
	return TRUE;
    else if (((index(valid_menu_classes,'M') != (char *)0) &&
...
{
    if ((obj != uchain) &&
	(((index(valid_menu_classes,'S') != (char *)0) && obj->unpaid) ||
	(index(valid_menu_classes, obj->oclass) != (char *)0)))
	return TRUE;
    else
	return FALSE;
...
int what;		/* should be a long */
{
	...
	if (!u.uswallow) {
		...
		if (notake(youmonst.data)) {
		    if (!autopickup)
			pline("Eres físicamente incapaz de recoger nada.");
		    else
			check_here(FALSE);
		    return (0);
		}
	...
	if (flags.menu_style != MENU_TRADITIONAL || iflags.menu_requested) {
		...
		char buf[QBUFSZ];
		Sprintf(buf, "¿Elegir %d de cuál?", count);
		...
		n = query_objlist("¿Recoger cuál?", objchain,
...
menu_pickup:
		...
		if (!all_of_a_type) {
		    char qbuf[BUFSZ];
		    Sprintf(qbuf, "¿Recoger %s?",
			safe_qbuf("", sizeof("¿Recoger ?"), doname(obj),
					an(simple_typename(obj->otyp)), "algo"));
	...
	*pick_list = (menu_item *) 0;
	...
	if ((qflags & ALL_TYPES) && (ccount > 1)) {
		...
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
		       (qflags & WORN_TYPES) ? "Todos los tipos llevados" : "Todos los tipos",
			MENU_UNSELECTED);
		invlet = 'b';
	} else
		invlet = 'a';
	do {
	    ...
	    pack++;
	    if (invlet >= 'x') {
		impossible("query_category: demasiadas categorias");
		return 0;
	    }
	} while (*pack);
	...
	if (do_unpaid) {
		...
		any.a_int = 'S';
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			"Artículos sin pagar", MENU_UNSELECTED);
	}
	...
	if (qflags & BILLED_TYPES) {
		...
		any.a_int = 'x';
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			 "Artículos sin pagar ya usados", MENU_UNSELECTED);
	}
	if (qflags & CHOOSE_ALL) {
		...
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			(qflags & WORN_TYPES) ?
			"Auto-elegir todos los artículos llevados" :
			"Auto-elegir todos los artículos", MENU_UNSELECTED);
	}
	...
	if (do_blessed) {
		...
		any.a_int = 'B';
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			"Cosas conocidas como Benditas", MENU_UNSELECTED);
	}
	if (do_cursed) {
		invlet = 'M';
		any.a_void = 0;
		any.a_int = 'M';
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			"Cosas conocidas como Malditas", MENU_UNSELECTED);
	}
	if (do_uncursed) {
		invlet = 'I';
		any.a_void = 0;
		any.a_int = 'I';
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			"Cosas conocidas como Inmalditas", MENU_UNSELECTED);
	}
	if (do_buc_unknown) {
		invlet = 'X';
		any.a_void = 0;
		any.a_int = 'X';
		add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE,
			"Cosas de estado desconocido de B/M/I",
			MENU_UNSELECTED);
	}
	end_menu(win, qstr);
	n = select_menu(win, how, pick_list);
...
int *wt_before, *wt_after;
{
    ...
    *wt_after  = wt;
	...
	if (container) {
	    Sprintf(where, "en %s", the(xname(container)));
	    verb = "traer";
	} else {
	    Strcpy(where, "yaciendo aquí");
	    verb = telekinesis ? "adquirir" : "levantar";
	}
    } else {
	/* lint supppression */
	*obj_nambuf = *where = '\0';
    ...
    if (qq > 0) {
	if (qq < count) {
	    boolean fem = isfeminine(obj_nambuf);
	    pline("Puedes %s sólo %s de %s %s %s.",
		verb,
		(qq == 1L) ? (fem? "una" : "un")
			   : (fem? "algunas" : "algunos"),
		fem? "las" : "los",
	...
	*wt_after = wt;
    ...
    if (!container) Strcpy(where, "aquí");  /* slightly shorter form */
#ifndef GOLDOBJ
    if (invent || u.ugold) {
#else
    if (invent || umoney) {
	...
	if (isfeminine(obj_nambuf)) {
	    prefx1 = (obj->quan == 1L) ? "" : "aún una ";
	    prefx2 = "es demasiado pesada para ";
	} else {
	    prefx1 = (obj->quan == 1L) ? "" : "aún uno ";
	    prefx2 = "es demasiado pesado para ";
	}
	suffx  = "";
    }
    Hay("%s %s, pero %s%s%s%s.",
	  obj_nambuf, where,
...
boolean telekinesis;
{
	...
	No_puedes("poner tu %s cerca de est%c %s.",
			body_part(HAND),
    ...
    *cnt_p = carry_count(obj, container, *cnt_p, telekinesis, &old_wt, &new_wt);
...
#endif
	pline("Tu mochila no puede admitir ningunos más artículos.");
	result = -1;	/* nothing lifted */
    } else {
	result = 1;
	...
	if (next_encumbr > prev_encumbr) {
		...
		Strcpy(qbuf,
			...
			moderateloadmsg);
		Sprintf(eos(qbuf), " %s. ¿Continuar?",
			safe_qbuf(qbuf, sizeof(" . ¿Continuar?"),
				doname(obj), an(simple_typename(obj->otyp)), "algo"));
		obj->quan = savequan;
		switch (ynq(qbuf)) {
...
unsigned padlength;
{
	...
	if (len_lastR >= textleft) {
	    impossible("safe_qbuf: last_resort demasiado grande con %u caracteres.",
		       len_lastR);
	    return "";
	}
...
boolean telekinesis;	/* not picking it up directly by hand */
{
	int res, nearload;
#ifndef GOLDOBJ
	const char *where = (obj->ox == u.ux && obj->oy == u.uy) ? "aquí" :
			    distu(obj->ox, obj->oy) <= 9 ? "ahí"
			    : "allí";
#endif
	...
	if (obj == uchain) {    /* do not pick up attached chain */
	    ...
	    if (gold_capacity <= 0L) {
		Hay("%ld pieza%s de oro %s, pero no puedes traer más.",
		      obj->quan, plur(obj->quan), where);
		return 0;
	    } else if (gold_capacity < count) {
		pline("Puedes %s sólo %s de los %ld piezas de oro que yacen %s.",
		    telekinesis ? "adquirir" : "traer",
		    gold_capacity == 1L ? "uno" : "algunos", obj->quan, where);
		pline("%s %ld pieza%s de oro.",
		...
		if (poly_when_stoned(youmonst.data) && polymon(PM_STONE_GOLEM))
			...
			pline("Tocar %s es un error fatal.", kbuf);
			instapetrify(kbuf);
		    return -1;
		}
	    } else if (is_rider(&mons[obj->corpsenm])) {
		pline("A tu %s, el cadáver de repente se mueve...",
			telekinesis ? "adquisición atentada" : "toque");
		...
		pline("%s pergamino%s se %s polvo como lo%s levantas.",
			(obj->quan > 1L) ? "Los" : "El",
			plur(obj->quan),
			otense(obj, "hace"),
			plur(obj->quan));
...
encumber_msg()
{
    ...
    if(oldcap < newcap) {
	switch(newcap) {
	case 1: pline("Tus movimientos están retardados un poco por tu carga.");
		break;
	case 2: pline("Rebalanceas tu carga.  El movimiento es difícil.");
		break;
	case 3: pline("%s bajo tu carga pesada.  El movimiento es muy difícil.",
		    stagger(youmonst.data, "Te tambaleas"));
		break;
	default: pline("¡%s puedes mover de ninguna manera con esta carga!",
		newcap == 4 ? "Escasamente" : "Ya no");
		break;
	}
	flags.botl = 1;
    } else if(oldcap > newcap) {
	switch(newcap) {
	case 0: pline("Tus movimientos ahora no están impedidos.");
		break;
	case 1: pline("Tus movimientos están retardados sólo un poco por tu carga.");
		break;
	case 2: pline("Rebalanceas tu carga.  El movimiento aún es difícil.");
		break;
	case 3: pline("%s bajo tu carga.  El movimiento aún es muy difícil.",
		    stagger(youmonst.data, "Te tambaleas"));
		break;
	}
	flags.botl = 1;
...
#endif
			No_puedes("alcanzar %s.", surface(x, y));
		return FALSE;
	} else if (is_pool(x, y) || is_lava(x, y)) {
		/* at present, can't loot in water even when Underwater */
		No_puedes("saquear las cosas que están en %s.",
		    is_lava(x, y) ? "la lava" : "el agua");
		return FALSE;
	} else if (nolimbs(youmonst.data)) {
		pline("Sin brazos ni piernas, no puedes saquear nada.");
		return FALSE;
	} else if (!freehand()) {
		const char *mano = body_part(HAND);
		pline("Sin un%s %s libre, no puedes saquear nada.",
			isfeminine(mano)? "a" : "", mano);
		return FALSE;
	}
	return TRUE;
}
...
doloot()	/* loot a container on the floor or loot saddle from mon. */
{
    ...
    const char *dont_find_anything = "No encuentras nada";
    struct monst *mtmp;
    char qbuf[BUFSZ];
    int prev_inquiry = 0;
...
lootcont:
	...
	for (cobj = level.objects[cc.x][cc.y]; cobj; cobj = nobj) {
	    ...
	    if (Is_container(cobj)) {
		const char *caja =
			safe_qbuf("", sizeof("Hay  aquí, ¿saquearlo?"),
			     doname(cobj), an(simple_typename(cobj->otyp)),
			     "una caja");
		Sprintf(qbuf, "Hay %s aquí, ¿saquearl%c?",
			caja, isfeminine(caja)? 'a' : 'o');
		c = ynq(qbuf);
		...
		if (cobj->olocked) {
		    pline("Hmmm, parece cerrad%c con llave.",
			isfeminine(xname(cobj))? 'a' : 'o');
		    continue;
		}
		if (cobj->otyp == BAG_OF_TRICKS) {
		    int tmp;
		    pline("Con cuidado, abres la bolsa...");
		    pline("¡Desarolla un juego enorme de dientes y te muerde!");
		    tmp = rnd(10);
		    if (Half_physical_damage) tmp = (tmp+1) / 2;
		    losehp(tmp, "bolsa carnivora", KILLED_BY_AN);
		    makeknown(BAG_OF_TRICKS);
		    timepassed = 1;
		    continue;
		}

		pline("Con cuidado, abres %s...", the(xname(cobj)));
...
gotit:
		if (coffers) {
	    verbalize("Gracias por su contribución para reducir la deuda.");
			...
			delobj(goldob);
			pline("El tesoro acepta tu contribución.");
		    } else {
			dropx(goldob);
		    }
		}
		...
		dropy(goldob);
#endif
		pline("Muy bien, ahora hay botín aquí.");
	    }
	}
    } else if (IS_GRAVE(levl[cc.x][cc.y].typ)) {
	Hay("que cavar el sepulcro para saquearlo efectivamente...");
    }
    /*
     * 3.3.1 introduced directional looting for some things.
     */
    if (c != 'y' && mon_beside(u.ux, u.uy)) {
	if (!get_adjacent_loc("¿Robar en qué dirección?", "Lugar de saquear inválido",
	...
	if (cc.x == u.ux && cc.y == u.uy) {
	    ...
	    underfoot = FALSE;
	if (u.dz < 0) {
	    pline("%s que saquear en %s.", dont_find_anything,
		ceiling(cc.x, cc.y));
	    timepassed = 1;
	    return timepassed;
	}
	...
	if (!underfoot) {
	    if (container_at(cc.x, cc.y, FALSE)) {
		if (mtmp) {
		    No_puedes("saquear nada %sahí mientras %s te obstruya.",
			    prev_inquiry ? "más " : "", mon_nam(mtmp));
		    return timepassed;
		} else {
		    Hay("que estar a una caja para saquearlo.");
		}
	    } else {
		pline("%s %sahí que saquear.", dont_find_anything,
			(prev_inquiry || prev_loot) ? "else " : "");
		return timepassed;
	    }
	}
    } else if (c != 'y' && c != 'n') {
	pline("%s %s que saquear.", dont_find_anything,
		    underfoot ? "aquí" : "ahí");
    }
    return (timepassed);
}
...
boolean *prev_loot;
{
	...
	if (passed_info) *passed_info = 1;
	    Sprintf(qbuf, "¿Quieres quitar la silla %s?",
		al(x_monnam(mtmp, ARTICLE_THE, (char *)0, SUPPRESS_SADDLE, FALSE)));
	if ((c = yn_function(qbuf, es_ynqchars, ynqchars, 'n')) == 'y') {
		if (nolimbs(youmonst.data)) {
		    No_puedes("hacer eso sin brazos ni piernas."); /* not body_part(HAND) */
		    return (0);
		}
		if (otmp->cursed) {
		    pline("No puedes.  La silla parece pegada %s.",
			al(x_monnam(mtmp, ARTICLE_THE, (char *)0,
		...
		otmp = hold_another_object(otmp, "¡Dejas caer %s!", doname(otmp),
					(const char *)0);
		timepassed = rnd(3);
...
register struct obj *obj;
{
	...
	if (!current_container) {
		...
		pline("Debes de bromear.");
		return 0;
	} else if (obj == current_container) {
		pline("Eso sería un ejercicio topological interesante.");
		return 0;
	} else if (obj->owornmask & (W_ARMOR | W_RING | W_AMUL | W_TOOL)) {
		Norep("No puedes %s %s que llevas puesto.",
			Icebox ? "refrigerar" : "almacenar", something);
		return 0;
	} else if ((obj->otyp == LOADSTONE) && obj->cursed) {
		obj->bknown = 1;
	      pline("La%s piedra%s no dejará%s tu persona.",
	 ...
	 */
	    pline("%s no puede confinarse en tales arreos.", The(xname(obj)));
	    return 0;
	} else if (obj->otyp == LEASH && obj->leashmon != 0) {
		pline("%s está junta con tu mascota.", The(xname(obj)));
		return 0;
	} else if (obj == uwep) {
		if (welded(obj)) {
			weldmsg(obj);
	...
	if (obj->otyp == ICE_BOX || Is_box(obj) || obj->otyp == BOULDER ||
		...
		No_puedes("caber %s dentro de %s.", buf,
	...
	if (Icebox && !age_is_relative(obj)) {
		...
		pline("Conforme pones %s al dentro, ¡eres volad%c por una explosión mágica!",
		      doname(obj), poly_gender()==1? 'a' : 'o');
		...
		if (!floor_container)
			...
			panic("in_container:  bolsa no encontrada.");

		losehp(d(6,6),"explosión mágica", KILLED_BY_AN);
		current_container = 0;	/* baggone = TRUE; */
	}

	if (current_container) {
	    Strcpy(buf, the(xname(current_container)));
	    pline("Pones %s en %s.", doname(obj), buf);
...
register struct obj *obj;
{
	...
	if (is_pick(obj) && !obj->unpaid && *u.ushops && shop_keeper(*u.ushops)) {
		...
		verbalize("¡Malcriad%c solapad%c! ¡Váyase de aquí con ese pico!",
			fem, fem);
	}
	...
	prinv(loadlev ?
	      (loadlev < MOD_ENCUMBER ?
	       "Tienes un poco de dificultad sacando" :
	       "Tienes dificultad sacando") : (char *)0,
	      otmp, count);
...
struct obj *item;
{
    ...
    if (item->dknown)
	pline("¡%s %s haber desvanecido!", Doname2(item), otense(item, "parece"));
    else
	pline("¡%s que %s se desaparece!", Blind ? "Noticias" : "Ves", doname(item));
...
struct obj *box;
{
    static NEARDATA const char sc[] = "El Gato de Schroedinger";
    struct obj *deadcat;
	...
	if (!canspotmon(livecat))
	    pline("Piensas que %s te ha rozado %s.", something, body_part(FOOT));
	else
	    pline("¡%s al dentro de la caja aún está viv%c!",
		  Monnam(livecat), mon_gender(livecat)? 'a' : 'o');
	(void) christen_monst(livecat, sc);
    } else {
	const char *mname = Hallucination ? rndmonnam() : "gato";
	...
	pline("!%s al dentro de la caja está muert%c!",
	      The(mname), isfeminine(mname)? 'a' : 'o');
    }
    box->owt = weight(box);
    return;
}
...
register int held;
{
	...
	char qbuf[BUFSZ], emptymsg[BUFSZ], pbuf[QBUFSZ], engpbuf[QBUFSZ];
	long loss = 0L;
	int cnt = 0, used = 0,
	    menu_on_request;
	...
	if (nohands(youmonst.data)) {
		...
		pline("No tienes %s libre.", body_part(HAND));
		return 0;
	}
	if (obj->olocked) {
	    const char *que = xname(obj);
	    pline("%s parece cerrad%c con llave.", The(que), isfeminine(que)? 'a' : 'o');
	    if (held) Hay("que dejarlo para abrirlo con llave.");
	    return 0;
	} else if (obj->otrapped) {
	    if (held) pline("Abres %s...", the(xname(obj)));
	...
	if (loss)	/* magic bag lost some shop goods */
	    pline("Debes %ld %s por %sartículo%s perdido%s.",
		loss, currency(loss), (loss>1)? "un " : "", plur(loss), plur(loss));
	obj->owt = weight(obj);	/* in case any items were lost */

	if (!cnt) {
	    const char *name = Yname2(obj);
	    Sprintf(emptymsg, "%s %sestá vací%c.", name,
		    quantum_cat ? "ya " : "",
		    isfeminine(name)? 'a' : 'o');
	}

	if (cnt || flags.menu_style == MENU_FULL) {
	    Strcpy(qbuf, "¿Quieres sacar algo de ");
	    Sprintf(eos(qbuf), "%s?",
		    safe_qbuf(qbuf, 1, yname(obj), ysimple_name(obj), "esto"));
...
ask_again2:
	...
	if (flags.menu_style != MENU_FULL) {
	    Sprintf(qbuf, "¿Quieres meter %s?", something);
	    Strcpy(pbuf, es_ynqchars);
	    Strcpy(engpbuf, ynqchars);
	...
	if (loot_in) {
		...
		/* traditional code */
		menu_on_request = 0;
		if (query_classes(select, &one_by_one, &allflag, "meter",
				   invent, FALSE,
#ifndef GOLDOBJ
				   (u.ugold != 0L),
#endif
...
boolean put_in;
{
    ...
    char buf[BUFSZ];
    const char *takeout = "Sacar", *putin = "Meter";
	...
	Sprintf(buf,"¿%s qué tipo de objetos?", put_in ? putin : takeout);
	...
	Sprintf(buf,"¿%s qué?", put_in ? putin : takeout);
...
boolean outokay, inokay;
{
    ...
    char buf[BUFSZ];
    int n;
    const char *menuselector = iflags.lootabc ? "abc" : "sma";
    ...
    if (outokay) {
	any.a_int = 1;
	Sprintf(buf,"Sacar %s %s", something, del(the(xname(obj))));
    ...
    if (inokay) {
	any.a_int = 2;
	Sprintf(buf,"Meter %s en %s", something, the(xname(obj)));
	...
	any.a_int = 3;
	add_menu(win, NO_GLYPH, &any, *menuselector, 0, ATR_NONE,
			"Ambas de las cosas arriba", MENU_UNSELECTED);
    }
    end_menu(win, prompt);
