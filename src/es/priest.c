...
char *pname;		/* caller-supplied output buffer */
{
	...
	unsigned i;
	boolean fem = Hallucination? isfeminine(what) : mon_gender(mon);
	char decl = fem? 'a' : 'o';

	Strcpy(pname, mon_article(mon)? "la " : "el ");
...
register int roomno;
{
	...
	if(!temple_occupied(u.urooms0)) {
		...
		if (can_speak) {
		    ...
		    pline("%s entona:",
			canseemon(priest) ? Monnam(priest) : "Una voz cercana");
		    priest->ispriest = save_priest;
		}
		msg2 = 0;
		if(sanctum && Is_sanctum(&u.uz)) {
		    if(priest->mpeaceful) {
			msg1 = "Infiel, ¡habéis entrado el Santuario de Moloch!";
			msg2 = "¡Idos!";
			priest->mpeaceful = 0;
			set_malign(priest);
		    } else
			msg1 = "¡Desecráis este lugar por vuestra presencia!";
		} else {
		    Sprintf(buf, "Peregrin%c, ¡entráis un lugar %s!",
			    flags.female? 'a' : 'o',
			    !shrined ? "desecrado" : "sagrado");
		    msg1 = buf;
		}
		if (can_speak) {
		    verbalize(msg1);
		...
		if(!sanctum) {
			...
			pline("Tienes un sentido%s pavoroso...",
				(!shrined) ? "" : " extraño");
		    else pline("Experimentas un extraño sentido de paz.");
		}
	    } else {
		switch(rn2(3)) {
		  case 0: pline("Tienes un sentido horroroso..."); break;
		  case 1: Te_sientes("como si estuvieras siendo mirad%c.",
				poly_gender()==1? 'a' : 'o'); break;
		  default: pline("Un escalofrío corre través de tu %s.",
		...
		if(!rn2(5)) {
			...
			pline("¡Un fantasma enormo aparece cerca de ti!");
		    else pline("¡Percibes una presencia cercana!");
		    mtmp->mpeaceful = 0;
		    set_malign(mtmp);
		    if(flags.verbose)
...
register struct monst *priest;
{
	...
	if(priest->mflee || (!priest->ispriest && coaligned && strayed)) {
	    pline("¡%s no quiere hacer nada contigo!", Monnam(priest));
	    priest->mpeaceful = 0;
	    return;
	}
	...
	if(!histemple_at(priest,priest->mx,priest->my) ||
	    ...
	    static const char *cranky_msg[4] = {
		"Peregrino, yo no hablaría más con vos."
		"Tendríais palabras, ¿eh?  ¡Os daré una palabra o dos!",
		"¿Hablar?  ¡Aquí está lo que tengo para decir!",
		"Peregrina, yo no hablaría más con vos."
	    };

	    if(!priest->mcanmove || priest->msleeping) {
		pline("¡%s se escapa de su ensueño!", Monnam(priest));
		priest->mfrozen = priest->msleeping = 0;
		priest->mcanmove = 1;
	    }
	    priest->mpeaceful = 0;
	    verbalize(cranky_msg[rn2(3)+(flags.female? 1 : 0)]);
	    return;
	}
	...
	if(priest->mpeaceful && *in_rooms(priest->mx, priest->my, TEMPLE) &&
		  !has_shrine(priest)) {
	    verbalize("¡Idos!  Desecráis este sagrado lugar con vuestra presencia.");
	    priest->mpeaceful = 0;
	    return;
	}
#ifndef GOLDOBJ
	if(!u.ugold) {
		...
		if (priest->mgold > 0L) {
		    ...
		    /*
		     * LENGUA:  A "bit" is an eighth of a US dollar.  The
		     * origin of this expression lies, as it happens, in old
		     * Spanish coinage.  The _peso_ or "piece of eight" was
		     * equal to eight _reales_.  The US dollar, in the
		     * beginning, was pegged to the _peso_, and the _real_
		     * was called a "bit."
		     * Thus "bit" is translated here as _real_.
		     */
		    /* LENGUA:  Better translation for ale than cerveza? */
		    pline("%s te da %s por una cerveza.", Monnam(priest),
			(priest->mgold == 1L) ? "un real" : "dos reales");
		    if (priest->mgold > 1L)
			u.ugold = 2L;
		    else
			u.ugold = 1L;
	...
	if(!money_cnt(invent)) {
		...
		if (pmoney > 0L) {
		    /* Note: two bits is actually 25 cents.  Hmm. */
		    pline("%s te da %s por una cerveza.", Monnam(priest),
			(pmoney == 1L) ? "un real" : "dos reales");
		     money2u(priest, pmoney > 1L ? 2 : 1);
#endif
		} else
		    pline("%s predica las virtudes de la pobreza.", Monnam(priest));
		exercise(A_WIS, TRUE);
	    } else
		pline("%s no está interesad%c.", Monnam(priest),
			mon_gender(priest)? 'a' : 'o');
	    return;
	} else {
	    long offer;

	    pline("%s te pide una contribución para el templo.",
			Monnam(priest));
	    if((offer = bribe(priest)) == 0) {
		verbalize("¡Lamentaréis vuestra acción!");
		if(coaligned) adjalign(-1);
	    } else if(offer < (u.ulevel * 200)) {
#ifndef GOLDOBJ
		if(u.ugold > (offer * 2L)) verbalize("Tacañ%c.", poly_gender()==1? 'a' : 'o');
#else
		if(money_cnt(invent) > (offer * 2L)) verbalize("Tacañ%c.", poly_gender()==1? 'a' : 'o');
#endif
		else {
		    verbalize("Gracias por vuestra contribución.");
		    /*  give player some token  */
		    exercise(A_WIS, TRUE);
		}
	    } else if(offer < (u.ulevel * 400)) {
		verbalize("Sois de hecho una persona pía.");
#ifndef GOLDOBJ
		if(u.ugold < (offer * 2L)) {
#else
		if(money_cnt(invent) < (offer * 2L)) {
			...
			adjalign(1);
		    verbalize("Os confiero una bendición.");
		    incr_itimeout(&HClairvoyant, rn1(500,500));
		}
	    } else if(offer < (u.ulevel * 600) &&
		      u.ublessed < 20 &&
		      (u.ublessed < 9 || !rn2(u.ublessed))) {
		verbalize("Vuestra devoción ha sido recompensada.");
		if (!(HProtection & INTRINSIC))  {
			HProtection |= FROMOUTSIDE;
			if (!u.ublessed)  u.ublessed = rn1(3, 2);
		} else u.ublessed++;
	    } else {
		verbalize("Vuestra magnánima generosidad se aprecia mucho.");
#ifndef GOLDOBJ
...
struct monst *priest;
{
	...
	switch(rn2(3)) {
	case 0:
	    pline("%s ruge de enojo:  \"¡Sufriréis!\"",
			a_gname_at(ax, ay));
	    break;
	case 1:
	    pline("La voz de %s resuena:  \"¡Como os atrevéis a lastimar a mi sirviente!\"",
			a_gname_at(ax, ay));
	    break;
	default:
	    pline("%s ruge:  \"¡Profanáis mi santuario!\"",
			a_gname_at(ax, ay));
	    break;
	}
