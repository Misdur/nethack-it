...
struct obj *obj;	/* item to make known if effect can be seen */
{
	...
	if (petrify) {
	    ...
	    if (flags.verbose) pline("%s está retardándose.", Monnam(mon));
	} else if (adjust > 0 || mon->mspeed == MFAST)
	    pline("%s de repente se mueve %smás rápidamente.", Monnam(mon), howmuch);
	else
	    pline("%s parece mover %smás lentamente.", Monnam(mon), howmuch);
...
outer_break:
	...
	if (!creation) {
		...
		if (old)
		    Sprintf(buf, " se quita %s y", distant_name(old, doname));
		else
		    buf[0] = '\0';
		pline("%s%s se pone %s.", Monnam(mon),
	...
	if (!creation && (unseen ^ !canseemon(mon))) {
		if (mon->minvis && !See_invisible) {
			...
			makeknown(best->otyp);
		} /* else if (!mon->minvis) pline("¡%s aparece de repente!", Amonnam(mon)); */
	}
}
...
boolean polyspot;
{
	...
	if (breakarm(mdat)) {
		...
		if ((Is_dragon_scales(otmp) &&
		    ...
		    pline("¡%s rompe afuera de su armadura!", Monnam(mon));
		else
		...
		if (otmp->oartifact) {
		    if (vis)
			pline("¡%s %s se cae!", The(cloak_simple_name(otmp)),
			      del(mon_nam(mon)));
		    if (polyspot) bypass_obj(otmp);
		    m_lose_armor(mon, otmp);
		} else {
		    if (vis)
			pline("¡%s %s se rasga!", The(cloak_simple_name(otmp)),
			      del(mon_nam(mon)));
		    else
			Oyes("un sonido de rasgadura.");
		    m_useup(mon, otmp);
		}
	    }
#ifdef TOURIST
		...
		if (vis)
		    pline("¡La camisa %s se rasga en trizas!", del(mon_nam(mon)));
		else
		    Oyes("un sonido de rasgadura.");
		m_useup(mon, otmp);
	    }
	    ...
	    if ((otmp = which_armor(mon, W_ARM)) != 0) {
		if (vis)
		    pline("¡La armadura %s se cae a su alrededor!",
				 del(mon_nam(mon)));
		else
		    Oyes("un thud.");
		if (polyspot) bypass_obj(otmp);
		...
		if (vis) {
			...
			pline("¡%s %s se cae, insostenida!",
				The(cloak_simple_name(otmp)),
				del(mon_nam(mon)));
		    else
			pline("¡%s se contrae afuera de su %s!", Monnam(mon),
		...
		if (vis) {
		    if (sliparm(mon->data))
			pline("¡%s se escurre través de su camisa!",
					Monnam(mon));
		    else
			pline("¡%s se vuelve muy demasiado pequeñ%c para su camisa!",
					Monnam(mon), mon_gender(mon)? 'a' : 'o');
		}
		if (polyspot) bypass_obj(otmp);
		m_lose_armor(mon, otmp);
	    }
	...
	if (handless_or_tiny) {
		...
		if (vis)
		    pline("¡%s deja caer sus guantes%s!", Monnam(mon),
					MON_WEP(mon) ? " y arma" : "");
		if (polyspot) bypass_obj(otmp);
		m_lose_armor(mon, otmp);
	    }
		...
		if (vis)
		    pline("¡%s no puede retener más su escudo!", Monnam(mon));
		else
		    Oyes("un clank.");
		if (polyspot) bypass_obj(otmp);
		m_lose_armor(mon, otmp);
	...
	if (handless_or_tiny || has_horns(mdat)) {
		...
		if (vis)
		    pline("¡El yelmo %s se cae %s!",
			  del(mon_nam(mon)), al(surface(mon->mx, mon->my)));
		else
		    Oyes("un clank.");
		if (polyspot) bypass_obj(otmp);
		m_lose_armor(mon, otmp);
	...
	if (handless_or_tiny || slithy(mdat) || mdat->mlet == S_CENTAUR) {
		...
		if (vis) {
		    if (is_whirly(mon->data))
			pline("¡Las botas %s se caen!",
				       del(mon_nam(mon)));
		    else pline("¡Las botas %s son %s%cs afuera de sus pies!",
			del(mon_nam(mon)),
			verysmall(mdat) ? "resbalad" : "empujad",
			mon_gender(mon)? 'a' : 'o');
		}
		if (polyspot) bypass_obj(otmp);
		m_lose_armor(mon, otmp);
	...
	if (!can_saddle(mon)) {
		...
		m_lose_armor(mon, otmp);
		if (vis)
		    pline("La silla %s se cae.", del(mon_nam(mon)));
	    }
	    if (mon == u.usteed)
		goto noride;
	} else if (mon == u.usteed && !can_ride(mon)) {
	noride:
	    No_puedes("cabalgar %s más.", mon_nam(mon));
	    if (touch_petrifies(u.usteed->data) &&
			!Stone_resistance && rnl(3)) {
		char buf[BUFSZ];

		pline("%s.", mon_nam_acc(u.usteed, "Tocas"));
		Sprintf(buf, "caer desde %s",
