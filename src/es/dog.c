...
boolean quietly;
{
	...
	do {
		...
		if (!pm) {
		  if (!quietly)
		    pline("No parece haber nada disponible para un familiar.");
		  break;
		}
	    }
	    ...
	    if (otmp && !mtmp) { /* monster was genocided or square occupied */
	 	if (!quietly)
		   pline("¡La estatuilla se retuerce y se rompe en pedazos!");
		break;
	    }
	} while (!mtmp && --trycnt > 0);
	...
	if (otmp) { /* figurine; resulting monster might not become a pet */
		...
		if (chance == 2) { /* hostile (cursed figurine) */
		    if (!quietly)
			pline("Tienes un mal presentimiento acerca de esto.");
		    mtmp->mpeaceful = 0;
		    set_malign(mtmp);
		}
	    }
...
makedog()
{
	...
	if (!*petname && pettype == PM_LITTLE_DOG) {
	    ...
	    if(Role_if(PM_CAVEMAN)) petname = "Lobo";   /* Conan El Guerrero */
	    if(Role_if(PM_SAMURAI)) petname = "Hachi";     /* Shibuya Station */
	    if(Role_if(PM_BARBARIAN)) petname = "Idefix";  /* Obelix */
	    if(Role_if(PM_RANGER)) petname = "Sirio";     /* Orion's dog */
	}
	...
	switch (xyloc) {
	 ...
	 case MIGR_PORTAL:
		...
		if (t) {
		    ...
		    impossible("mon_arrive: ¿no hay portal correspondente?");
		} /*FALLTHRU*/
	 default:
...
long nmv;		/* number of moves */
{
	...
	if (nmv < 0L) {			/* crash likely... */
	    panic("¿catchup de tiempo futuro?");
	    /*NOTREACHED*/
	    return;
	} else if (nmv == 0L) {		/* safe, but should'nt happen */
	    impossible("¿catchup de ahora?");
	} else
#endif
	...
	if (!mtmp->mtame && mtmp->mleashed) {
	    ...
	    impossible("¿alcanzando a monstruo amarrado?");
...
boolean pets_only;	/* true for ascension or final escape */
{
	...
	for (mtmp = fmon; mtmp; mtmp = mtmp2) {
		...
		if (mtmp->mtame && mtmp->meating) {
			if (canseemon(mtmp))
			    pline("%s aún está comiendo.", Monnam(mtmp));
			stay_behind = TRUE;
			...
			if (canseemon(mtmp))
			    pline("%s aún está atrampad%c.", Monnam(mtmp),
				  mon_gender(mtmp)? 'a' : 'o');
			stay_behind = TRUE;
		}
#ifdef STEED
		...
		if (stay_behind) {
			if (mtmp->mleashed) {
				pline("Su correa se suelta de repente.");
				m_unleash(mtmp, FALSE);
			}
			continue;
		}
		...
		pline("La correa %s se afloja.", del(mon_nam(mtmp)));
		m_unleash(mtmp, FALSE);
	    }
	}
}
...
register struct obj *obj;
{
	...
	if (mtmp->mtame && obj) {
		...
		if (canseemon(mtmp)) {
		    ...
		    pline("%s%s prende %s%s",
			  !big_corpse ? "" : "¡",
			  Monnam(mtmp), the(xname(obj)),
			  !big_corpse ? "." : ", r vice versa!");
		} else if (cansee(mtmp->mx,mtmp->my))
		    pline("%s.", Tobjnam(obj, "se para"));
...
boolean was_dead;
{
	...
	if(!quietly && cansee(mtmp->mx, mtmp->my)) {
		...
		if (haseyes(mtmp->data)) {
			pline("%s %s mirarte en %s.",
				Monnam(mtmp),
				mtmp->mpeaceful ? "parece incapaz de" :
					    "se rehusa a",
				the(body_part(EYE)));
		} else 
			pline("%s evita tu mirada.",
				Monnam(mtmp));
	    }
	}
    } else {
