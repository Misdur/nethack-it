...
const char *name;	/* if null, then format `obj' */
{
	...
	if (!name) {
	    if (!obj) panic("thitu: ¿name & obj ambos null?");
	    name = strcpy(onmbuf,
	    ...
	    if (!strncmpi(name, "el ", 3) ||
		    !strncmpi(name, "la ", 3) ||
		    !strncmpi(name, "los ", 4) ||
		    !strncmpi(name, "las ", 4) ||
		    !strncmpi(name, "un ", 3) ||
		    !strncmpi(name, "una ", 4) ||
		    !strncmpi(name, "unos ", 5) ||
		    !strncmpi(name, "unas ", 5)) kprefix = KILLED_BY;
	}
	...
	if(u.uac + tlev <= rnd(20)) {
		if(Blind || !flags.verbose) pline("Te falla.");
		else pline("Casi te golpea %s.", onm);
		return(0);
	} else {
		if(Blind || !flags.verbose) pline("¡Eres golpead%c!", poly_gender()==1? 'a' : 'o');
		else pline("¡Te golpea %s!", onm);

		if (obj && objects[obj->otyp].oc_material == SILVER
				&& hates_silver(youmonst.data)) {
			dam += rnd(20);
			pline("¡La plata quema tu carne!");
			exercise(A_CON, FALSE);
		}
		if (is_acid && Acid_resistance)
			pline("No parece dañarte.");
		else {
			if (is_acid) pline("¡Te quema!");
...
int x,y;
{
	...
	if (create && !((mtmp = m_at(x, y)) && (mtmp->mtrapped) &&
		...
		if (!objgone) {
			if (!flooreffects(obj,x,y,"cae")) { /* don't double-dip on damage */
...
boolean verbose;  /* give message(s) even when you can't see what happened */
{
	...
	if (tmp < rnd(20)) {
		...
		else if (verbose) pline("Es fallado.");
	    }
		...
		const char *excl0;
		const char *excl1 = exclam(damage);
		const char *mon = Monnam(mtmp);
		excl0 = excl1[0]=='!'? "¡" : excl1[0]=='?'? "¿" : "";
		if (strcmp(mon, "Él") == 0)
		    pline("%sEs golpeado%s", excl0, excl1);
		else
		    pline("%s%s es golpead%c%s", excl0, Monnam(mtmp),
			    mon_gender(mtmp)? 'a' : 'o', excl1);
	    }

	    if (otmp->opoisoned && is_poisonable(otmp)) {
		if (resists_poison(mtmp)) {
		    if (vis) pline("El veneno no parece %s.",
				   mon_nam_acc(mtmp, "afectar"));
		} else {
		    if (rn2(30)) {
			damage += rnd(6);
		    } else {
			if (vis) pline("El veneno era mortal...");
			damage = mtmp->mhp;
		    }
		}
	    }
		...
		if (vis) pline("¡La plata quema la carne %s!",
				del(mon_nam(mtmp)));
		else if (verbose) pline("¡Se quema su carne!");
	    }
	    if (otmp->otyp == ACID_VENOM && cansee(mtmp->mx,mtmp->my)) {
		if (resists_acid(mtmp)) {
		    if (vis || verbose)
			pline("No se afecta %s.", Monnam(mtmp));
		    damage = 0;
		} else {
		    if (vis) pline("¡El ácido %s!", mon_nam_acc(mtmp, "quema"));
		    else if (verbose) pline("¡Se quema!");
		}
	    }
	    mtmp->mhp -= damage;
	    if (mtmp->mhp < 1) {
	...
	register struct obj *obj;
{
	...
	if (singleobj->cursed && (dx || dy) && !rn2(7)) {
		...
		if(is_ammo(singleobj))
		    pline("¡%s maldispara!", Monnam(mon));
		else
		    pline("¡%s como %s l%c tira!",
			  Tobjnam(singleobj, "se resbala"), mon_nam(mon),
			  isfeminine(xname(singleobj))? 'a' : 'o');
	    }
	    dx = rn2(3)-1;
	    dy = rn2(3)-1;
	...
	while(range-- > 0) { /* Actually the loop is always exited by break */
		...
		if ((mtmp = m_at(bhitpos.x, bhitpos.y)) != 0) {
			...
			if (singleobj->otyp > LAST_GEM) {
			    pline("Ases %s.", the(xname(singleobj)));
			    pline("No te interesa la basura %s.",
				del(mon_nam(mon)));
			    makeknown(singleobj->otyp);
			    dropy(singleobj);
			} else {
			    pline("Aceptas el regalo %s en el ánimo de que era intentado.",
				del(mon_nam(mon)));
			    (void)hold_another_object(singleobj,
				"Ases, pero dejas caer, %s.", xname(singleobj),
				"Ases:");
			}
			break;
		    }
			...
			case EGG:
				...
				impossible("monstruo tirando tipo de huevo %d",
					singleobj->corpsenm);
				hitu = 0;
				break;
			    }
			...
			if(singleobj->otyp == CREAM_PIE) {
			    if(!Blind) pline("!Yecch!  Lo tienes sobre tod%c tu %s.",
				       isfeminine(body_part(FACE))? 'a' : 'o',
				       body_part(FACE));
			    else Hay("%s pegajoso sobre tod%c tu %s.",
				       something,
				       isfeminine(body_part(FACE))? 'a' : 'o',
				       body_part(FACE));
			    ...
			    /* venom in the eyes */
			    if(!Blind) pline("El veneno te ciega.");
			    else pline("Tu%s %s se escoce%s.",
				      (num_eyes == 1) ? "" : "s",
...
register struct attack *mattk;
{
	...
	if(mtmp->mcan) {
	    ...
	    if(flags.soundok)
		pline("Un traqueteo seco viene de la garganta %s.",
		                      del(mon_nam(mtmp)));
	    return 0;
	}
	if(lined_up(mtmp)) {
		switch (mattk->adtyp) {
			...
			break;
		    default:
			impossible("malo tipo de ataque en spitmu");
				/* fall through */
		    case AD_ACID:
		...
		if(!rn2(BOLT_LIM-distmin(mtmp->mx,mtmp->my,mtmp->mux,mtmp->muy))) {
		    if (canseemon(mtmp))
			pline("¡%s escupe veneno!", Monnam(mtmp));
		    m_throw(mtmp, mtmp->mx, mtmp->my, sgn(tbx), sgn(tby),
			distmin(mtmp->mx,mtmp->my,mtmp->mux,mtmp->muy), otmp);
		    nomul(0);
		    return 0;
	...
	register struct attack  *mattk;
{
	...
	if(lined_up(mtmp)) {
		...
		} else impossible("Arma de respiro %d usada", typ-1);
	    }
	}
	return(1);
}
...
int whodidit;	/* 1==hero, 0=other, -1==just check whether it'll pass thru */
{
    ...
    /* LENGUA:   need translations for the onomatopoeia */
    if (hits && whodidit != -1) {
