 ...
 */
	switch(ptr->mlet) {
		...
		if(is_mercenary(ptr)) {
			...
			default: impossible("¿mercenario extraño %d?", monsndx(ptr));
				mac = 0;
				break;
		    }
...
boolean ghostly;
{
	...
	if ((int) mvitals[mndx].born >= lim && !(mons[mndx].geno & G_NOGEN) &&
		...
		if (wizard) pline("Automáticamente se extinguieron %s.",
...
register int	mmflags;
{
	...
	if(ptr){
		...
		if (wizard && (mvitals[mndx].mvflags & G_EXTINCT))
		    pline("Explícitamente creando monstruo extinto %s.",
			mons[mndx].mname);
#endif
	} else {
...
#ifdef DEBUG
		pline("rndmonst: ¡no mons communes!");
#endif
 ...
 */
		...
		if (ct < 0 || ct > 127)
		    panic("rndmonst: cuenta mala [#%d: %d]", mndx, ct);
 ...
 */
	...
	if (mndx == SPECIAL_PM || uncommon(mndx)) {	/* shouldn't happen */
	    impossible("rndmonst: mal `mndx' [#%d]", mndx);
	    return (struct permonst *)0;
	}
	return &mons[mndx];
}
...
int	spc;
{
	...
	if(class < 1 || class >= MAXMCLASSES) {
	    impossible("¡mkclass llamada con mala clase!");
	    return((struct permonst *) 0);
	}
...
struct monst *mtmp, *victim;
{
	...
	if ((int)++mtmp->m_lev >= mons[newtype].mlevel && newtype != oldtype) {
		...
		if (sensemon(mtmp))
		    pline("Como %s se crece a %s, ¡se %s!", mon_nam(mtmp),
			an(ptr->mname),
			nonliving(ptr) ? "expira" : "muere");
...
struct obj *bag;
{
	...
	impossible("mala bolsa de trucos");
    } else if (bag->spe < 1) {
