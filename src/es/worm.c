    ...
    struct obj *weap;
{
	...
	curr = curr->nseg;
	if (!curr) {
	    impossible("cut_worm:  no segmento a (%d,%d)", (int) x, (int) y);
	    return;
	}
    }
	...
	if (flags.mon_moving)
	    pline("Parte de la cola %s está cortada.", del(mon_nam(worm)));
	else
	    pline("Cortas una parte de la cola desde %s.", mon_nam(worm));
    ...
    if (flags.mon_moving)
	pline("%s está cortad%c en mitad.", Monnam(worm),
	      mon_gender(worm)? 'a' : 'o');
    else
	pline("Cortas %s en mitad.", mon_nam(worm));
}
...
/*  if (!wnum) return;  bullet proofing */
	...
	impossible("place_worm_tail_randomly: ¡wormno existe sin cola!");
	return;
    }
