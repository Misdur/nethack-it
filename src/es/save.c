...
dosave0()
{
	...
	HUP if (iflags.window_inited) {
		...
		(void) close(fd);
		clear_nhwindow(WIN_MESSAGE);
		pline("Parece que hay archivo viejo de guardar.");
		if (yn("¿Sobreescribir el archivo viejo?") == 'n') {
		    compress(fq_save);
		    return 0;
		}
	    }
	...
	fd = create_savefile();
	if(fd < 0) {
		HUP pline("No puedo abrir el archivo de guardar.");
	...
	if (iflags.checkspace) {
	    ...
	    if (needed > fds) {
		HUP {
		    pline("No hay bastante espacio en el disco de guardar (SAVE).");
		    pline("Se necesita %ld bytes pero hay sólo %ld.", needed, fds);
		}
		flushout();
		(void) close(fd);
...
savestateinlock()
{
	...
	if (flags.ins_chkpt || havestate) {
		...
		if (fd < 0) {
		    pline("%s", whynot);
		    pline("Tal vez alguien lo haya quitado.");
		    killer = whynot;
		    done(TRICKED);
		    return;
		}
		...
		if (hackpid != hpid) {
		    Sprintf(whynot,
			    "¡Pid de nivel #0 (%d) no iguala la nuestra (%d)!",
			    hpid, hackpid);
		    pline("%s", whynot);
		    killer = whynot;
		    done(TRICKED);
...
int mode;
{
	...
	if(fd < 0) panic("¡Guardar en archivo malo!");	/* impossible */
#ifdef MFLOPPY
	count_only = (mode & COUNT_SAVE);
#endif
...
int fd;
{
    if (outbufp) {
	outbufp = 0;
	panic("cerrando archivo con dados aún no escritos");
    }
    outrunlength = -1;
    compressing = FALSE;
    return;
...
#endif
		panic("No puedo escribir %u bytes al archivo #%d", num, fd);
	}
    } else {
	bwritefd = fd;
	for (; num; num--, bp++) {
...
#ifdef UNIX
    if(bw_fd >= 0)
	panic("buffers dobles inesperados");
    bw_fd = fd;
    if((bw_FILE = fdopen(fd, "w")) == 0)
	panic("buffer de archivo %d falló", fd);
#endif
    buffering = TRUE;
}
    ...
    int fd;
{
	...
	if(fflush(bw_FILE) == EOF)
	    panic("¡flujo de archivo de guardar falló!");
    }
#endif
    return;
}
...
register unsigned num;
{
	...
	if (buffering) {
	    if(fd != bw_fd)
		panic("escribir sin buffer a fd %d (!= %d)", fd, bw_fd);
...
#endif
		panic("No puedo escribir %u bytes a archivo #%d", num, fd);
	}
}
...
register struct obj *otmp;
{
	...
	while(otmp) {
	    ...
	    if (perform_bwrite(mode)) {
		if (otmp->oartifact) {
		    /* Save an artifact under the English name */
		    struct obj *otmp3;
		    unsigned int xl2;
		    const char *en_name;
		    unsigned int namelth;

		    en_name = artiname_en(otmp->oartifact);
		    namelth = strlen(en_name) + 1;
		    xl2 = otmp->oxlth + namelth;
		    otmp3 = newobj(xl2);
		    memcpy((genericptr_t)otmp3, (genericptr_t)otmp,
			   otmp->oxlth + sizeof(struct obj));
		    otmp3->onamelth = (uchar)namelth;
		    strcpy(ONAME(otmp3), en_name);
		    bwrite(fd, (genericptr_t) &xl2, sizeof(int));
		    bwrite(fd, (genericptr_t) otmp3, xl2 + sizeof(struct obj));

		    free((genericptr_t) otmp3);
		}
		else {
		    /* Original object-save procedure */
...
# ifdef TOS
	...
	if (_copyfile(from, to))
		panic("No puedo copiar %s a %s", from, to);
# else
