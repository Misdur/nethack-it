...
STATIC_VAR NEARDATA const char * const odd_skill_names[] = {
    "no destreza",
    "manos desnudas",		/* use barehands_or_martial[] instead */
    "combate de dos armas",
    "cabalgar",
    "armas de pola",
    "sable",
    "martillo",
    ...
    "hechizos de divinación",
    "hechizos de encantamienta",
    "hechizos clericales",
    "hechizos de escapar",
    "hechizos de materia",
};
...
STATIC_VAR NEARDATA const char * const barehands_or_martial[] = {
    "combate de manos desnudas", "artes marciales"
};
...
int skill;
{
	Te_sientes("más confidente en tus destrezas%s.",
		skill == P_NONE ?
			"" :
		skill <= P_LAST_WEAPON ?
			" de armas" :
...
boolean polyspot;
{
	...
	if (!attacktype(mon->data, AT_WEAP)) {
		...
		if (cansee(mon->mx, mon->my)) {
			pline("%s deja caer %s.", Monnam(mon),
				distant_name(obj, doname));
		    newsym(mon->mx, mon->my);
		}
		/* might be dropping object into water or lava */
		if (!flooreffects(obj, mon->mx, mon->my, "deja caer")) {
...
register struct monst *mon;
{
	...
	switch(mon->weapon_check) {
		...
		case NEED_PICK_OR_AXE:
			...
			break;
		default: impossible("¿weapon_check %d para %s?",
	...
	if (obj && obj != &zeroobj) {
		...
		if (mw_tmp && mw_tmp->cursed && mw_tmp->otyp != CORPSE) {
			...
			Sprintf(welded_buf, "%s soldad@%s a su%s %s",
				otense(mw_tmp, "está"),
				plur(mw_tmp->quan),
				bimanual(mw_tmp) ? "s" : "",
				mon_hand);
			...
			if (obj->otyp == PICK_AXE) {
			    ...
			    *strchr(welded_buf, '@') = 'a';
			    pline("Porque %s arma%s %s %s,",
				  mw_tmp->quan>1L? "las" : "el",
				  plur(mw_tmp->quan),
				  del(mon_nam(mon)),
				  welded_buf);
			    pline("%s no puede blandir aquel%s %s.",
				Monnam(mon), isfeminine(que)? "la" : "", que);
			} else {
			    const char *que;
			    que = doname(obj);
			    pline("%s trata de blandir %s.", Monnam(mon), que);
			    que = xname(mw_tmp);
			    *strchr(welded_buf, '@') = isfeminine(que)? 'a' : 'o';
			    pline("¡%s %s %s!",
				  The(que), del(mon_nam(mon)), welded_buf);
			}
			mw_tmp->bknown = 1;
		    }
		    mon->weapon_check = NO_WEAPON_WANTED;
		...
		mon->weapon_check = NEED_WEAPON;
		if (canseemon(mon)) {
		    pline("¡%s blande %s!", Monnam(mon), doname(obj));
		    if (obj->cursed && obj->otyp != CORPSE) {
			pline("¡%s %s %s!",
			    Tobjnam(obj, "se suelda"),
			    al(the(mbodypart(mon,HAND))), del(mon_nam(mon)));
			obj->bknown = 1;
		    }
		}
		if (artifact_light(obj) && !obj->lamplit) {
		    ...
		    if (canseemon(mon))
			pline("¡%s fuerte en %s %s!",
			    Tobjnam(obj, "brilla"), 
			    the(mbodypart(mon,HAND)), del(mon_nam(mon)));
		}
		obj->owornmask = W_WEP;
		return 1;
	}
...
char *buf;
{
	...
	case P_UNSKILLED:    ptr = "Inhábil";   break;
	case P_BASIC:	     ptr = "Básic@";    break;
	case P_SKILLED:	     ptr = "Hábil";     break;
	case P_EXPERT:	     ptr = "Expert@";   break;
	/* these are for unarmed combat/martial arts only */
	case P_MASTER:	     ptr = "Maestr@";   break;
	case P_GRAND_MASTER: ptr = "Gran Maestr@"; break;
	default:	     ptr = "Desconocido"; break;
    }
    Strcpy(buf, ptr);
    return buf;
}
...
int skill;
{
    ...
    pline("Eres ahora %smás hábil en %s.",
    	P_SKILL(skill) >= P_MAX_SKILL(skill) ? (flags.female? "la " : "el ") : "",
	P_NAME(skill));
}
...
const static struct skill_range {
    ...
    { P_FIRST_H_TO_H, P_LAST_H_TO_H, "Destrezas de Combate" },
    { P_FIRST_WEAPON, P_LAST_WEAPON, "Destrezas de Armas" },
    { P_FIRST_SPELL,  P_LAST_SPELL,  "Destrezas de Hechizos" },
};
...
enhance_weapon_skill()
{
...
#ifdef WIZARD
	if (wizard && yn("¿Avanzar destrezas sin practicar?") == 'y')
	    speedy = TRUE;
#endif
	...
	do {
		...
		if (eventually_advance > 0) {
		    Sprintf(buf,
			    "(Destreza%s marcada%s por \"*\" puede%s avanzarse %s.)",
			    plur(eventually_advance),
			    plur(eventually_advance),
			    eventually_advance>1L? "n" : "",
			    (u.ulevel < MAXULEV) ?
				"cuando tengas más experiencia" :
				"si aberturas de destreza se ponen disponibles");
		    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE,
			     buf, MENU_UNSELECTED);
		}
		if (maxxed_cnt > 0) {
		    Sprintf(buf,
		  "(Destreza%s marcada%s por \"#\" no puede%s avanzarse más.)",
			    plur(maxxed_cnt),
			    plur(maxxed_cnt),
			    maxxed_cnt>1L? "n" : "");
		    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE,
	  ...
	  for (pass = 0; pass < SIZE(skill_ranges); pass++)
		...
		if ((gen = strchr(sklnambuf, '@')) != NULL)
		    *gen = poly_gender()==1? 'a' : 'o';
#ifdef WIZARD
		if (wizard) {
	    ...
	    Strcpy(buf, (to_advance > 0) ? "Escoge una destreza para avanzar:" :
					   "Destrezas actuales:");
#ifdef WIZARD
	    if (wizard && !speedy)
		Sprintf(eos(buf), "  (%d abertura%s disponible%s)",
		...
		for (n = i = 0; i < P_NUM_SKILLS; i++) {
		    if (can_advance(i, speedy)) {
			if (!speedy) Te_sientes("que podrás ser más peligros%c!",
				poly_gender()==1? 'a' : 'o');
			n++;
			break;
		    }
...
struct obj *weapon;
{
    ...
    static const char bad_skill[] = "weapon_hit_bonus: mala destreza %d";
...
struct obj *weapon;
{
	...
	switch (P_SKILL(type)) {
	    default: impossible("weapon_dam_bonus: mala destreza %d",P_SKILL(type));
		     /* fall through */
	    case P_ISRESTRICTED:
	    case P_UNSKILLED:	bonus = -2; break;
	    case P_BASIC:	bonus =  0; break;
...
register struct obj *obj;
{
	...
	end_burn(obj, FALSE);
	if (canseemon(mon))
	    pline("%s en %s %s %s de brillar.", The(xname(obj)),
		  the(mbodypart(mon,HAND)), del(mon_nam(mon)),
		  otense(obj, "deja"));
    }
    obj->owornmask &= ~W_WEP;
