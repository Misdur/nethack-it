...
/* LENGUA:  These are replaced by isalpha2, toupper2 and tolower2 */
#if 0
boolean
    ...
    const char *s;
{
    ...
    if (!strcmpi2(s, "él") || !strcmpi2(s, "ella"))
	Strcpy(buf, isupper2(s[0])? "Suyo" : "suyo");
    else
	Sprintf(buf, "de %s", s);
    return buf;
}
...
#ifdef OVL2
/* LENGUA:  This will probably disappear.  It's not the number but the gender
   of the modified noun that determines the suffix. */
const char *
    ...
    int  y;
{
    ...
    if (y == 0)
	panic("división por cero en rounddiv");
    else if (y < 0) {
	divsgn = -divsgn;  y = -y;
    }
    if (x < 0) {
...
pmatch_top:
    ...
    s = deaccent[(unsigned char)*strng++];
    p = deaccent[(unsigned char)*patrn++];
    if (!p)			/* end of pattern */
...
#ifdef OVL2
/* LENGUA: replaced by strncmpi2; see ctype2.c */
#ifndef STRNCMPI
int
...
# if 0
    ...
    assert( &tolower2 != 0 );			/* can't be unsafe macro */
# endif
