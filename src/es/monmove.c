...
register struct monst *mtmp;
{
	if (flags.verbose) {
		...
		pline("¡¡BUM!!  Ves una puerta estallando.");
	   ...
	   mtmp->mcansee && m_canseeu(mtmp) && !rn2(3)) {
		...
		if(couldsee(mtmp->mx, mtmp->my)) {
		  ...
		  if(levl[x][y].looted & D_WARNED) {
			verbalize("¡Párese, ladr%s!  ¡Usted está bajo aresto!",
				flags.female? "ona" : "ón");
			(void) angry_guards(!(flags.soundok));
		  } else {
			verbalize("¡Hey, deje de picar esa cerradura!");
...
boolean fleemsg;
{
	if (u.ustuck == mtmp) {
		...
		pline("¡Eres liberad%c!", poly_gender()==1? 'a' : 'o');
	    }
	}
	...
	if (!first || !mtmp->mflee) {
		...
		pline("¡%s se vuelve para huir!", (Monnam(mtmp)));
	    mtmp->mflee = 1;
	}
}
...
/*	Pre-movement adjustments	*/
	...
	if(nearby && mdat->msound == MS_BRIBE &&
	   mtmp->mpeaceful && !mtmp->mtame && !u.uswallow) {
		if (mtmp->mux != u.ux || mtmp->muy != u.uy) {
			pline("%s cuchichea al aire.",
			    cansee(mtmp->mux, mtmp->muy) ? Monnam(mtmp) : "Él");
			...
			if (is_demon(youmonst.data)) {
			  /* "Good hunting, brother" */
			    ...
			    pline("¡%s se pone enojad%c!", Amonnam(mtmp),
					mon_gender(mtmp)? 'a' : 'o');
			    mtmp->mpeaceful = 0;
	...
	if (mdat == &mons[PM_WATCHMAN] || mdat == &mons[PM_WATCH_CAPTAIN])
		...
		if (canseemon(mtmp))
			pline("%s concentra.", Monnam(mtmp));
		if (distu(mtmp->mx, mtmp->my) > BOLT_LIM * BOLT_LIM) {
			pline("Percibes una onda leve de energía psíquica.");
			goto toofar;
		}
		pline("¡Una onda de energía psíquica te inunda!");
		if (mtmp->mpeaceful &&
		    (!Conflict || resist(mtmp, RING_CLASS, 0, 0)))
			pline("Siente bastante calmante.");
		else {
			register boolean m_sen = sensemon(mtmp);

			if (m_sen || (Blind_telepat && rn2(2)) || !rn2(10)) {
				int dmg;
				pline("¡Se agarra a tu %s!",
					m_sen ? "telepatía" :
					Blind_telepat ? "telepatía latente" : "mente");
				dmg = rnd(15);
				if (Half_spell_damage) dmg = (dmg+1) / 2;
				losehp(dmg, "ráfaga psíquica", KILLED_BY_AN);
			}
		}
		for(m2=fmon; m2; m2 = nmon) {
			...
			if ((telepathic(m2->data) &&
				...
				if (cansee(m2->mx, m2->my))
				    pline("Se agarra %s.", al(mon_nam(m2)));
				m2->mhp -= rnd(15);
				if (m2->mhp <= 0)
...
register struct monst *mtmp;
{
	if (sticks(youmonst.data) && mtmp==u.ustuck && !u.uswallow) {
		pline("¡%s no puede escaparse de ti!", Monnam(mtmp));
		return(TRUE);
	}
	return(FALSE);
}
	...
	if(mmoved == 1 || mmoved == 3) {
		...
		if(IS_DOOR(levl[mtmp->mx][mtmp->my].typ)
			...
			if (flags.verbose && canseemon(mtmp))
			    pline("%s %s bajo la puerta.", Monnam(mtmp),
			...
			if(btrapped) {
			    ...
			    if (flags.verbose) {
				if (canseeit)
				   pline("Ves una puerta destrabándose y abriéndose.");
				else if (flags.soundok)
				   Oyes("una puerta destrabándose y abriéndose.");
			    }
			...
			if(btrapped) {
			    ...
			    if (flags.verbose) {
				if (canseeit)
				     pline("Ves una puerta abriéndose.");
				else if (flags.soundok)
				     Oyes("una puerta abriéndose.");
			    }
			    here->doormask = D_ISOPEN;
			...
			if(btrapped) {
			    ...
			    if (flags.verbose) {
				if (canseeit)
				    pline("Ves una puerta chocándose abierta.");
				else if (flags.soundok)
				    Oyes("una puerta chocándose abierta.");
			    }
			...
			if (flags.verbose && canseemon(mtmp))
			    Norep("%s %s %s los barros de hierro.", Monnam(mtmp),
				  locomotion(ptr, "pasa"),
				  passes_walls(ptr) ? "través de" : "entre");
		}
