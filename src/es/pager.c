    ...
    char *buf, *monbuf;
{
	...
	if (mtmp != (struct monst *) 0) {
	    ...
	    Sprintf(buf, "%s%s%s",
		    ...
		    (mtmp->mpeaceful && accurate) ? (mon_gender(mtmp)? "pacífica " : "pacífico ") : "",
		    name);
	    if (u.ustuck == mtmp)
		Strcat(buf, (Upolyd && sticks(youmonst.data)) ?
			(mon_gender(mtmp)? ", detenida" : ", detenido") : ", deteniéndote");
	    if (mtmp->mleashed)
		...
		if (ways_seen > 1 || !normal) {
			...
			Strcat(monbuf, "infravisión");
			if (ways_seen-- > 1) Strcat(monbuf, ", ");
		    }
		    if (tp_sensemon(mtmp)) {
			Strcat(monbuf, "telepatía");
			if (ways_seen-- > 1) Strcat(monbuf, ", ");
		    }
		    if (useemon && xraydist > 0 &&
			    distu(mtmp->mx, mtmp->my) <= xraydist) {
			/* Eyes of the Overworld */
			Strcat(monbuf, "visión astral");
			if (ways_seen-- > 1) Strcat(monbuf, ", ");
		    }
		    if (Detect_monsters) {
		    ...
		    if (MATCH_WARN_OF_MON(mtmp)) {
		    	char wbuf[BUFSZ];
			if (Hallucination)
				Strcat(monbuf, "delusión paranoica");
			else {
	...
	break;
    case S_cloud:
	Strcpy(buf, Is_airlevel(&u.uz) ? "área nubiosa" : "nube de niebla/vapor");
	break;
    default:
    ...
    boolean user_typed_name, without_asking;
{
	...
	dbase_str += 7;
    else if (!strncmp(dbase_str, "pacífico ", 9))
	dbase_str += 9;
    else if (!strncmp(dbase_str, "pacífica ", 9))
	dbase_str += 9;
...
bad_data_file:	impossible("el archivo 'data' en formato malo");
	...
	if (user_typed_name || without_asking || yn("¿Más info?") == 'y') {
	    winid datawin;
	...
	pline("No tengo ninguna información sobre estas cosas.");
    ...
    boolean quick;	/* use cursor && don't search for "more info" */
{
	...
	getlin("¿Especificar qué? (escribe la palabra)", out_str);
	...
	if ((from_screen ?
	    ...
	    found += append_str(out_str, "tú");	/* tack on "or you" */
...
#define is_cmap_drawbridge(i) ((i) >= S_vodbridge && (i) <= S_hcdbridge)
	...
	if (found) {
	    ...
	    pline("Nunca he oído nada de tales cosas.");
	}
...
doidtrap()
{
	...
	pline("No puedo ver ninguna trampa allí.");
	return 0;
}
...
dowhatdoes()
{
	...
	introff();
#endif
	q = yn_function("¿Qué comando?", (char *)0, (char *)0, '\0');
#if defined(UNIX) || defined(VMS)
	intron();
#endif
	...
	int *sel;
{
	...
	end_menu(tmpwin, "Elige un artículo:");
