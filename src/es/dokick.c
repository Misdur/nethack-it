...
static const char kick_passes_thru[] = "patada pasa a través sin hacer daño";
...
register boolean clumsy;
{
	...
	if (mon->mhp > 0 && martial() && !bigmonst(mon->data) && !rn2(3) &&
		...
		if(goodpos(mdx, mdy, mon, 0)) {
			pline("%s se tambalea por el golpe.", Monnam(mon));
...
register xchar x, y;
{
	...
	if (Upolyd && attacktype(youmonst.data, AT_KICK)) {
		...
		if (mon->data == &mons[PM_SHADE] &&
		    ...
		    /* doesn't matter whether it would have hit or missed,
		       and shades have no passive counterattack */
		    pline("Tu %s %s.", kick_passes_thru, al(mon_nam(mon)));
	...
	if(Levitation && !rn2(3) && verysmall(mon->data) &&
	   !is_flyer(mon->data)) {
		pline("¡Flotando en el aire, fallas por mucho!");
	...
	if(i < (j*3)/10) {
		if(!rn2((i < j/10) ? 2 : (i < j/5) ? 3 : 4)) {
			...
			pline("Tu torpe patada no hace ningún daño.");
			(void) passive(mon, FALSE, 1, AT_KICK);
			return;
		}
	...
	if(!rn2(clumsy ? 3 : 4) && (clumsy || !bigmonst(mon->data)) &&
	   ...
	   mon->data->mmove >= 12) {
		if(!nohands(mon->data) && !rn2(martial() ? 5 : 3)) {
		    pline("%s detiene tu %spatada.", Monnam(mon),
				clumsy ? "torpe " : "");
		    (void) passive(mon, FALSE, 1, AT_KICK);
		    return;
		} else {
			...
			pline("%s %s, evitando %s tu %spatea.", Monnam(mon),
				(can_teleport(mon->data) ? "se teleporta" :
				 is_floater(mon->data) ? "flota" :
				 is_flyer(mon->data) ? "vuela" :
				 (nolimbs(mon->data) || slithy(mon->data)) ?
					"se desliza" : "salta"),
				clumsy ? "fácilmente" : "ágilmente",
				clumsy ? "torpe " : "");
			(void) passive(mon, FALSE, 1, AT_KICK);
			return;
		    }
...
register struct obj *gold;
{
	...
	if(!likes_gold(mtmp->data) && !mtmp->isshk && !mtmp->ispriest
		...
		if (canseemon(mtmp)) {
		    pline("%s %s sin daño.", The(xname(gold)),
			      mon_nam_acc(mtmp, otense(gold, "golpea")));
		    msg_given = TRUE;
		}
	} else {
#ifdef GOLDOBJ
		...
		if (cansee(mtmp->mx, mtmp->my))
		    pline("%s atrapa el oro.", Monnam(mtmp));
#ifndef GOLDOBJ
		...
		if (mtmp->isshk) {
			...
			if (robbed) {
				...
				pline("La suma cubre%s sus pérdidas recientes.",
				      !robbed ? "" : " en parte");
...
#endif
				    pline("Tienes %ld %s de crédito.",
					  ESHK(mtmp)->credit,
					  currency(ESHK(mtmp)->credit));
				} else verbalize("¡Gracias, canalla!");
			}
		} else if (mtmp->ispriest) {
			if (mtmp->mpeaceful)
			    verbalize("Gracias por su contribución.");
			else verbalize("¡Gracias, canalla!");
			...
			if (goldreqd) {
		     ...
		     if (mtmp->mpeaceful)
			    verbalize("Es suficiente.  ¡Váyase!");
		     else verbalize("¡No es suficiente, cobarde!");
		}
...
struct obj *obj;
{
	...
	if (costly && loss) {
	    if (!insider) {
		pline("¡Causaste %ld %s de daño!", loss, currency(loss));
		make_angry_shk(shkp, x, y);
	    } else {
		pline("Debes %s %ld %s por objetos destruidos.",
		    al(mon_nam(shkp)), loss, currency(loss));
	    }
	}
}
...
xchar x, y;
{
	...
	if ((trap = t_at(x,y)) != 0 &&
		...
		/* LENGUA:  "tizzy" (jerga) -> ? */
		No_puedes("patear %s que está en %s!", something,
			 Hallucination ? "una tizzy" :
			 (trap->ttyp == WEB) ? "una telaraña" : "un hoyo");
		return 1;
	}

	if(Fumbling && !rn2(3)) {
		pline("Tu torpe patada erró.");
		return(1);
	}
	...
	if(kickobj->otyp == CORPSE && touch_petrifies(&mons[kickobj->corpsenm])
	    ...
	    pline("Pateas %s con tus %s desnud%cs.",
		  the(corpse_xname(kickobj, TRUE)),
		  makeplural(pie),
		  isfeminine(pie)? 'a' : 'o');
		...
		Sprintf(kbuf, "patear %s sin botas",
	...
	if (IS_ROCK(levl[x][y].typ) || closed_door(x, y)) {
	    ...
	    if (!flooreffects(kickobj, u.ux, u.uy, "cae")) {
	...
	if(Is_box(kickobj)) {
		...
		if (kickobj->olocked) {
			...
			pline("¡Abres la cerradura de golpe!");
			kickobj->olocked = 0;
			kickobj->obroken = 1;
			...
			pline("La tapa se vuela abierta, y entonces cae cerrada.");
			if (otrp) (void) chest_trap(kickobj, LEG, FALSE);
			return(1);
		    }
		}
	...
	if (slide && !Blind) {
	    pline("¡Whee!  %s se %s a través %s.", Doname2(kickobj),
		otense(kickobj, "desliza"),
                del(surface(x, y)));
	}
	...
	if(flooreffects(kickobj,bhitpos.x,bhitpos.y,"cae")) return(1);
...
char *buf;
{
	...
	else if (IS_DOOR(maploc->typ)) what = "una puerta";
	else if (IS_TREE(maploc->typ)) what = "un árbol";
	else if (IS_STWALL(maploc->typ)) what = "una pared";
	else if (IS_ROCK(maploc->typ)) what = "una roca";
	else if (IS_THRONE(maploc->typ)) what = "un trono";
	else if (IS_FOUNTAIN(maploc->typ)) what = "una fuente";
	else if (IS_GRAVE(maploc->typ)) what = "una lápida";
#ifdef SINKS
	else if (IS_SINK(maploc->typ)) what = "un fregadero";
#endif
	else if (IS_ALTAR(maploc->typ)) what = "un altar";
	else if (IS_DRAWBRIDGE(maploc->typ)) what = "una puente levadizo";
	else if (maploc->typ == STAIRS) what = "la escalera";
	else if (maploc->typ == LADDER) what = "una escalera de mano";
	else if (maploc->typ == IRONBARS) what = "una barra de hierro";
	else what = "algo extraño";
	return strcat(strcpy(buf, "patear "), what);
}
...
dokick()
{
	...
	if (nolimbs(youmonst.data) || slithy(youmonst.data)) {
		pline("No tienes piernas con las que patear.");
		no_kick = TRUE;
	} else if (verysmall(youmonst.data)) {
		pline("Eres demasiado pequeñ%c para pegar patadas.",
			poly_gender()==1? 'a' : 'o');
		no_kick = TRUE;
#ifdef STEED
	} else if (u.usteed) {
		if (yn_function("¿Patear tu corcel?", es_ynchars, ynchars, 'y') == 'y') {
		    pline("%s.", mon_nam_acc(u.usteed, "Pateas"));
		    kick_steed();
		    return 1;
		} else {
		    return 0;
		...
		pline("Tu%s %s%s no está%s en ninguna condición para patear.",
             wl == BOTH_SIDES? "s" : "",
		     bp,
		     feminize((wl == LEFT_SIDE) ? " izquierdo" :
			(wl == RIGHT_SIDE) ? " derecho" : "", poly_gender()==1),
		     (wl == BOTH_SIDES) ? "n" : "");
		no_kick = TRUE;
	} else if (near_capacity() > SLT_ENCUMBER) {
		pline("Tu carga es demasiado pesada para balancearte para patear.");
		no_kick = TRUE;
	} else if (youmonst.data->mlet == S_LIZARD) {
		pline("Tus piernas no pueden patear efectivamente.");
		no_kick = TRUE;
	} else if (u.uinwater && !rn2(2)) {
		pline("Tu pateada en cámara lenta no golpea nada.");
		no_kick = TRUE;
	} else if (u.utrap) {
		switch (u.utraptype) {
		    case TT_PIT:
			pline("No hay bastante espacio para patear aquí abajo.");
			break;
		    case TT_WEB:
		    case TT_BEARTRAP:
			No_puedes("mover tu %s!", body_part(LEG));
			break;
		    default:
			break;
		}
	...
	if(u.uswallow) {
		switch(rn2(3)) {
		case 0:  No_puedes("mover tu %s!", body_part(LEG));
			 break;
		case 1:  if (is_animal(u.ustuck->data)) {
				pline("%s eructa ruidosamente.", Monnam(u.ustuck));
				break;
			 }
		default: pline("Tu débil patada no tiene ningún efecto."); break;
		}
		return(1);
	}
	if (Levitation) {
		...
		if (isok(xx,yy) && !IS_ROCK(levl[xx][yy].typ) &&
		    ...
		    pline("No tienes nada de que agarrarte.");
		    return(0);
		}
	}
	...
	if (is_pool(x, y) ^ !!u.uinwater) {
		...
		pline("Salpicas agua por todos lados.");
		return 1;
	}
	...
	if(!IS_DOOR(maploc->typ)) {
		if(maploc->typ == SDOOR) {
			...
			pline("¡Crash!  ¡Tu patada %s una puerta secreta!",
		...
		if(maploc->typ == SCORR) {
		    if(!Levitation && rn2(30) < avrg_attrib) {
			pline("!Crash!  ¡Tu patada abre un pasadizo secreto!");
			exercise(A_DEX, TRUE);
			maploc->typ = CORR;
			if (Blind)
			    feel_location(x,y);	/* we know it's gone */
		...
		if(IS_THRONE(maploc->typ)) {
			...
			if (Blind)
			    pline("¡CRASH!  Lo destruyes.");
			else {
			    pline("¡CRASH!  Destruyes el trono.");
			    newsym(x, y);
			}
			exercise(A_DEX, TRUE);
			return(1);
			...
			if (Blind)
			    pline("¡Arrancas %s a patadas!", something);
			else {
			    pline("¡Tu patada arranca algunas monedas ornamentales y joyas!");
			    newsym(x, y);
			}
		...
		if(IS_ALTAR(maploc->typ)) {
		    if(Levitation) goto dumb;
		    pline("Pateas %s.",(Blind ? something : "el altar"));
		    if(!rn2(3)) goto ouch;
		    altar_wrath(x, y);
		    exercise(A_DEX, TRUE);
		    return(1);
		...
		if(IS_FOUNTAIN(maploc->typ)) {
		    if(Levitation) goto dumb;
		    pline("Pateas %s.",(Blind ? something : "la fuente"));
		    if(!rn2(3)) goto ouch;
		    /* make metal boots rust */
		    if(uarmf && rn2(3))
			if (!rust_dmg(uarmf, "botas metales", 1, FALSE, &youmonst)) {
				pline("Tus botas se mojan.");
		...
		if(IS_TREE(maploc->typ)) {
			...
			if ( !rn2(6) && !(mvitals[PM_KILLER_BEE].mvflags & G_GONE) )
			    Oyes("un zumbido bajo."); /* a warning */
			goto ouch;
		    }
			...
			if (is_plural(treefruit)) {
			    const char *fruta = xname(treefruit);
			    pline("¡Algun%cs %s caen del arbol!",
				  isfeminine(fruta)? 'a' : 'o',
				  xname(treefruit));
			} else
			    pline("¡%s cae del arbol!", An(xname(treefruit)));
			...
			if (nfall != nfruit) {
			    ...
			    fruta = xname(treefruit);
			    pline("%ld %s qued%s colgad%c%s en las ramas.",
				  nfruit-nfall, fruta,
				  (nfruit-nfall) > 1 ? "aron" : "ó",
				  isfeminine(fruta)? 'a' : 'o',
				  (nfruit-nfall) > 1 ? "s" : "");
			...
			if ( made )
			    pline("¡Has atraído a los ocupantes anteriores del árbol!");
			else
			    pline("Hueles miel rancia.");
		...
		if(IS_SINK(maploc->typ)) {
		    ...
		    if(rn2(5)) {
			if(flags.soundok)
			    pline("¡Klunk!  Los tubos vibran ruidosamente.");
			else pline("¡Klunk!");
			exercise(A_DEX, TRUE);
			return(1);
			...
			if (Blind)
			    Oyes("un sonido chorreante.");
			else
			    pline("¡Una rezuma %s se se brota por el desagüe!",
			...
			/* can't resist... */
			pline("¡Vuelve %s!", (Blind ? something :
				washerndx==PM_INCUBUS? "el lavador"
						     : "la lavadora"));
			...
			pline("¡Flupp!  %s.", (Blind ?
				      ...
				      "Una suciedad fangosa brota del desagüe."));
			if(!(maploc->looted & S_LRING)) { /* once per sink */
			    if (!Blind)
				pline("Ves un anillo brillando en medio.");
		...
		if (maploc->typ == STAIRS || maploc->typ == LADDER ||
			...
			goto dumb;
ouch:
		    pline("!Ouch!  ¡Eso duele!");
		    exercise(A_DEX, FALSE);
		    exercise(A_STR, FALSE);
		    if (Blind) feel_location(x,y); /* we know we hit it */
		    if (is_drawbridge_wall(x,y) >= 0) {
			pline("El puente levadizo no es afectado.");
...
dumb:
		...
		if (martial() || ACURR(A_DEX) >= 16 || rn2(3)) {
			pline("Pateas el espacio vacío.");
			if (Blind) feel_location(x,y);
		} else {
			pline("¡Movimiento tonto!  Te tuerces un músculo.");
			exercise(A_STR, FALSE);
	...
	if(rnl(35) < avrg_attrib + (!martial() ? 0 : ACURR(A_DEX))) {
		...
		/* break the door */
		if(maploc->doormask & D_TRAPPED) {
		    if (flags.verbose) pline("Pateas la puerta.");
		    exercise(A_STR, FALSE);
		    maploc->doormask = D_NODOOR;
		    b_trapped("puerta", FOOT);
		} else if(ACURR(A_STR) > 18 && !rn2(5) && !shopdoor) {
		    pline("¡Al patear la puerta, se rompe en pedazos!");
		    exercise(A_STR, TRUE);
		    maploc->doormask = D_NODOOR;
		} else {
		    pline("¡Al patear la puerta, se abre de golpe!");
		    exercise(A_STR, TRUE);
		    maploc->doormask = D_BROKEN;
		}
		if (Blind)
		...
		if (in_town(x, y))
		  for(mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
			...
			if (canspotmon(mtmp))
			    ...
			    Oyes("a alguien gritando:");
			verbalize("¡Alto, ladr%s!  ¡Estás arrestad%c!",
		...
		for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
			...
			if(levl[x][y].looted & D_WARNED) {
			    ...
			    verbalize("¡Hey, deja de dañar esa puerta!");
...
xchar x, y, dlev;
{
	...
	if (dct && cansee(x,y)) {	/* at least one object fell */
	    const char *np = (dct == 1L? "" : "s");
	    const char *vp = (dct == 1L? "" : "n");

	    if (missile)
		pline("Desde el impacto, otro%s objeto%s cae%s.",
		      np, np, vp);
	    else if (oct == dct)
		pline("%s objeto%s adyacente%s cae%s %s.",
		      dct == 1L ? "El" : "Todos los", np, np, vp, gate_str);
	    else
		pline("Uno%s de los objeto%s adyacente%s cae%s %s.",
		      np, np, np, vp, gate_str);
	}

	if(costly && shkp && price) {
		if(ESHK(shkp)->robbed > robbed) {
		    pline("¡Has quitado %ld %s de bienes!", price, currency(price));
		    if(cansee(shkp->mx, shkp->my)) {
			if(ESHK(shkp)->customer[0] == 0)
			    (void) strncpy(ESHK(shkp)->customer,
					   plname, PL_NSIZ);
			if(angry)
			    pline("¡%s está enfurecid%c!",
				  Monnam(shkp), mon_gender(shkp)? 'a' : 'o');
			else pline("\"%s, usted es ladr%s!\"", plname,
				  poly_gender()==1? "ona" : "ón");
		    } else  pline("una grita, \"¡Ladr%s!\"",
				  poly_gender()==1? "ona" : "ón");
		    hot_pursuit(shkp);
		    (void) angry_guards(FALSE);
		    return;
		...
		if(ESHK(shkp)->debit > debit) {
		    ...
		    pline("Debes %s %ld %s por bienes perdidos.",
			  al(mon_nam(shkp)),
			  amt, currency(amt));
		}
	}
...
long num;
{
	...
	if(num) { /* means: other objects are impacted */
	    Sprintf(eos(obuf), " %s otro%s objeto%s",
		    otense(otmp, "golpea"),
		    num > 1L ? "s" : "",
		    num > 1L ? "s" : "");
	    if(nodrop)
		...
		Sprintf(eos(obuf), " y %s %s.",
			otense(otmp, "cae"), gate_str);
	    pline("%s", obuf);
	} else if(!nodrop)
	    pline("%s %s %s.", obuf, otense(otmp, "cae"), gate_str);
}
