...
found_it:;
	...
	if (!isok(*x,*y)) {
	    impossible("get_location:  (%d,%d) fuera de límites", *x, *y);
...
#ifdef DEBUG
				if(!vault)
				    debugpline("área extraña [%d,%d] en check_room.",x,y);
#endif
...
struct mkroom *broom;
{
	...
	if (trycnt > 100) {
		impossible("create_door: ¡No se puede encontrar un lugar apto!");
		return;
	}
	add_door(x,y,broom);
    ...
    xchar walls; /* any of W_NORTH | W_SOUTH | W_EAST | W_WEST (or W_ANY) */
{
    ...
    impossible("No podía crear puerta secreta en ningunas paredes 0x%x", walls);
}
...
struct mkroom	*croom;
{
	...
	if (mtmp) {
		...
		switch (m->appear) {
		    case M_AP_NOTHING:
			impossible(
		"create_monster: mon tiene apariencia, \"%s\", pero no tipo",
				m->appear_as.str);
			break;
			...
			if (i == MAXPCHARS) {
			    impossible(
				"create_monster: No puedo encontrar rasgo \"%s\"",
				m->appear_as.str);
			} else {
			...
			if (i == NUM_OBJECTS) {
			    impossible(
				"create_monster: No puedo encontrar objeto \"%s\"",
				m->appear_as.str);
			} else {
		    ...
		    default:
			impossible(
		"create_monster: tipo de apariencia de mon no implementado [%d,\"%s\"]",
...
struct mkroom	*croom;
{
	...
	switch(o->containment) {
	    ...
	    case 1:
		if (!container) {
		    impossible("create_object: no caja");
		    break;
		}
		remove_object(otmp);
	    ...
	    default: impossible("¿tipo de contenemienta %d?", (int) o->containment);
	}
...
#ifdef DEBUG
		debugpline("dig_corridor: malos coords : (%d,%d) (%d,%d).",
			   xx,yy,tx,ty);
#endif
		return FALSE;
	}
...
dlb *fd;
{
	...
	while(n--) {
		...
		if (room_not_needed || nroom >= MAXNROFROOMS) {
		    if (!room_not_needed)
			impossible("¡Demasiados cuartos en nuevo nivel!");
		    light_region(&tmpregion);
		    continue;
		}
	...
	while(n--) {
		...
		if (!create_drawbridge(x, y, tmpdb.dir, tmpdb.db_open))
		    impossible("No puedo crear puente levantizo.");
	}
