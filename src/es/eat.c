...
static const struct { const char *txt; int nut; boolean precedes; } tintxts[] = {
	{"frito",	  60, FALSE },	/* LENGUA:  Inglés, "deep fried" */
	...
	register struct obj *food;
{
	...
	if (u.uhs != SATIATED) {
		if (!food || food->otyp != AMULET_OF_STRANGULATION)
			...
			Te_sientes("como un%s!", poly_gender()==1? "a glotona" : " glotón");
	}
...
boolean allowmsg;
{
	if (!CANNIBAL_ALLOWED() && your_race(&mons[pm])) {
		if (allowmsg) {
			...
			pline("¡Eres un%s caníbal!  ¡Lamentarás esto!",
				poly_gender()==1? "a" : "");
		}
...
register int pm;
{
	...
	switch(pm) {
		...
		{ char buf[BUFSZ];
		    ...
		    Sprintf(buf, "comió imprudentemente el cuerpo de %s",
			    mons[pm].mname);
		    killer = buf;
...
fix_petrification()
{
	...
	delayed_killer = 0;
	if (Hallucination)
	    pline("Qué lástima - ¡has arruinado una futura pieza de %sarte!",
...
register struct permonst *ptr;
{
	...
	switch (type) {
		...
		if(!(HFire_resistance & FROMOUTSIDE)) {
			...
			*/
			pline(Hallucination ? "Te escalofrías." :
			      "Sientes un escalofrío momentario.");
...
register int pm;
{
	...
	switch(pm) {
		...
		if (youmonst.data->mlet != S_MIMIC && !Unchanging) {
		    char buf[BUFSZ];
		    No_puedes("resistir la tentación de imitar una %s.",
		...
		if (HFast & INTRINSIC) {
			HFast &= ~INTRINSIC;
			pline("Pareces más lent%c.",
				poly_gender()==1? 'a' : 'o');
		} else {
			HFast |= FROMOUTSIDE;
			pline("Pareces más rápid%c.",
				poly_gender()==1? 'a' : 'o');
		}
		break;
	    case PM_LIZARD:
	 ...
	 /* case PM_SANDESTIN: */
		if (!Unchanging) {
		    Te_sientes("un cambio alcanzándote.");
		    polyself(FALSE);
		}
		break;
	    case PM_MIND_FLAYER:
		...
		if (ABASE(A_INT) < ATTRMAX(A_INT)) {
			...
			pline("Por alguna razón, esto supo blando.");
		}
...
opentin()		/* called during each move whilst opening a tin */
{
	...
	pline("Tienes éxito de abrir la lata.");
	if(tin.tin->spe != 1) {
	    if (tin.tin->corpsenm == NON_PM) {
		pline("Descubres que es vacía.");
	    ...
	    if (!tin.tin->cursed)
		pline("¡Ésta te hace sentir como %s!",
...
no_opener:
		pline("No es tan fácil abrir esta lata.");
		if(Glib) {
...
struct obj *obj;
{
	...
	if(!rn2(4)) {
		...
		flags.soundok = 0;
		nomul(-rnd(10));
		nomovemsg = "Estás consciente de nuevo.";
		afternmv = Hear_again;
		return(1);
	}
	return(0);
	...
	register struct obj *otmp;
{
	...
	if (mnum != PM_ACID_BLOB && !stoneable && rotted > 5L) {
		...
		const char *carne = 
		      mons[mnum].mlet == S_FUNGUS ? "a vegetación hongosa" :
		...
		if (Sick_resistance) {
			...
			if (!uniq)
			    ...
			    Sprintf(buf, "cadáver podrido de %s",
				    mons[mnum].mname);
		...
		tp++;
		pline("Tienes un caso muy malo de ácido del estómago."); /* not body_part() */
		losehp(rnd(15), "cadáver ácido", KILLED_BY_AN);
	} else if (poisonous(&mons[mnum]) && rn2(5)) {
		tp++;
		pline("¡Ecch - eso debe de ser envenenado!");
		if(!Poison_resistance) {
			losestr(rnd(4));
			losehp(rnd(15), "cadáver envenenado", KILLED_BY_AN);
	...
	/* now any corpse left too long will make you mildly ill */
		...
		Te_sientes("%senferm%c.", (Sick) ? "muy " : "",
			poly_gender()==1? 'a' : 'o');
		losehp(rnd(8), "cadáver", KILLED_BY_AN);
	}
	...
	if (!tp && mnum != PM_LIZARD && mnum != PM_LICHEN &&
		...
		if (!retcode) pline("El cadáver se pudre completamente.");
		if (carried(otmp)) useup(otmp);
		else useupf(otmp, 1L);
		retcode = 2;
	    }
...
struct obj *otmp;
{
	switch(otmp->otyp) {
	    case FOOD_RATION:
		if(u.uhunger <= 200)
		    /* LENGUA:  Inglés, "Oh wow, like, superior, man!" */
		    /* LENGUA:  Inglés, "That food really hit the spot!" */
		...
		if (carnivorous(youmonst.data) && !humanoid(youmonst.data))
		    pline("¡Esa ración de tripas era sorprendientemente buena!");
		else if (maybe_polyd(is_orc(youmonst.data), Race_if(PM_ORC)))
		    /* LENGUA: Inglés, "Tastes great! Less filling!"  From
		       a beer commercial. */
		    pline(Hallucination ? "¡Sabe bien!  ¡Menos llenón!" :
			  "Mmm, tripas... ¡no malo!");
		else {
...
struct obj *otmp;
{
	pline("Magia se extende través de tu cuerpo como digieres el %s.",
	...
	if (!rn2(otmp->oclass == RING_CLASS ? 3 : 5)) {
	  switch (otmp->otyp) {
		...
		if (!(u.uprops[objects[typ].oc_oprop].intrinsic & FROMOUTSIDE))
		    pline("Magia se extende través de tu cuerpo como digeres el %s.",
			  otmp->oclass == RING_CLASS ? "anillo" : "amuleto");
		...
		switch (typ) {
		  ...
		  case RIN_INVISIBILITY:
		    if (!oldprop && !EInvis && !BInvis &&
					!See_invisible && !Blind) {
			newsym(u.ux,u.uy);
			pline("Tu cuerpo adquiere una transparencía %s...",
...
eatspecial() /* called after eating non-food */
{
	...
	if ((otmp->otyp == TRIDENT) && !otmp->cursed)
	{
		/* LENGUA:  Esloganes de noticias de chicle (Trident(R) y
		   Wrigley's(R) */
		pline(Hallucination ? "Cuatro de cinco dentistas están de acuerdo." :
				"¡Esto era satisfacción pura de masticar!");
		exercise(A_WIS, TRUE);
	}
...
register struct obj *otmp;
{
	switch(otmp->otyp) {
		...
		if (Upolyd) {
			...
			killer_format = KILLED_BY_AN;
			killer = "terrón podrido de jalea real";
			done(POISONING);
		    }
		}
...
struct obj *otmp;
{
	...
	if (cadaver && acidic(&mons[mnum]) && !Acid_resistance) {
		Sprintf(buf, "%s un poco ácid%c%s. %s",
...
doeat()		/* generic "eat" command funtion (see cmd.c) */
{
	...
	if (Strangled) {
		pline("¿Si no puedes inhalar el aire, como puedes consumir sólidos?");
		return 0;
	}
	...
	if (!is_edible(otmp)) {
	    ...
	    /* let them eat rings */
	    No_puedes("comer %s que estás llevando.", something);
	    return 0;
	}
	if (is_metallic(otmp) &&
...
register int num;
{
	...
	if(u.uhunger >= 2000) {
		...
		if (!victual.eating || (victual.eating && !victual.fullwarn)) {
		    pline("Estás teniendo dificultad de comer lo todo.");
	...
	if(newhs != u.uhs) {
		...
		case HUNGRY:
			if (Hallucination) {
			    ...
			    pline((!incr) ? "Sólo tienes hambre ahora." :
		...
		case WEAK:
			...
			"Los munchies siguen frustrando tus habilidades.");
			    ...
			    pline((!incr) ? "Te sientes débil ahora." :
				  (u.uhunger < 45) ? "Te sientes débil." :
				   "Empiezas a sentirte débil.");
			if (incr && occupation &&
			    (occupation != eatfood && occupation != opentin))
			    stop_occupation();
			break;
	...
	int corpsecheck; /* 0, no check, 1, corpses, 2, tinnable corpses */
{
	...
	if (feeding && metallivorous(youmonst.data)) {
		...
		Sprintf(qbuf, "Hay una trampa para osos aquí (%s); ¿Comerlo?",
		...
		if (gold->quan == 1L)
		    Sprintf(qbuf, "Hay una pieza de oro aquí; ¿Comerla?");
		else
		    Sprintf(qbuf, "Hay %ld piezas de oro aquí; ¿Comerlas?",
			    gold->quan);
		if ((c = yn_function(qbuf, es_ynqchars, ynqchars, 'n')) == 'y') {
		    return gold;
		} else if (c == 'q') {
...
register struct obj *obj;
{
	...
	if (uneaten_amt > full_amount) {
	    impossible(
	  "cosa comido en parte (%ld) más nutritiva que no comida (%ld)",
