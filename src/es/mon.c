...
register struct monst *mtmp;
{
	...
	switch(mndx) {
		...
		if (mtmp->mrevived && rn2(20)) {
			if (canseemon(mtmp))
			   pline("El cuerno recientemente crecido de %s se desmenuza a polvo.",
				mon_nam(mtmp));
		} else
...
register struct monst *mtmp;
{
	...
	if (!is_clinger(mtmp->data) && !likes_lava(mtmp->data)) {
		...
		if (cansee(mtmp->mx,mtmp->my))
		    pline("%s %s.", Monnam(mtmp),
			  ...
			  "se hierve completamente" : "se quema al crespo");
		mondead(mtmp);
	    }
	    else {
		if (--mtmp->mhp < 1) {
		    if (cansee(mtmp->mx,mtmp->my))
			pline("%s se rende al fuego.", Monnam(mtmp));
		    mondead(mtmp);
		}
		else if (cansee(mtmp->mx,mtmp->my))
		    pline("%s se quema un poco.", Monnam(mtmp));
	    }
	    if (mtmp->mhp > 0) {
	...
	if (!is_clinger(mtmp->data)
	    ...
	    if (cansee(mtmp->mx,mtmp->my)) {
		    pline("%s se ahoga.", Monnam(mtmp));
	    }
	    if (u.ustuck && u.uswallow && u.ustuck == mtmp) {
	    /* This can happen after a purple worm plucks you off a
		flying steed while you are over water. */
		pline("%s se sume como el agua se flue al dentro y te vacía.",
			Monnam(mtmp));
	    }
	    mondead(mtmp);
	    if (mtmp->mhp > 0) {
	...
	register struct monst *mtmp;
{
	...
	for (otmp = level.objects[mtmp->mx][mtmp->my];
		...
		/* KMH -- Don't eat indigestible/choking objects */
			...
			pline("¡%s come %s!", Monnam(mtmp),
				distant_name(otmp,doname));
		    else if (flags.soundok && flags.verbose)
			Oyes("un sonido de estrujar.");
	...
	register struct monst *mtmp;
{
	...
	for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp2) {
		...
		++ecount;
		if (ecount == 1) {
			Sprintf(buf, "%s engolfa %s.", Monnam(mtmp),
			    distant_name(otmp,doname));
		} else if (ecount == 2)
			Sprintf(buf, "%s engolfa unos objetos.", Monnam(mtmp));
	...
	register struct monst *mtmp;
{
	...
	if (cansee(mtmp->mx, mtmp->my) ) {
		...
		pline("%s algo de %s.", Mon_verb_mon(mtmp, "recoge", (struct monst *)0),
			mat_idx == GOLD ? "oro" : "dinero");
	    newsym(mtmp->mx, mtmp->my);
	}
    }
}
...
#endif
		if (cansee(mtmp->mx,mtmp->my) && flags.verbose)
			pline("%s %s.", Mon_verb_mon(mtmp, "recoge", (struct monst *)0),
...
impossible("Un monstruo miró una trampa muy extraña de tipo %d.", ttmp->ttyp);
			    continue;
			}
...
dmonsfree()
{
	...
	impossible("dmonsfree: %d quitados no igualan %d pendientes",
...
register struct monst *mtmp, *mtmp2;
{
	...
	if (otmp->where != OBJ_MINVENT || otmp->ocarry != mtmp)
	    panic("replmon: inconsistencia de minvent");
#endif
	otmp->ocarry = mtmp2;
    }
    mtmp->minvent = 0;
...
register struct monst *mon;
{
	...
	if (fmon == (struct monst *)0)  panic ("relmon: no fmon disponible.");
	...
	if(mon == fmon) fmon = fmon->nmon;
		...
		else	    panic("relmon: mon no en la lista.");
	}
}
...
struct monst *mtmp;
{
	...
	if (lifesave) {
		...
		if (cansee(mtmp->mx, mtmp->my)) {
			pline("Pero espera...");
			pline("¡El medallón %s empieza a alumbrar!",
				del(mon_nam(mtmp)));
			makeknown(AMULET_OF_LIFE_SAVING);
			if (attacktype(mtmp->data, AT_EXPL)
			    || attacktype(mtmp->data, AT_BOOM))
				pline("¡%s se reconstituye!", Monnam(mtmp));
			else
				pline("¡%s se ve mucho mejor!", Monnam(mtmp));
			pline("¡El medallón se desmenuza a polvo!");
		}
		...
		if (mvitals[monsndx(mtmp->data)].mvflags & G_GENOD) {
			if (cansee(mtmp->mx, mtmp->my))
			    pline("Desgraciadamente, %s todavía está genocidad%c...",
...
boolean was_swallowed;			/* digestion */
{
	...
	if (mdat == &mons[PM_VLAD_THE_IMPALER] || mdat->mlet == S_LICH) {
		...
		pline("El cuerpo %s se desmenuza a polvo.", del(mon_nam(mon)));
	    return FALSE;
	}
	...
	for(i = 0; i < NATTK; i++) {
		...
		if (was_swallowed && magr) {
		    if (magr == &youmonst) {
			pline("¡Hay una explosión en tu %s!",
			      body_part(STOMACH));
	    		Sprintf(killer_buf, "la explosión de %s",
				an(mdat->mname));
			if (Half_physical_damage) tmp = (tmp+1) / 2;
			losehp(tmp, killer_buf, KILLED_BY_AN);
		    } else {
			if (flags.soundok) Oyes("una explosión.");
			magr->mhp -= tmp;
			if (magr->mhp < 1) mondied(magr);
			if (magr->mhp < 1) { /* maybe lifesaved */
			    if (canspotmon(magr))
				pline("¡%s se rasga abiert%c!", Monnam(magr),
					mon_gender(magr)? 'a' : 'o');
			} else if (canseemon(magr))
			    pline("%s parece tener indigestión.",
				  Monnam(magr));
		    }
	    	...
	    	Sprintf(killer_buf, "explosión %s", del(the(mdat->mname)));
	    	killer = killer_buf;
...
register struct monst *mdef;
{
...
#endif
				obj_resists(obj, 0, 0)) {
			if (flooreffects(obj, x, y, "cae")) continue;
	...
	if (wasinside) {
		if (is_animal(mdef->data)) {
			...
			pline("%s través de una abertura en %s %s.",
				locomotion(youmonst.data, "Saltas"),
				isfeminine(que)? "la nueva" : "el nuevo",
				que);
		}
	}
}
...
int how;
{
	...
	if ((mdef->wormno ? worm_known(mdef) : cansee(mdef->mx, mdef->my))
		&& fltxt) {
	    const char *mon = Monnam(mdef);
	    if (strcmp(mon, "Él") == 0)
		pline("¡Es %s%s%s!",
			nonliving(mdef->data) ? "destruido" : "matado",
			*fltxt ? " por " : "",
			*fltxt ? the(fltxt) : ""
		     );
	    else
		pline("¡%s es %s%c%s%s!", mon,
			nonliving(mdef->data) ? "destruid" : "matad",
			mon_gender(mdef)? 'a' : 'o',
			*fltxt ? " por " : "",
			*fltxt ? the(fltxt) : ""
		     );
	...
	if (be_sad && mdef->mhp <= 0)
	    Te_sientes("triste por un momento, y entonces pasa el sentido.");
}
	...
	int	dest;
{
	...
	if (dest & 1) {
	    const char *verb = nonliving(mtmp->data) ? "destruyes" : "matas";

	    if (!wasinside && !canspotmon(mtmp))
		pline("¡Lo %s!", verb);
	    else {
		pline("¡%c%s %s!", toupper2(verb[0]), verb+1,
		    al(!mtmp->mtame ? mon_nam(mtmp) :
			    x_monnam(mtmp,
...
cleanup:
	...
	if(is_human(mdat) && (!always_hostile(mdat) && mtmp->malign <= 0) &&
	   ...
	   u.ualign.type != A_CHAOTIC) {
		HTelepat &= ~INTRINSIC;
		change_luck(-2);
		pline("¡Tú asesin%c!", poly_gender()==1? 'a' : 'o');
	...
	if (is_unicorn(mdat) &&
		...
		change_luck(-5);
		Te_sientes("pecador%s...", poly_gender()==1? "a" : "");
	}
	...
	if (mtmp->m_id == quest_status.leader_m_id) {		/* REAL BAD! */
	    ...
	    pline("Eso era %suna mala idea...",
	    		u.uevent.qcompleted ? "probablemente " : "");
	} else if (mdat->msound == MS_NEMESIS)	/* Real good! */
	    adjalign((int)(ALIGNLIM/4));
	else if (mdat->msound == MS_GUARDIAN) {	/* Bad */
	    adjalign(-(int)(ALIGNLIM/8));
	    if (!Hallucination) pline("Eso fue probablemente una mala idea...");
		...
		if (!Hallucination) Oyes("el retumbo de trueno distante...");
		else Oyes("el aplauso de la audiencia!");
    ...
    register struct monst *mtmp;
{
	...
	if(canseemon(mtmp))
	    pline("%s se solidifica...", Monnam(mtmp));
	if (newcham(mtmp, &mons[PM_STONE_GOLEM], FALSE, FALSE)) {
	    if(canseemon(mtmp))
		pline("Es ahora %s.", an(mtmp->data->mname));
	} else {
	    if(canseemon(mtmp))
		pline("... y vuelve a normal.");
	}
    } else
	impossible("¡No puedo petrificar %s!", a_monnam(mtmp));
}
...
static const char *poiseff_str[] = {

	"Te sientes más débil", "Tu cerebro se arde",
	"Tu discernimiento está dañado", "Tus músculos no te obedecen",
	"Te sientes muy enferm@", "Tienes ronchas"
};
	...
	int typ;
{
	...
	if (strchr(str, '@') != (char *)0) {
		...
		str2 = nextobuf();
		Strcpy(str2, poiseff_str[typ]);
		while ((p = strchr(str2, '@')) != (char *)0)
			*p = fem;
                str = str2;
	}
...
int  typ, fatal;
{
	...
	if(Poison_resistance) {
		if(!strcmp(string, "ráfaga")) shieldeff(u.ux, u.uy);
		pline("Parece que el veneno no te afecta.");
		return;
	}
	...
	} else if (!strncmpi(pname, "el ", 3) ||
	    !strncmpi(pname, "la ", 3) ||
	    !strncmpi(pname, "los ", 4) ||
	    !strncmpi(pname, "las ", 4) ||
	    !strncmpi(pname, "un ", 3) ||
	    !strncmpi(pname, "una ", 4) ||
	    !strncmpi(pname, "unos ", 5) ||
	    !strncmpi(pname, "unas ", 5)) {
	...
	if(u.uhp < 1) {
		...
		done(strstri(pname, "veneno") ? DIED : POISONING);
	}
	(void) encumber_msg();
}
...
register struct monst *mtmp;
{
	...
	if (couldsee(mtmp->mx, mtmp->my)) {
		if (humanoid(mtmp->data) || mtmp->isshk || mtmp->isgd)
		    pline("¡%s se pone enojad%c!", Monnam(mtmp), mon_gender(mtmp)? 'a' : 'o');
	...
	if (!flags.mon_moving &&	/* should always be the case here */
		...
		if (!DEADMONSTER(mon) && mon->data == q_guardian && mon->mpeaceful) {
		    ...
		    if (!mon_gender(mon)) fem = FALSE;
		}
		if (got_mad && !Hallucination) {
		    if (got_mad == 1)
			pline("Parece que %s %s esté enojad%c también...",
				fem ? "la" : "el",
				q_guardian->mname,
				fem ? 'a' : 'o');
		    else
			pline("Parece que %s %s estén enojad%cs también...",
				fem ? "las" : "los",
...
struct monst *mon;
{
	...
	if (wizard && iflags.mon_polycontrol) {
		...
		do {
			Sprintf(pprompt,
				"¿Cambia %s en qué tipo de monstruo? [teclea el nombre]",
				mon_nam(mon));
			getlin(pprompt,buf);
			mndx = name_to_mon(buf);
			if (mndx < LOW_PM)
				No_puedes("%s en esto.", mon_nam_acc(mon, "polimorfear"));
			else break;
		} while(++tries < 5);
...
/* and the same for maximum hit points */
	...
	if (msg) {
	    ...
	    mtmp->mnamelth = 0;
	    pline("¡%s se vuelve %s!", oldname,
		  mdat == &mons[PM_GREEN_SLIME] ? "fango" :
	...
	if (!(mtmp->misc_worn_check & W_ARMG))
	    mselftouch(mtmp, "No más resistiente de petrificación, ",
			!flags.mon_moving);
	m_dowear(mtmp, FALSE);
...
int damtype, dam;
{
	...
	if (mon->mhp < mon->mhpmax) {
		...
		pline("%s parece más san%c.", Monnam(mon), mon_gender(mon)? 'a' : 'o');
	}
    }
}
...
register boolean silent;
{
	...
	if(ct) {
		...
		if(slct) pline("¡%s guarda%s se despierta%s!",
				slct > 1 ? "Los" : "El",
				slct > 1 ? "s" : "",
				slct > 1 ? "n" : "");
		if(nct || sct) {
			/* LENGUA: deal with gender here? */
			if(nct) pline("¡%s guarda%s se pone%s enojado%s!",
				nct > 1 ? "Los" : "El",
				nct > 1 ? "s" : "",
				nct > 1 ? "n" : "",
				nct > 1 ? "s" : "");
			else if(!Blind)
				pline("¡Ves a %sguarda%s enojado%s acercándose!",
				  sct == 1 ? "un " : "", sct > 1 ? "s" : "",
				  sct > 1 ? "s" : "");
		} else if(flags.soundok)
			Oyes("el sonido agudo de un silbado de un guarda.");
	    }
	    return(TRUE);
	}
	return(FALSE);
...
short otyp;
{
	...
	switch(mtmp->m_ap_type) {
		...
		if (otyp == SPE_HEALING || otyp == SPE_EXTRA_HEALING) {
		    /* LENGUA: do the color, and vívido, need to match the
		       gender of the object? */
		    pline("%s parece un %s más vívido que antes.",
