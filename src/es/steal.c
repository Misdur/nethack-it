...
register struct monst *mtmp;
{
	...
	if (gold && ( !u.ugold || gold->quan > u.ugold || !rn2(5))) {
	    ...
	    delobj(gold);
	    newsym(u.ux, u.uy);
	    pline("%s de prisa agarra algo de oro desde el espacio entre tus %s!",
	    ...
	    pline("Tu billetera siente más ligera.");
	    mtmp->mgold += tmp;
	if (!tele_restrict(mtmp)) (void) rloc(mtmp, FALSE);
	    mtmp->mavenge = 1;
	    monflee(mtmp, 0, FALSE, FALSE);
...
stealarm()
{
	...
	for(otmp = invent; otmp; otmp = otmp->nobj) {
		...
		for(mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
			...
			if(DEADMONSTER(mtmp)) impossible("stealarm(): monstruo muerto robando"); 
			if(!dmgtype(mtmp->data, AD_SITM)) /* polymorphed */
			    goto botm;
			if(otmp->unpaid)
			    subfrombill(otmp, shop_keeper(*u.ushops));
			freeinv(otmp);
			pline("¡%s te roba %s!", Monnam(mtmp), doname(otmp));
...
boolean unchain_ball;	/* whether to unpunish or just unwield */
{
	...
	if (obj->owornmask & W_ARMOR) {
	    if (obj == uskin) {
		impossible("¿Quitando escamas embedidas?");
...
nothing_to_steal:
	    ...
	    if(Blind)
	      pline("Alguien trata de robarte, pero no encuentra nada que robar.");
	    else
	      pline("¡%s trata de robarte, pero no hay nada que robar!",
		Monnam(mtmp));
...
gotobj:
	...
	if (monkey_business) {
		...
		static const char * const how[] = { "robar","asir","agarrar","tomar" };
		boolean plural;
 cant_take:
		plural = (otmp->quan>1) || (otmp == uarmf) || (otmp == uarmg);
		pline("%s trata de %s tu%s %s pero no puede.",
	...
	if((otmp->owornmask & (W_ARMOR | W_RING | W_AMUL | W_TOOL))){
		...
		case ARMOR_CLASS:
			...
			boolean plural = (otmp->quan>1) || (otmp == uarmf) || (otmp == uarmg);
			int slowly;
			...
			/* LENGUA: should ugly this up a bit so that "Ella"
			   will be implicit */
			if(flags.female)
			    pline("%s te cautiva.  Alegramente %s tu%s %s.",
				  !seen ? "Ella" : Monnam(mtmp),
				  curssv ? "la dejas tomar" :
				  slowly ? "empiezas a quitarte" : "la das",
				  plural? "s" : "", equipname(otmp));
			else
			    pline("%s te seduce y %s tu%s %s.",
				  !seen ? "Ella" : Adjmonnam(mtmp, "hermosa"),
				  curssv ? "te ayuda de quitarte" :
				  slowly ? "empiezas a quitarte" : "te quitas",
				  plural? "s" : "", equipname(otmp));
			named++;
		    ...
		    break;
		default:
		    impossible("Trató de robar una cosa llevada extraña. [%d]",
			       otmp->oclass);
		}
	}
	...
	freeinv(otmp);
	pline("%s te robó %s.", named ? "Ella" : Monnam(mtmp), doname(otmp));
...
register struct obj *otmp;
{
	...
	if (u.uswallow && mtmp == u.ustuck && !Blind)
	    pline("%s.", Tobjnam(otmp, "se extingue"));
	snuff_otmp = TRUE;
    }
...
struct monst *mtmp;
{
	...
	pline("¡%s te robó %s!", Monnam(mtmp), doname(otmp));
...
boolean verbosely;
{
	...
	pline("%s deja caer %s.", Monnam(mon), distant_name(obj, doname));
    if (!flooreffects(obj, omx, omy, "cae")) {
...
boolean is_pet;		/* If true, pet should keep wielded/worn items */
{
	...
	if (mtmp->mgold) {
		...
		if (is_pet && cansee(omx, omy) && flags.verbose)
			pline("%s deja caer %ld pieza%s de oro.", Monnam(mtmp),
				g, plur(g));
		mtmp->mgold = 0L;
	}
#endif
