...
boolean exclude_cookie; 
{
	...
	if (rumors) {
		...
		if (true_rumor_size == 0L) {	/* if this is 1st outrumor() */
			...
			Sprintf(rumor_buf, "Error leyendo \"%.80s\".",
				RUMORFILE);
			return rumor_buf;
		    }
		}
		...
		switch (adjtruth = truth + rn2(2)) {
		  ...
		  case -1:  beginning = false_rumor_start;
			...
			break;
		  default:
			    impossible("valor de verdad extraña para rumor");
	    ...
	    } while(count++ < 50 && exclude_cookie && (strstri(rumor_buf, "fortuna") || strstri(rumor_buf, "lástima")));
	    (void) dlb_fclose(rumors);
	    if (count >= 50)
		impossible("¿No puedo encontrar rumor no de galleta?");
	    else
		exercise(A_WIS, (adjtruth > 0));
	} else {
		pline("¡No puedo abrir archivo de rumores!");
...
int mechanism;
{
	static const char fortune_msg[] =
		"Esta galleta tiene un pedacito de papel al dentro.";
	const char *line;
	char buf[BUFSZ];
	...
	if (reading) {
		...
		if (mechanism == BY_COOKIE)
			pline(fortune_msg);
		pline("¡Qué lástima que no puedas leerlo!");
	    	return;
	    }
	}
	line = getrumor(truth, buf, reading ? FALSE : TRUE);
	if (!*line)
		line = "El archivo de rumores de NetHack está cerrado para la renovación.";
	switch (mechanism) {
	    case BY_ORACLE:
	 	/* Oracle delivers the rumor */
		pline("Fiel a su palabra, la Orácula dice%s: ",
		  (!rn2(4) ? " con indiferencia" : (!rn2(3) ? " casualmente" :
		  (rn2(2) ? " apáticamente" : ""))));
		verbalize("%s", line);
		exercise(A_WIS, TRUE);
		return;
	...
	if (oracles) {
		...
		if (delphi)
		    putstr(tmpwin, 0, special ?
		          "La Orácula desdeñosamente te quita todo tu dinero y dice:" :
		          "La Orácula medita por un momento y entonces entona:");
		else
		    putstr(tmpwin, 0, "El mensaje dice:");
		putstr(tmpwin, 0, "");
		...
		pline("¡No puedo abrir el archivo de oráculos!");
...
register struct monst *oracl;
{
	...
	if (!oracl) {
		pline("No hay nadie aquí para consultar.");
		return 0;
	} else if (!oracl->mpeaceful) {
		pline("%s no está de ningún humor para consultaciones.", Monnam(oracl));
		return 0;
#ifndef GOLDOBJ
	} else if (!u.ugold) {
#else
...
#endif
		pline("No tienes dinero.");
		return 0;
	}

	Sprintf(qbuf,
		"\"¿Quieres una consultación menor?\" (%d %s)",
...
#endif
		    pline("¡No tienes ni bastante dinero por ésta!");
		    return 0;
		}
		u_pay = minor_cost;
		break;
