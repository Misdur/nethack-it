    ...
    int fd;
{
    ...
    mread(fd, (genericptr_t) &count, sizeof(count));
    if (count >= MAXLINFO)
	panic("cuenta de información de nivel más grande (%d) que tamaño alocado", count);
	...
	dlb	*stream;
{
	...
	if((cnt = dlb_fread(ptr, size, nitems, stream)) != nitems) {
	    panic(
 "¡Fin de archivo prematuro en archivo de descripción de calabozo!\r\nSe esperaban %d bytes - había %d.",
...
const char	*s;
{
	...
	panic("No se puede resolver número de calabozo por nombre \"%s\".", s);
	/*NOT REACHED*/
	return (xchar)0;
}
	...
	const char *s;
{
	...
	for(curr = sp_levchn; curr; curr = curr->next)
	    if (!strcmpi2(s, curr->proto)) break;
	return curr;
}
	...
	struct proto_dungeon *pd;
{
	...
	if (pd) {
	    ...
	    if (i == pd->n_brs) panic("find_branch: no se encuentra %s", s);
	} else {
		...
		/* LENGUA:  Dungeon names are untranslated as yet */
		if (!strcmpi2(dnam, s) ||
			(!strncmpi2(dnam, "The ", 4) && !strcmpi2(dnam + 4, s)))
		    break;
	    }
...
struct proto_dungeon *pd;
{
	...
	panic("parent_dnum: no se puede resolver rama.");
	/*NOT REACHED*/
	return (xchar)0;
}
	...
	int *adjusted_base;
{
	...
	if (chain >= 0) {		 /* relative to a special level */
	    ...
	    if (!levtmp) panic("level_range: ¡nivel encadenado vacío!");
	...
	if (base < 1 || base > lmax)
	    panic("level_range: valor de base fuera de límites");
    ...
    struct tmpbranch *tbr;
{
    ...
    impossible("correct_branch_type: tipo de rama desconocido");
    return BR_STAIR;
}
   ...
   boolean extract_first;
{
	...
	if (!curr) panic("insert_branch: no encontrada");
	if (prev)
	    prev->next = curr->next;
	else
	    branches = curr->next;
    ...
    int nth;
{
    ...
    panic("pick_level:  se acabaron los niveles válidos");
    return 0;
}
    ...
    struct proto_dungeon *pd;
{
	...
	fprintf(stderr,"%s: probando %d [ ", lev->proto, lev->dlevel.dlevel);
	for (i = 1; i <= MAXLEVEL; i++)
...
init_dungeons()		/* initialize the "dungeon" structs */
{
	...
	if (!dgn_file) {
	    char tbuf[BUFSZ];
	    Sprintf(tbuf, "¡No se puede abrir descripción de calabozo - \"%s",
		DUNGEON_FILE);
...
# endif
	...
	if (!check_version(&vers_info, DUNGEON_FILE, TRUE))
	    panic("Descripción de calabozo no válida.");
	...
	if (n_dgns >= MAXDUNGEON)
	    panic("init_dungeons: demasiados calabozos");
	...
	for (i = 0; i < n_dgns; i++) {
		...
		panic("init_dungeon: demasiados niveles especiales");
	    /*
		...
		panic("init_dungeon:  no se pudo ubicar niveles");
#ifdef DDEBUG
	    fprintf(stderr, "--- fin del calabozo %d ---\n", i);
	    fflush(stderr);
	    getchar();
#endif
		...
		panic("init_dungeon: demasiadas ramas");
	    for(; cb < pd.n_brs; cb++)
		Fread((genericptr_t)&pd.tmpbranch[cb],
					sizeof(struct tmpbranch), 1, dgn_file);
	}
 ...
 *	LENGUA:  I hate just as much not being able (yet) to translate them
 */
	quest_dnum = dname_to_dnum("La Misión");
	sokoban_dnum = dname_to_dnum("Sokoban");
	mines_dnum = dname_to_dnum("Las Minas de los Gnomos");
	tower_dnum = dname_to_dnum("La Torre de Vlad");
...
boolean noquest;
{
	...
	for(i = 0; i < n_dgns; i++) {
	    ...
	    if(!strcmp(dungeons[i].dname, "La Misión") && noquest) continue;
...
xchar	ledgerno;
{
	...
	panic("número de nivel fuera de límites [ledger_to_dnum(%d)]", (int)ledgerno);
	/*NOT REACHED*/
	return (xchar)0;
}
...
u_on_sstairs() {	/* place you on the special staircase */
	...
	if (sstairs.sx) {
	    ...
	    xchar x, y;
#ifdef DEBUG
	    pline("u_on_sstairs: elegiendo lugar al azar");
#endif
#define badspot(x,y) ((levl[x][y].typ != ROOM && levl[x][y].typ != CORR) || MON_AT(x, y))
	    ...
	    panic("u_on_sstairs: ¡No se puede reubicar jugador!");
#undef badspot
	}
}
...
int levnum;
{
	...
	if (levnum <= 0) {
		...
		do {
		    ...
		    if (!br)
			panic("get_level: no se puede encontrar calabozo padre");
    ...
    const char *s;
{
    ...
    if (!br) panic("dgn_entrance: no se puede encontrar entrada de %s", s);
...
d_level	*lev;
{
	...
	if (dndest.nlx > 0)
	    ...
	    impossible("¿No hay linde a la Torre del Mago?");
	return FALSE;
}
 ...
 */
/* LENGUA:  Leave the translation for later */
schar
lev_by_name(nam)
const char *nam;
{
	...
	*(eos(buf) - 6) = '\0';
    ...
    if (!strcmpi2(nam, "gehennom") || !strcmpi2(nam, "hell")) {
    ...
    int type;
{
	...
	case BR_NO_END1: return "Conección";
	case BR_NO_END2: return "Escalera de una sola mano";
	case BR_STAIR:	 return "Escalera";
    }
    return " (desconocido)";
}
...
xchar *rdgn;
{
	...
	if (nlev > 1)
	    Sprintf(buf, "%s: niveles de %d a %d", dptr->dname, dptr->depth_start,
						dptr->depth_start + nlev - 1);
	else
	    Sprintf(buf, "%s: nivel %d", dptr->dname, dptr->depth_start);

	/* Most entrances are uninteresting. */
	if (dptr->entry_lev != 1) {
	    if (dptr->entry_lev == nlev)
		Strcat(buf, ", entrada desde abajo");
	    else
		Sprintf(eos(buf), ", entrada en %d",
	...
	if (br->end1.dnum == n_dgns) {
	    if (first) {
	    	if (!bymenu) {
		    putstr(win, 0, "");
		    putstr(win, 0, "Ramas flotantes");
		}
		first = FALSE;
	    }
	    Sprintf(buf, "   de %s a %s",
	...
	end_menu(win, "Teleportarte a qué nivel:");
	...
	putstr(win, 0, "");
	Sprintf(buf, "Posición de invocación @ (%d,%d), héroe @ (%d,%d)",
	...
	putstr(win, 0, "");
	if (trap)
	    Sprintf(buf, "Portal @ (%d,%d), héroe @ (%d,%d)",
		trap->tx, trap->ty, u.ux, u.uy);
	else
	    Sprintf(buf, "Portal no fué encontrado.");
	putstr(win, 0, buf);
    }
