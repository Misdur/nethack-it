...
#  ifdef PERMANENT_MAILBOX
		pline("No puedo obtener estado de MAIL=\"%s\".", mailbox);
		mailbox = 0;
#  else
		omstat.st_mtime = 0;
#  endif
    ...
    register int tx, ty;		/* destination of mail daemon */
{
	...
	newsym(fx, fy);
	verbalize("Este lugar está tan atestado.  Voy a salida.");
...
struct mail_info *info;
{
	...
	if (info->object_nam) {
		...
		if ( namelth <= 0 || namelth >= (int) obj->onamelth )
		    impossible("entrega de correo rota");
		else
		    *(ONAME(obj) + namelth) = '\0';
...
ckmailstatus()
{
	...
	if (--mustgetmail <= 0) {
		static struct mail_info
			deliver = {MSG_MAIL,"Tengo una carta para usted",0,0};
		newmail(&deliver);
		mustgetmail = -1;
	}
}
...
{
    static char *junk[] = {
    "Desatende la carta última por favor.",
    "Bienvenid@ a Nethack.",
#ifdef AMIGA
    "Sólo Amiga lo hace posible.",
    "CATS tienen todas las respuestas.",
#endif
    /* LENGUA:  I'm the one responsible for the Spanish version, not the
       Devteam. */
    "Reporta errores a <rchason@smart.net>.",
    "Invitación: Visita la página Web de NetHack a http://www.nethack.org!",
    "Invitación: Visita la página Web de NetHack en Español a http://spanish-nethack.sourceforge.net!"
    };

    if (Blind) {
	pline("Desgraciadamente no puedes ver lo que se escriba ahí.");
    } else
	pline("Dice:  \"%s\"", junk[rn2(SIZE(junk))]);
...
#  ifdef PERMANENT_MAILBOX
		pline("No puedo obtener estado de MAIL=\"%s\" más.", mailbox);
		mailbox = 0;
#  else
		nmstat.st_mtime = 0;
#  endif
...
#  ifndef NO_MAILREADER
			MSG_MAIL, "Tengo una carta para usted",
#  else
			/* suppress creation and delivery of scroll of mail */
			MSG_OTHER, "Tiene usted correo en el mundo exterior",
#  endif
			0, 0
		    };
		    newmail(&deliver);
